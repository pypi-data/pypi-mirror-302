...
from al360_tai_core_flask import FlaskHelper  # , environment_detector
    ...
    def load_widget_file(self, path):
        ...
        with open(js_path, "r", encoding="utf-8") as f:
            ...
            content = content.replace(
                "__al360_tai_app_id__", f'al360_tai_widget_{self.id}')
            content = content.replace(
                '"__al360_tai_config__"', f'`{json.dumps(self.config)}`')
            model_data = json.dumps(self.model_data,
                                    default=serialize_json_safe)
            content = content.replace(
                '"__al360_tai_model_data__"',
                f'`{model_data}`')
            return content
