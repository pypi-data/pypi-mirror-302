...
class ReactServicer(react_pb2_grpc.ReactServicer):
    ...
    def __init__(
        ...
        for service_name in middleware_by_service_name.keys():
            ...
            state_tag = _state_type_tag(state_type)
            self._service_name_by_state_tag[state_tag] = service_name

        self._stop_websockets_serve = asyncio.Event()

    def _service_name_for_state_ref(
    ...
    async def serve(self, websocket):
        with use_application_id(self._application_id):
            try:
                ...
                service_name = self._service_name_for_state_ref(state_ref)
