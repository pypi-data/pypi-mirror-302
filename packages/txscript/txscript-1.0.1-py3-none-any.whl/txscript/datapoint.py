...
from typing import TYPE_CHECKING, Any, Generator, Iterable, List, NoReturn, Optional, SupportsIndex, overload

from .builtins import datetime, default_to, is_empty
...
class Field(abc.ABC):
    ...
    def parent(self) -> Optional[Field]:
        ...
        if parent_schema["category"] == "multivalue":
    ...
    def from_data(schema_id: str, field_source: Fields) -> Field:
        ...
        schema = field_source._data.schema[schema_id]
        if schema["category"] == "multivalue":
            # Make referring to the multivalue id and the inner child id
            # equivalent.
            schema_id = schema["child"]
            attrs = field_source._data.objects[schema_id]
            assert attrs
...
class MultivalueDatapointField(MultivalueField):
    ...
    @needs_updates
    def append(self, value: Any) -> None:
        if self.parent and self.parent.schema["category"] == "tuple":
...
class MultivalueTupleField(MultivalueField):
...
class FieldValueBase:
    ...
    `field.date_due.datapoint.rir_confidence`.
    ...
    directly accessing the `field` object.
      * `field.amount.datapoint.id` will work.
      * `x = field.amount; x.datapoint.id` will work, but is not recommended as confusing.
      * `x = field.amount; x += 1; x.datapoint.id` will not work (x is now plain float).
    ...
    def __getattr__(self, attrname: str) -> Any:
        ...
        if attrname in ("id", "value", "rir_confidence", "options"):
            ...
            raise AttributeError(f"Consider accessing .{attrname} as .datapoint.{attrname}")
        else:
    ...
    def __getattr__(self, attrname: str) -> Any:
        if attrname == "options":
            if "options" not in self._field.attrs:
                self._field.attrs["options"] = [{**op} for op in self._field.schema["options"]]
...
class TableRow(FieldValueBase):
    ...
    def _row_formula_context(self, t: TxScriptAnnotationContent) -> Generator[TxScriptAnnotationContent, None, None]:
        ...
        try:
            yield t._with_field(self)  # type: ignore[arg-type]
        finally:
