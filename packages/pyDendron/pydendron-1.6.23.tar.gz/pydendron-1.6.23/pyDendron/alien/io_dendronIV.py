...
class IODendronIV:
    ...
    def to_dataset(self, keycode_parent='Dataset', root_idx=ROOT, trash_keycode='Trash', workshop_keycode='Workshop', clipboard_keycode='Clipboard'):
        """
        ...
        self.dataset.new_root(keycode_parent, root_idx)
    ...
    def compact(self, drop_duplicate=False, chronology_to_set=False):
        """
    ...
    def project_to_set(self, idx_root=ROOT):
        def create_set(path, project_sets):
            ...
            for p in path.parts:
                ...
                if p not in project_sets:
                    self.idx += 1
                    #perror(f"   new p: {p} {idx_parent[-1]} {self.idx}")
                    self.dataset.new(keycode=p, category=SET, idx_parent=idx_parent[-1], idx=self.idx)
                    idx_parent.append(self.idx)
                    project_sets[p] = copy.copy(idx_parent)
                else:
                    idx_parent = copy.copy(project_sets[p])
                    #perror(f"   get p: {p} {idx_parent[-1]}")
            return idx_parent
        ...
        for project, df in self.dataset.sequences.groupby(PROJECT):
            ...
            child_mask = self.dataset.components.index.get_level_values(IDX_CHILD).isin(df.index)
            root_mask = self.dataset.components.index.get_level_values(IDX_PARENT).isin([idx_root])
    ...
    def read(self, directory, keycode_parent='Dendron-IV'):
        ...
        self.compact(drop_duplicate=True, chronology_to_set=True)
        self.project_to_set()
