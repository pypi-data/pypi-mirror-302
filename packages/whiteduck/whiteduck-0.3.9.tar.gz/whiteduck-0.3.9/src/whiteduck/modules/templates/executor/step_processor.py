...
from whiteduck.modules.templates.steps.dependency_wizard_step import DependencyWizardStep
...
def process_steps(
    template: Template, steps: List[Step], variables: List[Variable], modules: List[Module], prompts: List[Prompt]
) -> None:
    """
    ...
    for step in steps:
        ...
        try:
            if step_type == "prompt_id":
                ...
                DependencyWizardStep(step, template.dependency_groups).execute(variables)
                pass
            else:
