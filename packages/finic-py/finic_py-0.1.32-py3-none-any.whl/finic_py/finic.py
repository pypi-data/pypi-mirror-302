...
class Finic:
    ...
    async def save_browser_context(self, context: Union[BrowserContext, AsyncBrowserContext], browser_id: Optional[str] = None):
        if self.environment == FinicEnvironment.LOCAL:
            ...
            if not browser_id:
                return
            if isinstance(context, AsyncBrowserContext):
                browser_context = await context.storage_state()
            else:
    ...
    async def launch_browser_async(self,  **kwargs) -> Tuple[AsyncPage, AsyncBrowserContext]:
        video_dir = os.path.join(os.getcwd(), "session_recording")
        # If it doesn't exist, create it
        if not os.path.exists(video_dir):
            os.makedirs(video_dir)
