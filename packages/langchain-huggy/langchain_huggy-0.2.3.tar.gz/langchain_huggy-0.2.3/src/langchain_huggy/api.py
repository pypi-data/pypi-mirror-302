    ...
    try:
        ...
        if request.stream:
            return StreamingResponse(
                stream_response(huggy_llm.stream, messages, request.model, request.conversation),
...
async def generate(request: GenerateRequest):
    try:
        if request.stream:
            return StreamingResponse(
                stream_response(huggy_llm.stream, request.prompt, request.model, request.conversation),
                media_type="text/event-stream"
            )
        else:
...
async def stream_response(stream_func, messages, model, conversation):
    try:
        for chunk in stream_func(messages, model_name=model, conversation=conversation):
            yield f"data: {chunk}\n\n"
        yield "data: [DONE]\n\n"
    except Exception as e:
