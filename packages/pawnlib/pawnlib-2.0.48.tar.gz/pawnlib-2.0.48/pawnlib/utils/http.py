...
from pawnlib.typing.converter import append_suffix, append_prefix, hex_to_number, FlatDict, FlatterDict, flatten, const, shorten_text, StackList, replace_path_with_suffix
...
from pawnlib.utils.log import ConsoleLoggerAdapter, AppLogger\
...
class IconRpcHelper:
    ...
    def get_balance(self, url=None, address=None, is_comma=False, is_tint=True, return_as_hex=False, return_as_decimal=False):
        """
        ...
        if is_valid_token_address(address):
            ...
            # Return the balance in hex if requested
            if return_as_hex:
                return _balance

            # Convert balance from hex to number
            ...
            if return_as_decimal:
                ...
                # print(f"---------------- balance_float={balance_float}")
                # balance_float = float(balance_float)  # If necessary, convert Decimal back to float
                # print(f"last---------------- balance_float={balance_float}")
    ...
    def check_transaction_loop(self, tx_hash=None, url=None,  is_compact=True, status=None, is_block_time=False, max_attempts=None):
        ...
        while True:
            ...
            if max_attempts is not None and count >= max_attempts:
                detailed_message = (
                    f"Reached maximum attempts ({max_attempts}) for transaction {shorten_text(tx_hash)}.\n"
                    f"Response: {resp}"
                )
                self._print_final_result(detailed_message, is_compact, resp)
...
class AsyncCallWebsocket:
    ...
    async def _connect(self, api_path: str = ""):
        ...
        pawn.console.log(f"self.ws_url={self.ws_url}")
        try:
    ...
    async def _handle_communication(self):
        try:
            # Check if on_send callback is available
            if self.on_send:
                # Call on_send (coroutine or function)
                message = await self.on_send() if asyncio.iscoroutinefunction(self.on_send) else self.on_send()

                pawn.console.log(f"{self.on_send} => {message} {type(message)}")

                if message and isinstance(message, str):
                    # Check if WebSocket is connected and not closed
                    pawn.console.log(f"closed => {self._ws.closed}")
                    if self.is_connected and self._ws and not self._ws.closed:
                        pawn.console.log(f"Sending message... {self.ws_url}")
                        await self._ws.send_str(message)
                        pawn.console.log("Message sent successfully.")
                        if self.verbose > 1:
                            self.logger.debug(f"Sent: {message}")
                    else:
                        self.logger.error("WebSocket is closed or not connected, cannot send message.")
                        await self.reconnect()  # Attempt to reconnect if WebSocket is closed
                else:
                    self.logger.warn("Message is either None or not a string.")
            ...
            while True:
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    self.logger.error(f"WebSocket connection error: {msg.data}")
                    await self.reconnect()
                    break
                else:
                    self.logger.warn(f"Unknown message type: {msg.type}")

        except aiohttp.ClientConnectionError as e:
            self.logger.error(f"WebSocket connection error during communication: {e}")
            await self.reconnect()

        except Exception as e:
            self.logger.error(f"Unexpected error during WebSocket communication: {e}")
            await self.close()
...
class AsyncGoloopWebsocket(AsyncCallWebsocket):
    ...
    def __init__(self, url, verbose=0, timeout=10, blockheight=0, sec_thresholds=4, monitoring_target=None, ignore_ssl=True, network_info=None, on_send=None, on_receive=None, logger=None):
        self.url = url
        self.verbose = verbose
        self.timeout = timeout
        self.blockheight = blockheight
    ...
    async def get_block_hash(self, tx_hash) -> int:
        """
        Get the latest block height.
        """
        response = await self.execute_rpc_call(method='icx_getBlockByHash', params={"hash": tx_hash}, return_key="result")
        return response
    ...
    async def get_preps(self):
        """
        Get P-Reps (Public Representatives) information.
        """
    ...
    async def get_tx_result(self, tx_hash, max_attempts=5):
        ...
        while attempt < max_attempts:
            ...
            if isinstance(tx_result, dict):
                ...
                self.logger.info(f"{flatten_tx_result}")
    ...
    async def execute_rpc_call(self, method=None, params: dict = {}, return_key=None, governance_address=None, return_on_error=True):
        ...
        :param return_key: If provided, return only the value of this key from the response.
        :param governance_address: If provided, execute the call against a governance address.
        :param return_on_error: Whether to return the response body even if the status code is not 200.
        """
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, data=request_data, headers=headers) as resp:
                ...
                if resp.status == 200:
                    ...
                    # Return the response even if status is not 200, if return_on_error is True
                    if return_on_error:
                        try:
                            response_json = await resp.json()
                            return self.handle_response_with_key(response_json, return_key=return_key)
    ...
    async def request_blockheight_callback(self):
        if self.blockheight == 0:
            self.blockheight = await self.get_last_blockheight()
    ...
    async def parse_blockheight(self, response=None):
        """
        Example implementation of block height parsing
        """
        response_json = json.loads(response)
        self.blockheight_now = response_json.get("height")
        self.logger.info(f"Block height parsed: {self.blockheight_now}")
