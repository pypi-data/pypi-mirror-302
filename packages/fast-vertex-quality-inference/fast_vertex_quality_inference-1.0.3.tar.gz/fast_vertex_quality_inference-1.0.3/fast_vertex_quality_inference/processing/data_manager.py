...
class tuple_manager:
	...
	def map_branch_names(self):
		...
		temps = []
		for i in range(3):
			if self.particles[i] in self.daughter_particle_names and self.particles[i] == self.daughter_particle_names[i]:
				...
				temps.append(i)
			branch_names = [branch.replace(self.daughter_particle_names[i],replace_string) for branch in branch_names]
		for i in temps:
			replace_string = f'{self.particles[i][0]}_temp_{self.particles[i][1:]}'
			branch_names = [branch.replace(replace_string,self.particles[i]) for branch in branch_names]
	...
	def smearelectronE(self, E_smearing_output, particle):
		for column in E_smearing_output.columns:
			dim = column[-1]
			self.tuple[f"{particle}_P{dim}"] = (E_smearing_output[column]*(self.tuple[f"{particle}_P{dim}_TRUE"]+1E-4))+self.tuple[f"{particle}_P{dim}_TRUE"]
	...
	def recompute_combined_particles(self, combined_particles):
		...
		for combined_particle in combined_particles:
			...
			for param in ['M','PX','PY','PZ']:
				self.tuple[f'{combined_particle}_{param}'] = compute_combined_param(daughter_info, param)
			self.tuple[f'{combined_particle}_P'] = np.sqrt(self.tuple[f'{combined_particle}_PX']**2 + self.tuple[f'{combined_particle}_PY']**2 + self.tuple[f'{combined_particle}_PZ']**2)
			self.tuple[f'{combined_particle}_PT'] = np.sqrt(self.tuple[f'{combined_particle}_PX']**2 + self.tuple[f'{combined_particle}_PY']**2)
	...
	def recompute_reconstructed_mass(self):
		...
		PE = np.sqrt(
			df[f"{i}_M"]**2 + df[f"{i}_PX"] ** 2 + df[f"{i}_PY"] ** 2 + df[f"{i}_PZ"] ** 2
		) + np.sqrt(
			df[f"{j}_M"]**2 + df[f"{j}_PX"] ** 2 + df[f"{j}_PY"] ** 2 + df[f"{j}_PZ"] ** 2
		) + np.sqrt(
			df[f"{k}_M"]**2 + df[f"{k}_PX"] ** 2 + df[f"{k}_PY"] ** 2 + df[f"{k}_PZ"] ** 2
		)
	...
	def write(self, new_branches_to_keep, output_location=None):
		...
		branch_swaps[self.intermediate] = self.intermediate_particle_name
		...
		for column in columns:
			for to_swap in list(branch_swaps.keys()):
				if column[:len(to_swap)] == to_swap:
					new_columns.append(column.replace(to_swap, branch_swaps[to_swap]))
					break
	...
	def add_branches(self, data_to_add):
		new_dataframes = []
		for branch in list(data_to_add.keys()):
			new_dataframes.append(pd.DataFrame({branch: data_to_add[branch]}))
		new_frame = pd.concat(new_dataframes, axis=1)
		self.tuple = pd.concat([self.tuple, new_frame], axis=1)


	def get_branches(self, branches, transformers=None, numpy=False, scale_factor=1.):
		data = self.tuple[branches]*scale_factor
	...
	def append_conditional_information(self):
		...
		for idx, particle in enumerate(self.particles):
			...
			self.tuple[f'{particle}_TRUEID'] = self.tuple[f'{particle}_ID_TRUE']
