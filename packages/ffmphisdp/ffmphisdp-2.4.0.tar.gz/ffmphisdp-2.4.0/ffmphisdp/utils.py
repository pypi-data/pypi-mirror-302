...
def probe_with_cache(file_path: str):
    """This helper will probe a video file and cache the result
    This is to prevent calling s3 multiple times for the same video
    ...
    """
    if file_path in probe_cache:
        return probe_cache[file_path]
    probe = ffmpeg.probe(file_path)
    probe_cache[file_path] = probe
    return probe
...
def decode_all_frames(
    ...
    probe = probe_with_cache(file_path)
