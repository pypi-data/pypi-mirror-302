...
_lily_dir_:str = __file__.removesuffix(f"{__name__.split(".")[1]}.py")
...
class Logger:
    ...
    def log(self, level, msg):
        ...
        self.text += datetime.datetime.now().strftime("[%m/%d/%Y-%H:%M:%S]") + f": Lily: %s: %s" % ({self.LOG_INFO: "info", self.LOG_WARNING: "warning", self.LOG_ERROR: "error", self.LOG_FATAL: "fatal error", self.LOG_SYSTEM: "system info"}[level], msg) + "\n"
        
        if level > self.LOG_NONE: #and self.dump:
            set_text_attr(5)
            print(f"{stamp} ", end = "", flush = True)
            set_text_attr(7)
            print("Lily: ", end = "", flush = True)
