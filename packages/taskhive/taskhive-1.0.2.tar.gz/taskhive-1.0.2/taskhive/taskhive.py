...
class TaskHive(Generic[W, R]):
        ...
        with Manager() as manager:
            ...
            if isinstance(self.works, Sized):
                ...
                with tqdm(total=total, desc=desc, position=0, *args, **kwargs) as pbar:
                    self._run_with_progress_bar(lock, availableWorkerIds, processingCallback, finishedCallback, errorCallback, pbar)
            else:
                with yaspin(text=desc) as spinner:
                    self._run_with_spinner(lock, availableWorkerIds, processingCallback, finishedCallback, errorCallback, spinner)

    def _run_with_progress_bar(self, lock, availableWorkerIds, processingCallback, finishedCallback, errorCallback, pbar):
    ...
    def _run_with_spinner(self, lock, availableWorkerIds, processingCallback, finishedCallback, errorCallback, spinner):
        if self.numWorkers == 1:
            try:
                for work in self.works:
                    result = self.workProcessingCallback(lock, availableWorkerIds, processingCallback, work)
