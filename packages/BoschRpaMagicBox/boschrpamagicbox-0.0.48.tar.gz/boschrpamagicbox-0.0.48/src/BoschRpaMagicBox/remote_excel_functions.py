...
def load_excel_data(username, password, server_ip, server_name, share_name, remote_file_path, sheet_name='Sheet1', port=445):
...
def save_excel_file(username, password, server_ip, server_name, share_name, remote_file_path, workbook, port=445):
...
def manage_workbook_sheet(username, password, server_ip, server_name, share_name, remote_file_path, rename_sheet_dict=None, add_sheet_name_list=None, delete_sheet_name_list=None,
    ...
    target_workbook, _ = load_excel_data(username, password, server_ip, server_name, share_name, remote_file_path)
    ...
    save_excel_file(username, password, server_ip, server_name, share_name, remote_file_path, target_workbook, port)
...
def save_dataframe_into_excel(username, password, server_ip, server_name, share_name, remote_file_path, sheet_name, saved_data, start_cell_location='A1',
    ...
    """
    target_workbook, target_sheet = load_excel_data(username, password, server_ip, server_name, share_name, remote_file_path, sheet_name, port)
...
def copy_worksheet(username, password, server_ip, server_name, share_name, source_file_path, source_sheet_name, target_file_path, target_sheet_name, port=445):
    ...
    Args:
        ...
        server_name(str):This is the server name (url), e.g. szh0fs06.apac.bosch.com
        share_name(str): This is the share_name of public folder, e.g. GS_ACC_CN$
        source_file_path(str): This is the public file path that file will be copied under share name folder
        target_file_path(str): This is the public file path that file will be saved under share name folder
        port(int): This is the port number of the server name
    ...
    source_workbook, source_sheet = load_excel_data(username, password, server_ip, server_name, share_name, source_file_path, source_sheet_name, port)
    target_workbook, _ = load_excel_data(username, password, server_ip, server_name, share_name, target_file_path, target_sheet_name, port)
    ...
    for row in source_sheet.iter_rows():
        for cell in row:
            ...
            if cell.has_style:
                ...
                new_cell.number_format = cell.number_format  # 复制数字格式
    ...
    save_excel_file(username, password, server_ip, server_name, share_name, target_file_path, target_workbook, port)
