...
def solve(
    ...
    Args:
        problem_file (str): Path to the optimization problem file (.mps/.qubo, either plain or .gz).
        ...
        context (str, optional): Billing context to run the job in. Defaults to "".
    ...
    """
    client = HTTPSClient(api_key=ctx.obj.api_key, server=ctx.obj.server)
...
@app.command()
def logs(ctx: Context, job_id: str, item: int = 0) -> None:
...
@app.command()
def monitor(ctx: Context, job_id: str) -> None:
...
@app.command()
def status(ctx: Context, job_id: str, item: int = 0) -> None:
...
@app.command()
def solution(ctx: Context, job_id: str, item: int = 0) -> None:
...
@app.command()
def time_billed(ctx: Context, job_id: str) -> None:
...
def main(ctx: Context) -> None:
    ...
    ctx.obj = AppState(api_key, server)
