...
class Pipeline:
    ...
    def __get_ports(self, n1 : Node):
        return [n1.__dict__()['inputs'], n1.__dict__()['outputs']]
    ...
    def __n1_is_output(self, n1 : Node, port_name1 : str, n2 : Node, port_name2 : str ):
        ...
        n1_isOutput = False
        if port_name1 in n1p[0].values():
            n1_isOutput = False
        if port_name1 in n1p[1].values():
            n1_isOutput = True
        if port_name1 not in n1p[0].values() and port_name1 not in n1p[1].values():
            raise ValueError(f'Port {port_name1} not found in {n1.__class__.__name__}.')
        if port_name2 not in n2p[0].values() and port_name2 not in n2p[1].values():
