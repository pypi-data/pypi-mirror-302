...
class ClassError(DomoError):
    ...
    def __init__(
        ...
        auth = auth or (res and res.auth) or (cls_instance and cls_instance.auth)
        ...
        entity_name = entity_name or (cls_instance and getattr(cls_instance, cls_name_attr))

        
        entity_str = f"{entity_name}"
        
        if entity_id:
            entity_str = f"{entity_id} - {entity_str}" 
