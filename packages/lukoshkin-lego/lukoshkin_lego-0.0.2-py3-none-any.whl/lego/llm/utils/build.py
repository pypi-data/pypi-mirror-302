...
from lego.llm.settings import CustomLLMChatSettings, LLMProvider, RetrialPolicy
...
def build_llm_provider(provider_alias: str) -> LLMProvider:
    """A function to register an LLM provider."""
    return LLMProvider(_env_prefix=f"{provider_alias}_")


def build_llm(
    llm_alias: str, key_idx: int, use_async: bool = False, **compl_kwargs
) -> router.ChatCompletion | router.AsyncChatCompletion:
    """
    ...
    """
    provider, model = AVAILABLE_PROVIDERS[llm_alias]
    provider = build_llm_provider(provider)
    ...
    return (
        ...
        if use_async
        else router.ChatCompletion(**router_kwargs)
    )


def build_generic_router(
    core_llm_alias: str, fallback_llm_alias: str, use_async: bool = False
    ...
    base_services = [
        build_llm(core_llm_alias, i, use_async=use_async) for i in range(3)
    ]
    fallback_services = [
        build_llm(fallback_llm_alias, i, use_async=use_async) for i in range(2)
    ]
    router_kwargs = {
