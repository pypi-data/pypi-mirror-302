...
# -*- coding: utf-8 -*-
"""
IVIS_data/CodeLibs/FoxLabel/libs/shape.py
line 144: draw_vertex: hàm vẽ hình vuông/tròn ở các góc của ROIs


trong hàm: def close_enough(self, p1, p2): IVIS_data/CodeLibs/FoxLabel/libs/canvas.py
    epsilon = 24.0 : dòng 35 IVIS_data/CodeLibs/FoxLabel/libs/canvas.py
    là khoảng cách để hiển thị cái đuòng màu đỏ lên
"""

AppVersion = "2024.08 - V1.2" 
...
# sys.path.insert(0,'IVIS_data/CodeLibs/FoxLabel')
...
class taLibs:
    def fnResize1Img(fn, Resized_W, Resized_H, ResizeType='same'):
        img = cv2.imread(fn)
        if ResizeType == 'same':
            img = cv2.resize(img, dsize=(Resized_W, Resized_H), interpolation=cv2.INTER_CUBIC)
    ...
    def LoadConfigFromfiles(ymlPath: str = 'IVIS_data/config.yml'):
        print('LoadConfigFromfiles()')
        if not exists(ymlPath):  # Create new config file
            with open('IVIS_data/system.ta', "r") as ymlfile:
                cfg0 = yaml.load(ymlfile, Loader=yaml.FullLoader)
            taLibs.fnSaveConfigFile(cfg0)
            pass
    ...
    def fnSaveConfigFile(cfg, ymlPath='IVIS_data/config.yml'):
        with open(ymlPath, 'w') as yaml_file:
    ...
    def put_text_top(img, txt, h, tbox=None, sbox=None, scolor=None):
        font = cv2.FONT_HERSHEY_SIMPLEX
        Line1 = (h, h)
        Line2 = (h, 2 * h)
        fontScale = img.shape[0] / 1200
        ...
        cv2.putText(img, txt, Line1, font, fontScale, BgColor, thickness * 2, lineType)
        cv2.putText(img, txt, Line1, font, fontScale, fontColor, thickness, lineType)
        if tbox:
            cv2.putText(img, tbox, Line2, font, fontScale, BgColor, thickness * 2, lineType)
            cv2.putText(img, tbox, Line2, font, fontScale, fontColor, thickness, lineType)
        if scolor:
            for k, [mID, scolor] in enumerate(zip(sbox, scolor)):
                ...
                cv2.putText(img, f"Box:{mID}", line, font, fontScale, (0, 0, 0), thickness * 2, lineType)
                cv2.putText(img, f"Box:{mID}", line, font, fontScale, scolor, thickness, lineType)
        return img

    def Rect(image, p1, p2, color, thickness):
        # cv2.rectangle(image, p1, p2, (255,255,255), thickness=thickness*2+4)
        cv2.rectangle(image, p1, p2, (0, 0, 0), thickness=thickness * 2)
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.setWindowTitle(f'{__appname__} -- [Dev maintain: nt.anh.fai@gmail.com] - ver: {AppVersion}]')
        ...
        open_dir = action(get_str('openDir'), self.open_dir_dialog, 'u', 'open', get_str('openDir'))
        ...
        copy_prev_bounding = action(get_str('copyPrevBounding'), self.copy_previous_bounding_boxes, 'f', 'copy')  # , get_str('copyPrevBounding'))
        copy_prev_bound_ta = action('FoxAI: Copy last (any) boxes', self.taCopyLastbox, 'Ctrl+1', 'copy last box')  # , get_str('copyPrevBounding'))

        ta_Extract_Video2imgs_then_Del_vid = action('Video => images folder', self.fnExtract_All_Frame_invideo_to_image_Folder, 'Ctrl+2', 'Extract video 2 image')  # , get_str('copyPrevBounding'))
        fnimage_Folder_2_Video = action('Images in folder => Video (Overwrite if exists)', self.fnimage_Folder_2_Video, 'Ctrl+3', 'Images folder => Video')  # , get_str('copyPrevBounding'))
        Label_All_Images_inthis_dir_same = action('Label All Images in this dir same last saved', self.fnLabel_All_Images_inthis_dir_same, 'Ctrl+9',
                                                  'Label All Images in this dir same last saved')  # , get_str('copyPrevBounding'))
        ...
        # save = action(get_str('save'), self.save_file_as,  # self.save_file,
        #               'Ctrl+S', 'save', get_str('saveDetail'), enabled=True)
        save = action(get_str('save'), self.save_file, 'S', 'save', get_str('saveDetail'), enabled=True)
        ...
        save_as = action(get_str('saveAs'), self.save_file_as, 'Ctrl+S', 'save-as', get_str('saveAsDetail'), enabled=True)
        ...
        create_mode = action(get_str('crtBox'), self.set_create_mode, '1', 'new', get_str('crtBoxDetail'), enabled=False)
        create = action(get_str('crtBox'), self.create_shape, '1', 'new', get_str('crtBoxDetail'), enabled=False)
        edit_mode = action(get_str('editBox'), self.set_edit_mode, '2', 'edit', get_str('editBoxDetail'), enabled=False)
        copy = action(get_str('dupBox'), self.copy_selected_shape, '3', 'copy', get_str('dupBoxDetail'), enabled=False)
        delete = action(get_str('delBox'), self.delete_selected_shape, 'z', 'delete', get_str('delBoxDetail'), enabled=False)
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, copy_prev_bounding,
                     copy_prev_bound_ta,
                     ta_Extract_Video2imgs_then_Del_vid,
                     fnimage_Folder_2_Video,
                     Label_All_Images_inthis_dir_same,
                     self.menus.recentFiles, save, save_format, save_as, close, reset_all, delete_image, quit))
        ...
        self.statusBar().showMessage('%s started. [Dev maintain: nt.anh.fai@gmail.com]' % __appname__)
        self.statusBar().show()
    ...
    def add_label(self, shape):
        ...
        # item.setBackground((128,0,0,128))
    ...
    def save_labels(self, annotation_file_path):
        ...
        try:
            if self.label_file_format == LabelFileFormat.PASCAL_VOC:
                ...
                fn = 'IVIS_data/CodeLibs/FoxLabel/classes.txt'
                if not exists(fn):
                    fn = 'classes.txt'
                    if not exists(fn):
                ...
                with open(fn) as ff:
                    classlist = ff.read().splitlines()
                self.label_file.save_yolo_format(annotation_file_path, shapes, self.file_path, self.image_data, classlist,  # self.label_hist,
                                                 self.line_color.getRgb(), self.fill_color.getRgb())
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            self.status(f"Loaded {os.path.basename(unicode_file_path)}, {image.width()}x{image.height()}")
            ...
            self.setWindowTitle(__appname__ + ' ' + file_path + ' ' + counter + f'  [Dev maintain: nt.anh.fai@gmail.com - ver: {AppVersion}]')
    ...
    def show_bounding_box_from_annotation_file(self, file_path, onlyCurrentDir=False):
        if onlyCurrentDir:
            self.load_yolo_txt_by_filename(file_path)
        ...
        if self.default_save_dir is not None:
            ...
            # name, ext = os.path.splitext(base_name)[0]
            # txt_path = join(dirname(file_path), name) + '.txt'
    ...
    def open_dir_dialog(self, _value=False, dir_path=None, silent=False):
        ...
        if target_dir_path == '': return
        self.last_open_dir = target_dir_path
        fis_videos = taLibs.getFiles(sPath=target_dir_path, ext=('.avi', '.mp4'))
        ...
        self.file_path = self.m_img_list[0]  # Chủ động set file path = path mới.
        if self.file_path:
    ...
    def open_prev_image(self, _value=False):
        ...
        if self.cur_img_idx - 1 >= 0:
            ...
            message_box = QMessageBox.information(self, "Begin of List", "This is first image!!!")
            return
    ...
    def open_next_image(self, _value=False):
        ...
        if self.file_path is None:
            ...
            if self.cur_img_idx + 1 < self.img_count:
                ...
                p = filename.rfind('.')
                labeltxt = filename[:p] + ".txt"

                # labeltxt = filename.replace(".jpg", ".txt")

                if not exists(labeltxt):
                    with open(labeltxt, 'w') as ff:
                        ff.write("")
            else:
                message_box = QMessageBox.information(self, "End of List", "This is last image!!!")
                return
    ...
    def open_file(self, _value=False):
        ...
        if filename:
            ...
            self.cur_img_idx = 0
            self.img_count = 1
            # TODO: TACT: Change label dir to here
            self.default_save_dir = dirname(filename)
            self.load_file(filename)

    def save_file(self, _value=False):
        self._save_file('ta_last_box.txt')
        print('saved to ta_last_box.txt')
        # filename=self.save_file_dialog(remove_ext=False)
        # filename = self.file_path.replace('.jpg', '.txt')
        # base_name = basename(self.file_path)
        # name, ext = os.path.splitext(base_name)[0]
        # filename = join(dirname(self.file_path), name) + '.txt'

        p = self.file_path.rfind('.')
        filename = self.file_path[:p] + ".txt"
    ...
    def save_file_as(self, _value=False):
        ...
        self._save_file('ta_last_box.txt')
        print('Saved as to ta_last_box.txt')
    ...
    def taCopyLastbox(self):
        prev_file_path = 'ta_last_box.txt'
        self.show_bounding_box_from_annotation_file(prev_file_path, onlyCurrentDir=True)
        # self.canvas.set_editing()
        self.set_dirty()

    def fnExtract_All_Frame_invideo_to_image_Folder(self):
        path = os.path.dirname(ustr(self.file_path)) if self.file_path else '.'
        filters = "Video files (*.avi *.mp4);;All Files (*)"
        filename, _ = QFileDialog.getOpenFileName(self, '%s - Choose Video File' % __appname__, path, filters)
        if filename:
            if isinstance(filename, (tuple, list)):
                filename = filename[0]
            ifn = filename.replace('.avi', '')
            ifn = ifn.replace('.mp4', '')

            os.makedirs(ifn, exist_ok=True)
            vidcap = cv2.VideoCapture(filename)
            success, image = vidcap.read()
            cnt = 1
            while success:
                cv2.imwrite(ifn + f"/{basename(ifn)}__{cnt:06d}.jpg", image)
                success, image = vidcap.read()
                cnt += 1
            vidcap.release()
            os.startfile(ifn)

    def fnLabel_All_Images_inthis_dir_same(self):
        prev_file_path = 'ta_last_box.txt'

        print("Đánh nhãn hết các file chưa có label, dùng nhãn save lần cuối, đánh trong thư mục sau:")
        print(self.default_save_dir)
        with open(prev_file_path, 'r') as fr:
            contents = fr.read()
            fis=[]  
            for D, _, F in os.walk(self.default_save_dir):
                for fn in F:
                    if fn.endswith(('.jpg', '.jpeg', '.png')):
                        ext = fn.split('.')[-1]
                        ffn_txt = fn.replace(ext, 'txt')
                        ffn_txt = join(self.default_save_dir, ffn_txt)
                        fis.append(ffn_txt)
            for ffn_txt in fis:
                if not exists(ffn_txt):
                    with open(ffn_txt, 'w') as fw:
                        fw.write(contents)
        print("Done!")
        pass

    def fnimage_Folder_2_Video(self):
        path = os.path.dirname(ustr(self.file_path)) if self.file_path else '.'
        filters = "JPG Files (*.jpg);;All Files (*)"
        filename, _ = QFileDialog.getOpenFileName(self, '%s - Choose a image File' % __appname__, path, filters)
        if filename:
            if isinstance(filename, (tuple, list)):
                filename = filename[0]
            image_folder = dirname(filename)
            video_name = image_folder + '.avi'
            print('Save video to:', video_name)
    ...
    def copy_previous_bounding_boxes(self, onlyCurrentDir=False):
        # try:
...
def get_main_app(image_dir='', argv=None):
    """
    ...
    argparser.add_argument("image_dir", nargs="?", default=image_dir)
    argparser.add_argument("class_file",
                           default=os.path.join(os.path.dirname(__file__), "data", "predefined_classes.txt"),
                           nargs="?")
    argparser.add_argument("save_dir", nargs="?", default=dirname(image_dir))
...
def Fmain(image_dir=''):
    ...
    print("Start!")
    app, _win = get_main_app(image_dir, sys.argv)
    return app.exec_()
