...
class TorqueCurrentFOC:
    ...
    def __init__(self, output: ampere, max_abs_duty_cycle: float = 1.0, deadband: ampere = 0.0, override_coast_dur_neutral: bool = False, limit_forward_motion: bool = False, limit_reverse_motion: bool = False, use_timesync: bool = False):
    ...
    def _send_request(self, network: str, device_hash: int) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlTorqueCurrentFOC(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, self.output, self.max_abs_duty_cycle, self.deadband, self.override_coast_dur_neutral, self.limit_forward_motion, self.limit_reverse_motion, self.use_timesync))
