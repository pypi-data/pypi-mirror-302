# -*- coding: utf-8 -*-
"""
torchgdm field propgation: near-fields and far-fields
"""
# %%
import warnings
...
def _nearfield(
    ...
    **kwargs,
    ...
    # clean up
    if e_sca.device.type.startswith("cuda"):
...
def _nearfield_h(
    ...
    **kwargs,
    ...
    if h_sca.device.type.startswith("cuda"):
        torch.cuda.empty_cache()
...
@batched(batch_kwarg="r_probe", arg_position=2, out_dim_batch=1, default_batch_size=256)
def _field_grad(
    sim,
    wavelength: float,
    r_probe: torch.Tensor,
...
def ff(
    ...
    **kwargs,
    ...
    if r_probe is None:
        from torchgdm.tools.geometry import coordinate_map_spherical

        r_probe = coordinate_map_spherical()  # full hemisphere
        _r = r_probe["r_probe"]
    elif type(r_probe) == dict:
        _r = r_probe["r_probe"]
    else:
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        surface = tg.tools.geometry.coordinate_map_square_2d(200, 21, 300)
    ...
    surface_spherical = tg.tools.geometry.coordinate_map_spherical_lower()
    t0 = time.time()
