...
from fluidattacks_core.types.dynamo.items import (
    ...
    StakeholderMetadataItem,
    ToeInputMetadataItem,
    VulnerabilityMetadataItem,
...
class OrganizationFaker(metaclass=MetaBuilder):
    ...
    creation_date: str = FIXED_DATE_2024
    ...
    def build(self) -> OrganizationMetadataItem:
        return {
...
class GroupFaker(metaclass=MetaBuilder):
    ...
    set_policies: Callable[[GroupPolicies], Self]
    ...
    def build(self) -> GroupMetadataItem:
        return {
...
class VulnerabilityFaker(metaclass=MetaBuilder):
    ...
    report_date: str = FIXED_DATE_2024
    source: str = "MACHINE"
    priority: int = 125
    efficacy: Decimal = Decimal("0")
    ...
    set_efficacy: Callable[[Decimal], Self]
    ...
    def build(self) -> VulnerabilityMetadataItem:
        ...
        if self.bug_tracking_system_url:
            result["bug_tracking_system_url"] = self.bug_tracking_system_url

        if self.severity_level:
            result["severity_score"] = fake_severity_score(self.severity_level)

        if self.zero_risk:
            result["zero_risk"] = self.zero_risk
...
class FindingFaker(metaclass=MetaBuilder):
    ...
    unreliable_indicators: FindingUnreliableIndicators = {
        ...
        "unreliable_newest_vulnerability_report_date": FIXED_DATE_2024,
        "unreliable_verification_summary": {
            "verified": 0,
            "requested": 0,
            "on_hold": 0,
        },
        "unreliable_oldest_vulnerability_report_date": FIXED_DATE_2024,
        "unreliable_oldest_open_vulnerability_report_date": FIXED_DATE_2024,
    }
    ...
    set_unreliable_indicators: Callable[[FindingUnreliableIndicators], Self]
    set_evidences: Callable[[FindingEvidences], Self]

    def __init__(self, finding_id: str, group_name: str) -> None:
        self.finding_id = finding_id
        self.group_name = group_name

    def build(self) -> FindingMetadataItem:
        return {
...
class GitRootFaker(metaclass=MetaBuilder):
    ...
    machine_executions_date: MachineExecutionsDate = {
        "apk": FIXED_DATE_2024,
        "cspm": FIXED_DATE_2024,
        "dast": FIXED_DATE_2024,
        "sast": FIXED_DATE_2024,
        "sca": FIXED_DATE_2024,
    }
    ...
    set_machine_executions_date: Callable[[MachineExecutionsDate], Self]
    ...
    def build(self) -> GitRootMetadataItem:
        return {
...
class EnvironmentUrlFaker(metaclass=MetaBuilder):
    ...
    set_cloud_name: Callable[[str], Self]
    set_use_egress: Callable[[bool], Self]
    set_use_vpn: Callable[[bool], Self]
    set_use_ztna: Callable[[bool], Self]
    ...
    def build(self) -> RootEnvironmentUrlItem:
        return {
...
class ToeInputFaker(metaclass=MetaBuilder):
    ...
    be_present_until: str = FIXED_DATE_2024
    seen_first_time_by: str = CREATED_BY
    seen_at: str = FIXED_DATE_2024
    attacked_by: str = CREATED_BY
    attacked_at: str = FIXED_DATE_2024
    first_attack_at: str = FIXED_DATE_2024
    modified_by: str = CREATED_BY
    modified_date: str = FIXED_DATE_2024
...
class StakeholderFaker(metaclass=MetaBuilder):
    # Required attributes
    email: str
    first_name: str
    last_name: str
    ...
    # Optional attributes
    enrolled: bool = True
    last_login_date: str = FIXED_DATE_2024
    is_registered: bool = True
    is_concurrent_session: bool = False
    legal_remember: bool = True
    registration_date: str = FIXED_DATE_2024

    # Setters
    # Meta methods must be defined for linting support
    set_enrolled: Callable[[bool], Self]
    set_last_login_date: Callable[[str], Self]
    set_is_registered: Callable[[bool], Self]
    set_is_concurrent_session: Callable[[bool], Self]
    set_legal_remember: Callable[[bool], Self]
    set_registration_date: Callable[[str], Self]
    ...
    def build(self) -> StakeholderMetadataItem:
        return {
            ...
            "is_concurrent_session": self.is_concurrent_session,
            "is_registered": self.is_registered,
            "jira_client_key": "",
            "last_login_date": self.last_login_date,
            "last_name": self.last_name,
            "legal_remember": self.legal_remember,
            "login": {
                "modified_by": CREATED_BY,
                "modified_date": FIXED_DATE_2024,
                "expiration_time": 999999,
                "browser": "chrome",
                "country_code": "054",
                "device": "test-device",
...
class OrganizationAccessFaker(metaclass=MetaBuilder):
    # Required attributes
    email: str
    organization_id: str
    role: str
    ...
    # Setters
    # Meta methods must be defined for linting support
    set_has_access: Callable[[bool], Self]
    set_modified_date: Callable[[str], Self]
    set_modified_by: Callable[[str], Self]
    ...
    def build(self) -> OrganizationAccessItem:
        return {
            "pk": f"USER#{self.email}",
            "email": self.email,
            "organization_id": self.organization_id,
            "sk": f"ORG#{self.organization_id}",
            "state": {
                "modified_date": self.modified_date,
                "modified_by": self.modified_by,
