...
try:
    ...
    class SimSwerveDrivetrain:
        ...
        def update(self, dt: second, supply_voltage: volt, modules_to_apply: list[SwerveModule]):
            ...
            states: list[SwerveModuleState] = [SwerveModuleState() for _ in modules_to_apply]
            for i in range(0, len(self._modules)):
                drive_motor = modules_to_apply[i].drive_motor.sim_state
                steer_motor = modules_to_apply[i].steer_motor.sim_state
                cancoder = modules_to_apply[i].cancoder.sim_state

                drive_motor.orientation = (
                    ChassisReference.Clockwise_Positive
                    if self._modules[i].drive_motor_inverted
                    else ChassisReference.CounterClockwise_Positive
                )
                steer_motor.orientation = (
                    ChassisReference.Clockwise_Positive
                    if self._modules[i].steer_motor_inverted
                    else ChassisReference.CounterClockwise_Positive
                )
                ...
                self._modules[i].drive_motor.setInputVoltage(
                    self.add_friction(drive_motor.motor_voltage, self._modules[i].drive_friction_voltage)
                )
                self._modules[i].steer_motor.setInputVoltage(
                    self.add_friction(steer_motor.motor_voltage, self._modules[i].steer_friction_voltage)
                )

                self._modules[i].drive_motor.update(dt)
                self._modules[i].steer_motor.update(dt)

                drive_motor.set_raw_rotor_position(self._modules[i].drive_motor.getAngularPosition() / (2 * math.pi) * self._modules[i].drive_gearing)
                drive_motor.set_rotor_velocity(self._modules[i].drive_motor.getAngularVelocity() / (2 * math.pi) * self._modules[i].drive_gearing)

                steer_motor.set_raw_rotor_position(self._modules[i].steer_motor.getAngularPosition() / (2 * math.pi) * self._modules[i].steer_gearing)
                steer_motor.set_rotor_velocity(self._modules[i].steer_motor.getAngularVelocity() / (2 * math.pi) * self._modules[i].steer_gearing)

                # CANcoders see the mechanism, so don't account for the steer gearing
                cancoder.set_raw_position(self._modules[i].steer_motor.getAngularPosition() / (2 * math.pi))
                cancoder.set_velocity(self._modules[i].steer_motor.getAngularVelocity() / (2 * math.pi))

                states[i] = modules_to_apply[i].get_current_state()
