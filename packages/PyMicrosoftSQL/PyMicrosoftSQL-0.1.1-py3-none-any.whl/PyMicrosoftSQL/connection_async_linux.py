...
libodbcinst_path  = os.path.join(lib_dir, "libodbcinst.so.2")
libmsodbcsql_path = os.path.join(lib_dir, "libmsodbcsql-18.4.so.1.1")

libodbcinst = ctypes.CDLL(libodbcinst_path)
odbc = ctypes.CDLL(libmsodbcsql_path)

async_event_mgr_obj = AsyncSQLEventManager(odbc.SQLDriverConnectW, odbc.SQLExecDirectW)

class ConnectionAsyncLinux:
    ...
    def _allocate_environment_handle(self):
        ret = odbc.SQLAllocHandle(const.SQL_HANDLE_ENV, None, ctypes.byref(self.henv))
        self._check_ret(ret, const.SQL_HANDLE_ENV, self.henv)

    def _set_environment_attributes(self):
        ret = odbc.SQLSetEnvAttr(self.henv, const.SQL_ATTR_ODBC_VERSION, ctypes.c_void_p(const.SQL_OV_ODBC3_80), 0)
        self._check_ret(ret, const.SQL_HANDLE_ENV, self.henv)
    ...
    def _connect_to_server(self):
        ...
        if ret == const.SQL_STILL_EXECUTING:
            # self.async_lib.wait(conn_event)
            conn_event.wait()
