    ...
    def del_helpers_entities(self):
        ...
        for configuration in self.all_configs.helpers_entities:
            full_cfg = self.file_io.read_with_type(path=
                configuration.path + configuration.filename, file_type=configuration.type_cfg
            )

            try:
                automation_editor = AutomationsEditor(
                    configuration=configuration, full_cfg=full_cfg, host_cfg=self.all_configs.host_info
                )
                new_cgf, ids_list = automation_editor.delete_helpers_entities(helpers_list=ids_list)
