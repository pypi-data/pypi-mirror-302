...
pub fn get_normalized_imports(
    ...
    if include_string_imports {
        ...
        result_imports.extend(import_visitor.normalized_imports.into_iter());
        result_imports.extend(string_import_visitor.normalized_imports.into_iter());

        return Ok(result_imports);
    } else {
        return Ok(import_visitor.normalized_imports);
    }
}
