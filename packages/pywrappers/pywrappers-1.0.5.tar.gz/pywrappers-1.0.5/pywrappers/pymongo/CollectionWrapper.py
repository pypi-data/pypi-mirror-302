...
class CollectionWrapper:
    ...
    # --- Update Methods ---   
    def update_one(self, filter: Dict[str, Any], update: Dict[str, Any], upsert: bool = False, replace: bool = False) -> UpdateResult:
        """Update a single document matching the filter."""
        if not replace and not any(key.startswith('$') for key in update):
            update = {"$set": update}
        return self.collection.update_one(filter, update, upsert=upsert)

    def update_many(self, filter: Dict[str, Any], update: Dict[str, Any], upsert: bool = False, replace: bool = False) -> UpdateResult:
    ...
    def update(self, spec: Dict[str, Any], document: Dict[str, Any], upsert: bool = False, multi: bool = False, replace: bool = False) -> UpdateResult:
        """
        ...
        """
        if not replace and not any(key.startswith('$') for key in document):
