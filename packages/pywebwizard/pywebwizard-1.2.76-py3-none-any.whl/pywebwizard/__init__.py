...
from .utils.functions import required_fields, is_valid_url, action
    ...
    def _register_actions(self):
        for name in dir(self):
            ...
            if callable(attr) and hasattr(attr, '_action_name'):
                ...
                self.actions_mapping[action_name] = attr
    ...
    def _get_keepass_entry_password(self, entry_path):
        if self.keepass_db:
            ...
            if entry:
                ...
                logging.error(f"[KEEPASS] Entry '{entry_name}' doesn't exist in group '{current_group.name}' ({entry_path}).")
                return ""
        else:
...
class PyWebWizard(PyWebWizardBase):
    ...
    def __init__(self, spell_file: str, spell_book: list = None, headers: dict = None,
        ...
        """
        super().__init__(spell_file, headers, destruct_on_finish, external_functions_mapping, keepass_file, keepass_password)
