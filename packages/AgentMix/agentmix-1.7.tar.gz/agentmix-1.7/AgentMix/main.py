...
class OmniAgent:
    # Define your agent's initialization and operations here
    async def run(self):
        print("Running AgentMix...")

def main():
    if not get_api_key():
        print(f"{Color.YELLOW}Please enter your Groq API key in the config file or when prompted.{Color.RESET}")
        ...
        while True:
            ...
            if self.current_agent.name == "Coder":
                ...
                if save_file == 'y':
                    ...
                    if not os.path.exists(self.coder_files_dir):
                        os.makedirs(self.coder_files_dir)
