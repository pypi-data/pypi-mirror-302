...
from langchain_huggy.core._base_llm import BaseHuggyLLM
from langchain_huggy.utils.const import AVAILABLE_MODEL_LIST
...
class HuggyLLM():
    ...
    def _update_dependencies(self,model_name,messages,conversation):
        ...
        if not conversation: #Default conversation is False
            self.system_prompt = curr_sys_prompt if curr_sys_prompt!=self.system_prompt else self.system_prompt
