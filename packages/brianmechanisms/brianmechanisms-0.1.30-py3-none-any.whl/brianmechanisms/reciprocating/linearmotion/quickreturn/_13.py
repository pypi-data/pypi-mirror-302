...
class mechanism13:
    ...
    def Settings(options, center_distance = 0, fig = None, ax = None, grid=False, rotations = 1, speed = 0.5):
        ...
        for index, instance in enumerate(options):
            ...
            instance["radius2"] = mechanism13.calculate_radius2(instance.get('radius1', 0), forward_angle)
    ...
    def plot(self, instance_number=None):
        ...
        ax.set_title(f'Mechanism 13 (n={len(instances)})')
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.grid(True)
        ax.axis('off')
    ...
    def update(self, interval=100, frames=100):
        ...
        def animate(frame):
            ...
            for inst in self.settings['instances']:
                ...
                x_end_circle2 = radius2 * np.cos(stop_angle_2 + angle) + origin+ rack_displacement
                y_end_circle2 = radius2 * np.sin(stop_angle_2 + angle)
                x_start_circle2 = radius2 * np.cos(start_angle_2 + angle) + origin+ rack_displacement
                y_start_circle2 = radius2 * np.sin(start_angle_2 + angle)
                ...
                ax.plot([-inst["x_limit"] / 2 + origin , inst["x_limit"] / 2 + origin ], [radius1, radius1], color='blue', linestyle='--')
                ax.plot([-inst["x_limit"] / 2 + origin , inst["x_limit"] / 2 + origin ], [-radius2, -radius2], color='red', linestyle='--')
                ax.plot([-inst["x_limit"] / 2 + origin , -inst["x_limit"] / 2 + origin ], [radius1, -radius2], color='black', linestyle='-')
                ax.plot([inst["x_limit"] / 2 + origin , inst["x_limit"] / 2 + origin ], [radius1, -radius2], color='black', linestyle='-')

                ax.plot(origin+rack_displacement, 0, 'ko')

            ax.set_aspect('equal', adjustable='box')
            ax.set_title(f'Mechanism 13 (n={len(self.settings["instances"])})')
            ax.set_xlabel('X')
            ax.set_ylabel('Y')
            ax.grid(True)
