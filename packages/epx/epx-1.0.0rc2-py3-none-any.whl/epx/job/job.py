...
class Job:
    ...
    def _copy_and_rename_file(
        ...
        """Copy and rename ref files"""

        for file in ref_files:
            if os.path.isfile(ref_files[file]):
                shutil.copy(ref_files[file], f"{output_path}/{file}")
            else:
                raise RuntimeError(f"Not found the file with name = {file}")
