"""
Code for SQLite3 operations

Functions:
- create_db(db_path, script)
- get_connection(db_path, row_factory)
- fetch_all(db_path, query, data)
- fetch_one(db_path, query, data)
- insert(db_path, query, data)
"""
...
from .queries import song_count_by_product

def create_db(db_path: str, script: str):
    """Builds the project database tables and inserts data if it's part of the build script.

    Warning: pass the script as a string from open(path/to/file).read(). Don't pass the file path!

    Parameters:
    db_path (str): full path to the project database
    script (str): the SQL script used to build the database
