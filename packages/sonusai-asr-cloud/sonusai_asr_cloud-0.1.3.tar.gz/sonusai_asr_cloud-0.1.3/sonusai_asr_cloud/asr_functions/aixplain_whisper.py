...
def aixplain_whisper(audio: AudioT, **config) -> ASRResult:
    ...
    if model_name is None:
        raise SonusAIError("config must specify 'model'")

    env_var = "AIXP_WHISPER_" + model_name.upper()
    model_key = getenv(env_var)
    if model_key is None:
        raise SonusAIError(f"{env_var} environment variable does not exist")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        while True:
            try:
                ...
                if count >= retry:
                    raise SonusAIError(f"Whisper exception: {e.args}") from e
