...
def _get_node_edge_dict(label_array, edge_array, label, dilate_xy, dilate_z):
    ...
    label_array = nettracer.dilate_3D(label_array, dilate_xy, dilate_xy, dilate_z) #Dilate the label to see where the dilated label overlaps
    ...
    edge_array = nettracer.remove_zeros(edge_array)  # Remove zeros
    edge_array = len(edge_array)
...
def quantify_edge_node(nodes, edges, search = 0, xy_scale = 1, z_scale = 1, resize = None):
    ...
    edge_quants = create_node_dictionary(nodes, edges, num_nodes, dilate_xy, dilate_z) #Find which edges connect which nodes and put them in a dictionary.
    
    network_analysis.save_singval_dict(edge_quants, 'NodeID', 'Edge Skele Quantity', 'edge_node_quantity.xlsx')