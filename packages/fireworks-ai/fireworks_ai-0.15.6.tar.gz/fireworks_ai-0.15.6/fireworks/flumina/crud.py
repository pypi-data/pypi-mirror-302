...
async def list_models(account_id: str, pageToken: Optional[str] = None) -> Dict[str, Any]:
...
async def create_model(account_id: str, model_id: str, model_data: Dict[str, Any]) -> Dict[str, Any]:
    url = f"https://api.fireworks.ai/v1/accounts/{account_id}/models"
    headers = {
        "Authorization": f"Bearer {get_api_key()}",
...
async def get_model_upload_endpoint(account_id: str, model_id: str, filename_to_size: Dict[str, int]) -> Dict[str, Any]:
...
async def list_deployments(account_id: str, pageToken: Optional[str] = None) -> Dict[str, Any]:
...
async def get_deployment(account_id: str, deployment_id: str) -> Dict[str, Any]:
    url = f"https://api.fireworks.ai/v1/accounts/{account_id}/deployments/{deployment_id}"
    headers = {
        "Authorization": f"Bearer {get_api_key()}"
    }
...
async def create_deployment(account_id: str, deployment_data: Dict[str, Any]) -> Dict[str, Any]:
    url = f"https://api.fireworks.ai/v1/accounts/{account_id}/deployments"
    headers = {
        "Authorization": f"Bearer {get_api_key()}",
        "Content-Type": "application/json"
...
async def delete_deployment(account_id: str, deployment_id: str, hard: Optional[bool] = None) -> Dict[str, Any]:
    url = f"https://api.fireworks.ai/v1/accounts/{account_id}/deployments/{deployment_id}"
    headers = {
        "Authorization": f"Bearer {get_api_key()}"
    }
