...
from maspy.learning.ml_utils import RecordConstructorArgs
from maspy.learning.core import Model, Wrapper, ActType, ObsType, RenderFrame
...
class RenderCollection(
    ...
    def __init__(
        self,
        env_model: Model[ObsType, ActType],
        ...
        assert env_model.model_render_mode is not None
        assert not env_model.model_render_mode.endswith("_list")

        self.frame_list: list[RenderFrame] = [] # type: ignore
        self.pop_frames = pop_frames
        self.reset_clean = reset_clean

        self.metadata = deepcopy(self.env_model.metadata)
        if f"{self.env_model.model_render_mode}_list" not in self.metadata["render_modes"]:
            self.metadata["render_modes"].append(f"{self.env_model.model_render_mode}_list")
...
class RecordVideo(Wrapper[ObsType, ActType, ObsType, ActType], RecordConstructorArgs):
    ...
    def __init__(
        ...
        if env_model.model_render_mode in {None, "human", "ansi"}:
            raise ValueError(
                f"Render mode is {env_model.model_render_mode}, which is incompatible with RecordVideo.",
                "Initialize your environment with a render_mode that returns an image, such as rgb_array.",
            )

        assert episode_trigger is not None or step_trigger is not None, "Must specify at least one trigger"
        ...
        self.recorded_frames: list[RenderFrame] = [] # type: ignore[valid-type]
        self.render_history: list[RenderFrame] = [] # type: ignore[valid-type]
    ...
    def step(
        ...
        obs, rew, terminated, truncated, info = self.env_model.step(action)
        self.step_id += 1
    ...
    def render(self) -> RenderFrame | list[RenderFrame]:
        ...
        render_out: RenderFrame | list[RenderFrame] | None = super().render() 
        assert render_out is not None, "render() returned None"
    ...
    def stop_recording(self):
        ...
        if len(self.recorded_frames) == 0:
            ...
            print("No video saved because i don't want to install moviepy.")
    ...
    def __del__(self):
        ...
        if len(self.recorded_frames) > 0:
            print("Unable to save last video! Did you call close()?")


class HumanRendering(Wrapper[ObsType, ActType, ObsType, ActType], RecordConstructorArgs):
    ...
    def __init__(self, env_model: Model[ObsType, ActType]):
        ...
        assert (
            self.env_model.model_render_mode in self.ACCEPTED_RENDER_MODES
        ), f"Expected env.render_mode to be one of {self.ACCEPTED_RENDER_MODES} but got '{env_model.model_render_mode}'"
        assert (
            "render_fps" in self.env_model.metadata
