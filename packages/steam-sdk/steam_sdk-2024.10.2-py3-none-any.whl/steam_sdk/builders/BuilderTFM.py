...
class BuilderTFM:
    ...
    def __assignTurnsToSections(self):
        ...
        self.__setAttribute(self.General, 'el_order_sections', np.array(list(dict.fromkeys(el_order_sections))).astype(int))
        self.__setAttribute(self.General, 'el_order_to_apertures', sections_to_apertures[np.array(list(dict.fromkeys(el_order_sections))).astype(int)-1])
    ...
    def __calculate_Inductance_Sections(self) -> dict:
        ...
        self.__setAttribute(self.General, 'L_mag', np.sum(M_block))
    ...
    def change_coupling_parameter(self, output_path: str):
        ...
        if self.Options.flag_PC:
            ...
            self.General.lib_path = change_library_EqLoop(self.General.lib_path, 'PC', self.frequency, self.PC.L, np.array([]), self.PC.M, groups=groups,force_new_name=self.General.lib_path)
        ...
        if self.Options.flag_IFCC:
            ...
            self.General.lib_path = change_library_EqLoop(self.General.lib_path, 'IFCC', self.frequency, self.IFCC.L, self.IFCC.R, self.IFCC.M, groups=groups,force_new_name=self.General.lib_path)
