...
class ganada:
	...
	def delete_from_doc_start_to_nth_char_end(self, input_no):
		...
		if input_no > 0:
			self.selection.MoveRight(Unit=self.vars["letter_type_vs_enum"]["char"], Count=input_no, Extend=1)  # Extend = 0 은 이동을 시키는 것이다
		else:
	...
	def delete_from_doc_start_to_nth_word_end(self, input_no):
		...
		self.move_cursor_to_doc_start()
		if input_no > 0:
			self.selection.MoveRight(Unit=self.vars["letter_type_vs_enum"]["word"], Count=input_no, Extend=1)  # Extend = 0 은 이동을 시키는 것이다
		else:
	...
	def draw_line_color_for_table(self, table_obj, inside_color="bla", outside_color="bla"):
		...
		"""
		table_obj.Borders.InsideColorIndex = self.vars["colorname_vs_enum"][inside_color]
		table_obj.Borders.OutsideColorIndex = self.vars["colorname_vs_enum"][outside_color]
	...
	def draw_line_style_for_table(self, table_obj, inside_line="-", outside_line="-"):
		...
		"""
		table_obj.Borders.InsideLineStyle = self.vars["line_vs_enum"][inside_line]
		table_obj.Borders.OutsideLineStyle = self.vars["line_vs_enum"][outside_line]
	...
	def find_text_all_in_selection_with_colored(self, search_text):
		"""
	...
	def find_text_in_selection_start_only(self, search_text):
		"""
	...
	def get_xy_n_text_for_nth_word_in_doc(self, input_no):
		...
		jf_result = self.jf.search_all_by_jf_sql("[공백:1~]",result)
	...
	def insert_no_colored_table_at_selection_end(self, x_no, y_no):
		"""
		추가하기 : 커서위치에 테이블삽입
	...
	def insert_table_at_para_end(self, para_no, table_xy=[5, 5]):
		"""
		추가하기 : 없애도 되는 것
		선택한 문단뒤에 테이블을 만든다
		형태적인 분류 - active_doc(화일) > sentence(문장) > word(한 단어) > character(한글자)
	...
	def insert_table_at_selection_end(self, x_no, y_no):
		"""
		추가하기 : 커서위치에 테이블삽입
	...
	def make_new_shape(self, shape_name=None, pxy_list=None, size_whlist=[100, 100], transparency_0_1=None,
		...
		:return:
		"""
		shape_enum = self.vars["shape name_vs_ enum"][shape_name]
	...
	def make_new_table(self, row_line_no=3, col_line_no=3):
		"""
		만들기 : 기본적인 형태의 테이블 객체를 만든다

		:param row_line_no:
		:param col_line_no:
		"""

		new_table_obj = self.doc.Tables.Add(Range=self.selection.Range, NumRows=row_line_no,
											NumColumns=col_line_no,
											DefaultTableBehavior=0, AutoFitBehavior=0)
		new_table_obj.Cell(1, 3).range.ParagraphFormat.Alignment = 0

		return new_table_obj

	def make_new_table_with_black_line(self, row_line_no=3, col_line_no=3):
		"""
		만들기 : 검정색으로 테이블 만들기
		...
		for no in [-1, -2, -3, -4, -5, -6]:
			new_table_obj.Borders(no).LineStyle = 1
		new_table_obj.Rows.Height = 10
		return new_table_obj
	...
	def move_cursor_from_doc_start_to_nth_letter_type_start(self, letter_type, input_no):
		...
		if input_no > 0:
			if letter_type_no in [3, 2, 1]:
				self.selection.MoveRight(Unit=letter_type_no, Count=input_no) #self.vars["letter_type_vs_enum"]["char"]
			else:
				self.selection.MoveDown(Unit=letter_type_no, Count=input_no)
		else:
			if letter_type_no in [3, 2, 1]:
	...
	def paint_highlight_for_selection(self, input_color):
		...
		:param input_color: 색이름
		"""
		self.selection.Range.HighlightColorIndex = self.vars["colorname_vs_enum"][input_color]

	def paint_highlight_for_selection_by_1_10(self, input_no):
		"""
		...
		self.selection.Range.HighlightColorIndex = color_no_list[input_no - 1]
	...
	def paint_highlight_from_char_no1_to_char_no2(self, input_no1, input_no2, input_color="blu"):
		...
		my_range.HighlightColorIndex = self.vars["colorname_vs_enum"][input_color]
	...
	def select_from_selection_end_to_one_line_end(self, input_no=1):
		...
		"""
		self.selection.Collapse(0)  # 0 : end, 1:start
		self.word_application.Selection.MoveDown(Unit=self.vars["letter_type_vs_enum"]["line"], Count=input_no, Extend=1)
	...
	def write_text_in_selected_shape(self, input_text=None, font_size=None, font_bold=None):
		"""
		...
		img = self.selection.InlineShapes(1)
		shape = img.ConvertToShape()
		if input_text: shape.TextFrame.TextRange.Text = input_text
		if font_size: shape.TextFrame.TextRange.Font.Size = font_size
		if font_bold: shape.TextFrame.TextRange.Font.Bold = font_bold

	def write_text_in_table_at_nth_cell(self, table_no, input_no=1, input_text=""):
		"""
		글쓰기 : 테이블의 n번째 셀에 값넣기
		...
		print(x, y)
		table.Cell(x, y).Range.Text = str(input_text)

	def write_text_in_table_at_xy_cell(self, input_table_no, xy, input_text):
		"""
