...
from .utils import create_dir_tree, dummy_fn
...
def _get_storage_instance(fs_class: type[fsspec.AbstractFileSystem]):
    class Storage(fs_class):
        ...
        def get(self, remote_path, local_path, recursive=False):
            ...
            """
            self.log_fn(
                msg=f"Copying remote file(s) at {remote_path} to local at {local_path}"
            )
            super().get(remote_path, local_path, recursive)

        def get_file(self, rpath, lpath, **kwargs):
            create_dir_tree(fsspec.filesystem("file"), lpath)
            return super().put_file(rpath, lpath, **kwargs)
        ...
        def put_file(self, lpath, rpath, **kwargs):
            ...
            return super().put_file(lpath, rpath, **kwargs)
        ...
        def _remove_prefix(
            ...
            if remove_root_folder is True:
                files = [file.partition("/")[2] for file in files]

            if remove_prefix is not None:
                files = [file.replace(remove_prefix, "", 1) for file in files]
        ...
        def list_files(
            ...
            detail : bool, optional
                If True, return a list of dictionaries with details about the files,
                by default False.
        ...
        def glob_files(
            self, pattern, remove_root_folder=True, remove_prefix=None, detail=False
        ):
            ...
            """
            self.log_fn(msg=f"Listing remote files matching {pattern}")
