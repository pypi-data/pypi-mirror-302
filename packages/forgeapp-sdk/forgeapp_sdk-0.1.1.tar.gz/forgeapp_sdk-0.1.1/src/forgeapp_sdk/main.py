...
from forgeapp_sdk import superjson
...
from .handlers import ForgeActionHandler, ForgePageHandler, IOResponseHandler
from .types import ForgeError, NotInitializedError
...
forge_context_var: ContextVar[
    tuple[IO, Union[ActionContext, PageContext]]
] = ContextVar("forge_context_var")
...
class Forge:
    class Routes:
        ...
        def add(self, slug: str, action_or_page: Union[Action, Page]):
            if isinstance(action_or_page, Page):
                action_or_page._on_change = self._forge._handle_routes_change

            self._forge._routes[slug] = action_or_page
            self._forge._handle_routes_change()

        def remove(self, slug: str):
            try:
                action_or_page = self._forge._routes[slug]
                if isinstance(action_or_page, Page):
                    action_or_page._on_change = None

                del self._forge._routes[slug]
                self._forge._handle_routes_change()
            except KeyError:
                pass

    _logger: Logger
    _endpoint: str = "wss://forgeapp.io/websocket"
    _http_endpoint: str
    _api_key: str

    _retry_forge_seconds: float = 3
    _max_resend_attempts: int = 10
    _send_timeout_seconds: float = 5
    _ping_timeout_seconds: float = 5
    _ping_forge_seconds: float = 30
    ...
    _action_handlers: dict[str, ForgeActionHandler]
    _page_handlers: dict[str, ForgePageHandler]
    _on_error: Optional[Callable[[ForgeErrorProps], None]]
    ...
    def __init__(
        ...
        *,
        ...
        on_error: Optional[Callable[[ForgeErrorProps], None]] = None,
    ):
        ...
        self._retry_forge_seconds = retry_forge
        ...
        self._ping_forge_seconds = ping_forge
    ...
    def _walk_routes(self):
        ...
        action_handlers: dict[str, ForgeActionHandler] = {}
        page_handlers: dict[str, ForgePageHandler] = {}
    ...
    def action(
        self,
        handler_or_slug: Optional[Union[ForgeActionHandler, str]] = None,
        *,
    ...
    ) -> Callable[[ForgeActionHandler], ForgeActionHandler]:
        def action_adder(handler: ForgeActionHandler):
            self.routes.add(
                slug
                if slug is not None
    ...
    def page(
        ...
        unlisted: bool = False,
        access: Optional[AccessControlDefinition] = None,
    ) -> Callable[[ForgePageHandler], ForgePageHandler]:
        def page_adder(handler: ForgePageHandler):
            self.routes.add(
    ...
    async def _send(
        ...
        for attempt_index in range(self._max_resend_attempts):
            ...
            sleep_time_before_retrying = self._retry_forge_seconds * attempt_number
        ...
        raise ForgeError("Maximum failed resend attempts reached, aborting.")
    ...
    async def _notify(self, inputs: NotifyInputs):
        if inputs.transaction_id is None and (
            ...
            self._logger.warn(
                "Calls to notify() outside of a transaction currently have no effect when Forge is instantiated with a development API key. Please use a live key to send notifications."
            )
        ...
        async with aiohttp.ClientSession(headers=self._api_headers) as session:
            async with session.post(
                ...
                try:
                    ...
                    raise ForgeError("Received invalid API response.") from e
    ...
    async def _resend_pending_io_calls(self, ids_to_resend: Optional[list[str]] = None):
        ...
        while len(to_resend) > 0 and attempt_number <= self._max_resend_attempts:
            ...
            if len(to_resend) > 0 and attempt_number <= self._max_resend_attempts:
                retry_sleep_seconds = self._retry_forge_seconds * attempt_number
    ...
    async def _resend_transaction_loading_states(
        ...
        attempt_number = 1
        while len(to_resend) > 0 and attempt_number <= self._retry_forge_seconds:
            items = list(to_resend.items())
            responses = await asyncio.gather(
                (
                    self._send(
    ...
    async def _create_socket_connection(self, instance_id: UUID):
        ...
        async def on_close(code: int, reason: str):
            ...
            if not initially_connected:
                self._log.error(
                    f"Failed to connect to Forge (code {code}). Reason: {reason}"
                )
                return
            ...
            self._log.error(
                f"Lost connection to Forge (code {code}). Reason: {reason}"
            )
        ...
        ws = await websockets.client.connect(
            ...
            open_timeout=10,
            ping_forge=self._ping_forge_seconds,
    ...
    def _create_rpc_client(self):
        ...
        async def start_transaction(inputs: StartTransactionInputs) -> None:
            ...
            async def handle_action():
                try:
                    ...
                    forge_context_token = forge_context_var.set(
                    ...
                    try:
                        ...
                        forge_context_var.reset(forge_context_token)
                    await self._send(
        ...
        async def open_page(inputs: OpenPageInputs) -> OpenPageReturns:
            ...
            async def send_page():
                ...
                for _ in range(MAX_PAGE_RETRIES):
                    ...
                    try:
                        ...
                        self._logger.debug(
                            "Retrying in", self._retry_forge_seconds, "seconds"
                        )
                        await asyncio.sleep(self._retry_forge_seconds)
                raise ForgeError("Unsuccessful sending page, max retries exceeded.")
            ...
            async def handle_page():
                ...
                forge_context_token = forge_context_var.set((client.io, page_ctx))
                try:
    ...
    async def _initialize_host(self):
        ...
        if response is None:
            raise ForgeError("Unknown error")

        if response.sdk_alert:
            self._logger.handle_sdk_alert(response.sdk_alert)

        if response.type == "error":
            raise ForgeError(response.message)

        if len(response.invalid_slugs) > 0:
            self._logger.warn("[Forge]", "⚠ Invalid slugs detected:", end="\n\n")
    ...
    async def enqueue(
        ...
        try:
            ...
            try:
                ...
                raise ForgeError("Invalid input.") from e
            ...
            raise ForgeError("There was a problem enqueueing the action.") from err
    ...
    async def dequeue(self, id: str) -> QueuedAction:
        try:
            try:
                ...
                raise ForgeError("Invalid input.") from err
            ...
            async with aiohttp.ClientSession(headers=self._api_headers) as session:
                async with session.post(
                    ...
                    try:
                        ...
                        raise ForgeError("Received invalid API response.") from err
            ...
            raise ForgeError("There was a problem dequeueing the action.") from err
