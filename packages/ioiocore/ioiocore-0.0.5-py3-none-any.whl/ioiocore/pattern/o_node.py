...
class ONode:
    ...
    def _set_metadata(self, metadata : list[dict]):
        if metadata is not None and len(metadata) != len(self.OutputPorts):
            raise ValueError(f'Dimensions of metadata and outputs must be identical if set. \'{len(metadata)}\' != \'{len(self.OutputPorts)}\'. Please override the \'init\' function of your node and set valid metadata by calling \'self._set_metadata(metadata)\'.')
        if self.OutputPorts is not None and len(self.OutputPorts) > 0:
