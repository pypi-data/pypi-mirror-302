...
def form_list_item(attributes, title, pairs, queue, delims=[', ']):
    try:
        item = {}
...
def combine_list_elements(attributes, elems, delims, idx) -> str:
    ...
    for i, elem in enumerate(elems):
        if isinstance(elem, list):
            string_add = combine_list_elements(attributes, elem, delims, idx+1)
        else:
            try:
                string_add = str(attributes.get(elem, '')).strip()
            except Exception as e: continue

        if string_add in [None, '']: continue 
        string += string_add 

        if i == len(elems) - 1: break
        string += delims[idx]
