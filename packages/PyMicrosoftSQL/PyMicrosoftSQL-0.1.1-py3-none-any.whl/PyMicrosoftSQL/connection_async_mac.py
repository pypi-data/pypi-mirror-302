...
from PyMicrosoftSQL.async_sql_event_manager import AsyncSQLEventManager, Event
...
odbc = ctypes.CDLL(libmsodbcsql_path, mode=ctypes.RTLD_GLOBAL)
async_event_mgr_obj = AsyncSQLEventManager(odbc.SQLDriverConnectW, odbc.SQLExecDirectW)

class ConnectionAsyncMac:
    ...
    def _initialize(self):
        ...
        self._set_statement_attributes()
    
    def _allocate_environment_handle(self):
        ret = odbc.SQLAllocHandle(const.SQL_HANDLE_ENV, const.SQL_NULL_HANDLE, ctypes.byref(self.henv))
        self._check_ret(ret, const.SQL_HANDLE_ENV, self.henv)

    def _set_environment_attributes(self):
        ret = odbc.SQLSetEnvAttr(self.henv, const.SQL_ATTR_ODBC_VERSION, ctypes.c_void_p(const.SQL_OV_ODBC3), 0)
        self._check_ret(ret, const.SQL_HANDLE_ENV, self.henv)

    def _allocate_connection_handle(self):
        ret = odbc.SQLAllocHandle(const.SQL_HANDLE_DBC, self.henv, ctypes.byref(self.hdbc))
        self._check_ret(ret, const.SQL_HANDLE_DBC, self.hdbc)

    def _connect_to_server(self):
        conn_event = Event(handle=self.hdbc, event_type=const.SQL_HANDLE_DBC)
        async_event_mgr_obj.add_conn_handle(conn_event)
        ...
        self._check_ret(ret, const.SQL_HANDLE_DBC, self.hdbc)
        if ret == const.SQL_STILL_EXECUTING:
            conn_event.wait()

    def _allocate_statement_handle(self):
        ret = odbc.SQLAllocHandle(const.SQL_HANDLE_STMT, self.hdbc, ctypes.byref(self.hstmt))
        self._check_ret(ret, const.SQL_HANDLE_STMT, self.hstmt)
    
    def _set_statement_attributes(self):
        ret = odbc.SQLSetStmtAttrW(
            self.hstmt,
            const.SQL_ATTR_ASYNC_ENABLE,
            ctypes.c_void_p(1),
            const.SQL_IS_INTEGER
        )
        self._check_ret(ret, const.SQL_HANDLE_STMT, self.hstmt)

    def execute(self, query):
        stmt_event = Event(handle=self.hstmt, event_type=const.SQL_HANDLE_STMT)
        async_event_mgr_obj.add_stmt_handle(stmt_event)
        ret = odbc.SQLExecDirectW(self.hstmt, ctypes.c_char_p(query.encode('utf_16_le')), const.SQL_NTS)
        self._check_ret(ret, const.SQL_HANDLE_STMT, self.hstmt)
        self.query_ret = ret
        self.query_stmt_event = stmt_event

    def wait_for_query_completion(self):
        if self.query_ret == const.SQL_STILL_EXECUTING:
            self.query_stmt_event.wait()

    def fetch_data(self):
        data = []
        num_cols = ctypes.c_long()
        ret = odbc.SQLNumResultCols(self.hstmt, ctypes.byref(num_cols))
        self._check_ret(ret, const.SQL_HANDLE_STMT, self.hstmt)
        ...
        for col in range(1, num_cols.value + 1):
            ...
            ret = odbc.SQLDescribeColW(self.hstmt, col, col_name, self.buffer_length, ctypes.byref(name_len), None, None, None, None)
            self._check_ret(ret, const.SQL_HANDLE_STMT, self.hstmt)
            columns.append(from_buffer_u(col_name))
        
        while True:
            ret = odbc.SQLFetch(self.hstmt)
            self._check_ret(ret, const.SQL_HANDLE_STMT, self.hstmt)
            ...
            for i in range(1, num_cols.value + 1):
                ret = odbc.SQLGetData(self.hstmt, i, 1, self.buffer, self.buffer_length, ctypes.byref(self.indicator))
                if ret not in (const.SQL_SUCCESS, const.SQL_SUCCESS_WITH_INFO):
    ...
    def _check_ret(self, ret, handle_type, handle):
        odbc.SQLGetDiagRecW.argtypes = [ctypes.c_short, ctypes.c_void_p, ctypes.c_short, ctypes.c_char_p, ctypes.POINTER(ctypes.c_int), ctypes.c_char_p, ctypes.c_short, ctypes.POINTER(ctypes.c_short)]
        odbc.SQLGetDiagRecW.restype = ctypes.c_short
        ...
        if ret not in (const.SQL_SUCCESS, const.SQL_SUCCESS_WITH_INFO):
            ...
            while 1:
                ret = odbc.SQLGetDiagRecW(handle_type, handle, number_errors, sql_state, ctypes.byref(native_error), error_message, 1024, ctypes.byref(text_length))
                if ret == const.SQL_NO_DATA:
                    if len(err_list):
    ...
    def close(self):
        odbc.SQLDisconnect(self.hdbc)
        odbc.SQLFreeHandle(const.SQL_HANDLE_DBC, self.hdbc)
        odbc.SQLFreeHandle(const.SQL_HANDLE_ENV, self.henv)
