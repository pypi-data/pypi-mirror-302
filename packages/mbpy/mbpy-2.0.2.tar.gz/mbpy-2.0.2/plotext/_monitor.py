...
##############################################
    ...
    def _draw_image(self, image, marker = None, style = None, fast = False, grayscale = False):
        ...
        self.set_size([sz  for sz in ut.terminal_size()])
        size = ut.update_size(image, [min(sz / 2, imsz) for sz, imsz in zip(self.size, image.size)],resample=False)
        # image = image.resize(size, resample = True)
         # Resize the image with resampling for better quality.
        resample_method = Image.LANCZOS if False else Image.NEAREST
