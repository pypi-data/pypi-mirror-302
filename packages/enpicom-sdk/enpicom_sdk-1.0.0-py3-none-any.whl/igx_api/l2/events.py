...
def _create_paho_client(client_id: str, topic: str, on_event: OnEventCallback) -> Client:
...
class EventListener:
    ...
    **Do not use this class directly, use `UserEventListener` or `OrganizationEventListener` instead.**
    """

    _paho_client: Client

    def __init__(self, on_event: OnEventCallback, client_id: str, topic: str):
...
class UserEventListener(EventListener):
    def __init__(self, on_event: OnEventCallback, client_id: str | None = None):
        ...
        Args:
            ...
            client_id (str | None, optional): A unique identifier for the client. Defaults to a random UUID. Take care when providing a
        ...
        client_id = f"{get_event_root_topic()}-user-{user_id}_{client_id}"
...
class OrganizationEventListener(EventListener):
    def __init__(self, on_event: OnEventCallback, client_id: str | None = None):
        ...
        client_id = client_id or str(uuid.uuid4())
        client_id = f"{get_event_root_topic()}-organization-{organization_id}_{client_id}"
