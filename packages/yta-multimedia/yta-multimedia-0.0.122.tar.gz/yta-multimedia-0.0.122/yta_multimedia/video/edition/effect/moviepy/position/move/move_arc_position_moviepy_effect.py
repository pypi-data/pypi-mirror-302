...
class MoveArcPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    """
    def apply(self, initial_position: Union[ScreenPosition, CoordinateCorner, CoordinateCenter] = ScreenPosition.OUT_LEFT, final_position: Union[ScreenPosition, CoordinateCorner, CoordinateCenter] = ScreenPosition.CENTER, arc_is_bottom: bool = False, max_arc_height: float = 300):
        """
        ...
        return self.apply_over_video(background_video, initial_position, final_position, arc_is_bottom, max_arc_height)

    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], initial_position: Union[ScreenPosition, CoordinateCorner, CoordinateCenter] = ScreenPosition.OUT_LEFT, final_position: Union[ScreenPosition, CoordinateCorner, CoordinateCenter] = ScreenPosition.CENTER, arc_is_bottom: bool = False, max_arc_height: float = 300):
        """
        TODO: Write well
        ...
        effect = self.video.set_position(lambda t: arc_movement(t, initial_position, final_position, self.video.duration, self.video.fps, arc_is_bottom, max_arc_height)).set_start(0).set_duration(self.video.duration)
