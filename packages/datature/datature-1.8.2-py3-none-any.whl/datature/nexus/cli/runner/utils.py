...
from datature.nexus.cli.runner.config import GPUInfo, RunnerConfig, SystemSpecs
    ...
    try:
        config_map = k8s.client.V1ConfigMap(
            ...
            data={
                ...
                "docker_registry": os.getenv("DOCKER_REGISTRY", "docker.datature.io"),
                "api_url": os.getenv(
...
def setup_docker_registry_credentials(
    ...
    try:
        docker_config_dict = {
            "auths": {
                os.getenv("DOCKER_REGISTRY", "docker.datature.io"): {
                    "username": "secret-key",
...
def apply_deployment(
    ...
    try:
        deployment = k8s.client.V1Deployment(
            ...
            spec=K8S_CONFIG.get_deployment_spec(
                runner_config.kubectl_namespace, logs_folder
            ),
        )
...
def format_runner_status(runner_config: RunnerConfig) -> None:
    ...
    if len(runner_config.status.runs) > 0:
        ...
        for run in runner_config.status.runs:
            ...
            print(
                f"| {run_hash:<9} {run.image_name:<15} {run.status:<12} {run.started:<20} "
