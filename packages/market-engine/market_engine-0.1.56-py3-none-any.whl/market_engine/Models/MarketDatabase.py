...
class MarketDatabase:
    ...
    FROM item_statistics
    WHERE item_id IN (%s) AND platform=%s AND order_type='closed'
    ORDER BY datetime
    """
    ...
    SELECT i.item_name, COALESCE(s.wa_price, 0) as last_average_price
    FROM items i
    LEFT JOIN (
        SELECT item_id, wa_price
        FROM (
            SELECT item_id, wa_price,
    ...
    def get_item_price_history(self, item_ids: List[str], platform: str = 'pc') -> Dict[str, Dict[str, str]]:
        ...
        query = self._GET_PRICE_HISTORY_QUERY % placeholders
    ...
    def get_item_demand_history(self, item_ids: List[str], platform: str = 'pc') -> Dict[str, Dict[str, str]]:
        ...
        """
        placeholders = ','.join(['%s'] * len(item_ids))
        query = self._GET_DEMAND_HISTORY_QUERY % placeholders
        results = self.execute_query(query, *item_ids, platform, fetch='all')
