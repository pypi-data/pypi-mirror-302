        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a web crawling task planner. Your goal is to analyze the user's instruction and determine if any web crawling is explicitly requested. Only return crawl tasks for URLs directly provided by the user. Do not guess or infer any URLs. Exclude all video, image, or related asset links (e.g., .png, .jpg, .mp4, etc.).\n\n"
                    "For each valid crawling task, provide:\n"
                    "- crawl_url: The URL provided by the user to crawl, corrected if necessary.\n"
                    ...
                    '    ]\n'
                    "}\n\n"
                    "If no valid crawling URLs are provided by the user, return an empty JSON: {}"
                )
            },
            {
                "role": "user",
                "content": f"Analyze this prompt and determine if any web crawling is explicitly requested with provided URLs. If yes, provide the crawling details in JSON format, correcting any incomplete URLs to fully valid ones. If no valid crawling URLs are given, return an empty JSON:\n{prompt}\n"
            }
        ]
