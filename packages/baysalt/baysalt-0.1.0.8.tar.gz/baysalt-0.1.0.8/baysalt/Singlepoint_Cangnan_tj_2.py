...
class Cangnan_Single:
    ...
    def getfore(self, _date, _idx):
        ...
        fh_css,fh_csm,fh_csb,fh_cds,fh_cdm,fh_cdb=self.getcurrent(_date,_idx,self.p)
        return str(format(hs_rtn, '.2f')).rjust(12, " "), \
               ...
               str(format(lm_rtn, '.2f')).rjust(12, " "), \
                  str(format(fh_css, '.2f')).rjust(12, " "), \
                  str(format(fh_csm, '.2f')).rjust(12, " "), \
                    str(format(fh_csb, '.2f')).rjust(12, " "), \
                    str(format(fh_cds, '.2f')).rjust(12, " "), \
                    str(format(fh_cdm, '.2f')).rjust(12, " "), \
                    str(format(fh_cdb, '.2f')).rjust(12, " ")
    ...
    def getcurrent(self,_date,_idx,p):
        ...
        :param p: fvcom网格点的索引
        :return: "currentspeeds","currentspeedm","currentspeedb","currentdirs","currentdirm","currentdirb"
        """
        # 时间处理，将UTC转为CST
        if _idx>=120:
            currentspeeds, currentspeedm, currentspeedb, currentdirs, currentdirm, currentdirb=np.nan,np.nan,np.nan,np.nan,np.nan,np.nan
            return currentspeeds, currentspeedm, currentspeedb, currentdirs, currentdirm, currentdirb
        date1 = datetime.datetime.strptime(f'{_date}00', '%Y%m%d%H')
        _idx = _idx - 8  # 向前8个钟头，找到CST当天的0点
        date2 = date1 + datetime.timedelta(hours=_idx)
        ...
        u = f_c.variables['u'][:]
        v = f_c.variables['v'][:]
        currentspeeds = np.sqrt(u[idx,0, p] ** 2 + v[idx,0, p] ** 2)
        currentdirs= np.arctan2(v[idx,0, p], u[idx,0, p]) * 180 / np.pi
        currentspeedm = np.sqrt(u[idx,7, p] ** 2 + v[idx,7, p] ** 2)
        currentdirm = np.arctan2(v[idx,1, p], u[idx,1, p]) * 180 / np.pi
        currentspeedb = np.sqrt(u[idx,14, p] ** 2 + v[idx,14, p] ** 2)
        currentdirb = np.arctan2(v[idx,14, p], u[idx,14, p]) * 180 / np.pi

        currentspeeds=np.round(currentspeeds,2)
        currentspeedm=np.round(currentspeedm,2)
        currentspeedb=np.round(currentspeedb,2)
        currentdirs=np.round(currentdirs,2)
        currentdirm=np.round(currentdirm,2)
        currentdirb=np.round(currentdirb,2)
    ...
    def get_current_sp(self,_date):
        ...
        """
        _date = str(_date)
        filename = os.path.join(fvcom_dir, _date + '/output' + '/forecast_0001.nc')
        f_c = netCDF4.Dataset(filename)  # fvcom nc
    ...
    @staticmethod
    def getJsonData(date, fc_hs, fc_dir, fc_t01, fc_PP, fc_phs1, fc_ptp1, fc_lm, fc_css, fc_csm, fc_csb, fc_cds, fc_cdm,fc_cdb):
        ...
        postData['mwl'] = fc_lm
        postData['currentspeeds'] = Cangnan_Single.nan2str(fc_css)
        postData['currentspeedm'] = Cangnan_Single.nan2str(fc_csm)
        postData['currentspeedb'] = Cangnan_Single.nan2str(fc_csb)
        postData['currentdirs'] = Cangnan_Single.nan2str(fc_cds)
        postData['currentdirm'] = Cangnan_Single.nan2str(fc_cdm)
        postData['currentdirb'] = Cangnan_Single.nan2str(fc_cdb)
        return postData
    ...
    def grttxt(self, _date):
        ...
        # 流场数据开头
        fh_css='currentspeeds'.rjust(12, " ")  # 表面流速
        fh_csm='currentspeedm'.rjust(12, " ")  # 中层流速
        fh_csb='currentspeedb'.rjust(12, " ")  # 底层流速
        fh_cds='currentdirs'.rjust(12, " ")  # 表面流向
        fh_cdm='currentdirm'.rjust(12, " ")  # 中层流向
        fh_cdb='currentdirb'.rjust(12, " ")  # 底层流向
        ...
        for k in range(1):
            ...
            with open(file, "w") as fl:
                fl.writelines([fh_time, fh_hs, fh_dir, fh_t01, fh_lm, fh_PP, fh_phs1, fh_ptp1,fh_css,fh_csm,fh_csb,fh_cds,fh_cdm,fh_cdb,"\n"])
                # 写入文件头----------------------------------end
                ...
                for i in range(max(tmp) + 1):  # +72
                    ...
                    fc_hs, fc_t01, fc_dir, fc_PP, fc_phs1, fc_ptp1, fc_lm,fc_css,fc_csm,fc_csb,fc_cds,fc_cdm,fc_cdb = self.getfore(_date0, i)  # 获得数据

                    if fc_PP.strip() == "--":  # 谱峰周期为空时，采用2.99代替
                        fc_PP = str(2.99).rjust(12, " ")
                    if i in m_hours:
                        fl.writelines([fc_time, fc_hs, fc_dir, fc_t01, fc_lm, fc_PP, fc_phs1, fc_ptp1, fc_css,fc_csm,fc_csb,fc_cds,fc_cdm,fc_cdb,"\n"])
    ...
    def grtjson(self, _date):
        ...
        for k in range(1):
            ...
            for i in range(max(tmp) + 1):
                ...
                fc_css, fc_csm, fc_csb, fc_cds, fc_cdm, fc_cdb=self.getcurrent(_date0,i,self.p)

                if np.isnan(fc_PP):  # 谱峰周期为空时，采用2.99代替
                    fc_PP = 2.99

                JsonData = self.getJsonData(fc_time, fc_hs, fc_dir, fc_t01, fc_lm, fc_PP, fc_phs1, fc_ptp1,fc_css,fc_csm,fc_csb,fc_cds,fc_cdm,fc_cdb)  # dict()
...
if __name__ == '__main__':
    ...
    location_cnhd1 = {"No": "cnhd1", "name": "苍南核电1", "lon": 120.600, "lat": 27.260}
    name_cnhd1 = "cnhd1_120.521E_27.201N_"
    c=Cangnan_Single(120.600, 27.260, input_path, output_path_txt, output_path_json, name_cnhd1, **location_cnhd1)
    #c.grttxt('20230721')
    print('json')
    c.grtjson('20230721')
    print('end')
