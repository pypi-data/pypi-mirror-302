...
class GatePainter:
    ...
    def __init__(self, mpl_ax, qpu_fov: FieldOfView):
        # gates:
        self.artist_objs = {}
        ...
        self.artist_objs["GlobalHyperfineZRotation"] = GlobalGateArtist(
            mpl_ax=mpl_ax,
            xmin=qpu_fov.xmin,
            width=qpu_fov.width,
            ymin=qpu_fov.ymin,
            height=qpu_fov.height,
            color=_gate_color_code["GlobalHyperfineZRotation"],
        )
    ...
    def process_gates(self, gates: List[GateEvent]) -> List:
        ...
        for gate in gates:
            match gate.cls_name:
                case "GlobalCZGate":
                    self.artist_objs[gate.cls_name].set_visible(True)
                case "GlobalHyperfineRotation":
                    self.artist_objs[gate.cls_name].set_visible(True)
                case "GlobalHyperfineZRotation":
                    self.artist_objs[gate.cls_name].set_visible(True)
                case "TopHatCZGate":
                    obj = self.artist_objs[gate.cls_name]
                    obj.update_data(
                        ymin=gate.kwargs["y_min"],
                        ymax=gate.kwargs["y_max"],
                        ymin_keepout=gate.kwargs["y_min_keepout"],
                        ymax_keepout=gate.kwargs["y_max_keepout"],
                    )
                    obj.set_visible(True)
                case "LocalHyperfineZRotation" | "LocalHyperfineRotation":
                    raise ValueError(
                        "cannot have local hyperfineZrotation, it should be already lowered to Pos variant!"
                    )
                case "LocalHyperfineZRotationPos" | "LocalHyperfineRotationPos":
                    obj = self.artist_objs[gate.cls_name]
                    obj.update_data(
                        xy=[
                            (x, gate.kwargs["y_row_loc"])
                            for x in gate.kwargs["varargs"]
                        ]
                    )
                    obj.set_visible(True)
