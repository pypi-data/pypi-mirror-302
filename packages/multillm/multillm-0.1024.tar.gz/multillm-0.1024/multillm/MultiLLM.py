...
def task_A(model_name,prompt,taskid = None, convid = None, q=None, model=None):
    #model = MultiLLM.model_registry[model_name]
    if not model:
        return 
    response, is_code = model.get_response(prompt,taskid, convid)
    q.put((model_name, response, is_code))
...
class MultiLLM(object):
    ...
    def __init__(self, config=None, models=[], model_names=[], debug=False):
    ...
    def run_llm_query(cfg,reward,q):
        global out_file_name
        cfg.RewardPredictor["reward_column"] = reward
        RewPred = RewardPredictor(dataframe = dataframe, config = cfg, uname=args.username, tid=args.taskid, output_file=args.output_file)
        cfg.RewardPredictor["incremental"] = args.incremental
        
        RewPred.create_predictor()
        RewPred.generate_knobs()
        q.put(RewPred.out_file_name_pred)
        
   
    def run_macos(self, prompt, action_chain, rank_chain, taskid=None, convid = None, 
                detail=False):
        """
        """
        responses = {}
        q = Queue()
        args = [(model_name,prompt,taskid, convid,q, MultiLLM.model_registry[model_name]) for model_name in self.model_names]
        print("args: {0} " .format(args))

        """
        Example Args:
        
        [('PHI3', Prompt('please explain quantum entanglement'), None, None),
        ('GEMMA', Prompt('please explain quantum entanglement'), None, None),
        ('GPT', Prompt('please explain quantum entanglement'), None, None)]

        """
        
        print("model_names: {0}" .format(self.model_names))
        is_code_list = []
        ...
        for i in range(len(args)):
            ...
            p = Process(target=task_A, args=args[i])
            p.start()
            jobs.append(p)
        
        for p in jobs:
            print("running job {0}" .format(p))
            p.join()
            model_name,response, is_code  = q.get()
