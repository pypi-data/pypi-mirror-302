...
class TableSchemasManager(metaclass=Singleton):
    def __init__(self) -> None:
        ...
        self._tables: dict[glue.Schema, bool] = {}
        self._schemas_cache: dict[str, dict[str | Versions, glue.Schema]] = {}
        self._operation_manager = DataApplication().operation_manager

    def set_schemas_cache(self, class_name: str, schema: glue.Schema) -> None:
        for class_version in [Versions.LATEST, schema.version]:
            self._schemas_cache.setdefault(class_name, {})
            self._schemas_cache[class_name][class_version] = schema
    ...
    def register_table(
        ...
        *,
        ...
        self._tables[schema] = is_created
        self.set_schemas_cache(schema.name.lower(), schema)
        ...
        if using == LAKEHOUSE_DB_ALIAS:
            result = self._operation_manager.perform_schema_command_lakehouse(
                command=glue.SchemaCommand(
                    ...
                    root_transaction_id=AmsdalTransactionManager()._get_root_transaction_id(),
