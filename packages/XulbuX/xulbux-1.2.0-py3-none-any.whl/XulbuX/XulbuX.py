...
class Path:
  @staticmethod
  def get_dir(cwd:bool = False, base_dir:bool = False) -> str|list:
    paths = []
  ...
  def extend(path:str, search_in:str|list[str] = None, raise_error:bool = False) -> str:
    ...
    if os.path.isabs(path):
      ...
      base_dir = Path.get_dir(base_dir=True)
...
class File:
  ...
  def _make_path(filename:str, filetype:str, search_in:str|list[str] = None, prefer_base_dir:bool = True) -> str:
    ...
    except FileNotFoundError: return os.path.join(Path.get_dir(base_dir=True), filename) if prefer_base_dir else os.path.join(os.getcwd(), filename)
...
class EnvVars:
  ...
  def has_path(path:str = None, cwd:bool = False, base_dir:bool = False) -> bool:
    ...
    if base_dir: path = Path.get_dir(base_dir=True)
  ...
  def add_path(add_path:str = None, cwd:bool = False, base_dir:bool = False, persistent:bool = True) -> None:
    ...
    if base_dir: add_path = Path.get_dir(base_dir=True)
...
class FormatCodes:
  ...
  **Special formatting when param `default_color` is set to a color:**
  ...
  @staticmethod
  def __get_default_ansi(default_color:hexa|rgb, format_key:str = None, brightness_steps:int = None, _modifiers:tuple[str, str] = ('+l', '-d')) -> str|None:
