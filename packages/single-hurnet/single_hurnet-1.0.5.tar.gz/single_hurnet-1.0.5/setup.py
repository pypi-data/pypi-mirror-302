...
class PostInstallCommand(install):
    def run(self):
        def online_environment():
            try:
                is_google_colab = 'google.colab' in modules
                is_colab = 'COLAB_RELEASE_TAG' in environ
                is_kaggle = 'KAGGLE_KERNEL_RUN_TYPE' in environ
                is_jupyter = 'JUPYTER_PATH' in environ
                return any([is_google_colab, is_colab, is_kaggle, is_jupyter])
            except: return False
        if not online_environment():
            print('Local environment detected.')
            install.run(self)
            install_dir = self.install_lib
            source_file = path.join(install_dir, package_name, package_name + '.py')
            compiled_file = path.join(install_dir, package_name, package_name + '.pyc')
            cache_dir = path.join(install_dir, package_name, '__pycache__')
            try:
                if not path.isfile(source_file):
                    print(f"Source file not found: {source_file}")
                    return
                compile(source_file, cfile=compiled_file, doraise=True)
                remove(source_file)
                if path.isdir(cache_dir): rmtree(cache_dir)
            except Exception as error: print('ERROR during compilation: '+str(error))
        else:
            print('REMOTE environment detected.')
            install.run(self)
