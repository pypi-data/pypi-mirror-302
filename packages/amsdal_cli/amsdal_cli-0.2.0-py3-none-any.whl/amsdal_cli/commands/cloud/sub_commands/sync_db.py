...
def sync_db_command(
    ...
    *,
    ...
    _copy_data(config_manager.get_config())
...
def _copy_data(config: AmsdalConfig) -> None:
    for from_name, to_name in (
        ('remote_historical', 'local_history'),
        ('remote_state', 'local_state'),
    ):
        from_creds = config.connections[from_name].credentials
        from_backend = import_class(config.connections[from_name].backend)
        to_creds = config.connections[to_name].credentials
        to_backend = import_class(config.connections[to_name].backend)
        ...
        for schema in from_connection.query_schema():
            ...
            for data in from_connection.query(_query):
                to_connection.run_data_command(
                    glue.DataCommand(
                        mutations=[
                            glue.InsertData(
