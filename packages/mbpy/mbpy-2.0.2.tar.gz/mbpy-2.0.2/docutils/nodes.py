...
# import xml.dom.minidom as dom # -> conditional import in Node.asdom()  # noqa: ERA001
#                                    and document.asdom()
...
class Node:
    ...
    def walk(self, visitor):
        ...
        visitor.document.reporter.debug(
            f"docutils.nodes.Node.walk calling dispatch_visit for {self.__class__.__name__}"
        )
        try:
            try:
...
class reprunicode(str):
    ...
    def __init__(self, s):
        warnings.warn(
            "nodes.reprunicode() is not required with Python 3" " and will be removed in Docutils 0.21 or later.",
            DeprecationWarning,
            stacklevel=2,
        )
        super().__init__()
...
def ensure_str(s):
    ...
    """
    warnings.warn(
        "nodes.ensure_str() is not required with Python 3" " and will be removed in Docutils 0.21 or later.",
        DeprecationWarning,
        stacklevel=2,
    )
    return s
...
class Element(Node):
    ...
    def set_class(self, name):
        ...
        warnings.warn(
            "docutils.nodes.Element.set_class() is deprecated; " " and will be removed in Docutils 0.21 or later.",
...
class NodeVisitor:
    ...
    def dispatch_visit(self, node):
        ...
        self.document.reporter.debug(
            f"docutils.nodes.NodeVisitor.dispatch_visit calling {method.__name__} for {node_name}"
        )
        return method(node)
    ...
    def dispatch_departure(self, node):
        ...
        self.document.reporter.debug(
            "docutils.nodes.NodeVisitor.dispatch_departure calling {} for {}".format(method.__name__, node_name)
        )
        return method(node)
