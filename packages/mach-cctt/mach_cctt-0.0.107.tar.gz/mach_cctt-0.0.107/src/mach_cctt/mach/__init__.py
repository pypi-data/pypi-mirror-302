...
from mach_client import ChainId, Token, client, config as client_config, utility
...
async def run(
    *,
    ...
    while True:
        ...
        if (risk_analyis := risk_manager.check_order(src_token, dest_token, quote)).reject:
        ...
        log.info(
            f"Waiting for source balance to be withdrawn ({src_balance=}, {
        ...
        log.info(
            f"Source balance withdrawn, waiting to receive destination token ({dest_balance=}, {
