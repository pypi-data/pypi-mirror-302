...
from aivm_config import default_config, generate_proxy_config
...
class ProxyServicer(ssgrpc.ProxyServicer):
    ...
    async def GetServerConfiguration(
        ...
        logging.info("Forwarding GetServerConfiguration Request")
    ...
    async def GetPrediction(
        ...
        logging.info("Forwarding GetPrediction Request")
        ...
        for i, address in enumerate(self.server_addresses):
            if i < len(request.shares):
                logging.info(f"Sending shares to {address}")
        ...
        try:
            ...
            response = sspb2.ClientShares(shares=results)
            return response
        except grpc.RpcError as e:
            context.set_code(e.code())
            context.set_details(e.details())
    ...
    async def GetPreprocessing(
        ...
        tasks = []
        logging.info("Forwarding GetPreprocessing Request")
        for address in self.server_addresses:
    ...
    async def SendModel(self, request_iterator, context):
        logging.info("Forwarding SendModel Request")
        ...
        for address, stub in self.model_stubs.items():
            logging.debug(f"Sending model to {address}")
            tasks.append(stub.SendModel(chunks))
...
class ProxyServer:
    ...
    async def start(self):
        ...
        logging.info(f"Proxy server listening on {listen_addr}")
...
async def run_server(config):
    server = ProxyServer(config["nodes"], config["proxy_addr"], config["proxy_port"])
...
def main():
    ...
    parser.add_argument("--node_config", type=str, help="Path to the config file")
    args = parser.parse_args()

    # Load configuration from YAML or use default
    if args.node_config:
        config = load_yaml_config(args.node_config)
    else:
        config = generate_proxy_config(default_config)
    try:
        asyncio.run(run_server(config))
    except KeyboardInterrupt:
        print("Server shutdown due to keyboard interrupt.")
