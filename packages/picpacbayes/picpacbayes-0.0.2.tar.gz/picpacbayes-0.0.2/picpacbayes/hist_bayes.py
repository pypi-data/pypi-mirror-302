...
Macro information on the evolution of a PBayesSolver task
"""
...
class HistBayesLog:
    ...
    Stored data can be accessed through methods:
        ...
        pbayes_scores (:math:`PB_{Cat}(\theta)`),
    ...
    def __init__(self, proba_map: ProbaMap, n: int):
        ...
        self._pbayes_scores = np.zeros(n)
    ...
    def add(
        ...
        self._pbayes_scores[n0 : (n0 + n)] = scores
    ...
    def get(self, k: int = 1) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        return self.proba_pars(k), self.pbayes_scores(k), self.KLs(k), self.means(k)
    ...
    def pbayes_scores(self, k: Optional[int] = None) -> np.ndarray:
        """
        ...
        return self._pbayes_scores[init : self._n_filled]
    ...
    def best(self) -> tuple[np.ndarray, float]:
        ...
        pars, scores = self.proba_pars(), self.pbayes_scores()
    ...
    def save(self, name: str, path: str = ".", overwrite: bool = True) -> str:
        ...
        proba_pars, pbayes_scores, KLs, means = self.get_all()
        io.rw_arr.save(os.path.join(acc_path, "proba_pars.json"), proba_pars)
        io.rw_arr.save(os.path.join(acc_path, "pbayes_scores.json"), pbayes_scores)
    ...
    def load(cls, path: str):
        ...
        pbayes_scores = io.rw_arr.load(os.path.join(path, "pbayes_scores.json"))
        ...
        n = len(proba_pars)
        hist = cls(proba_map, n)
        hist.add(proba_pars, pbayes_scores, KLs, means) 
        return hist
