...
try:
    ...
    logger.debug("Pandas not installed. Skipping reset_dataframe_index function.")
...
def convert_element(value):
    ...
    if isinstance(val, Iterable) and has_nan(val):
        return None
    return val


def has_nan(iterable: Iterable):
    return any(math.isnan(x) for x in iterable if isinstance(x, (int, float)))
