...
def pandas_convert_odfv_only(
    tree: NodeRef, odfv_input: pyarrow.RecordBatchReader, secret_resolver: Optional[SecretResolver] = None
) -> PandasExecNode:
    assert tree_contains(tree, MultiOdfvPipelineNode)
    return _convert_to_pandas_nodes(tree, odfv_input, secret_resolver)
...
def _convert_to_pandas_nodes(
    ...
    if logical_tree_node.__class__ in node_mapping:
        ...
        return node_mapping[logical_tree_node.__class__].from_node_inputs(
            logical_tree_node, input_node, secret_resolver=secret_resolver
        )
    else:
