    ...
    decorator=swagger_auto_schema(
        ...
        operation_description="""
            ...
            **Note:** Imported data is verified against a project *label_config* and must
            ...
            Update this example to specify your authorization token and Labelo instance host, then run the following from
            the command line.
            ...
            Update this example to specify your authorization token, Labelo instance host, and file name and path,
...
class ImportAPI(generics.CreateAPIView):
    ...
    def _save(self, tasks):
        ...
        project.save_compressed_thumbnails(tasks)
...
class CompressedFileResponse(generics.RetrieveAPIView):
    permission_classes = (IsAuthenticated,)
    ...
    def get(self, *args, **kwargs):
        ...
        filename = kwargs['filename']
        # XXX needed, on windows os.path.join generates '\' which breaks FileUpload
        file = settings.COMPRESSED_DIR + (
            '/' if not settings.COMPRESSED_DIR.endswith('/') else '') + filename
        logger.debug(f'Fetch uploaded file by user {request.user} => {file}')
        file_upload = FileUpload.objects.filter(file=file).last()
        if not file_upload.has_permission(request.user):
            return Response(status=status.HTTP_403_FORBIDDEN)
        ...
        if file.storage.exists(file.name):
            content_type, encoding = mimetypes.guess_type(str(file.name))
            content_type = content_type or 'application/octet-stream'
            return RangedFileResponse(request, file.open(mode='rb'),
