...
class TaskAdapter:
  ...
  def from_task_v1(self) -> task.Task:
    ...
    task_dict = {
      ...
      'creation_date': datetime(self.task_v1_data['date_created']),
      'date_range': self.task_v1_data['lookback_days'],
      'from_days_ago': self.task_v1_data['from_days_ago'],
      'exclusion_level': enums.ExclusionLevelEnum.AD_GROUP.value,
      'placement_types': self._build_placement_types(),
      'status': task.TaskStatus.ACTIVE,
      'exclusion_rule': self._build_condition_rules(),
      'task_output': self._build_task_output(),
    }
  ...
  def _build_task_output(self) -> task.TaskOutput:
    if self.task_v1_data['email_alerts'] and self.task_v1_data['schedule']:
      return task.TaskOutput.EXCLUDE_AND_NOTIFY
    if not self.task_v1_data['email_alerts'] and self.task_v1_data['schedule']:
      return task.TaskOutput.EXCLUDE
    if self.task_v1_data['email_alerts'] and not self.task_v1_data['schedule']:
