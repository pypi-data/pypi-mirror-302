...
def target_func(deltaX, x0, fw, fl):
    return cal_angle(fw, fl, x0, x0 - deltaX) - 30.0


def create_dist_list(fw, fl, height):
    ret = [0]
    while sum(ret) < height:
        z0 = -sum(ret)
        r = bisect(target_func, 100, 20000, args=(z0 * 1000, fw, fl))
        int_part = int(r / 500)
...
def easy_tower(tower_id, cc, height, ref_ground, l_fun, w_fun, delta_h, delta_l, s):
    dist = create_dist_list(w_fun, l_fun, height)
    fem_list = []
