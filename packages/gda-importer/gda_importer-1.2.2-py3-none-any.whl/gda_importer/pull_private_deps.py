...
def main():
    ...
    for name, data in packages.items():
        ...
        if args.dry_run:
            print(
                "Here is your command. It might need quotes. Omit --dry_run to execute"
                " (with specified tokens inserted)."
            )
            total_cmd = installer(name, data["extras"], data["dep"], insert_token=False)
            for line in total_cmd:
                print(f"  $ {' '.join(line)}")
        else:
            total_cmd = installer(name, data["extras"], data["dep"], insert_token=True)
            for line in total_cmd:
