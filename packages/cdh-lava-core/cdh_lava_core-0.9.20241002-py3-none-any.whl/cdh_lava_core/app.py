...
def get_rstudio_user_id():
    ...
    if 'RS_SERVER_URL' in os.environ  or 'rstudio' in request.headers.get('Host', '').lower():
        logger.info(f"RS_SERVER_URL found")
        ...
        logger.info(f"RS_SERVER_URL not found")
    return None
...
@cdc_files_bp.route("/dashboards/dependency_graph/<operation_id>")
def dependency_graph(operation_id):
    ...
    with tracer.start_as_current_span("dependency_graph"):
        try:
            ...
            api_key = os.getenv("APPLICATION_INSIGHTS_API_KEY_DEV")

            chart_html = tracer_singleton.graph_ai_dependencies(operation_id, api_key, DATA_PRODUCT_ID, ENVIRONMENT)
            calling_page_url = request.args.get("calling_page")

            tracer_singleton.force_flush()
            return render_template(
                "dashboards/dependency_graph.html", calling_page_url=calling_page_url, chart=chart_html
            )
