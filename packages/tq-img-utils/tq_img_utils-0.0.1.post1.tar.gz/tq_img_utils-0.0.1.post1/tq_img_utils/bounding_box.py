...
class BoundingBox:
    ...
    def __init__(self, x: int, y: int, width: int, height: int):
        ...
        :raises ValueError: if the area of the bounding box is zero(left==right or top==bottom).
        """
        if x == (x + width) or y == (y + height):
            raise ValueError("The area of the bounding is zero(left==right or top==bottom).")
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    @staticmethod
    def get_from_min_max_format(region: Tuple[int, int, int, int]) -> 'BoundingBox':
        """ get bounding box from min_max_format. Min-Max format: [x_min, x_max, y_min, y_max] """
        x_min, x_max, y_min, y_max = region
        bbox = (x_min, y_min, x_max - x_min, y_max - y_min)
        return BoundingBox(*bbox)

    @staticmethod
    def get_from_two_points_format(region: Tuple[int, int, int, int]) -> 'BoundingBox':
        """ get bounding box from two_points_format. Two-Points format: [x_min, y_min, x_max, y_max] """
        x_min, y_min, x_max, y_max = region
        return BoundingBox.get_from_min_max_format((x_min, x_max, y_min, y_max))

    @staticmethod
    def get_from_vertexes_format(vertex: List[List[Union[float, int]]]) -> 'BoundingBox':
        """
    ...
    def get_extend_bounds(self, margin: int) -> 'BoundingBox':
        """ extend bounding(add margin) """
    ...
    def union(self, e: 'BoundingBox') -> 'BoundingBox':
        """
        get the union of these two bounding.
