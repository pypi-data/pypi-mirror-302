...
def get_verified_payload(token: str, tenant_id: str = "common", audience_uris: List[str] = None, token_version='v2.0') -> Dict[str, Any]:
    ...
    public_key = _get_public_key(kid, tenant_id, token_version)
    openid_config = _get_openid_config(tenant_id, token_version)
    try:
        payload = decode(
            token,
            public_key,
...
def _get_public_key(kid: str, tenant_id: str, token_version='v2.0'):
    ...
    try:
        ...
        for key in _get_jwk_keys(tenant_id, token_version):
            if key["kid"] == kid:
                x5c = key["x5c"]
...
def _get_jwk_keys(tenant_id: str, token_version='v2.0') -> List[Dict]:
    ...
    """
    jwks_uri = _get_openid_config(tenant_id, token_version).get("jwks_uri")
    if not jwks_uri:
...
@cached(cache=TTLCache(maxsize=16, ttl=3600))
def _get_openid_config(tenant_id: str, token_version='v2.0') -> Dict[str, Any]:
