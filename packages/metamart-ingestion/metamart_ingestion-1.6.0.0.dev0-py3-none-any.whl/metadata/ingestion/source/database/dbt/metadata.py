...
class DbtSource(DbtServiceSource):
    ...
    def __init__(self, config: WorkflowSource, metadata: MetaMart):
        super().__init__()
        self.config = config
            ...
            for key, manifest_node in manifest_entities.items():
                try:
                    ...
                    if table_entity:
                        ...
                        logger.warning(
                            f"Unable to find the table '{table_fqn}' in MetaMart"
                            "Please check if the table exists and is ingested in MetaMart"
        ...
        for key, manifest_column in manifest_columns.items():
            try:
                ...
                if manifest_column.meta:
                    ...
                    if (
                        dbt_column_meta.metamart
                        and dbt_column_meta.metamart.glossary
                    ):
                        dbt_column_tag_list.extend(
                            get_tag_labels(
                                metadata=self.metadata,
                                tags=dbt_column_meta.metamart.glossary,
                                include_tags=self.source_config.includeTags,
    ...
    def process_dbt_meta(self, manifest_meta):
        ...
        try:
            ...
            if dbt_meta_info.metamart and dbt_meta_info.metamart.glossary:
                dbt_table_tags_list.extend(
                    get_tag_labels(
                        metadata=self.metadata,
                        tags=dbt_meta_info.metamart.glossary,
                        include_tags=True,
                        tag_type=GlossaryTerm,
                    )
                    or []
            ...
            if dbt_meta_info.metamart and dbt_meta_info.metamart.tier:
                tier_fqn = dbt_meta_info.metamart.tier
                dbt_table_tags_list.extend(
                    get_tag_labels(
                        metadata=self.metadata,
