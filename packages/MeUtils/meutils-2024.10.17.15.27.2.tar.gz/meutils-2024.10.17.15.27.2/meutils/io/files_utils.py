...
async def to_url(file: Union[UploadFile, str, bytes], headers: Optional[dict] = None):
    ...
    try:
        ...
        logger.error(e)
        file_object = await Minio().put_object_for_openai(file=file, bucket_name="caches")
...
async def to_base64(file: Union[UploadFile, str, bytes]):
    ...
    content_type = isinstance(file, str) and mimetypes.guess_type(file)[0]
