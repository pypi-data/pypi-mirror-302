...
class TestMentatStorage(unittest.TestCase):
    ...
    def test_13_relapse(self):
        ...
        relapses = storage.search_relapsed_events('test@domain.org', 'low', ttltime, False)
        ...
        relapses = storage.search_relapsed_events('test@domain.org', 'low', ttltime - datetime.timedelta(seconds = 30), False)
        self.assertEqual(len(relapses), 0)
        relapses = storage.search_relapsed_events('test@domain.org', 'low', ttltime + datetime.timedelta(seconds = 30), False)
        self.assertEqual(len(relapses), 1)
        relapses = storage.search_relapsed_events('test@domain.org', 'low', ttltime + datetime.timedelta(seconds = 30), True)
        self.assertEqual(len(relapses), 1)
