...
class InstallBatch:
    def __init__(self):
        ...
        self.task_map = defaultdict(dict) # source name -> {task -> (start, end)}
...
class Client():
    ...
    def query(self, task:m.Task, rentrant=False, readonly=True, raw_results=False, inputs={}, format="pandas", tag=None, nowait_durable=None, headers={}, internal=False, abort_on_error=True) -> DataFrame|Any:
        ...
        with debugging.span("query", model=self._database, task=task, tag=tag, internal=internal) as end_span:
            ...
            source_map = build_source_map({"": {task: (0, code.count("\n"))}})
            if task.has_persist():
                readonly = False
            if self.dry_run:
                return DataFrame()
...
def build_source_map(tasks: Dict[str, Dict[m.Task, Tuple[int, int]]]):
    out = {} # file name -> (rel start, rel end) -> (python start, python end)
