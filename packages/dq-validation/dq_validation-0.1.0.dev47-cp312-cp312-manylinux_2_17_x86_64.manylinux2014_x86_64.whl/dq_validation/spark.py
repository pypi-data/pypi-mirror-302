...
from .utils import (
    ...
    update_report_with_passed_outcome,
    is_report_passed
)
...
def run(
    ...
    store_temp = False
    if bool(os.environ.get("VALIDATION_SPARK_STORE_TEMP")):
        store_temp = True
    ...
    for env, key in [
        ...
        ("VALIDATION_SPARK_DRIVER_MEMORY_OVERHEAD_FACTOR", "spark.driver.memoryOverheadFactor")
    ]:
        if bool(os.environ.get(env)):
            session_args["config"][key] = os.environ[env]
    ...
    with spark_session(
        ...
        **session_args,
        ...
        if store_temp:
            ...
            logger.info(f"Read and persisted table with {df.count()} rows")
        ...
        if drop_invalid_rows:
            ...
            update_report_with_passed_outcome(report_path)
        ...
        if is_report_passed(report_path):
            ...
            with tempfile.TemporaryDirectory(dir=temp_dir) as d:
                ...
                temp_output_path = os.path.join(d, "dataset.parquet")
                df.write.parquet(temp_output_path)

                if drop_invalid_rows:
                    num_rows_after_drop_invalid = ss.read.parquet(temp_output_path).count()
                    num_rows_dropped = num_rows_total - num_rows_after_drop_invalid
                    update_report_with_invalid_row_removal_result(report_path, num_rows_dropped)
