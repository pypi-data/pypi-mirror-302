...
class BaseHandler(BaseHandler):
    ...
    def show_share(self):
        ...
        # return self.app.options.auth and res and res.groups()[1] == str(self.current_user.id)
...
class DiskHandler(tornado.web.StaticFileHandler, BaseHandler):
    ...
    async def check(self, name):
        if not self.app.options.auth or self.current_user.id == 1:
            return True
        uid = name.split('/')[0]
            ...
            if str(key).startswith(str(path)):
                for doc in files:
                    if doc.path.name.lower().find(q) >= 0:
                        doc.name = str(doc.path).split('/', 1)[-1] if self.app.options.auth else str(doc.path)
                        entries.append(doc)
    ...
    async def get(self, name, include_body=True):
        ...
        if self.args.q:
            ...
            if path.is_file():
                ...
                if self.app.options.auth and not self.request.path.startswith('/disk/public'):
