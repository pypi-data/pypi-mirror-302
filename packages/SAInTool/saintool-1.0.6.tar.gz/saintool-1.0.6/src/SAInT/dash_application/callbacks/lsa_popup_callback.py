...
def register_lsa_popup_callback(dash_app, app):
    ...
    def update_lsa_popup(selected_data, n_clicks_lsa_popup, norm_denorm_value, loaded_data):
        changed_id = get_pressed_buttons()
        if "graph.clickData" in changed_id and selected_data is not None:
            _handle_graph_click(dash_app, app, selected_data)
        ...
        if "loaded_data.children" in changed_id:
            app.application.optimizer.reset()
        opt_div = app.application.optimizer.div_content
        return app.application.lsa_popup.is_open, app.application.lsa_popup.get_content(), selected_data, norm_denorm_value, opt_div

def _handle_graph_click(dash_app, app, selected_data):
    ...
    sample_idx = sample_dict["original_idx"]
    ds_name = sample_dict["ds_name"]
    app.application.optimizer.prepare(dash_app, app, sample_idx, ds_name)
