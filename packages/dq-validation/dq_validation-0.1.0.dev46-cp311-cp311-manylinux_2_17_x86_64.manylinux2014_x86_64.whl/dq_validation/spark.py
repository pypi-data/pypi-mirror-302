    ...
    with tempfile.TemporaryDirectory(dir=temp_dir) as d:
        ...
        with open(temp_merged_path, "wb") as temp_out:
            for ix, part in enumerate(csv_parts):
                ...
                logger.debug(f"Moved part file '{part}' into '{temp_merged_path}' ({ix + 1} / {len(csv_parts)})")
...
def find_duplicates_spark(df: DataFrame, unique_keys: List[List[int]], row_nr_column: str, has_error_col: str):
    ...
    for subset_columns_ix in unique_keys:
        ...
        # Filter duplicates
        duplicated_df = df_with_dup_flag.filter("is_duplicated == true").select(row_nr_column).persist(StorageLevel.DISK_ONLY)
...
def run(
        ...
        default_heap_size_extra_room: int = FOUR_GB_IN_BYTES
):
    config = read_config(config_path)

    session_args = {
        "heap_size_extra_room": default_heap_size_extra_room,
        "config": {},
    }

    if bool(os.environ.get("VALIDATION_SPARK_HEAP_SIZE_EXTRA_ROOM")):
        session_args["heap_size_extra_room"] =\
            int(os.environ["VALIDATION_SPARK_HEAP_SIZE_EXTRA_ROOM"])

    if bool(os.environ.get("VALIDATION_SPARK_NUM_PARTITIONS")):
        num_partitions = os.environ["VALIDATION_SPARK_NUM_PARTITIONS"]
        session_args["config"]["spark.sql.shuffle.partitions"] = num_partitions
        session_args["config"]["spark.default.parallelism"] = num_partitions

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_MEMORY")):
        session_args["config"]["spark.driver.memory"] = os.environ["VALIDATION_SPARK_DRIVER_MEMORY"]

    if bool(os.environ.get("VALIDATION_SPARK_MEMORY_FRACTION")):
        session_args["config"]["spark.memory.fraction"] = os.environ["VALIDATION_SPARK_MEMORY_FRACTION"]

    if bool(os.environ.get("VALIDATION_SPARK_MEMORY_STORAGE_FRACTION")):
        session_args["config"]["spark.memory.storageFraction"] =\
            os.environ["VALIDATION_SPARK_MEMORY_STORAGE_FRACTION"]

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_CORES")):
        session_args["config"]["spark.driver.cores"] =\
            os.environ["VALIDATION_SPARK_DRIVER_CORES"]

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_EXTRA_JAVA_OPTIONS")):
        session_args["config"]["spark.driver.extraJavaOptions"] =\
            os.environ["VALIDATION_SPARK_DRIVER_EXTRA_JAVA_OPTIONS"]

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_PROPERTIES_FILE")):
        session_args["config"]["spark.driver.extraJavaOptions"] =\
            f"-Dconfig.file={os.environ['VALIDATION_SPARK_DRIVER_PROPERTIES_FILE']}"

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_MEMORY_OVERHEAD")):
        session_args["config"]["spark.driver.memoryOverhead"] =\
            os.environ.get("VALIDATION_SPARK_DRIVER_MEMORY_OVERHEAD")

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_MEMORY_OVERHEAD_FACTOR")):
        session_args["config"]["spark.driver.memoryOverheadFactor"] =\
            os.environ.get("VALIDATION_SPARK_DRIVER_MEMORY_OVERHEAD_FACTOR")

    if bool(os.environ.get("VALIDATION_SPARK_STORE_TEMP_PARQUET")):
        store_temp_parquet = True
    else:
        store_temp_parquet = False

    logger.info(f"Creating spark session with args:\n{json.dumps(session_args, indent=2)}")

    session_args["config"] = list(session_args["config"].items())  # convert to list of tuples

    with spark_session(
        temp_dir,
        name="Validation",
        input_files=[annotated_dataset_path],
        **session_args,
    ) as ss:
        dataset_schema = create_spark_schema_from_validation_config(config, string_only=True)
        column_names = [f.name for f in dataset_schema]
        string_column_names = [
            column_names[jx] for jx, col in enumerate(config.root.config.columns)
        ...
        if should_check_uniqueness:
            ...
            after = datetime.now()
            logger.info(f"Uniqueness check took {(after - before).total_seconds() / 60} min")
        ...
        if is_passed:
            ...
            logger.info(f"Writing out {fmt} file took {(after - before).total_seconds() / 60} min")
