...
class GaiminAI:
    ...
    def generate_text(self, model: str, prompt: str, stream: bool = False) -> T2TGenerateResponse:
        return self.text_to_text.generate(model, prompt, stream)
    ...
    def generate_image(self, prompt: str, output_type: str = "png", num_inference_steps: int = 50) -> T2IResponse:
        return self.text_to_image.generate(prompt, output_type, num_inference_steps)
    ...
    async def agenerate_text(self, model: str, prompt: str, stream: bool = False) -> T2TGenerateResponse:
        return await self.text_to_text.agenerate(model, prompt, stream)
    ...
    async def achat(self, model: str, messages: List[T2TMessage]) -> T2TChatResponse:
        return await self.text_to_text.achat(model, messages)

    async def agenerate_image(self, prompt: str, output_type: str = "png", num_inference_steps: int = 50) -> T2IResponse:
        return await self.text_to_image.agenerate(prompt, output_type, num_inference_steps)

    async def agenerate_speech(self, text: str) -> T2SResponse:
        return await self.text_to_speech.agenerate(text)
