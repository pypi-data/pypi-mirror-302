...
def find_element(
    xml_tag_or_string: Union[str, BeautifulSoup, Tag], element_name
) -> Tag:
    """
...
def get_footnote_ids(tag: Tag, sep: str = ",") -> str:
    """Get the footnotes from the tag as a string"""
    return sep.join([el.attrs.get("id") for el in tag.find_all("footnoteId")])


def value_with_footnotes(tag: Tag, footnote_sep: str = ",") -> str:
...
def child_value(parent: Tag, child: str, default_value: str = None) -> str:
    """
...
def child_texts(parent: Tag, child: str) -> List[str]:
    """
    Get the text of the value tag within the child tag if it exists or None
...
def optional_decimal(parent: Tag, child: str) -> Optional[Decimal]:
...
def extract_child_text(tag: Tag, key: str, child_tag_name: str) -> Tuple[str, str]:
...
def extract_child_value(tag: Tag, key: str, child_tag_name: str) -> Tuple[str, str]:
