from legalopsrag.database.mongo_db import MongoDataBase
from legalopsrag.embeddings.vogayeai import VoyageAI
from legalopsrag.retrievers.mongodb_retriever import MongoDBRetriever
from legalopsrag.retrievers.retriever_general import RAG,ExtractLegalClause,ReviewLegalClause
from legalopsrag.splitter.spacy_splitter import SpacySplitter
from legalopsrag.azure.blob_storage import AzureBlobStorage
from legalopsrag.utils.utils_func import InputDataLegalExtractor,InputLegalClause
import logging
import dspy
import os
import json
...
async def call_llm(input_data:InputDataLegalExtractor):
    try:
        ...
        try:
            blob_storage_2=AzureBlobStorage(containername,'input-request-response')
            original_request_blob=f"{str(uuid.uuid4())}-original-request"
            blob_storage_2.upload_blob_file(blobname=original_request_blob,data=xml_request)

            original_request_blob_response=f"{original_request_blob}-response"
            blob_storage_2.upload_blob_file(blobname=original_request_blob_response,data=xml_response.answer)
        except Exception as exc:
            err=f"error:{exc}"
            raise Exception(err)
    except Exception as exc:
