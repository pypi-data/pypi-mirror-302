...
def clean_data(
    ...
    for col_name in columns:
        ...
        if col_type == "string":
            # Replace blank text fields with an empty string
            if replaceBlankForTextFields:
                df = df.withColumn(col_name,
                                   when(col(col_name).isNull(), '').otherwise(
                                       col(col_name)))
            ...
            if cleanNumbers:
                df = df.withColumn(col(col_name), regexp_replace(col(col_name), r'\d+', ''))
        ...
        # If the column is numeric, apply numeric-based operations
        elif col_type in ["int", "integer", "float", "double", "long", "short"]:
...
def write_df_locally(_df, attachmentFormat, attachmentName):
    # Convert the Spark DataFrame to a Pandas DataFrame on the driver
    import uuid
