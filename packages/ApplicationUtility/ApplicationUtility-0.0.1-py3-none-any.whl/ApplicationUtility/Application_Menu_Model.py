    ...
    def generate_menu_model(self):
        ...
        if self.connection:
            try:
                ...
                cursor.execute(f"SELECT * FROM APPL_MST WHERE APP_NAME = :app_name", app_name=self.app_name)
                ...
                # logger.log(f"json_data:::\t{json.loads(json_data)[0]}")
                ...
                # logger.log(f"json_data1:::\t{json.loads(json_data1)}")
                menu_json['navigation'] = json.loads(json_data1)

                logger.log(f"final result:::\t{menu_json}")
