...
class FileContentManager:
    ...
    async def read_file(file_path: str) -> str:
        ...
        try:
            ...
            logger.error(f" #### The `file manager agent` encountered an issue: File `{file_path}` not found. Returning an empty string.")
            return ""
    ...
    async def apply_changes(cls, file_path: str, blocks: List[Tuple[str, str]]) -> str:
        ...
        original_content = content  # Keep the original content for accurate replacements

        for search, replace in blocks:
            if search:
                # Normalize both the search block and the file content
                normalized_search = cls.normalize_text(search)
                normalized_content = cls.normalize_text(content)

                # Prepare the regex pattern
                pattern = re.escape(normalized_search)
                # Replace escaped whitespace with a pattern that matches any whitespace
                pattern = pattern.replace(r'\ ', r'\s+')
                regex = re.compile(pattern, re.DOTALL)

                # Search for the pattern in the normalized content
                match = regex.search(normalized_content)
                if match:
                    ...
                    diff = difflib.ndiff([normalized_search], [normalized_content])
