...
class Project(ProjectMixin, models.Model):
    ...
    require_comments_on_skip = models.BooleanField(
        _('Require comment on skip'), default=False, help_text='Annotators must leave a comment on skip'
    )
    ...
    is_pinned = models.BooleanField(_('pinned'), default=False, null=True, blank=True)
    workspace = models.ForeignKey('workspace.Workspaces', on_delete=models.CASCADE, related_name='projects', null=True, blank=True)

    ASSIGNEE_CHOICES = [
        ('AUTO', 'Auto'),
        ('MANUEL', 'Manuel'),
    ]
    task_distribution = models.CharField(max_length=100, choices=ASSIGNEE_CHOICES, null=True, default='AUTO')
    show_dm_to_annotators = models.BooleanField(
        _('show data manager to annotators'),
        default=False,
        help_text='Show data manager interface to annoators',
    )
    # compressed_thumbnail_images = models.JSONField(default=list, blank=True, null=True)
    # Review Settings
    review_distribution = models.CharField(max_length=100, choices=ASSIGNEE_CHOICES, null=True, default='AUTO')
    review_instruction = models.TextField(
        _('review instruction'), blank=True, null=True, default='',
        help_text='Review instructions in HTML format'
    )
    show_review_instruction = models.BooleanField(
        _('show review instruction'), default=False,
        help_text='Show review instructions to the reviewer before they start'
    )
    show_dm_to_reviewers = models.BooleanField(
        _('show datamanger to reveiwer'),default=True,
        help_text='Show datamanger to reveiwer'
    )

    # Image search
    description_ml_model = models.CharField(max_length=20, null=True)
    search_method = models.CharField(max_length=20, null=True, default="word")
    ml_params = models.JSONField(null=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            logger.error("Error initializing Project: %s", e)
            raise
    ...
    def save_compressed_thumbnails(self, tasks):
        """Save compressed images as thumbnail"""
        image_paths = []
        count = 0
        existing_compressed_files = FileUpload.objects.filter(project=self,
                                                              file__contains='_compressed.jpg')

        compressed_images = []
        thumbnail_map = {}  # Dictionary to map file_upload_id to thumbnail

        if existing_compressed_files.count() <= 2:
            for task in tasks:
                if count >= 5:
                    break
                try:
                    ...
                    image_paths.append((file_upload_id, file_path))
                    count += 1
                except (FileUpload.DoesNotExist, IOError, SyntaxError):
                    # File not found or not a valid image file, skip it
                    continue

            for file_upload_id, path in image_paths:
                relative_compressed_path, compressed_img_path = self.compress_image(
                    path)
                if relative_compressed_path and compressed_img_path:
                    compressed_images.append(relative_compressed_path)
                    # Create file uploads for the compressed images
                    thumbnail = FileUpload.objects.create(user=self.created_by,
                                                          project=self,
                                                          file=relative_compressed_path)
                    thumbnail_map[file_upload_id] = thumbnail

            # print("compressed images......", compressed_images)
            for test, img_path in zip(tasks, compressed_images):
                task = Task.objects.get(file_upload_id=test['file_upload_id'])
                task.compressed = thumbnail_map[test['file_upload_id']]
                task.save()

        return compressed_images

    def compress_image(self, file_path, base_dimension=200):
        resize_filter = getattr(Image, 'ANTIALIAS', Image.LANCZOS)

        try:
            img = Image.open(file_path)
            # Convert RGBA, LA or P (with transparency) to RGB
            if img.mode in ('RGBA', 'LA') or (
                    img.mode == 'P' and 'transparency' in img.info) or img.mode == 'P':
                img = img.convert('RGB')

            # Determine the longer dimension and scale accordingly
            if img.size[0] > img.size[1]:  # Width is larger
