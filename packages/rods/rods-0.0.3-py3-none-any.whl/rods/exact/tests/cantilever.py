...
def case4(P=100, nel=5):
    """
    fix-fix, force at midspan
    """
    mat = {
        'area':              1.00,
    ...
    def user_force_load(self):
        n_nodes = self.get_number_of_nodes()
        Q = np.zeros((6, n_nodes))
        Q[2,int(n_nodes//2)+1] = -P
        # Q[4,-1] = 8*0.8*np.pi
        # Q[5,-1] =-8*0.6*np.pi
        return Q

    analysis.degrees_of_freedom[-1][:6, 0] = False  # [current time, dof 0 through 5, node 0]
    analysis.degrees_of_freedom[-1][:6,-1] = False
    analysis.degrees_of_freedom[-1][ 0,-1] = True
    analysis.force_load = functools.partial(user_force_load, analysis)
    return analysis, locals()
...
def main(n="3"):
    ...
    for i in range(len(domain.time[:])):
        postproc.line_plot(domain, (-0.2,1.2), (-0.7,0.7), (-0.7,0.7), i)
    postproc.plt.show()
