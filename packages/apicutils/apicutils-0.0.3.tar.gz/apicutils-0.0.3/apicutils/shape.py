...
def get_pre_shape(xs: np.ndarray, exp_shape: tuple[int, ...]) -> tuple[int, ...]:
    r"""For an array of shape (i_1, ..., i_n, e_1, ..., e_m) and shape (e_1, ..., e_m),
    returns the shape (i_1, ..., i_n). This is an helper for vectorized functions.
    If array shape does not end with expected shape, raises ShapeError.
    """
    if exp_shape == ():
        return xs.shape
    n_dim = len(exp_shape)
