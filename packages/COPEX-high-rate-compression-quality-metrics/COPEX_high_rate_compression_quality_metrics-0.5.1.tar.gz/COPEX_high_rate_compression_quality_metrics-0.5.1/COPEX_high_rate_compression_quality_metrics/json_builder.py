...
import numpy as np
from COPEX_high_rate_compression_quality_metrics import metrics, global_variables
...
def create_summary_json_from_use_case_path_at_use_case_level(root_directory: str, satellite_type: str, test_case_number: any):
    """
    ...
    create_summary_json_at_use_case_level(json_path_list, os.path.join(decompressed_folder_path, output_name))
# ------------------------- DATA/SATELLITE LEVEL -------------------------
def make_json_data_level_name(satellite_type):
    now = datetime.now().strftime("%Y%m%d_%H%M%S")
    return "[" + satellite_type + "]_[" + now + "]_[sum].json"
...
def create_summary_json_at_satellite_type_level_from_data_folder(root_directory: str, satellite_type: str):
    ...
    data_summary_folder_path = os.path.join(data_folder_path, global_variables.summary_output_folder_name)
    ...
    for folder in data_folders:
        ...
        print("input_folder_path = ", json_summary_file_path)
