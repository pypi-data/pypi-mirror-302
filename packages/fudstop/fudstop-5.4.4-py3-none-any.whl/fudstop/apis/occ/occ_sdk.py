...
class occSDK:
    def __init__(self, host:str='localhost', port:int=5432, user:str='chuck', password:str='fud', database:str='market_data', webull_id_df=None, ticker_df=None):
        self.conn = None
        self.pool = None
        self.session = None
    ...
    async def get_probability(self, ticker, vola_type='ATM', term:int=30, db=None):
        try:
            stock_id = await self.get_stock_id(ticker)
            async with httpx.AsyncClient() as client:
                ...
                if data.status_code == 200:
                    ...
                    async with httpx.AsyncClient() as client:
                        ...
                        if data.status_code == 200:
                            ...
                            await db.batch_upsert_dataframe(df, table_name='prob', unique_columns=['ticker', 'expiry', 'vola_type', 'term'])
