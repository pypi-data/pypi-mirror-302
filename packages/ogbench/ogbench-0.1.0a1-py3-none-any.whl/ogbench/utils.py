...
def load_dataset(dataset_path, ob_dtype=np.float32, action_dtype=np.float32, compact_dataset=False):
    ...
    if compact_dataset:
        ...
        # Our goal is to make the dataset have the following structure:
        #                |<--- traj 1 --->|  |<--- traj 2 --->|
        # observations: [s0, s1, s2, s3, s4, s0, s1, s2, s3, s4, ...]
        # actions     : [a0, a1, a2, a3, a4, a0, a1, a2, a3, a4, ...]
...
def make_env_and_dataset(dataset_name, dataset_dir=DEFAULT_DATASET_DIR, compact_dataset=False, env_only=False):
