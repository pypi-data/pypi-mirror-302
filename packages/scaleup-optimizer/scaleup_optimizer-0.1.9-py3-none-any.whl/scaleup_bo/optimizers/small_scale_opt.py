...
class SmallScaleBayesianOptimizer(BaseOptimizer):
    ...
    def propose_next_point(self):
        ...
        def min_obj(X):
            ...
            acquisition_value = self.acq_func.evaluation(X.reshape(1, -1), self.Y_iters_small, self.gp)
    ...
    def optimize(self):
        ...
        self.Y_iters_small = np.array([self.objective_func(x) for x in self.X_iters_small]).astype(float)
        ...
        for _ in range(self.n_calls):
            ...
            self.Y_iters_small = np.append(self.Y_iters_small, Y_next).astype(float)
