...
class Job(RestContext):
    ...
    def create(
        self, name: str, job_options: Union[JobOptions, dict]
    ) -> batch_models.Job:
        ...
        """
        assert isinstance(job_options, (JobOptions, dict))
        if isinstance(job_options, dict):
            job_options = JobOptions(**job_options)
        job_metadata = JobMetadata(name=name, spec=job_options.parse_to_spec())
