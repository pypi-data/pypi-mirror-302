...
def loadable_modules(mgr, dummy):
    for m in mgr.loadable_modules:
        str = "RTC:"                        \
            + m["vendor"] + ":"             \
            + m["category"] + ":"           \
            + m["implementation_id"] + ":"  \
...
def components(mgr, dummy):
    for rtc in mgr.components:
        rtc_class = "RTC:"                                \
            + rtc._vendor + ":"                           \
            + rtc._category + ":"                         \
            + rtc._properties["implementation_id"] + ":"  \
            + rtc._properties["language"] + ":"           \
            + rtc._version
        print("instance name:\t" + rtc._instance_name)
        print("    class name:   " + rtc_class)
        print("    naming name:  " + rtc._properties["naming.names"])
        print("    host name:    " + rtc._properties["os.hostname"])
        print("    manager name: " + rtc._properties["manager.instance_name"])

def configuration(mgr, dummy):
    maxwidth = 0
    for k in mgr.configuration.keys():
        if maxwidth < len(k):
            maxwidth = len(k)
    maxwidth = maxwidth + 2
    for k in mgr.configuration.keys():
        spaces = " " * (maxwidth - len(k))
        print(k + ":" + spaces + mgr.configuration[k])

def set_configuration(mgr, keyvalue):
    (key, value) = keyvalue.split('=')
    mgr.set_config_parameter(key, value)
...
def main(argv=None, tree=None):
    def cmd_cb(option, opt, val, parser):
        ...
        elif opt == '-m' or opt == '--modules':
            parser.values.cmds.append((loadable_modules, val))
        elif opt == '-r' or opt == '--rtcs':
            parser.values.cmds.append((components, val))
        elif opt == '-g' or opt == '--config':
            parser.values.cmds.append((configuration, val))
        elif opt == '-s' or opt == '--setconfig':
            parser.values.cmds.append((set_configuration, val))
...
Create and remove components with a manager.'''
    ...
    parser.add_option('-m', '--modules', action='callback', callback=cmd_cb,
            help='Show loadable modules available on the manager.')
    parser.add_option('-r', '--rtcs', action='callback', callback=cmd_cb,
            help='Show running component on the manager.')
    parser.add_option('-g', '--config', action='callback', callback=cmd_cb,
            help='Getting configuration of the manager.')
    parser.add_option('-s', '--setconfig', action='callback', callback=cmd_cb,
            type='string', help='Setting configuration parameter value of the manager.')
