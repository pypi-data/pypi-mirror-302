...
class JobsClient(ApiContainer):
    ...
    def create(self, params, overwrite=False, run_now=False):
        """
        ...
        """
        with self.tracer.start_as_current_span("create_job"):
            try:
        ...
        with self.tracer.start_as_current_span("create_job"):
            try:
                ...
                if run_now:
                    ...
                    job_id = jobs_create_result["job_id"]
                    notebook_params = self.get_notebook_param(jobs_create_result)
    
                    if job_id:
                        self.logger.info(f"Running job immediately with ID: {job_id}")
                        if notebook_params:
                            self.run_now(job_id, notebook_params)         
                        else:
                            self.run_now(job_id)
                    else:
                        self.logger.error("Failed to retrieve job_id for immediate run.")
    ...
    def create_2_0(self, params):
        ...
        with self.tracer.start_as_current_span("create_job_2_0"):
            try:
                ...
                exc_info = sys.exc_info()
                self.logger.error_with_exception(error_msg, exc_info)
                raise
