...
from metadata.ingestion.ometa.ometa_api import C, MetaMart, T
...
) -> MetaMart[T, C]:  # pyright: ignore[reportInvalidTypeVarUse]
    """Create an MetaMart client

    Args:
        metadata_config (MetaMartConnection): OM connection config

    Returns:
        MetaMart: an OM client
    """
    try:
        metadata = MetaMart[T, C](metadata_config)
...
def get_chart_entities_from_id(
    chart_ids: List[str], metadata: MetaMart, service_name: str
