  ...
  create view "f1"."staging"."stg_ergast__lap__positions__dbt_tmp" as (
    with
raw_lap_times as (select * from "f1"."ingress"."ergast__lap_times"),
...
lap_positions as (
    select
        ...
        md5(cast(coalesce(cast(_session.session_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(driver.driver_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(lap_number as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as lap_id,
    ...
    inner join event_ids as race_session on lap_time.raceid = _session.ergast_race_id
