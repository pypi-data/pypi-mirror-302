...
async def ppu_flow(
    ...
    n = int(np.round(n)) or 1  # np.ceil(n)  # 向上取整
    ...
    if money > MODEL_PRICE.get(post, 0.1):  # 预扣费
        ...
        try:
            api_key and await asyncio.gather(*[appu(post, api_key=api_key, base_url=base_url)] * n)
        except Exception as e:
            logger.error(e)
            send_message(f"{e}\n\n{api_key}", title='ppu')
...
if __name__ == '__main__':
    ...
    async def main():
        try:
            async with ppu_flow(api_key="sk-VO6f9usHwxg8HRK30e0bA699BfA34F08dE7D9FEb161386", post="ppu-1"):
                logger.debug("消费了哦")
