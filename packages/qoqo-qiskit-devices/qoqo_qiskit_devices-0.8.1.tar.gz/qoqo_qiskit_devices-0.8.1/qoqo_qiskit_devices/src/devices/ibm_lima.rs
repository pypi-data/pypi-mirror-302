...
impl IBMLimaDeviceWrapper {
    ...
    pub fn new() -> Self {
        Python::with_gil(|py| {
            py.run("import warnings; warnings.warn(\"Device ibmq_lima has been retired. Setting noise information is not possible.\", category=DeprecationWarning, stacklevel=2)", None, None).unwrap();
        });
        Self {
...
impl IBMLimaDeviceWrapper {
    ...
    pub fn from_pyany(input: Py<PyAny>) -> PyResult<IBMLimaDevice> {
        Python::with_gil(|py| -> PyResult<IBMLimaDevice> {
