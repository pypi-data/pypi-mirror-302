                ...
                for anno in item["annotations"]:
                    for label in anno.get("labels", []):
                        ...
                        if not any(int(label_id) in sublist for sublist in self.label_id2index):
                            continue
                        task_id = int(label["parent_id"])
                        column_index = self.label_id2index[task_id][int(label_id)]
                        annotated_arrays[task_id][image_id_2_index[item["image_id"]], column_index] = 1  # 增加标注数量
    ...
    def _format_multitask_classification_result(self, metric_result: Any):
        ...
        for task_idx, result in enumerate(metric_result):
            annotation_result = AnnotationRatioMetricResult(
                labelName=self.task_labels[str(task_idx)]['label_name'],
                imageCount=[],
                annotatedImageCount=[],
                ratio=[]
            )
        ...
        image_annotation_metric = ImageAnnotationRatioMetric(
            labels=[label for sublist in self.labels for label in sublist],
            metrics=results
        )
