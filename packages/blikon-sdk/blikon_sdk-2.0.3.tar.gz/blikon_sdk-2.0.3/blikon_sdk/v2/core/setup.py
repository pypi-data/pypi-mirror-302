...
def _setup_httpx_client() -> None:
    ...
    try:
        ...
        httpx_client: httpx.AsyncClient = httpx.AsyncClient(timeout=httpx.Timeout(sdk_config.sdk_settings.http_timeout_sec, connect_timeout=sdk_config.sdk_settings.http_connect_sec))
...
def _log_succesfull_setup() -> None:
    ...
    log_service.info(f"{log_service.get_line()}")
    log_service.info(f"{log_service.get_line()}")
    log_service.info("Use the 'msg' function to deliver messages in the application's specified language.")
    log_service.info(f"Importing the 'msg' function: 'from blikon_sdk.v2.helpers.msg_helper import msg'")
    log_service.info(f"{log_service.get_line()}")
