...
def get_example_value(type_hint):
    ...
    if origin is None:
        if isinstance(type_hint, type) and issubclass(type_hint, BaseModel):
            example = {}
            for field_name, field_info in type_hint.__fields__.items():
                example[field_name] = get_example_value(field_info.outer_type_)
            return example
        else:
...
def add_json_instructions_to_messages(
    ...
    if response_model:
        ...
        for key, type_hint in type_hints.items():
            ...
            field_description = response_model.__fields__[key].field_info.description
