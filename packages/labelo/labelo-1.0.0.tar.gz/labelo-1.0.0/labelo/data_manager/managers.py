...
from labelo.core.utils.common import load_func
from labelo.core.utils.params import cast_bool_from_str
...
def get_fields_for_evaluation(prepare_params, user, skip_regular=True):
    ...
    if fields:
        from labelo.data_manager.functions import TASKS
...
def annotate_storage_filename(queryset: TaskQuerySet) -> TaskQuerySet:
    from labelo.data_manager.functions import intersperse
...
def annotate_reviwers(queryset):
    ...
    queryset = queryset.annotate(
        reviewers=Subquery(
            TaskAssignee.objects.filter(
                task=OuterRef('pk'),
                type='RV'
            ).values('task').annotate(count=Count('id')).values('count'),
            output_field=CharField()
        )
    )
    return queryset

def annotate_reviews_accepted(queryset):
    return queryset.annotate(
    reviews_accepted=Count(
        Case(
            When(annotations__review_status='Accept', then=1),
            output_field=IntegerField()
        )
    )
)

def annotate_reviews_rejected(queryset):
    from django.db.models import Count, Case, When, IntegerField
    return queryset.annotate(
    reviews_rejected=Count(
        Case(
            When(annotations__review_status='Reject', then=1),
            output_field=IntegerField()
        )
    )
)
...
settings.DATA_MANAGER_ANNOTATIONS_MAP = {
    ...
    'reviewers': annotate_reviwers,
    'reviews_accepted': annotate_reviews_accepted,
    'reviews_rejected': annotate_reviews_rejected,
}
