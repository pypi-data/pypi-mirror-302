...
@router.post("/submit_schema_form/{name}")
async def submit_schema_form(
    form_name: str, payload: dict, session: AsyncSessionDep, current_user: CurrentUser
):
    ...
    if not flow_class:
        raise HTTPException(
            status_code=404, detail=f"Flow for form {form_name} not found"
        )

    async def stream():
        result = await flow_class.execute(session, current_user, payload)
