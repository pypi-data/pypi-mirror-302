...
class ManipulationModelling():
    ...
    def setup_motion(self, C, K, homing_scale, acceleration_scale, accumulated_collisions, joint_limits, quaternion_norms):
	assert self.komo==None
        self.komo = ry.KOMO(C, K, 16, 2, accumulated_collisions)
        if homing_scale>0.:
		self.komo.addControlObjective([], 0, homing_scale)
    ...
    def add_helper_frame(self, type, parent, name, initFrame, markerSize):
    ...
    def grasp_cylinder(self, time, gripper, obj, palm, margin=.02):
        ...
        """
        size = self.C.getFrame(obj).getSize()[:2]
    ...
    def place_box(self, time, obj, table, palm, place_direction='z', margin=.02):
        ...
        obj_frame = self.C.getFrame(obj)
        boxSize = self.C.getFrame(obj).getSize()
    ...
    def straight_push(self, times, obj, gripper, table):
        ...
        if not self.komo.world.getFrame(helperStart, False):
            self.add_helper_frame(ry.JT.hingeZ, table, helperStart, obj, 0, .3)
        if not self.komo.world.getFrame(helperEnd, False):
            self.add_helper_frame(ry.JT.transXYPhi, table, helperEnd, obj, 0, .3)
    ...
    def sub_motion(self, phase, fixEnd, homing_scale=1e-2, acceleration_scale=1e-1, accumulated_collisions=True, quaternion_norms=False):
