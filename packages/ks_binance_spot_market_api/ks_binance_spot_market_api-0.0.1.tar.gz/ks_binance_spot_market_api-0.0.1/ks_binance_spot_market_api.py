...
class KsBinanceSpotMarketApi(BaseMarketApi):
    ...
    def query_contract(self, vt_symbol: str) -> tuple[KsRetCode, ContractData]:
        ...
        if target:
            ...
            target['lot_size'] = to_decimal(lot_filter["minQty"]) if lot_filter else Decimal(0)

            notional_filter = pydash.find(target['filters'], lambda x: x['filterType'] == 'NOTIONAL')
            min_notioncal = Decimal('0')
            if notional_filter:
                min_notioncal = Decimal(notional_filter.get('minNotional') or '0')
            target['min_notional'] = min_notioncal
        ...
        contract = ContractData(
            ...
            size=Decimal('1'),
            min_volume=target['lot_size'],
            pricetick=target['pricetick'],
            name=symbol,
    ...
    def query_contracts(
        ...
        if df:
            ...
            return KS_RET_OK, data[['vt_symbol', 'product', 'size', 'min_volume', 'pricetick', 'gateway']]
    ...
    def query_quotes(self, vt_symbols: list[str], df: bool = True) -> Union[KsRetCode, list[QuoteData]]:
        ...
        if self.coinmarketcap_api:
            ...
            data = data.merge(cap_df[['symbol', 'circular_market_cap', 'total_market_cap']], on='symbol', how='left')
            data.drop_duplicates(subset=['symbol'], keep='first', inplace=True) # 有些是重复数据
        else:
            data['circular_market_cap'] = ''
