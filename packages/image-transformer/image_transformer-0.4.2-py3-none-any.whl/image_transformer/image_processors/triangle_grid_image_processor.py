...
class TriangleGridImageProcessor(GenericGridImageProcessor):
    """
    ...
    def __init__(self, hexagon_size: int):
        """
        Initializes the TriangleGridImageProcessor with the given hexagon size.
    ...
    def draw_grid_element_at(self, context: GenericOutputBuilder, grid_element_position: Tuple[int, int], color: Tuple[int, int, int]) -> None:
        ...
        center_x, center_y = self.from_grid_coordinates_to_center_in_pixel_coordinates((q, r))

        triangle_vertices = []
        for i in range(6):
            angle_deg = 60 * i + 30
            angle_rad = np.radians(angle_deg)
            vertex_x = center_x + self.hexagon_size * np.cos(angle_rad)
            vertex_y = center_y + self.hexagon_size * np.sin(angle_rad)
            triangle_vertices.append((vertex_x, vertex_y))
        
        for i in range(6):
            triangle = [
                (center_x, center_y),
                triangle_vertices[i],
                triangle_vertices[(i + 1) % 6]
            ]
            context.add_polygon(triangle, color)
    ...
    def approximate_number_of_grid_elements(self: 'GenericGridImageProcessor', width: int, height: int) -> int:
        ...
        return int(num_hexagons_width * num_hexagons_height) * 6