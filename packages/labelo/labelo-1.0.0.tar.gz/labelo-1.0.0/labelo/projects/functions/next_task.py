...
from tasks.models import Annotation, Task, TaskAssignee
...
def get_not_solved_tasks_qs(
    ...
    if not assigned_flag:
        ...
        # otherwise, filtering out completed tasks is sufficient
            ...
            if not project.task_distribution == 'MANUEL':
...
def get_not_solved_tasks_with_assign_annotators(user, not_solved_tasks):
    task_ids = TaskAssignee.objects.filter(
        assignee=user,
        task__in=not_solved_tasks,
        type='AN'
    ).values('task_id')

    tasks = Task.objects.filter(id__in=Subquery(task_ids))
    return tasks if tasks.exists() else Task.objects.none()
...
def get_next_task(
    ...
    with conditional_atomic(predicate=db_is_not_sqlite):
        ...
        if project.task_distribution == 'MANUEL':
            not_solved_tasks = get_not_solved_tasks_with_assign_annotators(
                user,not_solved_tasks
            )
