...
class OCR:
    ...
    @staticmethod
    def slide_file(target_file, bg_file, algorithm=1):
        """
            滑块识别
        ...
        target_file_bytes = OCR.content_to_bytes(target_file)
        bg_file = open(bg_file, "rb").read()
        bg_file_bytes = OCR.content_to_bytes(bg_file)
    ...
    @staticmethod
    def slide_base64(target_b64str, bg_b64str, algorithm=1):
        """
            滑块识别
        ...
        if algorithm != 1:
            method = "compare"
        target_b64str = OCR.content_to_bytes(target_b64str)
        target_b64str = base64.b64encode(target_b64str).decode()
        bg_b64str = OCR.content_to_bytes(bg_b64str)
    ...
    @staticmethod
    def content_to_bytes(image_data):
