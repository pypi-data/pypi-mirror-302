...
from tasksch.triggers import WeeklyTrigger
...
def set_weekly_schedule_trigger(task: TaskDefinition):
    trigger: WeeklyTrigger = task.triggers.create_trigger(
        TriggerType.TASK_TRIGGER_WEEKLY
    )
    trigger.weeks_interval = 2
    trigger.week_days = True
...
def main(ts: TaskService, demo_folder: TaskFolder, args: DemoArgs):
    ...
    Create a scheduled weekly task that triggers once a week
    and writes to the demo log file.

    """
    task_name = "WEEKLY-DEMO"
    if args.suffix:
    ...
    set_weekly_schedule_trigger(task)
