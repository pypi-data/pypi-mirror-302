...
class Object_Detection_Plot:
    ...
    def draw(self, plot_type, blind_spot_args=None,  
              top_misses_args=None, classbased_table_args=None, mixed_args=None,
                overall_args=None, confidence_histogram_args=None, threshold=None,
                save_path=None):
        if plot_type == 'blind_spot':
            self._plot_blind_spot(blind_spot_args,threshold, save_path)
        elif plot_type == 'top_misses':
            self._plot_top_misses(top_misses_args, save_path)
        elif plot_type == 'classbased_table':
            self._plot_classbased_table_metrics(classbased_table_args,threshold, save_path)
        elif plot_type == 'mixed_plot':
            self._plot_mixed_metrics(mixed_args,threshold, save_path)        
        elif plot_type == 'overall_metrics':
            self._plot_overall_metrics(overall_args,threshold, save_path)
        elif plot_type == 'confidence_histogram':
           self._plot_confidence_histogram(confidence_histogram_args, save_path)
        else:
            raise ValueError(f"Unsupported plot type: {plot_type}")
    
    def _plot_blind_spot(self, blind_spot_args, threshold, save_path):
    ...
    def _plot_overall_metrics(self, overall_args, threshold, save_path):
    ...
    def _plot_top_misses(self, top_misses_args, save_path):
        ...
        if top_misses_args and isinstance(top_misses_args, dict):
        ...
        plt.figure(figsize=(12, 8))
        sns.barplot(x='mIoU', y='image_id', data=df, palette='viridis')
    ...
    def _plot_classbased_table_metrics(self,classbased_table_args, threshold, save_path):
        ...
        if classbased_table_args:
            ...
            if threshold:
                df = df[df.apply(lambda row: all(float(val) > threshold for val in row if val != "None"), axis=1)]        
        df = df.reset_index().rename(columns={'index': 'Class'})
        df = df.melt(id_vars=['Class'], var_name='Metric', value_name='Value')
        df['Value'] = df['Value'].astype(float)
        plt.figure(figsize=(12, 8))
        sns.barplot(x='Value', y='Class', hue='Metric', data=df, palette='viridis')
...
#histogram_save_path = save_path.replace('.png', '_histogram.png') if save_path else 'histogram.png'
    ...
    def _plot_mixed_metrics(self, mixed_args,threshold, save_path):
    ...
    def _plot_confidence_histogram(self, confidence_histogram_args, save_path):
        ...
        # Histogram
        plt.figure(figsize=(12, 8))
        sns.barplot(x='category', y='value', data=histogram_df, palette='viridis')
