...
class Canvas(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.drawing_rect_color = QColor(0, 0, 128, 100)
    ...
    def mouseMoveEvent(self, ev):
        ...
        for shape in reversed([s for s in priority_list if self.isVisible(s)]):
            ...
            if index is not None:
                ...
                current_height = abs(point1.y() - point3.y())
                self.parent().window().label_coordinates.setText(
                    '%s (Label), Width: %d, Height: %d / X: %d; Y: %d' % (shape.label, current_width, current_height, pos.x(), pos.y()))
                break
    ...
    def paintEvent(self, event):
        ...
        if self.verified:
            pal = self.palette()
            pal.setColor(self.backgroundRole(), QColor(184, 239, 38, 50))  # TA 128
            self.setPalette(pal)
        else:
            pal = self.palette()
            pal.setColor(self.backgroundRole(), QColor(232, 232, 232, 50))
            self.setPalette(pal)
