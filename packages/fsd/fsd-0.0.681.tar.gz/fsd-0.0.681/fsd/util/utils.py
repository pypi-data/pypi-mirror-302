...
def read_file_content(file_path):
    ...
    if not os.path.exists(file_path):
        logger.debug(f" #### `File Manager Agent`: File does not exist: `{file_path}`")
        return ""
    ...
    if mime_type == 'application/pdf':
        try:
            ...
            logger.debug(f" #### `File Manager Agent`: Failed to read PDF file `{file_path}`: `{e}`")
    ...
    # Handle DOC and DOCX files
        ...
        try:
            ...
            logger.debug(f" #### `File Manager Agent`: Failed to read DOC/DOCX file `{file_path}`: `{e}`")
    ...
    # Handle XLSX (Excel) files
        ...
        try:
            ...
            logger.debug(f" #### `File Manager Agent`: Failed to read XLSX file `{file_path}`: `{e}`")
    ...
    try:
        ...
        logger.debug(f" #### `File Manager Agent`: Failed to read text/code file `{file_path}`: `{e}`")
    ...
    try:
        ...
        logger.debug(f" #### `File Manager Agent`: Binary read attempt for file `{file_path}` failed: `{e}`")
...
def parse_payload(user_prompt_json):
    try:
        ...
        logger.debug(f" #### Received Plain Text Prompt: `{user_prompt}`")
