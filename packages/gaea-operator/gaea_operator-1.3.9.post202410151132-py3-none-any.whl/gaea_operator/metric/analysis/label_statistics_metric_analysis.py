...
class LabelStatisticMetricAnalysis(object):
    ...
    def update(self, predictions: List[Dict], references: List[Dict], **kwargs):
        ...
        for key, metrics in self.metric.items():
            if isinstance(metrics, list):
                ...
                if key in annotation_dict:
                    metrics.update(annotations=annotation_dict[key])
    ...
    def _format_input(self, annotations: List[Dict]):
        ...
        if self.category in ("Image/ImageClassification/MultiTask"):
            ...
            annotation_dict[ANNOTATION_CONFIDENCE_STATISTIC_METRIC] = []
            annotation_dict[ANNOTATION_AREA_STATISTIC_METRIC] = []
            annotation_dict[ANNOTATION_WIDTH_STATISTIC_METRIC] = []
            annotation_dict[ANNOTATION_HEIGHT_STATISTIC_METRIC] = []
    ...
    def _format_to_multitask_classification(self, annotations: List[Dict]):
            ...
            for item in annotations:
                ...
                for anno in item["annotations"]:
                    for idx in range(len(anno["labels"])):
                        pred_array = [np.zeros(len(label)) for label in self.labels]
                        if isinstance(anno["labels"][idx]["id"], str):
                            anno["labels"][idx]["id"] = int(anno["labels"][idx]["id"])
                        if math.isnan(anno["labels"][idx]["id"]):
                            continue
                        # 如果标注标签id不在label,则跳过（修改了标签但是标注没有同步修改）
                        if str(anno["labels"][idx]["id"]) not in \
                                self.label_id2index[int(anno["labels"][idx]["parent_id"])]:
                            continue
                        task_id = int(anno["labels"][idx]["parent_id"])
                        column_index = self.label_id2index[task_id][str(anno["labels"][idx]["id"])]
                        label_name = self.label_index2name[task_id][column_index]
                        if self.task_kind != self.manual_task_kind:
                            confidence = anno["labels"][idx].get("confidence", 1)
                            annotation_dict[ANNOTATION_CONFIDENCE_STATISTIC_METRIC][task_id][label_name]\
                                .append(confidence)
                        pred_array[int(anno["labels"][idx]["parent_id"])][column_index] = 1
                        annotation_dict[LABEL_STATISTIC_METRIC].append(pred_array)

            for key, anno in annotation_dict.items():
                if key == LABEL_STATISTIC_METRIC:
                    if len(anno) == 0:
                        annotation_dict[key] = [np.zeros(len(label)) for label in self.labels]
                # 过滤掉都是零值
                if key in [ANNOTATION_CONFIDENCE_STATISTIC_METRIC,
                           ANNOTATION_AREA_STATISTIC_METRIC,
                           ANNOTATION_WIDTH_STATISTIC_METRIC,
    ...
    def _format_multitask_classification_result(self, metric_result: Dict):
        ...
        metric = LabelStatisticsMetric(labels=label_names, metrics=[])
        label_count = LabelCountStatistic(name=LABEL_STATISTIC_METRIC,
                                          displayName="标签统计",
                                          result=[])
    ...
    def compute(self):
        ...
        for key, metrics in self.metric.items():
            ...
            if isinstance(metrics, list):
                ...
                results[key][metrics.name] = metrics.compute()
