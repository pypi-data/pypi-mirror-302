...
from igx_api.l2.types.filter import Condition, Filter, FilterId, TemplatedCondition, TemplatedFilter
...
class FilterApi:
    ...
    def create_filter(self, name: str, condition: Condition, shared: bool = False) -> Filter:
        ...
        Args:
            ...
            condition (igx_api.l2.types.filter.Condition): The condition of the filter.
            shared (bool): Whether the filter should be shared with other users in the organization. Defaults to False.

        Returns:
            igx_api.l2.types.filter.Filter: The newly created filter.

        Raises:
            igx_api.l2.client.api.filter_api.NameEmpty: If the name of the filter is empty.
    ...
    def create_templated_filter(self, name: str, condition: TemplatedCondition, shared: bool = False) -> TemplatedFilter:
        ...
        Args:
            name (str): The name of the filter.
            condition (igx_api.l2.types.filter.TemplatedCondition): The condition of the filter.
            shared (bool, optional): Whether the filter should be shared with other people in the organization. Defaults to False.

        Returns:
            igx_api.l2.types.filter.TemplatedFilter: The newly created filter.
    ...
    def get_filter_by_id(self, filter_id: FilterId) -> Filter:  # TODO: version parameter (when filter versioning is implemented)
        ...
        Args:
            filter_id (igx_api.l2.types.filter.FilterId): The ID of the filter to get.

        Returns:
            igx_api.l2.types.filter.Filter: The filter with the given ID.
    ...
    def get_templated_filter_by_id(self, filter_id: FilterId) -> TemplatedFilter:  # TODO: version parameter (when filter versioning is implemented)
        ...
        Returns:
            igx_api.l2.types.filter.TemplatedFilter: The templated filter with the given ID.
