...
class ControllerAgent:
    ...
    async def regular_code_task_pipeline(self, tier, finalPrompt, role, original_prompt_language):
        ...
        while True:
            if not isFirst:
                logger.info(" #### You are in code mode. Type your feedback to continue building, or click 'Exit' to leave code mode.")
                logger.info("#### code-mode")
    ...
    async def codingProgress(self, tier, finalPrompt, role, original_prompt_language):
        ...
        if tier == "Pro":
            commits = await self.get_pro_coding_requests(final_request, all_context, list(final_working_files), context_files, role, crawl_logs, original_prompt_language)
        else:
            commits = await self.get_coding_requests(final_request, all_context, list(final_working_files), context_files, role, crawl_logs, original_prompt_language)
