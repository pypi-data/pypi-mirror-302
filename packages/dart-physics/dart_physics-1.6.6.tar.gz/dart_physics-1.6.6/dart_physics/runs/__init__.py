...
def load_task_cfg(task): 
    ...
    if os.path.exists(os.path.join(_HERE, "..", "cfgs", f"{task}.yaml")):
        ...
        print("First tried to load task from ", os.path.join(_HERE, "..", "cfgs", f"{task}.yaml").absolute())
        print("Then tried to load task from ", os.path.join(_HERE, "..", "cfgs", f"{task}.py").absolute())
