...
class NewOracleBookkeepingDB:
    ...
    @convertToReturnValue
    def getInputOuputFilesForProd(self, prod_id, run_number):
        query_parts = [
        ...
        return {
            "Records": result,
            "ParameterNames": ["AncestorLFN", "DecendantLFN", "GotReplica", "FileType"],
    ...
    def getFilesWithMetadata(
        ...
        *,
        ...
        forceJoins = []
        if columns is None:
            columns = list(GETFILESWITHMETADATA_AVAILABLE_COLUMNS)
        selection = []
        for col in columns:
            if col not in GETFILESWITHMETADATA_AVAILABLE_COLUMNS:
    ...
    def getFilesSummary(
        ...
        selection = (
            "COUNT(DISTINCT fileid), SUM(f.EventStat), SUM(f.FILESIZE), SUM(f.luminosity), SUM(f.instLuminosity)"
        )
        return buildQuery(
            self.dbR_,
            selection,
