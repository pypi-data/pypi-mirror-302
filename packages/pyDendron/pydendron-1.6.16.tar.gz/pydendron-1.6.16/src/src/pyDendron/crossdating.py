...
COLS_D = {DIST_OVERLAP: 'Int32', DIST_OVERLAP_NAN: 'Int32', DIST: 'float32', D_RANK: 'Int32'}

COLS_BASE = {IDX_MASTER: 'Int32', IDX: 'Int32', 
        KEYCODE_MASTER: 'string', KEYCODE: 'string', 
        DATA_TYPE: 'string', 
        OFFSET: 'Int32', DATE_BEGIN_ESTIMATED: 'Int32', DATE_END_ESTIMATED: 'Int32', SYNC: 'string', 
        }

COLS = {**COLS_BASE, **COLS_R, **COLS_GLK, **COLS_D}

(RES_IDX_MASTER, RES_IDX, 
...
class CrossDatingWorker:
    ...
    def run_it(self, row, master_data):        
        results = []
        for idx_master, row_master in master_data.iterrows():
    ...
    def run(self, data=None, master_data=None, progress=None, stop_event=None):
        ...
        progress.reset(last_data)
        for i, idx in enumerate(self.data.index[:last_data]):
            row = self.data.iloc[i]
            master_start = i + 1 if self.self_crossdating else 0
            for idx_master, row_master in self.master_data.iloc[master_start:].iterrows():
                results += self.run_series(row, row_master) 
    ...
    def run_series(self, series, master_series):
        ...
        res_default[RES_IDX_MASTER:RES_DATA_TYPE+1] =  master_idx, series_idx,  master_keycode, series_keycode, self.data_type
...
class CrossDating(param.Parameterized):
    ...
    def results_to_df(self):
        ...
        if self.df_results is None:
            self.df_results = pd.DataFrame(np.array(self.results), columns=list(self.COLS.keys()))
        return self.df_results

    def get_cols_method(self):
    ...
    # https://en.wikipedia.org/wiki/Discounted_cumulative_gain
        ...
        dated = results[results[SYNC] == 'true']
        n = len(dated)
        if max_rank is not None:
        ...
        if dated['DCG'].sum() == np.inf:
            for idx, row in dated[[KEYCODE, KEYCODE_MASTER, score_key, rank_key, 'DCG']].iterrows():
                print(row.to_list())
...
#            srank += rank
    ...
    def density(self, score=None):
        def get_param(data):
            ...
            #aspect = self.param_density.aspect
        ...
        def get_figure(idx, keycode, y_range, width, height, x_range, formater, font_size):
        ...
        def set_legend(items, font_size):
            legend = Legend(items=items, 
                            ...
                            #padding = 1,
                            #margin = 5,
                            #glyph_width = round(4*aspect),
                            #glyph_height = round(4*aspect)
                            click_policy="hide"
                            )
        ...
        master_keycodes = {x: data.loc[data[IDX_MASTER] == x, KEYCODE_MASTER].iloc[0] for x in data[IDX_MASTER].unique()}
        legends = []
        rows = []
        source_vect = {}
        source = {}
        #n_colors = len(Blues256) - 100
        #colors = np.linspace(0, n_colors, len(data[IDX_MASTER].unique()), dtype=int)
        y_max = 0
        for idx, idx_data in data.groupby(IDX):
            colors_b = get_n_colors(len(idx_data[IDX_MASTER].unique()), Blues256)
            n_colors_b = len(colors_b)
            colors_r = get_n_colors(len(idx_data[IDX_MASTER].unique()), Turbo256)
            n_colors_r = len(colors_r)
            source_vect[idx] = {}
            source[idx] = {}
            for i, (idx_master, master_data) in enumerate(idx_data.groupby(IDX_MASTER)):
                pdf = gaussian_kde(master_data[score].dropna())
                y = pdf(x)
                y[0] = y[-1] = 0
                ly_max = np.nanmax(y)
                ...
                if not master_data[SYNC].isna().any():
                    mask = master_data[SYNC] == 'true'
                    ey = master_data.loc[mask, DATE_END_ESTIMATED].max()
                    dx = master_data.loc[mask, score].max()
                    if not np.isnan(dx):
                        dy = pdf(dx)[0]
                source_vect[idx][idx_master] = ColumnDataSource(data={'x': x, 'y': y}) 
                source[idx][idx_master] = ColumnDataSource(data={'dx': [dx], 'dy': [dy], 
                                                            'ey': [ey], 
                                                            'x0': [dx], 'x1': [dx], 
                                                            'y0': [0], 'y1': [dy], 
        ...
        for idx, idx_data in data.groupby(IDX):
            items = []
            keycode = idx_data[KEYCODE].iloc[0]
            p = get_figure(idx, keycode, y_range, *param)
            curve = []
            for i, (idx_master, master_data) in enumerate(idx_data.groupby(IDX_MASTER)):
                curve.append(p.patch(x='x', y='y', source=source_vect[idx][idx_master], alpha=0.2, color=colors_b[i % n_colors_b]))

            for i, (idx_master, master_data) in enumerate(idx_data.groupby(IDX_MASTER)):
                score_dated=source[idx][idx_master].data['dx'][0]
                if not np.isnan(score_dated):
                    label = f'{master_keycodes[idx_master]}: {round(float(score_dated), 3)}'
                    dated = p.scatter(x='dx', y='dy', source=source[idx][idx_master], color='color', size=self.param_density.bullet_size)#, legend_label=f'{master_keycodes[idx_master]}')
                    dated_seg = p.segment(x0='x0', x1='x1', y0='y0', y1='y1', source=source[idx][idx_master], color='color')
                    items.append((label, [dated, dated_seg, curve[i]]))
                else:
                    label = f'{master_keycodes[idx_master]}: undated'
    ...
    def map(self, score, data_dt=None, threshold=0, max_rank=None):
        ...
        geo = {row[IDX_CHILD]: lnglat_to_meters(row[SITE_LONGITUDE], row[SITE_LATITUDE]) for _, row in data_dt.iterrows()}
        ...
        for i, (idx, value) in enumerate(nodes.items()):
            if (geo[idx][0] is not None) and (geo[idx][1] is not None) and (idx in nodes.keys()):
                ...
                info.append({'i':i+1, IDX: idx, KEYCODE: keycodes[idx], score: value, 
                             'x': x, 'y': y, 'dx': dx, 'dy': dy, 
                             'size': size, 'anchor_str': anchor_str,
                            'legend_label': f'{i+1}: {keycodes[idx]} (sum of {score} = {round(value, 3)})'})
        ...
        if self.param_map.map_type == 2:
            for _, row in data.iterrows():
                ...
                x = [geo[idx][0], geo[idx_master][0]]
                y = [geo[idx][1], geo[idx_master][1]]
    ...
    def graph(self, score, threshold=0, max_rank=None):
        ...
        fig = figure(x_range=(-2, 2), y_range=(-2, 2),
           ...
           height=self.param_graph.height, width=self.param_graph.width, 
        ...
        for key, (x, y) in graph.layout_provider.graph_layout.items():
            ...
            #s = int(max(nodes[idx], 1) * self.param_graph.bullet_ratio) + 10
            s = 10
            ox = s if x > 0 else -s
            oy = -s if y > 0 else s
            anchor.append(a)
