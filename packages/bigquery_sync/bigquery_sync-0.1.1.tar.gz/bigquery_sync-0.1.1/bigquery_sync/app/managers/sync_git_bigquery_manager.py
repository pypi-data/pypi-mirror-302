...
class SyncGitBigqueryManager():
    ...
    def sync_from_bigquery_to_git(self, commit_and_push=False, datasets=None):
        commit_and_push = str(commit_and_push).lower() == 'true'
        ...
        for i in range(RETRY_COUNT):
            ...
            for project in self.projects:
                ...
                bigquery_view_manager.fetch_views(datasets=datasets)
            ...
            if commit_and_push:
                is_committed = self.git_manager.git_commit(message='Fetched the following updates from BigQuery:',
                                                           include_files_in_message=True)
                if not is_committed:
                    break
