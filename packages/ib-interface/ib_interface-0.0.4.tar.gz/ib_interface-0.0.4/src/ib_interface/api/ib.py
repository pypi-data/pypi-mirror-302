...
class IB:
    ...
    **user code may not block for too long**.
    ...
    def __repr__(self):
        conn = (
            f"connected to {self.client.host}:" f"{self.client.port} clientId={self.client.clientId}"
    ...
    def connect(
        ...
        """
        return self._run(self.connectAsync(host, port, clientId, timeout, readonly, account, raiseSyncErrors))
    ...
    def pnl(self, account="", modelCode="") -> List[PnL]:
        ...
        return [
            ...
            if (not account or v.account == account) and (not modelCode or v.modelCode == modelCode)
        ]
    ...
    def openTrades(self) -> List[Trade]:
        ...
        return [v for v in self.wrapper.trades.values() if v.orderStatus.status not in OrderStatus.DoneStates]
    ...
    def bracketOrder(
        self, action: str, quantity: float, limitPrice: float, takeProfitPrice: float, stopLossPrice: float, **kwargs
    ) -> BracketOrder:
        """
    ...
    def cancelPnL(self, account, modelCode: str = ""):
        ...
        if reqId:
            ...
            self._logger.error("cancelPnL: No subscription for " f"account {account}, modelCode {modelCode}")
    ...
    def cancelPnLSingle(self, account: str, modelCode: str, conId: int):
        ...
        if reqId:
            ...
            self._logger.error(
                "cancelPnLSingle: No subscription for " f"account {account}, modelCode {modelCode}, conId {conId}"
            )
    ...
    def reqRealTimeBars(
        self, contract: Contract, barSize: int, whatToShow: str, useRTH: bool, realTimeBarsOptions: List[TagValue] = []
    ) -> RealTimeBarList:
        """
        ...
        self.client.reqRealTimeBars(reqId, contract, barSize, whatToShow, useRTH, realTimeBarsOptions)
        return bars
    ...
    def reqHistoricalSchedule(
        ...
        """
        return self._run(self.reqHistoricalScheduleAsync(contract, numDays, endDateTime, useRTH))
    ...
    def reqHistoricalTicks(
        ...
        return self._run(
            self.reqHistoricalTicksAsync(
                contract, startDateTime, endDateTime, numberOfTicks, whatToShow, useRth, ignoreSize, miscOptions
            )
        )
    ...
    def reqTickByTickData(
        self, contract: Contract, tickType: str, numberOfTicks: int = 0, ignoreSize: bool = False
    ) -> Ticker:
        """
        ...
        self.client.reqTickByTickData(reqId, contract, tickType, numberOfTicks, ignoreSize)
        return ticker
    ...
    def reqMktDepth(
        self, contract: Contract, numRows: int = 5, isSmartDepth: bool = False, mktDepthOptions=None
    ) -> Ticker:
        """
    ...
    def reqFundamentalData(
        self, contract: Contract, reportType: str, fundamentalDataOptions: List[TagValue] = []
    ) -> str:
        """
    ...
    def reqScannerData(
        ...
        """
        return self._run(
            self.reqScannerDataAsync(subscription, scannerSubscriptionOptions, scannerSubscriptionFilterOptions)
        )
    ...
    def calculateImpliedVolatility(
        self, contract: Contract, optionPrice: float, underPrice: float, implVolOptions: List[TagValue] = []
    ) -> OptionComputation:
        """
        ...
        """
        return self._run(self.calculateImpliedVolatilityAsync(contract, optionPrice, underPrice, implVolOptions))

    def calculateOptionPrice(
        self, contract: Contract, volatility: float, underPrice: float, optPrcOptions: List[TagValue] = []
    ) -> OptionComputation:
        """
    ...
    def reqSecDefOptParams(
        ...
        """
        return self._run(
            self.reqSecDefOptParamsAsync(underlyingSymbol, futFopExchange, underlyingSecType, underlyingConId)
        )

    def exerciseOptions(
        self, contract: Contract, exerciseAction: int, exerciseQuantity: int, account: str, override: int
    ):
        """
    ...
    def reqHistoricalNews(
        ...
        return self._run(
            self.reqHistoricalNewsAsync(
                conId, providerCodes, startDateTime, endDateTime, totalResults, historicalNewsOptions
            )
        )
    ...
    async def qualifyContractsAsync(self, *contracts: Contract) -> List[Contract]:
        detailsLists = await asyncio.gather(*(self.reqContractDetailsAsync(c) for c in contracts))
        result = []
        for contract, detailsList in zip(contracts, detailsLists):
            if not detailsList:
                ...
                self._logger.warning(f"Ambiguous contract: {contract}, " f"possibles are {possibles}")
            else:
    ...
    def reqExecutionsAsync(self, execFilter: Optional[ExecutionFilter] = None) -> Awaitable[List[Fill]]:
    ...
    async def reqMarketRuleAsync(self, marketRuleId: int) -> Optional[List[PriceIncrement]]:
    ...
    def reqHistoricalTicksAsync(
        ...
        self.client.reqHistoricalTicks(
            reqId, contract, start, end, numberOfTicks, whatToShow, useRth, ignoreSize, miscOptions
        )
        return future
    ...
    async def reqScannerDataAsync(
        ...
        dataList = self.reqScannerSubscription(
            subscription, scannerSubscriptionOptions or [], scannerSubscriptionFilterOptions or []
        )
    ...
    def reqSecDefOptParamsAsync(
        ...
        reqId = self.client.getReqId()
        future = self.wrapper.startReq(reqId)
        self.client.reqSecDefOptParams(reqId, underlyingSymbol, futFopExchange, underlyingSecType, underlyingConId)
        return future
    ...
    async def reqHistoricalNewsAsync(
        ...
        self.client.reqHistoricalNews(reqId, conId, providerCodes, start, end, totalResults, historicalNewsOptions)
        try:
            await asyncio.wait_for(future, 4)
            return future.result()
        except asyncio.TimeoutError:
