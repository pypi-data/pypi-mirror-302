...
from tidsreg.utils import str_to_time, trunc
...
def test_truncation():
    ...
    assert trunc(text, None) == text
    assert trunc(text, 100) == text
    assert trunc(text, 19) == text
    assert trunc(text, 18) == "This is a long ..."
    assert trunc(text, 6) == "Thi..."
