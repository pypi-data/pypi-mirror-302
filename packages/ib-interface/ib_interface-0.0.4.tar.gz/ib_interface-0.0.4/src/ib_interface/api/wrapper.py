...
class Wrapper:
    ...
    def tickString(self, reqId: int, tickType: int, value: str):
        ...
        try:
            ...
            self._logger.error(f"tickString with tickType {tickType}: " f"malformed value: {value!r}")
    ...
    def historicalSchedule(
        self, reqId: int, startDateTime: str, endDateTime: str, timeZone: str, sessions: List[HistoricalSession]
    ):
    ...
    def error(self, reqId: int, errorCode: int, errorString: str, advancedOrderRejectJson: str):
        ...
        if errorCode == 165:
            ...
            if ticker:
                ...
                ticker.domTicks += [
                    MktDepthData(self.lastTime, 0, "", 2, 0, level.price, 0) for level in ticker.domAsks
                ]
                ticker.domTicks += [
                    MktDepthData(self.lastTime, 0, "", 2, 1, level.price, 0) for level in ticker.domBids
                ]
            ...
            if isinstance(bars, RealTimeBarList):
                ...
                self.ib.client.reqRealTimeBars(
                    reqId, bars.contract, bars.barSize, bars.whatToShow, bars.useRTH, bars.realTimeBarsOptions
                )
