...
DEFAULT_RESNET_SPECS = [('residual', 64, 3),('residual', 128, 3),('residual', 256, 3),('residual', 512, 3)]
CUSTOM_RESNET_SPECS = [[('residual', 2**filter_exp, repeat) for filter_exp in range(6, 10)] for repeat in range(1, 21)]

class MLBResNet(tf_keras.Model):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        super(MLBResNet, self).__init__(inputs=inputs, outputs=x, **kwargs)

    def train_step(self, data):
        features, labels = data
        one_hot_labels = tf.one_hot(labels, self._num_classes)
        with tf.GradientTape() as tape:
            outputs = self.__call__(features)
            tvars = self.trainable_variables
            cce = tf_keras.losses.CategoricalCrossentropy(from_logits=True, label_smoothing=0.0)
            loss = tf_utils.safe_mean(cce(one_hot_labels, outputs))
