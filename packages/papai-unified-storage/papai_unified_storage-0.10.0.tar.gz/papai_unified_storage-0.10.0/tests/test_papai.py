...
def test_list_bucket_objects(
    mock_bucket_config_with_tmp_dir, data_test_list_bucket_objects
):
...
def test_glob_bucket_objects(
    mock_bucket_config_with_tmp_dir, data_test_glob_bucket_objects
):
    remote_dir, _ = mock_bucket_config_with_tmp_dir
    (
        remote_paths_to_create,
...
def test_get_from_bucket(mock_bucket_config_with_tmp_dir, data_test_get_from_bucket):
...
def test_put_to_bucket(mock_bucket_config_with_tmp_dir, data_test_put_to_bucket):
    remote_dir, local_dir = mock_bucket_config_with_tmp_dir
    (
...
def test_put_to_bucket_with_versionning(
    mock_bucket_config_with_tmp_dir, data_test_put_to_bucket_with_versionning
):
    remote_dir, local_dir = mock_bucket_config_with_tmp_dir
    (
        local_paths_to_create,
    ...
    for i in range(n_uploads):
        files_uploaded = set(fsspec.filesystem("file").find(remote_dir + f"_{i}"))
        expected_files = set(
            ensure_in_list(
                joinpath(remote_dir + f"_{i}", expected_existing_remote_files)
            )
        )
        assert files_uploaded == expected_files
...
def test_remove_old_files(mock_bucket_config_with_tmp_dir, data_test_remove_old_files):
    remote_dir, _ = mock_bucket_config_with_tmp_dir
    ...
    create_files(joinpath(remote_dir, files_to_create))

    for file, utime in zip(files_to_create, files_utime):
    ...
    delete_old_files(
        ...
        [],
        fsspec.filesystem("file"),
        date_threshold=datetime_threshold,
        recursive=recursive,
    )
...
def test_remove_large_files(
    mock_bucket_config_with_tmp_dir, data_test_remove_large_files
):
    remote_dir, _ = mock_bucket_config_with_tmp_dir
    ...
    for file, size in zip(files_to_create, files_size):
