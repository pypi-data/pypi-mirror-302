...
def compose_columns(string_cell_columns: Array[Array[str]]) -> Array[CompositeColumn]:
    def _arrow1180(columns: Array[Array[str]], string_cell_columns: Any=string_cell_columns) -> CompositeColumn:
        return from_string_cell_columns(columns)

    return map_1(_arrow1180, group_columns_by_header(string_cell_columns), None)
...
def try_from_fs_worksheet(sheet: FsWorksheet) -> ArcTable | None:
    try: 
        ...
        if match_value is None:
            ...
            def _arrow1184(__unit: None=None) -> IEnumerable_1[Array[str]]:
                def _arrow1183(c: int) -> Array[str]:
                    def _arrow1182(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow1181(r: int) -> IEnumerable_1[str]:
                        ...
                        return collect(_arrow1181, range_big_int(1, 1, FsAddress__get_RowNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

                    return to_array_1(delay(_arrow1182))

                return map_2(_arrow1183, range_big_int(1, 1, FsAddress__get_ColumnNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

            composite_columns: Array[CompositeColumn] = compose_columns(map_1(fix_deprecated_ioheader, to_array_1(delay(_arrow1184)), None))
...
def to_fs_worksheet(index: int | None, table: ArcTable) -> FsWorksheet:
    ...
    if len(table.Columns) == 0:
        ...
        def _arrow1185(column_1: CompositeColumn, index: Any=index, table: Any=table) -> FSharpList[FSharpList[str]]:
            return to_string_cell_columns(column_1)

        def _arrow1186(column: CompositeColumn, index: Any=index, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
        ...
        columns: FSharpList[FSharpList[str]] = collect_1(_arrow1185, sort_by(_arrow1186, of_array(table.Columns), ObjectExpr1187()))
