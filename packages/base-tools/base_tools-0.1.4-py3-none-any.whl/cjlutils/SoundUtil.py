...
if __name__ == '__main__':
    sample_rate = 44100
    g1 = get_frequency_by_clef(ClefTypeEnum.TREBLE, 1, get_average_12_order(SolmizationEnum.SOL))
    be = get_frequency_by_clef(ClefTypeEnum.TREBLE, 1, get_average_12_order(SolmizationEnum.MI, -1))
    d = get_frequency_by_clef(ClefTypeEnum.TREBLE, 1, get_average_12_order(SolmizationEnum.RE))
    c = get_frequency_by_clef(ClefTypeEnum.TREBLE, 1, get_average_12_order(SolmizationEnum.DO))

    speed = 174
    arr = list(create_wave(g1, 3 * 60 / speed, sample_rate=sample_rate))
    arr += list(create_wave(be, 2 * 60 / speed, sample_rate=sample_rate))
    arr += list(create_wave(d, 1 * 60 / speed, sample_rate=sample_rate))
    arr += list(create_wave(c, 3 * 60 / speed, sample_rate=sample_rate))

    standard_arr = np.lnin
    sounddevice.play(arr)
