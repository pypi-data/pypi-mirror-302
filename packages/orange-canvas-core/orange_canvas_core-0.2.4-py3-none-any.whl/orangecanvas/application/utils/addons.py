...
class Command(Enum):
    Install = _tr.m[291, "Install"]
    Upgrade = _tr.m[292, "Upgrade"]
    Uninstall = _tr.m[293, "Uninstall"]
        ...
        try:
            if command == Install \
                ...
                self.setStatusMessage(
                    _tr.m[294, "Installing {}"].format(pkg.installable.name))
                if self.conda:
                    try:
                ...
                self.setStatusMessage(
                    _tr.m[295, "Upgrading {}"].format(pkg.installable.name))
                if self.conda:
                    try:
                ...
                self.setStatusMessage(
                    _tr.m[296, "Uninstalling {}"].format(pkg.local.name))
                if self.conda:
                    try:
            ...
            self.error.emit(
                _tr.m[297, "Command failed: python {}"].format(ex.cmd),
