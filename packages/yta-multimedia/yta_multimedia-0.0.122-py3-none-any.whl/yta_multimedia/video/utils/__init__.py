...
def parse_parameter_as_moviepy_clip(video: Union[VideoFileClip, CompositeVideoClip, ImageClip, ColorClip, VideoClip, str]):
    """
...
def concatenate_videos_ffmpeg(videos_abspath, output_abspath: str):
    ...
    command = 'ffmpeg -f concat -safe 0 -i ' + filename + ' -c copy ' + output_abspath
    run(command)
