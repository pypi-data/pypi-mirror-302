...
def RunProgram(
    ...
    pDir3="",
    RandColor=True,  # Mỗi ảnh sẽ có tập màu ngẫu nhiên, ROI cùng tên thì cùng màu
...
class taLibs:
    ...
    def LoadConfigFromfiles(ymlPath: str = "IVIS_data/config.yml"):
        ...
        os.makedirs(dirname(ymlPath), exist_ok=True)
        print(f"LoadConfigFromfiles('{ymlPath} ({os.path.realpath(ymlPath)})')")
        if not exists(ymlPath):  # Create new config file
            print("# Create new config file")
            os.makedirs(join(rootDir, "IVIS_data"), exist_ok=True)
            fnta = join(rootDir, "IVIS_data", "system.ta")
            if not exists(fnta):
...
"""
    ...
    def put_text_top(img, LINES, scolor=None, showAll=False):
        ...
        h = round((fontScale * img.shape[0]) // 22)  # 24
        Line1 = (h, h)  # (24,24)
        Line2 = (4 * h, 2 * h)  # (24,48)
        Line3 = (4 * h, 3 * h)  # (24,48)
        Line4 = (4 * h, 4 * h)  # (24,48)
...
def CheckYolo_Data(
    ...
    for fn in fis:  # Anh
        ...
        if not exists(fnLabel):
            noLabel.append(fn)
            bname = basename(fn)
            # fnDest=fn.replace(pImgdir, pDirErrorTxt)
            fnDest = f"{pDirErrorTxt}/{bname}"
            os.makedirs(dirname(fnDest), exist_ok=True)
            if exists(fnDest):
                bname = bname.replace(".jpg", f"_{tt2()}.jpg")
                shutil.move(fn, f"{pDirErrorTxt}/{bname}")
            else:
                shutil.move(fn, pDirErrorTxt)
    ...
    for fn in fis_videos:
        ...
        if not exists(fnLabel):
            ...
            fnDest = f"{pDirErrorTxt}/{bname}"
            if exists(fnDest):
                bname = bname.replace(".avi", f"_{tt2()}.avi")
                shutil.move(fn, f"{pDirErrorTxt}/{bname}")
            else:
                shutil.move(fn, pDirErrorTxt)
