...
async def create_family(fam: sqlm.CFamilyPost, request: Request) -> mdb_fam.CFamilyGet:
    ...
    return mdb_fam.CFamilyGet(**new_fam.model_dump())
...
async def update_family(
    ...
    if updated_family == None:
        raise HTTPException(status_code=404, detail="Family not found")
    return mdb_fam.CFamilyGet(**updated_family.model_dump())


@router.delete("/{id}/", dependencies=[Depends(sec.has_post_rights)])
def delete_family_by_id(id: int, delete_recursive: bool = True) -> dict[str, bool]:
    """
    ...
    * Request parameter: *delete_recursive* = true
    ...
    mdb_con.delete_family_by_id(family_id=id, delete_connected_items=delete_recursive)
    return {"ok": True}
