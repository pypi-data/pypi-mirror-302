...
from arize.logging import logger
from arize.logging.helpers import get_truncation_warning_message
...
ALLOWED_VECTOR_TYPES = (list,)
ALLOWED_ELEMENT_TYPES = (int, float)
T = TypeVar("T", bound=type)
try:
    import numpy as np

    ALLOWED_VECTOR_TYPES = ALLOWED_VECTOR_TYPES + (np.ndarray,)
    ...
    logger.debug("Numpy is not installed. Skipping numpy types for embedding vector")
try:
    import pandas as pd

    ALLOWED_VECTOR_TYPES = ALLOWED_VECTOR_TYPES + (pd.Series,)
except ImportError:
    logger.debug("Pandas is not installed. Skipping pandas types for embedding vector")
...
class Embedding(NamedTuple):
    ...
    def _validate_embedding_vector(
        ...
        if not self._is_valid_iterable(self.vector):
            raise TypeError(
        ...
        if not all(isinstance(val, ALLOWED_ELEMENT_TYPES) for val in self.vector):  # type: ignore
            raise TypeError(
    ...
    def _validate_embedding_data(
        self, emb_name: Union[str, int, float], data: Union[str, List[str]]
    ) -> None:
        """
        ...
        is_allowed_iterable = not is_string and self._is_valid_iterable(data)
    ...
    @staticmethod
    def _is_valid_iterable(data: Iterable) -> bool:
        """
        Validates that the input data field is of the correct iterable type. That is:
            1. List or
            2. numpy array (if numpy installed) or
            3. pandas Series (if pandas installed)
        ...
        """
        return any(isinstance(data, t) for t in ALLOWED_VECTOR_TYPES)
...
class LLMRunMetadata(NamedTuple):
    ...
    def validate(self) -> None:
        if self.total_token_count is not None and not isinstance(
            self.total_token_count,
            ALLOWED_ELEMENT_TYPES,  # type: ignore
        ):
        ...
        if self.prompt_token_count is not None and not isinstance(
        ...
        if self.response_token_count is not None and not isinstance(
        ...
        if self.response_latency_ms is not None and not isinstance(
...
class MultiClassPredictionLabel(NamedTuple):
    ...
    threshold_scores: Optional[Dict[str, Union[float, int]]] = None
