        ...
        if env == Environment.STUDIO_SSO:
            if access_token:
                try:
                    ...
                    logging.error(
                        f"Chat - Error getting Q profile ARN in SageMaker Studio: {str(e)}"
                    )

        elif self.is_md_environment(env):
            try:
                ...
                is_q_developer_enabled = False
                logging.error(f"Chat - Error getting Q settings in MD: {str(e)}")
