...
from lego.llm.settings import LlamaLLMChatSettings, LLMProvider
...
class RAGQueryEngine(SourcedQueryEngine):  # noqa: WPS214
    ...
    def _set_up_llm(
        ...
        if llm_settings.model in ANYSCALE_MODELS:
            llm_provider = LLMProvider(_env_prefix="anyscale_")
            provider_class = Anyscale
        else:
            llm_provider = LLMProvider(_env_prefix="openai_")
