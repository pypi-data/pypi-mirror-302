    ...
    async def process_creation(self, data):
        ...
        if data.get('Is_creating'):
            ...
            if new_files:
                ...
                logger.info(" #### `FileCreationManager`: No new files need to be created.")
        else:
            logger.info(" #### `FileCreationManager`: Creation flag is not set.")

        # Process moving of files
        moving_processes = data.get('Moving_files', [])
        if moving_processes:
            logger.info(" #### `FileCreationManager`: about to moving files")
            logger.info(moving_processes)
