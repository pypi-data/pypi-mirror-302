...
class brif:
    ...
    def fit(self, df, target_col):
        ...
        for j in range(p+1):
            if coltypes.iloc[j] == np.float64:
                vartypes[j] = "numeric"
                num_ind = num_ind + [j]
            elif coltypes.iloc[j] == np.int64:
                vartypes[j] = "integer"
                int_ind = int_ind + [j]
            elif coltypes.iloc[j] == object:
                vartypes[j] = "factor"
                fac_ind = fac_ind + [j]
            else:
