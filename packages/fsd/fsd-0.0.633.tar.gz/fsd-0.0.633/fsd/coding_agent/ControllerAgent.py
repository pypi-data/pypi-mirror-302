...
class ControllerAgent:
    ...
    async def regular_code_task_pipeline(self, tier, finalPrompt, role, original_prompt_language):
        ...
        if tier == "Pro":
            commits = await self.get_pro_coding_requests(idea_plan, all_context, list(final_working_files), context_files, role, crawl_logs, original_prompt_language)
        else:
            commits = await self.get_coding_requests(idea_plan, all_context, list(final_working_files), context_files, role, crawl_logs, original_prompt_language)
        ...
        self.repo.add_all_files(commits)
