...
class BasicFabricTrainer:
    ...
    def validation_step(
        self, batch: tuple[torch.Tensor, torch.Tensor], batch_idx: int, test: bool = False
    ) -> Dict[str, Any]:
        self.model.eval()
        with torch.no_grad():
            x, y = batch
    ...
    def train_epoch(self, train_loader: DataLoader, epoch_idx: int) -> Dict[str, float]:
        ...
        self.fabric.call(
            'on_train_epoch_end', trainer=self, metrics=metrics, epoch=epoch_idx
        )
        return metrics
    ...
    def val_epoch(
        ...
        for batch_idx, batch in enumerate(
            ...
            self.validation_step(batch, batch_idx, test)
        metrics = self.val_metric.compute()
        if not test:
            self.fabric.call(
                'on_validation_epoch_end', trainer=self, metrics=metrics, epoch=epoch_idx
            )
        return metrics
    ...
    def fit(
        ...
        for epoch in range(max_epochs):
            ...
            self.fabric.call(
                'on_train_epoch_end', trainer=self, metrics=train_metrics, epoch=epoch
            )
