...
class ScannerIntegration(ABC):
    ...
    def create_or_update_issue_from_finding(
        ...
        # Initialize the issue to None
        issue = None

        # Check if we should consolidate issues based on integrationFindingId
        if ScannerVariables.issueCreation.lower() != "perasset":
            # Find existing issues with the same integrationFindingId
            ...
            # Iterate through existing issues to find one that is open
            for existing_issue in existing_issues:
                if existing_issue.status != regscale_models.IssueStatus.Closed:
                    issue = existing_issue
                    break

            if issue:
                # Update the existing issue
                existing_asset_identifiers = (issue.assetIdentifier or "").split(delimiter)
    ...
    def _get_vulns_by_scan(self, all_vulns: list[regscale_models.Vulnerability]) -> list:
        """
        Fetch vulnerabilities that match the scanning tool
        :param list[regscale_models.Vulnerability] all_vulns: List of all vulneraibilities to check the scanningTool
        :return: List of matching vulnerabilities
        :rtype: list
        """
        vuln_list = []
        for vuln in all_vulns:
            if vuln := regscale_models.ScanHistory.get_object(vuln.scanId):
                if vuln.scanningTool == self.title:
    ...
    def close_outdated_vulnerabilities(self, current_vulnerabilities: Dict[int, Set[int]]) -> None:
        ...
        # filter by tool
        tool_vulns = self._get_vulns_by_scan(all_vulns=all_vulnerabilities)
        # current vulns
