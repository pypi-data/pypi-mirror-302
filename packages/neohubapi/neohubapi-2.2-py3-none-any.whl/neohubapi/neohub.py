...
class NeoHub:
    ...
    async def get_devices_data(self):
        ...
        for device in devices:
            ...
            for _, engineers_data_value in eng_hub_data.__dict__.items():
                ...
                if device_id is None or engineer_device_id is None:
                    self._logger.debug(f"Ignoring device '{device.device}', which has no ID and might be a repeater.")
                else:
                    if device_id == engineer_device_id:
