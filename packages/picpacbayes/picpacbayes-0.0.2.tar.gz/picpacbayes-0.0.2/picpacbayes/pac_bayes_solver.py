...
class PACBayesSolver(ABC):
    r"""
    Main class for Variational Inference Bayesian task solving.

    For a map to probability distributions \pi, PACBayesSolver solves the following
    minimisation task:

    argmin_{\theta} J(\theta) = \pi(\theta)[fun] + temperature * KL(\pi(\theta), \pi(\theta_0))

    NOTE:
        At this stage, PACBayesSolver class is not inherited from GenOptimizer
    """
