...
from igx_api.l2.types.filter import Filter, TemplatedFilter
...
class CollectionApi:
    ...
    def create_collection_from_csv(
        ...
        Args:
            ...
            reference_database_revision (igx_api.l2.types.reference_database.ReferenceDatabaseRevision | None): The reference database revision to use.
                If this is not provided, IGX will select the only one possible reference available for this import job or throw an error if there's more
                than one references available. Check igx_api.l2.client.api.reference_database_api.ReferenceDatabaseApi.get_reference_databases
                for available references.
            organism: (str | None): Name of the organism from which the imported data originates, used for running optional reference
                configuration validation before a collection import job is started. Apart from reference database validation it does not affect import flow.
    ...
    def create_collection_from_df(self, data_frame: pd.DataFrame) -> Execution[CollectionMetadata]:
        ...
        create_collection_execution = self.create_collection_from_csv(temp_file.name)
    ...
    def add_metadata(self, filter: Filter, annotation: import_metadata.Annotation) -> Execution[None]:
        ...
        support a templated filter, such as `add_metadata_from_csv` or `add_metadata_from_df`.
    ...
    def add_metadata_from_csv(
        ...
        """Import metadata from a CSV file to annotate collections, clones or sequences.
        ...
        Example:
            ...
            with IgxApiClient() as igx_client:
                ...
                igx_client.collection_api.add_metadata_from_csv(
                    filter=filter,
        ...
        def wait() -> None:
            ...
            import_metadata_request = openapi_client.ImportMetadataRequest(
                openapi_client.TemplatedSearchAndTag(
                    ...
                    filter_version=filter.version,
                    annotation=annotation.to_api_payload(),
                    template=openapi_client.FileTemplate(
                        type="file",
                        file_id=file.id,
                    ),
                )
    ...
    def add_metadata_from_df(
        ...
        Args:
            ...
            data_frame (pd.DataFrame): The DataFrame containing the metadata to import.
        ...
        return self.add_metadata_from_csv(filter, annotation, temporary_csv_file_path)
    ...
    def get_as_zip(
        ...
        Args:
            ...
            filter (igx_api.l2.types.filter.Filter): The filter to narrow down which collections, clones or sequences you wish to export.
    ...
    def get_as_df(self, collection_ids: list[CollectionId], filter: Filter, tag_ids: list[TagId] = DEFAULT_EXPORT_TAG_IDS) -> Execution[pd.DataFrame]:
