        ...
        for feature in self.fdw.materialized_fv_spec.aggregate_features:
            ...
            if isinstance(time_window, LifetimeWindowSpec):
                ...
                if self.sawtooth_aggregation_data:
                    batch_sawtooth_tile_size = self.sawtooth_aggregation_data.aggregations[
                    ...
                    window_start = window_start - (batch_sawtooth_tile_size or timedelta(seconds=1))
                time_deltas.update((window_start, time_window.window_end))
            else:
                msg = f"Invalid time_window type: {type(time_window)}"
                raise ValueError(msg)
