...
def transform_heatmap(heatmap, gamma=1.0):
...
def mask_by_distance(
    ...
    heatmap = transform_heatmap(heatmap, gamma=gamma)
    
    masks = heatmap > threshold
    masks = masks.cpu().numpy().astype(np.uint8)
    print(masks.sum(-1).sum(-1))
    
    if denoise:
        cleaned_masks = []
        for mask in masks:
            cleaned_mask, _ = clean_mask(mask, min_area=denoise_area_th)
