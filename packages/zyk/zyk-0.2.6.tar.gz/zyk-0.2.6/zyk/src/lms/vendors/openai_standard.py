...
class OpenAIStandard(VendorBase):
    ...
    async def _hit_api_async(self, model: str, messages: List[Dict[str, Any]], lm_config: Dict[str, Any], use_ephemeral_cache_only: bool = False) -> str:
        retry_count = getattr(self._hit_api_async, 'retry_count', 0)
        self._hit_api_async.retry_count = retry_count + 1
        
        if retry_count > 0 and retry_count % 5 == 0:
            print(f"Retrying async API call (attempt {retry_count})")
    ...
    def _hit_api_sync(self, model: str, messages: List[Dict[str, Any]], lm_config: Dict[str, Any], use_ephemeral_cache_only: bool = False) -> str:
        retry_count = getattr(self._hit_api_sync, 'retry_count', 0)
        self._hit_api_sync.retry_count = retry_count + 1
        
        if retry_count > 0 and retry_count % 5 == 0:
            print(f"Retrying sync API call (attempt {retry_count})")
