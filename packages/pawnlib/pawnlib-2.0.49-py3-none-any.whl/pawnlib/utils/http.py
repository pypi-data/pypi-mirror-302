...
from typing import Any, Dict, Iterator, Tuple, Union, Callable, Type, Optional
    ...
    async def _handle_communication(self):
        try:
            ...
            if self.on_send:
                ...
                function_name = self.on_send.__name__ if hasattr(self.on_send, '__name__') else 'Unknown'
                self.logger.debug(f"{function_name}  msg: {message} , type: {type(message)}")

                if message and isinstance(message, str):
                    # Check if WebSocket is connected and not closed
                    if self.is_connected and self._ws and not self._ws.closed:
                        self.logger.debug(f"Sending message... {self.ws_url}")
