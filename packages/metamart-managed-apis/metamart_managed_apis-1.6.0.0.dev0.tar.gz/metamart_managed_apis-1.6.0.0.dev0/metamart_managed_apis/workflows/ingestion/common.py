...
from metamart_managed_apis.api.utils import clean_dag_id
...
from metamart_managed_apis.utils.logger import set_operator_logger, workflow_logger
from metamart_managed_apis.utils.parser import (
...
class GetServiceException(Exception):
    ...
    def __init__(self, service_type: str, service_name: str):
        self.message = (
            ...
            " MetaMart client running in the Airflow host had issues getting"
...
class ClientInitializationError(Exception):
    """
    Exception to be thrown when couldn't initialize the Metamart Client
    """
...
def build_source(ingestion_pipeline: IngestionPipeline) -> WorkflowSource:
    ...
    try:
        ...
        raise ClientInitializationError(
            f"Failed to initialize the MetaMart client due to: {exc}."
            " Make sure that the Airflow host can reach the MetaMart"
            f" server running at {ingestion_pipeline.metaMartServerConnection.hostPort}"
...
def metadata_ingestion_workflow(workflow_config: MetaMartWorkflowConfig):
    """
...
def build_workflow_config_property(
    ...
    """
    Prepare the workflow config with logLevels and metaMartServerConfig
...
def send_failed_status_callback(workflow_config: MetaMartWorkflowConfig, *_, **__):
    """
    ...
    metadata_config = workflow_config.workflowConfig.metaMartServerConfig
...
def build_dag(
    task_name: str,
    ingestion_pipeline: IngestionPipeline,
    workflow_config: Union[MetaMartWorkflowConfig, MetaMartApplicationConfig],
    ...
    with DAG(**build_dag_configs(ingestion_pipeline)) as dag:
        ...
        CustomPythonOperator(
            ...
            # each DAG will call its own MetaMartWorkflowConfig
