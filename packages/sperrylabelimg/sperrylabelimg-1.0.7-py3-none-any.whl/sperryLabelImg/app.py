...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.common_active_dirs_path = None
        ...
        # Add some of the widgets to list_layout
        ...
        open = action(get_str('openFile'), self.open_file,  # noqa
    ...
    def setup_active_dir_dock(self):
        ...
        self.active_dir_dock = QDockWidget(self.tr("Active Directories BUSS"), self)
        self.active_dir_dock = QDockWidget(self.tr("Fixed"), self)
    ...
    def active_dir_item_clicked(self, item):
        for index in range(self.active_dir_list_widget.count()):
            if self.active_dir_list_widget.item(index).text() == item.text():
                break
        self.current_active_dir = self.active_dirs[index]
        self.statusBar().showMessage(f"Current active directory: {self.current_active_dir}")
    ...
    def add_active_dir(self):
        ...
        if dir_path:
            if not dir_path in self.active_dirs:
                ...
                self.common_active_dirs_path = os.path.commonpath(self.active_dirs)
                for dir_path in self.active_dirs:
                    self.active_dir_list_widget.addItem(os.path.relpath(dir_path, self.common_active_dirs_path))

            # Select the item
            item = self.active_dir_list_widget.item(self.active_dirs.index(dir_path))
            self.active_dir_list_widget.setCurrentItem(item)

            # Rename the Active Directories tab to the shorthand common dir name
            shortened_common_dirs_path = self.common_active_dirs_path.replace(HOME_DIR, "~")
            self.active_dir_dock.setWindowTitle(f"Active Directories -  {shortened_common_dirs_path}")
    ...
    def reset_active_dirs(self):
        ...
        self.active_dir_list_widget.clear()
        self.current_active_dir = None
        self.statusBar().showMessage("Active directories reset")
    ...
    def remove_selected_active_dir(self):
        ...
        if selected_items:
            ...
            self.remove_active_directory(directory)

    def remove_active_directory(self, directory):
        # Remove the directory from the active_dirs list
        if directory in self.active_dirs:
            self.active_dirs.remove(directory)

        # Remove the directory from the active_dir_list_widget
        items = self.active_dir_list_widget.findItems(directory, Qt.MatchExactly)
        for item in items:
        ...
        if self.current_active_dir == directory:
            self.current_active_dir = self.active_dirs[0] if self.active_dirs else None
    ...
    def scan_all_for_file(self, name: str) -> str:
        for directory in self.active_dirs:
            for root, dirs, files in os.walk(directory):
                for file in files:
                    if file == name:
                        return os.path.join(root, file)  # noqa
    ...
    def file_item_double_clicked(self, item=None):
        self.cur_img_idx = self.m_img_list.index(ustr(self.get_full_img_path(item.text())))
    ...
    def get_full_img_path(self, img_path: str) -> str:
        ...
        """
        return os.path.join(self.common_img_path, img_path)
    ...
    def open_dir_dialog(self, _value=False, dirpath=None, silent=False):
        ...
        if target_dir in self.active_dirs:
            ...
            self.current_active_dir = target_dir
        else:
    ...
    def import_dir_images(self, dir_path):
        ...
        if self.m_img_list:
            ...
            self.file_dock.setWindowTitle(f"File List - {shortened_common_img_dir}")
        else:
            self.file_dock.setWindowTitle("File List")

        for imgPath in self.m_img_list:
            item = QListWidgetItem(os.path.relpath(imgPath, self.common_img_path))
            self.file_list_widget.addItem(item)
    ...
    def open_file(self, _value=False):
        ...
        formats = ['*.%s' % fmt.data().decode("ascii").lower() for fmt in QImageReader.supportedImageFormats()]  # noqa
    ...
    def save_file(self, _value=False):
        ...
        current_ann_path = self.scan_all_for_file(saved_file_name)  # noqa
        annotation_is_in_dest_dir = os.path.dirname(current_ann_path) == self.current_active_dir
        if annotation_is_in_dest_dir:
            pass
        elif current_ann_path:
            os.remove(current_ann_path)

        img_is_in_dest_dir = os.path.dirname(self.file_path) == self.current_active_dir
        if img_is_in_dest_dir:
            pass
        elif self.file_path:
            new_img_location = os.path.join(self.current_active_dir, image_file_name)  # noqa
            shutil.move(self.file_path, new_img_location)  # noqa
            self.file_path = new_img_location
