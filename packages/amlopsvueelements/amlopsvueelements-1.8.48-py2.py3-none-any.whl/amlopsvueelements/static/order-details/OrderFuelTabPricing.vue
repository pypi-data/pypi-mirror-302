<template>
  <div class="w-full h-full flex flex-col gap-2">
    ...
    <div class="pricing-step bg-white w-full border border-transparent rounded-md">
      ...
      <div
        v-if="
          supplyFuel?.results?.length > 0 && !isLoadingSupplyFuel && !isLoadingSupplierFuelDetails
        "
      ...
      <div v-if="orderPricing && order && orderPricing.supplier_id" class="pricing-step-content">
        ...
        <div class="pricing-step-content-element flex">
          ...
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
            ...
            >
              <InputField
                :model-value="orderPricing.fuel_pricing?.client?.amount_total"
        ...
        >
          ...
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
            ...
            >
              <InputField
                class="roi-input w-full"
                :model-value="fee?.client?.amount_total"
                :is-white="true"
                placeholder=" "
                :disabled="true"
              >
...
import type { IFuelPricingObj, ISupplierFuel } from '@/types/order-details/pricing';
...
const { data: supplyFuel, callFetch: fetchSupplierFuel } = useFetch<
  any,
  (order: IOrder) => Promise<ISupplierFuel>
>(async (order: IOrder) => {
...
const { callFetch: selectFuelSupplier } = useFetch<
  any,
  (orderId: number, payload: { id: number; key: number }) => Promise<any>
...
const { callFetch: fetchOrderPricing } = useFetch<
  IFuelPricingObj,
  (orderId: number) => Promise<IFuelPricingObj | null>
...
const { callFetch: updateOrderPricing } = useFetch<any, (orderId: number) => Promise<any>>(
  async (orderId: number) => {
    ...
    orderPricing.value?.fees.forEach((fee: any) => {
      payload.fees.push({
        id: fee.client?.id,
        name: fee.supplier?.suppliers_fuel_fees_rates_row?.supplier_fuel_fee?.fuel_fee_category
          ?.name,
...
const { callFetch: updateOrderRoi } = useFetch<any, (orderId: number) => Promise<any>>(
  async (orderId: number) => {
