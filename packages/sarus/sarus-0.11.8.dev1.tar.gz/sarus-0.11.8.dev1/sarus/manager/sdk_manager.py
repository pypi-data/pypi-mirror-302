...
class SDKManager(Base):
    ...
    def infer_dataset_or_scalar(
        ...
        **named_arguments: t.Union[st.DataSpec, st.Transform],
        ...
        python_type, is_hint = self.infer_python_type_new_dataspec(
        ...
        def attach_python_type(dataspec: st.DataSpec) -> None:
            """Callback to attach the python type to a newly created dataspec"""
            attach_properties(
                dataspec,
                name=PYTHON_TYPE,
                properties={PYTHON_TYPE: python_type},
            )
            if is_hint:
                dataspec.manager()._unchecked_dataspecs.add(dataspec.uuid())

            return

        # Return the output type
        return dataspec_type, attach_python_type
