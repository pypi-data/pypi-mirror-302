...
"""
Converter logic to transform an MetaMart Table Entity
...
def ometa_to_sqa_orm(
    table: Table, metadata: MetaMart, sqa_metadata_obj: Optional[MetaData] = None
) -> DeclarativeMeta:
    """
    Given an MetaMart instance, prepare
...
def get_orm_schema(table: Table, metadata: MetaMart) -> str:
    """
...
def get_orm_database(table: Table, metadata: MetaMart) -> str:
    """get database name from database service

    Args:
        table (Table): table entity
        metadata (MetaMart): metadata connection to OM server instance
