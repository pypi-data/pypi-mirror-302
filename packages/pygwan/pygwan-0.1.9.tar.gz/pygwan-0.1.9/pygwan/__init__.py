...
class WhatsApp(object):
    ...
    def upload_media(self, media_path: str, mime_type: str) -> Union[str, None]:
        """
        Upload media to the WhatsApp cloud API

        Args:
            media_path[str]: Path of the media file to be uploaded
            mime_type[str]: Mime type of the media file e.g image/png, video/mp4
        ...
        Example:
            >>> from whatsapp import WhatsApp
            >>> whatsapp = WhatsApp(token, phone_number_id)
            >>> media_url = whatsapp.upload_media("path/to/file", "image/jpeg")
        """
        with open(media_path, "rb") as f:
            files = {
                "file": (media_path, f, mime_type),
            }
            data = {
                "type": mime_type,
                "messaging_product": "whatsapp"
            ...
            response = requests.post(
                f"{self.base_url}/{self.phone_number_id}/media", headers=self.headers, files=files, data=data
            )
            if response.status_code == 200:
