...
class S3Source(StorageServiceSource):
    ...
    def _load_metadata_file(self, bucket_name: str) -> Optional[StorageContainerConfig]:
        ...
        try:
            logger.info(
                f"Looking for metadata template file at - s3://{bucket_name}/{METAMART_TEMPLATE_FILE_NAME}"
            )
            response_object = self.s3_reader.read(
                path=METAMART_TEMPLATE_FILE_NAME,
                bucket_name=bucket_name,
                verbose=False,
            )
            ...
            logger.warning(
                f"No metadata file found at s3://{bucket_name}/{METAMART_TEMPLATE_FILE_NAME}"
            )
        except Exception as exc:
            logger.debug(traceback.format_exc())
            logger.warning(
                f"Failed loading metadata file s3://{bucket_name}/{METAMART_TEMPLATE_FILE_NAME}-{exc}"
            )
        return None
