...
class Markets:
    ...
    def get_available_markets(self) -> dict[str, dict[str, Any]]:
        """
    ...
    def _process_markets(self) -> dict[str, dict[str, Any]]:
        """
        Process the raw market data and structure it into a dictionary.

        :return: A dictionary containing the decoded market data.
        """
        available_tokens: dict[str, dict[str, str | int | bool]] = get_available_tokens(self.config)
        ...
        for raw_market in raw_markets:
            try:
                ...
                decoded_markets[raw_market[0]] = self._decode_market_data(
                    available_tokens=available_tokens, raw_market=raw_market, market_symbol=market_symbol
                )
                self.logger.info(f"Market processed: {raw_market[0]}")
            except KeyError:
                decoded_markets[raw_market[0]] = self._decode_market_data(
