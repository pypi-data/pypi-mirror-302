...
class TraverseDatasetItems:
    ...
    def item_filter(**kwargs) -> bool:
        ...
        <param length_of_result> 已经收集了元素数据的数量。若使用多线程处理，则不提供该信息为 None。
        <returns> True，保留数据；False，反之。
        """
        return True

    @classmethod
    def __item_processing(cls, file_abs_path, traverse_dir, external_raw_data, length_of_result=None):
        """
        对单个数据集 item 执行的操作
        :param file_abs_path: 需要处理的数据集 item 文件的绝对路径
        :param traverse_dir: 遍历的目录
        :param external_raw_data: 向 item_process 中传入的额外的数据
        :param length_of_result: 已经收集了元素数据的数量。若使用多线程处理，则不提供该信息默认为 None。
        :return: 如果 item_filter 为 True，则返回 tuple(file_abs_path, result of item_processor)；否则返回 None。
        """
        ...
        if cls.item_filter(traverse_dir=traverse_dir, file_abs_path=file_abs_path,
            ...
            return file_abs_path, processed_item_data
    ...
    @classmethod
    def traverse_dataset(cls, traverse_dir: str, item_file_type: str, external_raw_data: Dict[str, Any] = None,
        ...
        if multi_process:  # 多线程破处理
            ...
            for root, dirs, files in os.walk(traverse_dir):
                for file in files:
                    ...
                    data = cls.__item_processing(file_abs_path, traverse_dir, external_raw_data, len(result))
                    if data is not None:
                        result.append(data)
        return result
