...
from .split_acline import SplitTLine, SplitCable
...
class MovableMixin:                            # pylint: disable=abstract-method
    """
...
class ZComponent(Remotable):           # pylint: disable=too-many-public-methods
    """
    ...
    def _param_codec(self):
        ...
        codecs = self._pscad._zcmp_codec      # pylint: disable=protected-access
        key = self.__class__
...
class TLine(ACLine):       # pylint: disable=abstract-method, too-many-ancestors
    ...
    def split(self, dist_percent: float = 50) -> Dict[str, Point]:
        """
        Splits the Tline into two segments at the defined length percentage.

        Parameters
        ----------
        dist_percent: float
            The length percentage at which the line is split. Defaults to 50%.

        Returns
        -------
        Dict[str, Point]
            A dictionary containing name and location of connection ports.
            These can be the connecting points of external fault components.


        .. versionadded:: 3.0.3
        """

        return SplitTLine(self, dist_percent).split()
...
class Cable(ACLine):       # pylint: disable=abstract-method, too-many-ancestors
    ...
    def split(self, dist_percent: float = 50) -> Dict[str, Point]:
        """
        Splits the Cable into two segments at the defined length percentage.

        Parameters
        ----------
        dist_percent: float
            The length percentage at which the cable is split. Defaults to 50%.
        ...
        return SplitCable(self, dist_percent).split()
