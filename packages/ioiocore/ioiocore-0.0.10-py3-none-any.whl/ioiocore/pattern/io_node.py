...
class IONode(INode, ONode):
    def __init__(self, inputPorts : list[dict] = None , outputPorts : list[dict] = None, output_metadata : list[dict] = None):
        ...
        self.Metadata = {Metadata.INPUT_METADATA.value : self.InputMetadata, Metadata.OUTPUT_METADATA.value :self.OutputMetadata}
    ...
    def _init(self, data, input_metadata):
        output_metadata = self.init(data, input_metadata)
        if self.OutputPorts is not None and len(self.OutputPorts) > 0:
            if output_metadata is None:
                ONode._set_output_metadata(self, input_metadata)
            else:
                ONode._set_output_metadata(self, output_metadata)
        self.Metadata = {Metadata.INPUT_METADATA.value : self.InputMetadata, Metadata.OUTPUT_METADATA.value : self.OutputMetadata}
    ...
    @abstractmethod
    def init(self, data, input_metadata):
        pass
