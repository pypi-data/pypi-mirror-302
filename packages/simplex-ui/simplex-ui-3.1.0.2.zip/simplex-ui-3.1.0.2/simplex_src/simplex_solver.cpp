...
SimplexSolver::SimplexSolver(SimplexConfig &spconf)
	: SimplexConfig(spconf)
{
    ...
    if(m_select[Alignment_][ualign_] == TargetErrorLabel){
        ...
        rand_init(m_bool[Alignment_][alautoseed_], seed, m_procs, m_rank, &rand);
        for(int m = 0; m < m_M; m++){
...
void SimplexSolver::SetUndulatorData(UndulatorFieldData *ufdata, int nseg, int *type)
{
    ...
    *type = 0; // ideal
    if(find(m_segerr.begin(), m_segerr.end(), nseg) != m_segerr.end()){
        ...
        rand_init(m_bool[Und_][umautoseed_], seed, m_procs, m_rank, &rand);
        seed = rand.GetSeed();
...
{
    string input;
    LoadFile(m_rank, m_procs, m_string[EBeam_][partfile_], input);
...
void SimplexSolver::f_InitSimplexOutput()
{
    ...
    LoadFile(m_rank, m_procs, spxfile, input);
...
void rand_init(bool autoseed, int seed, int procs, int rank, RandomUtility *rand)
{
    if(autoseed){
        int seed = rand->Init();
        if(procs > 0){
...
void LoadFile(int rank, int mpiprocesses, const string filename, string &input)
{
    if(rank == 0){
