...
from igx_api.l2.types.collection import CollectionId, Receptor
...
class OptionalRestrictions(FromRawModel[openapi_client.ClusterOptionalRestrictions]):
    """Optional restrictions applied to sequences before clustering."""

    should_match_v_gene: bool | None = None
    """Determines if should cluster on the same V Gene."""
    should_match_j_gene: bool | None = None
    """Determines if should cluster on the same J Gene."""
    should_match_cdr3_length: bool | None = None
    """Determines if should cluster on the same CDR3 Amino Acid Length."""

    @classmethod
    def _build(cls, raw: openapi_client.ClusterOptionalRestrictions) -> "OptionalRestrictions":
        return cls(
            should_match_v_gene=raw.should_match_v_gene,
            should_match_j_gene=raw.should_match_j_gene,
            should_match_cdr3_length=raw.should_match_cdr3_length,
        )

    def to_inner(self) -> openapi_client.ClusterOptionalRestrictions:
        return openapi_client.ClusterOptionalRestrictions(
            should_match_v_gene=self.should_match_v_gene,
            should_match_j_gene=self.should_match_j_gene,
            should_match_cdr3_length=self.should_match_cdr3_length,
        )
...
class ClusterRun(FromRawModel[openapi_client.ExistingClusterRun]):
    ...
    """Collections that wer eused in the Cluster run."""
    receptor: Receptor
    ...
    optional_restrictions: OptionalRestrictions | None
    """Optional restrictions applied to sequences before clustering."""
    additional_options: AdditionalOptions | None
    """Additional Options for clustering configuration."""

    @classmethod
    def _build(cls, raw: openapi_client.ExistingClusterRun) -> "ClusterRun":
        return cls(
            ...
            sequence_features=[TagId(id) for id in raw.sequence_features],
            identities=SequenceFeatureIdentities.from_raw(raw.identities),
            optional_restrictions=OptionalRestrictions.from_raw(raw.optional_restrictions) if raw.optional_restrictions is not None else None,
