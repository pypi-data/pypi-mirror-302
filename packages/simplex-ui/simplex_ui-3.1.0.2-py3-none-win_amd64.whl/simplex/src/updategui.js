...
function UpdateEBeam()
{
    ...
    if(!chkprms.includes(null)){
        ...
        for(let j = 0; j < 2; j++){
            ...
            let avgbeta = GUIConf.input[PrePLabel][PreProcessPrmLabel.avbetaxy[0]][j];
            if(avgbeta != null){
                ebm[EBeamPrmsLabel.avgsize[0]][j]
                = Math.sqrt(avgbeta*nemitt+(espread*eta)**2)*1000.0;
            }
        }    
    }
    UpdateUndulatorSimCtrl();
...
function UpdateFELPrms()
{
    ...
    if(betasel == AvgBetaOpt){
        ...
        if(ebm[EBeamPrmsLabel.avgsize[0]][0] == null){
            betaval = -1;
        }
        else{
            GUIConf.input[PrePLabel] = GUIConf.GUIpanels[PrePLabel].JSONObj; // to save in the parameter file
            let avbeta = GUIConf.input[PrePLabel][PreProcessPrmLabel.avbetaxy[0]];
            betaval = Math.sqrt(avbeta[0]*avbeta[1]);
            felprm[FELPrmsLabel.avgbetavalue[0]] = betaval;
        }
    }
    else{
