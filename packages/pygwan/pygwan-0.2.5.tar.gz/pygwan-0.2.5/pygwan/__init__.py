...
class WhatsApp(object):
    ...
    def upload_pic(self, media_path: str, mime_type: str) -> str:
        """
        Upload media to the WhatsApp cloud API.

        Args:
            media_path[str]: Path of the media file to be uploaded.
            mime_type[str]: Mime type of the media file e.g. image/jpeg, video/mp4.

        Returns:
            str: Media ID if upload is successful, else None.
        """

        url = 'https://graph.facebook.com/v21.0/101879622956347/media'
        headers = {
            'Authorization': 'Bearer EAAOtcSk30jwBO3nd7EnDecj5wrOOUhBxk4ZC2w97PdcoZAYtCPGMkYYvQ8YU6OzmiwR8b8CffMJqZB0O4uTTF0eRIzx1kztcfgV8trjc2dT5jZAfpcJi9D7xKBzP5ifUL1zgnKIeJKoWn0VUBTQ4ikbIAqjBHZBELj17OxL8PGJZA7ObA90c5yaZBSxrZAc1VN2tsAZDZD'
        }
        # Ensure the path is correct and that the file exists
        file_path = media_path

        # Open the file in binary mode
        with open(file_path, 'rb') as image_file:
            files = {
                'file': (media_path, image_file, mime_type)
            }
            data = {
                'type': 'image/jpeg',
                'messaging_product': 'whatsapp'
