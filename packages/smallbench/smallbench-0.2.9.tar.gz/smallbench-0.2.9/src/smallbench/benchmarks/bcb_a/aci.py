    ...
    def __init__(self, bcb_backend: BCBEngine, synchronous: bool = False):
        ...
        self.actions = [
            ...
            ACIAction(
                ...
                action_arg_spec={
                    "unit_test_name": str,
                    "unit_test_dict": BaseModel,
                },  # Dict
                action_description="Add a unit test",
                action_context=f"""Add a unit test. The unit test information you submit must be in the format of a unit test: \n
class UnitTestDict(BaseModel):
    ...
    assertion_condition: str
    assertion_type: Literal["assertTrue", "assertRaises"] = "assertTrue"
\n\n It will be parsed via UnitTestDict(**unit_test_dict)


{unit_test_context}""",
                # format of a BCBUnitTest
                # BCBUnitTest
                # BCBUnitTest(**unit_test_dict)
                ##stringified_input_values: List[str]
    ...
    def get_submission_actions(self):
        if self.synchronous:
            test = ACIAction(
                ...
                action_arg_spec={},
                action_description="Test the submission against the existing unit tests",
                action_context="Test the submission against the existing unit tests - do not attempt to add new unit tests",
                transform=Transform(
