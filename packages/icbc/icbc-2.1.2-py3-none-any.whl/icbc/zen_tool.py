...
def taskkill_port(port=80,program="python",is_ask=True):
    local_port= "0.0.0.0:"+str(port)
    import re
    pid_set=re.findall(r"\d+\n",os.popen('netstat -ano | findstr '+local_port).read())
    if len(pid_set)>0:
        for i in pid_set:
            if program in os.popen('tasklist | findstr '+i.replace("\n","")).read():
                if is_ask:
                    yorn=input("检测到占用端口"+str(port)+'的python进程，请输入“y”确认强行关闭，不输入跳过：')
                    if yorn=="Y" or yorn =="y":
                        taskkill([i])
                else:
                    taskkill([i])
...
def pydoc(port=random(50000, 10000), package=None):
    ...
    try:
        ...
        dbp("端口被占用")
        dbp("尝试直接调用")
        os.system("python.exe -m pydoc -b")
