...
# this is required becuase any buttons not recognized in StringBundle will cause an error,
...
def scan_for_images(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            path = ustr(os.path.abspath(relative_path))  # noqa
            images.append(path)
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.com_dirs_path = None
        self.active_dir_container = None
        self.label_file = None
        self.image_data = None
        self.common_img_path = None
        self.active_dir_dock = None
        self.active_dir_list_widget = None
        self.remove_active_dir_button = None
        self.reset_active_dir_button = None
        self.add_active_dir_button = None
        ...
        open_dir = action(get_str('openDir'), self.add_active_dir,
        ...
        add_actions(self.menus.file,
                    (open_dir, open_annotation, copy_prev_bounding, self.menus.recentFiles, save, save_format, save_as, close, reset_all, delete_image, quit))
    ...
    def setup_active_dir_dock(self):
        self.active_dir_list_widget = QListWidget()
        ...
        self.remove_active_dir_button.clicked.connect(self.remove_selected_active_dir)
        button_layout.addWidget(self.remove_active_dir_button)

        # Add the new "Move to Active Directory" button
        self.move_to_active_dir_button = QPushButton(self.tr("Move Image"))
        self.move_to_active_dir_button.clicked.connect(self.move_image_to_active_dir)
        button_layout.addWidget(self.move_to_active_dir_button)
    ...
    def move_image_to_active_dir(self):
        ...
        # Get the selected image and its annotation file
        filename = self.m_img_list[self.cur_img_idx]
        strip_name = os.path.splitext(os.path.basename(filename))[0]
        annotation_file = self.scan_all_for_file(strip_name + XML_EXT)

        # Get the current active directory
        active_dir = self.current_active_dir
        ...
        # Construct the new paths for the image and its annotation file
        new_image_path = os.path.join(active_dir, os.path.basename(filename))
        new_annotation_path = os.path.join(active_dir, os.path.basename(annotation_file))

        # Move the image and its annotation file to the new paths
        shutil.move(filename, new_image_path)
        if annotation_file:
            shutil.move(annotation_file, new_annotation_path)

        # Update the image list
        self.m_img_list[self.cur_img_idx] = new_image_path

        # Update the UI elements, if needed
        self.update_file_menu()
        self.import_dir_images(self.current_active_dir)
        self.load_file(self.m_img_list[self.cur_img_idx])

    def export_repeating_file_names_to_cmd(self):
        """
        todo remove this debugging code
          this code was made to debug and keep track of when there are multiple copies of XMLs in a group of files
        :return:
        """
        ...
        def find_identical_filenames(start_directory):
            ...
            for root, dirs, files in os.walk(start_directory):
                for filename in files:
                    ...
                    identical_files[filename].append(full_path)
        ...
        start_dir = self.common_img_path  # Replace with your starting directory
    ...
    def active_dir_item_clicked(self, item):
        ...
        self.current_active_dir = self.active_dirs[index].replace("/", "\\")
        cur_active_dir = os.path.relpath(self.current_active_dir, self.com_dirs_path).replace("\\", "/")
        self.active_dir_dock.setWindowTitle(f"Active Directories (SAVE TO) >>  {cur_active_dir}")

    def add_active_dir(self):
        dir_path = QFileDialog.getExistingDirectory(self, self.tr("Select Active Directory"))
        if dir_path:

            if not dir_path in self.active_dirs:
                self.active_dir_list_widget.clear()

                self.active_dirs.append(dir_path)
                self.com_dirs_path = os.path.commonpath(self.active_dirs).replace("\\", "/")

                if self.com_dirs_path in self.active_dirs:
                    self.com_dirs_path = os.path.dirname(self.com_dirs_path)

                folder_name = self.com_dirs_path.replace("/", "\\")
                folder_name = folder_name.replace(HOME_DIR, "~")
                folder_item = QListWidgetItem(f"📂 {folder_name}")
                folder_item.setFlags(folder_item.flags() & ~Qt.ItemIsSelectable & ~Qt.ItemIsEnabled)
                self.active_dir_list_widget.addItem(folder_item)

                item_names = []
                if len(self.active_dirs) > 1:
                    for active_dir in self.active_dirs:
                        item_names.append(os.path.relpath(active_dir, self.com_dirs_path).replace('\\', '/'))
                elif len(self.active_dirs) == 1:
                    item_names.append(os.path.basename(self.active_dirs[0]))

                for item_name in item_names:
                    self.active_dir_list_widget.addItem(f"📁 {item_name}/")

            # Select the item
            item = self.active_dir_list_widget.item(self.active_dirs.index(dir_path) + 1)
            self.active_dir_list_widget.setCurrentItem(item)

            # Rename the Active Directories tab to the shorthand common dir name
            self.current_active_dir = dir_path.replace("/", "\\")
            cur_active_dir = os.path.relpath(self.current_active_dir, self.com_dirs_path).replace("\\", "/")
            self.active_dir_dock.setWindowTitle(f"Active Directories  (SAVE TO) >>  {cur_active_dir}")
    ...
    def remove_active_directory(self, directory, directory_text):
        # Remove the directory from the active_dirs list
        original_active_dir = self.current_active_dir
        if directory == self.current_active_dir.replace("\\", "/"):
            self.current_active_dir = os.path.dirname(self.file_path)

        if not self.may_continue():
            return

        self.current_active_dir = original_active_dir
        ...
        self.m_img_list = []
        for dir_ in self.active_dirs:
            self.m_img_list.extend(scan_for_images(dir_))
        self.sort_img_list()

        # update the image index so that if it doesn't exist in the new image list
        self.cur_img_idx = self.m_img_list.index(current_image_path) if current_image_path in self.m_img_list else 0

        # Update the file list widget
        # self.file_list_widget.clear()
        # if self.m_img_list:
        #     self.common_img_path = os.path.commonpath(self.m_img_list)
        #     shortened_common_img_dir = self.common_img_path.replace(HOME_DIR, "~")
        #     item = QListWidgetItem(f"📁 {shortened_common_img_dir}")
        #     item.setFlags(item.flags() & ~Qt.ItemIsSelectable & ~Qt.ItemIsEnabled)
        #     self.file_list_widget.addItem(f"📂 {shortened_common_img_dir}")
        #
        # for imgPath in self.m_img_list:
        #     item_name = os.path.relpath(imgPath, self.common_img_path).replace("\\", "/")
        ...
        #         # If the current image is still in the list, stay on it
        #         self.cur_img_idx = self.m_img_list.index(current_image_path)
        #     else:
        #         # If the current image was in the removed directory, open the next image
        #         self.cur_img_idx = 0
        #         if self.m_img_list:
        ...
        #
        # # If the removed directory was the current active directory, reset it
        # if self.current_active_dir == directory:
        #     self.current_active_dir = self.active_dirs[0].replace("/", "\\") if self.active_dirs else None
        #
        # # Update the UI
        # self.update_file_menu()
    ...
    def scan_all_images(self):
        images = []
        for read_dir in self.active_dirs:
            images.extend(scan_for_images(read_dir))
        return images

    def scan_all_for_file(self, name: str) -> str:
        for directory in self.active_dirs:
            for root, dirs, files in os.walk(directory):
                for file in files:
                    if file == name:
                        return os.path.join(root.replace("/", "\\"), file)  # noqa
    ...
    def file_item_double_clicked(self, item=None):
        ...
        if not full_image_file_path in self.m_img_list:
            self.cur_img_idx = 0
        else:
            self.cur_img_idx = self.m_img_list.index(full_image_file_path)
    ...
    def get_full_img_path(self, img_path: str) -> str:
        ...
        return os.path.join(self.common_img_path.strip(), ascii_image_path.strip().replace("/", "\\"))
    ...
    def save_labels(self, annotation_file_path):
        ...
        try:
            ...
            if self.label_file_format == LabelFileFormat.PASCAL_VOC:
                ...
                # if self.export_repeating_file_names_to_cmd():
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and self.file_list_widget.count() > 0:
            if unicode_file_path in self.m_img_list:
                index = self.m_img_list.index(unicode_file_path) + 1
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        ...
        xml_path = self.scan_all_for_file(basename + XML_EXT)
        txt_path = self.scan_all_for_file(basename + TXT_EXT)
        json_path = self.scan_all_for_file(basename + JSON_EXT)
    ...
    def open_dir_dialog(self, _value=False, dirpath=None, silent=False):
        ...
        if target_dir in self.active_dirs:
            self.last_open_dir = target_dir
            self.current_active_dir = target_dir.replace("/", "\\")
        else:
            self.active_dirs.append(target_dir)
    ...
    def import_dir_images(self, dir_path):
        ...
        if self.m_img_list:
            if len(self.m_img_list) > 1:
                ...
                self.common_img_path = os.path.dirname(self.m_img_list[0])
    ...
    def sort_img_list(self):
        rail_0 = [img for img in self.m_img_list if "_0_" in os.path.basename(img)]
        rail_1 = [img for img in self.m_img_list if "_1_" in os.path.basename(img)]

        rail_0 = sorted(rail_0, key=lambda x: int(os.path.basename(x).split("_")[-1].replace(".png", "")))
        rail_1 = sorted(rail_1, key=lambda x: int(os.path.basename(x).split("_")[-1].replace(".png", "")))

        self.m_img_list =  rail_0 + rail_1
    ...
    def save_file(self, _value=False):
        ...
        saved_file_name = os.path.splitext(image_file_name)[0] + XML_EXT

        ## Delete old annotation file
        old_ann_path = self.scan_all_for_file(saved_file_name)
        if os.path.exists(old_ann_path):
            os.remove(old_ann_path)

        ## Create and save new annotation file
        if self.current_active_dir:
            saved_path = os.path.join(self.current_active_dir, saved_file_name)
        else:
            saved_path = os.path.join(image_file_dir, saved_file_name)
            saved_path = saved_path if self.label_file else self.save_file_dialog(remove_ext=False)

        self._save_file(saved_path)

        # Moving the old image if the move is to a new directory

        if not os.path.dirname(self.file_path) == self.current_active_dir:
            ## Do the actual move
            img_save_path = os.path.join(self.current_active_dir.replace("/", "\\"), image_file_name)
            shutil.move(self.file_path, img_save_path)  # noqa

            ## Update game state to match the move
            self.file_path = img_save_path
