...
class DatasetPackageBuilder(Viewer):
    ...
    def __init__(self, dataset, parameters, treeview, **params):
        ...
        self.wcolumn = pn.widgets.AutocompleteInput(name='Column', min_characters=0, options=list(sequences_dtype_dict.keys()), placeholder='Select a column', width=bt_size*2, description='Select a column to filter.')
        self.wcolumn.param.watch(self.sync_wvalue, ['value'], onlychanged=True)
        self.woperator = pn.widgets.Select(name='operator', options=['==', '>', '<', '!=', 'contains' ], width=bt_size*2, description='Select an operator for filter.')
        self.wvalue = pn.widgets.AutocompleteInput(name='value', min_characters=0, restrict=False, options=[], placeholder='Enter a value', width=bt_size*2, description='Enter a value for filter.')
        ...
        self._layout = pn.Column(
                        self.wpath,
                        pn.Row(self.wcolumn, self.woperator, self.wvalue, self.bt_and_add, self.bt_or_add), 
    ...
    def sync_columns(self, event):
        """
        Synchronizes the hidden columns in the tabulator widget based on the selected columns in the treeview widget.
    ...
    def on_add_filter(self):
        ...
        Returns:
            None
        """
        if dtype_view[self.wcolumn.value] == 'string':
            if self.woperator.value == 'contains':
                self.wfilter.value += f'(`{self.wcolumn.value}`.str.contains("{self.wvalue.value}"))'
            else:
                self.wfilter.value += f'(`{self.wcolumn.value}` {self.woperator.value} "{self.wvalue.value}")'
        else:
            if self.woperator.value != 'contains':
                self.wfilter.value += f'(`{self.wcolumn.value}` {self.woperator.value} {self.wvalue.value})'
    ...
    def get_descendants(self, groups, groups_all, idx_parent):
        ...
        Parameters:
            ...
            idx_parent (int): The index of the parent node.
    ...
    def on_select(self, event):
        ...
        try:
            ...
            if len(self.treeview.path) < 1:
                ...
                idx_roots= self.treeview.path[-2] if len(self.treeview.path) > 1 else None
                #print('parent', self.add_parent.value, 'idx_parent:', idx_parent, 'idx_roots:', idx_roots, 'path:', self.treeview.path)
                if self.tg_level.value == 'current level':
                    if self.add_parent.value:
                        ...
                        res = self.dataset.get_data(idx_parent, idx_roots=idx_roots, max_depth=1)
                    else:
                    ...
                    if self.add_parent.value:
                        if idx_roots is None:
                            logger.warning('Add parent not possible with empty root')
                        res = self.dataset.get_data(idx_parent, idx_roots=idx_roots)
                    else:
                        res = self.dataset.get_data(idx_parent)
