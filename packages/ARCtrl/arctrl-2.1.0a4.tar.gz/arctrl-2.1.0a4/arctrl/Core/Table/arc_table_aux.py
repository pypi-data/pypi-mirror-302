...
def get_row_count(values: Any) -> int:
    if len(values) == 0:
        ...
        return 1 + max_by(projection, values.keys(), ObjectExpr615())[1]
...
def _007CIsUniqueExistingHeader_007C__007C(existing_headers: IEnumerable_1[CompositeHeader], input: CompositeHeader) -> int | None:
    if ((((input.tag == 3) or (input.tag == 2)) or (input.tag == 1)) or (input.tag == 0)) or (input.tag == 13):
        ...
        def _arrow616(h: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return True if (h.tag == 12) else False

        return try_find_index(_arrow616, existing_headers)

    elif input.tag == 11:
        def _arrow617(h_1: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return True if (h_1.tag == 11) else False

        return try_find_index(_arrow617, existing_headers)

    else: 
        def _arrow618(h_2: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return equals(h_2, input)

        return try_find_index(_arrow618, existing_headers)
...
def SanityChecks_validateColumnIndex(index: int, column_count: int, allow_append: bool) -> None:
    ...
    def _arrow619(__unit: None=None, index: Any=index, column_count: Any=column_count, allow_append: Any=allow_append) -> bool:
        x: int = index or 0
...
def SanityChecks_validateRowIndex(index: int, row_count: int, allow_append: bool) -> None:
    ...
    def _arrow620(__unit: None=None, index: Any=index, row_count: Any=row_count, allow_append: Any=allow_append) -> bool:
        x: int = index or 0
...
def SanityChecks_validate(headers: Array[CompositeHeader], values: Any, raise_exception: bool) -> bool:
    ...
    while en.System_Collections_IEnumerator_MoveNext() if is_valid else False:
        ...
        if (cell_is_not_freetext if (not cell.is_data) else False) if header_is_data else False:
            def _arrow621(message: str, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> None:
                raise Exception(message)

            def _arrow623(__unit: None=None, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> Callable[[str], None]:
                clo: Callable[[str], None] = to_console(printf("%s"))
                def _arrow622(arg: str) -> None:
                    clo(arg)

                return _arrow622

            (_arrow621 if raise_exception else _arrow623())(((("Invalid combination of header `" + str(header)) + "` and cell `") + str(cell)) + "`. Data header should contain either Data or Freetext cells.")
            is_valid = False

        if cell_is_not_freetext if header_is_freetext else False:
            def _arrow624(message_1: str, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> None:
                raise Exception(message_1)

            def _arrow626(__unit: None=None, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> Callable[[str], None]:
                clo_1: Callable[[str], None] = to_console(printf("%s"))
                def _arrow625(arg_1: str) -> None:
                    clo_1(arg_1)

                return _arrow625

            (_arrow624 if raise_exception else _arrow626())(((("Invalid combination of header `" + str(header)) + "` and cell `") + str(cell)) + "`. Freetext header should not contain non-freetext cells.")
            is_valid = False
...
def Unchecked_addColumn(new_header: CompositeHeader, new_cells: Array[CompositeCell], index: int, force_replace: bool, only_headers: bool, headers: Array[CompositeHeader], values: Any) -> None:
    ...
    if (has_duplicate_unique is None) if (index_1 < start_col_count) else False:
        def _arrow627(x: int, y: int, new_header: Any=new_header, new_cells: Any=new_cells, index: Any=index, force_replace: Any=force_replace, only_headers: Any=only_headers, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_column_index: int = max(_arrow627, start_col_count - 1, 0) or 0
    ...
    if not only_headers:
        ...
        def _arrow629(tupled_arg: tuple[int, int, CompositeCell], new_header: Any=new_header, new_cells: Any=new_cells, index: Any=index, force_replace: Any=force_replace, only_headers: Any=only_headers, headers: Any=headers, values: Any=values) -> Callable[[Any], None]:
            def _arrow628(values_1: Any) -> None:
                value: None = add_to_dict(values_1, (tupled_arg[0], tupled_arg[1]), tupled_arg[2])
                ignore(None)

            return _arrow628

        def _arrow631(tupled_arg_1: tuple[int, int, CompositeCell], new_header: Any=new_header, new_cells: Any=new_cells, index: Any=index, force_replace: Any=force_replace, only_headers: Any=only_headers, headers: Any=headers, values: Any=values) -> Callable[[Any], None]:
            def _arrow630(cells: Any) -> None:
                Unchecked_setCellAt(tupled_arg_1[0], tupled_arg_1[1], tupled_arg_1[2], cells)

            return _arrow630

        f: Callable[[tuple[int, int, CompositeCell], Any], None] = _arrow629 if (index_1 >= start_col_count) else _arrow631
...
def Unchecked_fillMissingCells(headers: Array[CompositeHeader], values: Any) -> None:
    ...
    class ObjectExpr633:
        ...
        def Equals(self) -> Callable[[int, int], bool]:
            def _arrow632(x: int, y: int) -> bool:
                return x == y
    ...
    column_key_groups: Any = of_array(Array_groupBy(projection, to_array(values.keys()), ObjectExpr633()), ObjectExpr634())
    for column_index in range(0, (column_count - 1) + 1, 1):
        ...
        if match_value is None:
            ...
            def _arrow635(tuple_1: tuple[int, int], headers: Any=headers, values: Any=values) -> int:
                return tuple_1[1]
            ...
            row_keys: Any = of_array_1(map(_arrow635, col_2, Int32Array), ObjectExpr636())
...
def Unchecked_addRow(index: int, new_cells: Array[CompositeCell], headers: Array[CompositeHeader], values: Any) -> None:
    ...
    if index < row_count:
        def _arrow637(x: int, y: int, index: Any=index, new_cells: Any=new_cells, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_row_index: int = max(_arrow637, row_count - 1, 0) or 0
...
def Unchecked_addRows(index: int, new_rows: Array[Array[CompositeCell]], headers: Array[CompositeHeader], values: Any) -> None:
    ...
    increase_row_indices: None
    if index < row_count:
        def _arrow638(x: int, y: int, index: Any=index, new_rows: Any=new_rows, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_row_index: int = max(_arrow638, row_count - 1, 0) or 0
        for row_index in range(last_row_index, index - 1, -1):
            for column_index in range(0, (column_count - 1) + 1, 1):
...
def Unchecked_alignByHeaders(keep_order: bool, rows: FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]]) -> tuple[Array[CompositeHeader], Any]:
    ...
    values: Any = Dictionary([], ObjectExpr639())
    def loop(col_i_mut: int, rows_2_mut: FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]], keep_order: Any=keep_order, rows: Any=rows) -> tuple[Array[CompositeHeader], Any]:
        while True:
            ...
            def _arrow640(arg: FSharpList[tuple[CompositeHeader, CompositeCell]], col_i: Any=col_i, rows_2: Any=rows_2) -> bool:
                return not is_empty(arg)

            if not exists(_arrow640, rows_2):
                return (headers, values)

            else: 
                def _arrow641(l: FSharpList[tuple[CompositeHeader, CompositeCell]], col_i: Any=col_i, rows_2: Any=rows_2) -> tuple[CompositeHeader, CompositeCell] | None:
                    return None if is_empty(l) else head(l)

                first_elem: CompositeHeader = pick(_arrow641, rows_2)[0]
