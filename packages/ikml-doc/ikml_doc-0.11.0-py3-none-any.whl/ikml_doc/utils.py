...
class Node(NodeParent):
    ...
    def to_dict(self, max_depth=-1):
        ...
        max_depth == 1, self and sibling nodes
        ...
        for child in self.children:
            if not child.is_attribute:
                dict_out["children"].append(child.to_dict(max_depth=max_depth-self.depth-1))
    ...
    def to_txt(self, exclude_root=False, max_depth=-1):
        if max_depth <= -1:
            max_depth = 9999
        d = self.tree_as_list(self, exclude_root=exclude_root, quoted_attr=False, max_depth=max_depth)
        return "\n".join(d)
