...
class ExplainerController:
    ...
    async def _prompt_user(self, action=None):
        if action:
            ...
            logger.info(" ### Click take action so Zinley can apply the above solution to the current project, or click Exit to leave.")
        else:
                ...
                if last_log:
                    ...
                    if action_status:
                        ...
                        if user_permission == "exit":
                            break
                        elif user_permission == "h":
                            await self.coder.explainer_code_task_pipeline()
                        else:
