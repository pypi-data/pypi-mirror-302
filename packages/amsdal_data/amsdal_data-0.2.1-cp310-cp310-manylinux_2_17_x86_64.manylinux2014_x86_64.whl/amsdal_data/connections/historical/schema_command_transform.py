...
class SchemaCommandExecutor:
    ...
    def execute(self) -> list[glue.Schema | None]:
        ...
        for _existing_schema, _mutation in items:
    ...
    def _transform_mutations(
        self,
    ) -> list[tuple[glue.Schema | None, glue.RegisterSchema | glue.DeleteSchema]]:
        result: list[tuple[glue.Schema | None, glue.RegisterSchema | glue.DeleteSchema]] = []
        ...
        for schema_mutation in self.schema_command.mutations:
            if schema_mutation.get_schema_reference().name in (
                TRANSACTION_TABLE,
                METADATA_TABLE,
                REFERENCE_TABLE,
                OBJECT_TABLE,
            ...
            if isinstance(schema_mutation, glue.AddConstraint) and isinstance(
                schema_mutation.constraint, glue.UniqueConstraint
            ):
                continue
        ...
        for (_schema_name, _mutation_type), _mutations in grouped.items():
            ...
            if _mutation_type is glue.DeleteSchema:
                ...
                _existing_schema = self._get_existing_schema(
                    ...
                    schema_version=delete_mutation.schema_reference.version,
                )
            ...
            result.append((_existing_schema, _mutation))
        return result
    ...
    def _transform_change_mutations_to_register(
    ...
    ) -> tuple[glue.Schema, glue.RegisterSchema]:
        ...
        return _existing_schema, _create_mutation
