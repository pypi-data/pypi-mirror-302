...
async def run(
    *,
    ...
    symbols = frozenset(("USDC", "USDT", "DAI"))
    ...
    while True:
        ...
        while True:
            ...
            rebalance_analysis = rebalance_manager(
                token_rates, portfolio_balance_pairs)
        ...
        for token in tokens:
            ...
            if exception:
                logger.error(
                    "An exception was thrown while withdrawing "
                    f"{token} from Aave:",
                    exc_info=e
                )
                yield WithdrawError(
        ...
        for token in tokens:
            ...
            if exception:
                logger.error(
                    "An exception was thrown while supplying "
                    f"{token} to Aave:",
                    exc_info=e
                )
                yield SupplyError(token, Decimal(amount) / 10**token.decimals, e)
            elif amount <= 0:
                continue
