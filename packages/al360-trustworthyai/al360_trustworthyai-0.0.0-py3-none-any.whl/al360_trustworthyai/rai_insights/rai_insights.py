...
"""Defines the AL360_TAIInsights class."""
...
from al360_erroranalysis._internal.cohort_filter import FilterDataWithCohortFilters
from al360_erroranalysis._internal.process_categoricals import process_categoricals
from al360_taiutils.data_processing import convert_to_list
from al360_taiutils.exceptions import (SystemErrorException,
                                 UserConfigValidationException)
from al360_taiutils.models import Forecasting, ModelTask, SKLearn
from al360_trustworthyai._interfaces import (Dataset, RAIInsightsData,
                                       TabularDatasetMetadata)
from al360_trustworthyai._internal._forecasting_wrappers import _wrap_model
from al360_trustworthyai._internal.constants import (FileFormats, ManagerNames,
                                               Metadata,
...
from al360_trustworthyai.managers.data_balance_manager import DataBalanceManager
...
from al360_trustworthyai.al360_tai_insights.al360_tai_base_insights import RAIBaseInsights
...
# AL360_TAIInsights object to store the model method outputs. Similarly, when
# serializing the AL360_TAIInsights object they need to mapped to file names.
...
# _OUTPUT_FIELDS contains the field names of AL360_TAIInsights. They are the options
...
class AL360_TAIInsights(RAIBaseInsights):
    """Defines the top-level Model Analysis API.
    Use AL360_TAIInsights to analyze errors, explain the most important
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """Creates an AL360_TAIInsights object.
        ...
        super(AL360_TAIInsights, self).__init__(
        ...
        self._validate_al360_tai_insights_input_parameters(
            model=model,
            train=train,
            test=test,
        ...
        self._classes = AL360_TAIInsights._get_classes(
            task_type=task_type,
            train=train,
            target_column=target_column,
            classes=classes
        ...
        self._feature_ranges = AL360_TAIInsights._get_feature_ranges(
    ...
    def _consolidate_categorical_features(
        ...
        Originally, only the AL360_TAIInsights constructor accepted
        ...
        if categorical_features is not None:
            warnings.warn("The categorical_features argument on the "
                          "AL360_TAIInsights constructor is deprecated and will "
            ...
            if categorical_features is None:
                ...
                if (set(categorical_features) ==
                    ...
                    raise UserConfigValidationException(
                        ...
                        'AL360_TAIInsights constructor and the categorical_features '
    ...
    def _validate_al360_tai_insights_input_parameters(
            self,
            model: Any,
            train: pd.DataFrame,
            test: pd.DataFrame,
            ...
            feature_metadata: FeatureMetadata,
            **kwargs):
        """Validate the inputs for the AL360_TAIInsights constructor.
    ...
    def _save_predictions(self, path):
        ...
        :param path: The directory path to save the AL360_TAIInsights to.
        :type path: str
        """
    ...
    def save(self, path):
        """Save the AL360_TAIInsights to the given path.

        :param path: The directory path to save the AL360_TAIInsights to.
        :type path: str
        """
        super(AL360_TAIInsights, self).save(path)
    ...
    def _load_metadata(inst, path):
        ...
        :param inst: AL360_TAIInsights object instance.
        :type inst: AL360_TAIInsights
        ...
        inst.__dict__['_' + Metadata.CLASSES] = AL360_TAIInsights._get_classes(
    ...
    @staticmethod
    def load(path):
        """Load the AL360_TAIInsights from the given path.

        :param path: The directory path to load the AL360_TAIInsights from.
        :type path: str
        :return: The AL360_TAIInsights object after loading.
        :rtype: AL360_TAIInsights
        """
        # create the AL360_TAIInsights without any properties using the __new__
        # function, similar to pickle
        inst = AL360_TAIInsights.__new__(AL360_TAIInsights)
        ...
        RAIBaseInsights._load(path, inst, manager_map,
                              AL360_TAIInsights._load_metadata)
        AL360_TAIInsights._load_predictions(inst, path)
        AL360_TAIInsights._load_large_data(inst, path)
