...
class GridAdventureEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        for x, y in product(range(width), range(height)):
            ...
            if sym_key == 'W' or sym == 'L':
                self.grid.set(x, y, inst())
            elif sym_key == 'K':
                color = _COLOR_TO_ARGUMENT[sym[1]] or 'red'
                self.grid.set(x, y, inst(color=color))
            elif sym_key == 'D':
                color = _COLOR_TO_ARGUMENT[sym[1]] or 'red'
                is_open = False
                is_locked = False
                ...
                self.grid.set(x, y, inst(color=color, is_open=is_open, is_locked=is_locked))
...
def make_grid_adventure(render_mode: str, with_wrapper: bool = True):
    env = GridAdventureEnv(render_mode=render_mode)
    if with_wrapper:
