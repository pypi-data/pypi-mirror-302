...
def format_assets_history(assets_history: Union[None, dict]):
    if assets_history:
        return pd.DataFrame(
            {
...
class URL:
    ...
    @typechecked
    def __call__(self, force: bool) -> Union[None, WriteResponse]:
...
class DataHandler(CustomPickleable):
    ...
    def use_revision(
        self,
        revision: Union[
            int, tuple[Union[int, None], ...], list[Union[int, None]], None
        ],
    ):
...
class Description:
    ...
    def update_user_entries(self, entries: dict):
        ...
        for entry in entries.copy():
            if entry not in self.VALID_USER_ENTRIES:
                ...
                if entry == "Data":
                    if isinstance(self.dict[entry], str):
                        self.dict[entry] = self.data_template
                    entries[entry] = {
                        "Data description": entries[entry],
                        "Data location": self.dict[entry]["Data location"],
                        "Data summary": self.dict[entry]["Data summary"],
                    }
        if len(entries) > 0:
    ...
    def _update_architecture(self, obj, entry):
        ...
        if to_update not in self.dict:
            self.dict[to_update] = {}
        value = getattr(obj, entry)
        if entry == "model":
            self.dict[to_update]["Model"] = (
    ...
    def _update_tech_perf(self, obj, _):
        columns = ["duration"]
        factors = [1]
        # converts to float, optionaly divides by a factor.
        dataframes = [
            convert_columns(
                obj.feature_engineering_execs.summary, columns, factors
            ),
            convert_columns(obj.split_execs.summary, columns, factors),
            convert_columns(obj.train_execs.summary, columns, factors),
            convert_columns(obj.validate_execs.summary, columns, factors),
            convert_columns(obj.test_execs.summary, columns, factors),
            convert_columns(obj.predict_execs.summary, columns, factors),
            convert_columns(obj.postprocess_execs.summary, columns, factors),
        ]
        ...
        self.dict["Technical performances"] = dataframes.T
    ...
    def _update_evaluation(self, obj, _):
        ...
        if len(validations) > 0:
            self.dict[to_update]["Validation metrics"] = validations[-1].results
        if len(tests) > 0:
            self.dict[to_update]["Test metrics"] = tests[-1].results
    ...
    def _update_data_location(self, obj, entry):
        ...
        if entry in self.DATA_LOCATION_CORRESPONDANCE:
            ...
            self.dict[parent]["Data location"][
                self.DATA_LOCATION_CORRESPONDANCE[entry]
            ] = location
        elif entry == "_predictions":
            if "Predictions" not in self.dict[parent]["Data location"]:
                self.dict[parent]["Data location"]["Predictions"] = {}
            for dataset in value:
                if value[dataset].pointer:
                    ...
                    location = None
                self.dict[parent]["Data location"]["Predictions"][dataset] = (
                    location
                )
    ...
    def _update_data_summary(self, obj, entry):  # noqa: C901
        ...
        if "Predictive variable(s)" not in self.dict[parent]["Data summary"]:
            self.dict[parent]["Data summary"]["Predictive variable(s)"] = {}
        if "Target variable(s)" not in self.dict[parent]["Data summary"]:
            self.dict[parent]["Data summary"]["Target variable(s)"] = {}

        if entry == "_predictive_variables":
            example = self.dict[parent]["Data summary"][
                "Predictive variable(s)"
            ].get("Input Example")
            self.dict[parent]["Data summary"]["Predictive variable(s)"] = value
            if example:
                self.dict[parent]["Data summary"]["Predictive variable(s)"][
                    "Input Example"
                ] = example
            ...
            self.dict[parent]["Data summary"]["Target variable(s)"] = value
            if example:
                self.dict[parent]["Data summary"]["Target variable(s)"][
                    "Output Example"
                ] = example
    ...
    def _update_rapporteurs(self, obj, _):
        entry = "Rapporteurs"
        self.dict[entry] = [dict(validator) for validator in obj._validators]

    def _reset_data_if_need_be(self):
        if "Data" not in self.dict:
            self.dict["Data"] = self.data_template
        if isinstance(self.dict["Data"], str):
            previous_data = self.dict["Data"]
            self.dict["Data"] = self.data_template
            self.dict["Data"]["Data description"] = previous_data
...
class ExecutionMetadata(BackwardCompatibleObject):
    ...
    renamed_attributes: ClassVar = {
        "date": {"old_name": "timestamp", "warning": None},
        "parameters": {"old_name": "kwargs", "warning": None},
    }
    new_attributes: ClassVar = {
        "model": {"default": "<OLD VERSION -- UNSPECIFIED>"},
        "version_id": {"default": "<OLD VERSION -- UNSPECIFIED>"},
    }
    deleted_attributes: ClassVar = {"args": {}}
...
class VersionNamedFile:
    ...
    def _is_last_version(self, version: Union[str, None], versions):
        if versions is None:
            versions = self._get_model_versions()
    ...
    def _get_version_path(self, version: Union[str, None] = None):
        if version is None:
    ...
    def rm(
        self,
        rev_or_ver: Union[str, int, None] = None,
        versions=None,
...
def get_data_sample(
    data: Union[np.ndarray, pd.DataFrame, pd.Series, SpecialDataFormat, None],
):
    if isinstance(data, (pd.DataFrame, pd.Series)):
