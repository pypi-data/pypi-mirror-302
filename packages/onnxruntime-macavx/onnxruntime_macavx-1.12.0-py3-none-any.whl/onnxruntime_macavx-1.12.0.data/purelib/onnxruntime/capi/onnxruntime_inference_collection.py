...
def get_ort_device_type(device_type, device_index):
    ...
    elif device_type == "macavx":
        return C.OrtDevice.macavx()
    elif device_type == "macavx_pinned":
        return C.OrtDevice.macavx_pinned()
...
def get_ort_memory_type(memory_type):
    if memory_type == "macavx_pinned":
        return C.OrtDevice.macavx_pinned_memory()
    else:
...
class Session:
    ...
    def run_with_ort_values_async(self, output_names, input_dict_ort_values, run_options=None, stream=None):
        """
        Async Compute the predictions.
        ...
        """
        if(stream is None):
            raise ValueError("You should set stream which can be created by ort.mcStreamCreateWithFlags(ort.mcStreamNonBlocking)")
        run_options_stream = run_options
        if(run_options_stream is None):
            run_options_stream = C.RunOptions()
        run_options_stream.stream = stream
        return self.run_with_ort_values(output_names, input_dict_ort_values, run_options_stream)
    ...
    def run_with_iobinding_async(self, iobinding, run_options=None, stream = None):
        """
        Async compute the predictions.
        ...
        if(run_options_stream is None):
            run_options_stream = C.RunOptions()
        run_options_stream.stream = stream
        return self.run_with_iobinding(iobinding, run_options_stream)
