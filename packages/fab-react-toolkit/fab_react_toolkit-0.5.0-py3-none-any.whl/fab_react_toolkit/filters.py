...
class BaseFilterTextContains(BaseFilter):
    def _handle_relation_contains(self, value):
        concat = []
        rel_interface = self.datamodel.get_related_interface(self.column_name)
        for col_name in rel_interface.list_columns.keys():
            col = getattr(rel_interface.obj, col_name)
            concat.append(cast(col, String))
            concat.append(" ")

        pk = rel_interface.get_pk()
        subquery = select(pk)
        subquery = subquery.filter(func.concat(*concat).ilike("%" + value + "%"))
        subquery = pk.in_(subquery)
        filter = (
            getattr(self.model, self.column_name).any
            if self.datamodel.is_relation_one_to_many(self.column_name)
            or self.datamodel.is_relation_many_to_many(self.column_name)
            else getattr(self.model, self.column_name).has
        )
...
class FilterGlobal(BaseFilterTextContains):
    ...
    def apply(self, query, value):
        if g.get("global_filter") and g.get("global_filter") != self.column_name:
            raise BadRequest(
                f"Global filter already set for column '{g.get('global_filter')}'"
            )
        g.global_filter = self.column_name
        ...
        for val in value:
            ...
            for col in list_cols:
                ...
                if self.datamodel.is_relation(col):
                    rel_filter_arr.append(self._handle_relation_contains(val))
                else:
                    concat_arr.extend([cast(field, String), " "])
            filters.append(
...
class FilterTextContains(BaseFilterTextContains):
    name = lazy_gettext("Contains as Text")
    arg_name = "ctt"
    ...
    def apply(self, query, value):
        ...
        for val in value:
            if self.datamodel.is_relation(self.column_name):
                filters.append(self._handle_relation_contains(val))
            else:
                filters.append(cast(field, String).ilike("%" + val + "%"))
        return query.filter(and_(*filters))
...
class SQLAFilterConverter(BaseFilterConverter):
    ...
    conversion_table = (
        ...
        (
            ...
            [FilterRelationManyToManyEqual, FilterGlobal, FilterTextContains],
        ),
        (
        ...
        (
            "is_enum",
            [FilterEqual, FilterNotEqual, FilterIn, FilterGlobal, FilterTextContains],
        ),
        (
            "is_text",
            [
