...
class StructEffPola3D(StructBase):
    """base class 3D point polarizability structure
    ...
    def __init__(
        ...
        for _adict in alpha_dicts:
            ...
            if (
                "alpha_pE" not in _adict
                and "alpha_mE" not in _adict
                and "alpha_pH" not in _adict
                and "alpha_mH" not in _adict
            ):
                raise ValueError(
                    "Polarizability-description dicts must contain at least one of: "
                    + "['alpha_pE', 'alpha_mH', 'alpha_mE', 'alpha_pH']."
                )
        ...
        for i, _adict in enumerate(alpha_dicts):
            if "alpha_pE" in _adict:
                alpha_pE_data.append(
                    self._interpolate_single_alpha(
                        wls=self.wavelengths_data,
                        a_data=_adict["alpha_pE"],
                        wl_data=_adict["wavelengths"],
                    )
                )
            if "alpha_mE" in _adict:
                alpha_mE_data.append(
                    self._interpolate_single_alpha(
                        wls=self.wavelengths_data,
                        a_data=_adict["alpha_mE"],
                        wl_data=_adict["wavelengths"],
                    )
                )
            if "alpha_pH" in _adict:
                alpha_pH_data.append(
                    self._interpolate_single_alpha(
                        wls=self.wavelengths_data,
                        a_data=_adict["alpha_pH"],
                        wl_data=_adict["wavelengths"],
                    )
                )
            if "alpha_mH" in _adict:
                alpha_mH_data.append(
                    self._interpolate_single_alpha(
                        wls=self.wavelengths_data,
                        a_data=_adict["alpha_mH"],
                        wl_data=_adict["wavelengths"],
                    )
                )
        ...
        self.zeros_spectral = torch.zeros(
            (len(self.positions), len(self.wavelengths_data), 3, 3),
            dtype=DTYPE_COMPLEX,
            device=self.device,
        )

        # effective polarizabilities of each meshpoint at each wavelength: shape (Npos, Nwl, 3, 3)
        if len(alpha_pE_data) != 0:
            self.alpha_pE_data = torch.stack(alpha_pE_data)
        else:
            self.alpha_pE_data = self.zeros_spectral

        if len(alpha_mH_data) != 0:
            self.alpha_mH_data = torch.stack(alpha_mH_data)
        else:
            self.alpha_mH_data = self.zeros_spectral

        if len(alpha_pH_data) != 0:
            self.alpha_pH_data = torch.stack(alpha_pH_data)
        else:
            self.alpha_pH_data = self.zeros_spectral

        if len(alpha_mE_data) != 0:
            self.alpha_mE_data = torch.stack(alpha_mE_data)
        else:
            self.alpha_mE_data = self.zeros_spectral

        # case of a single point-polarizability: expand dimensions
        if len(self.alpha_pE_data.shape) == 3:
            self.alpha_pE_data = self.alpha_pE_data.unsqueeze(0)
        if len(self.alpha_mH_data.shape) == 3:
            self.alpha_mH_data = self.alpha_mH_data.unsqueeze(0)
        if len(self.alpha_pH_data.shape) == 3:
            self.alpha_pH_data = self.alpha_pH_data.unsqueeze(0)
        if len(self.alpha_mE_data.shape) == 3:
            self.alpha_mE_data = self.alpha_mE_data.unsqueeze(0)

        # selfterms are zero.
        self.selfterms_pE_data = self.zeros_spectral
        self.selfterms_mE_data = self.zeros_spectral
        self.selfterms_pH_data = self.zeros_spectral
        self.selfterms_mH_data = self.zeros_spectral
        ...
        # verify dimensions
        if (
            (self.alpha_pE_data.shape != self.zeros_spectral.shape)
            or (self.alpha_mH_data.shape != self.zeros_spectral.shape)
            or (self.alpha_pH_data.shape != self.zeros_spectral.shape)
            or (self.alpha_mE_data.shape != self.zeros_spectral.shape)
        ):
            raise ValueError(
                "Wrong shape of a polarizability tensor list. "
                + "Needs to be (N_dipoles, N_wavelengths, 3, 3)."
            )
    ...
    def create_lookup(self):
        ...
        for i_wl, wl in enumerate(self.wavelengths_data):
            self.lookup_pE[float(wl)] = self.alpha_pE_data[:, i_wl, :, :]
        self.lookup_mE = {}
        for i_wl, wl in enumerate(self.wavelengths_data):
            self.lookup_mE[float(wl)] = self.alpha_mE_data[:, i_wl, :, :]
        self.lookup_pH = {}
        for i_wl, wl in enumerate(self.wavelengths_data):
            self.lookup_pH[float(wl)] = self.alpha_pH_data[:, i_wl, :, :]
        self.lookup_mH = {}
        for i_wl, wl in enumerate(self.wavelengths_data):
            self.lookup_mH[float(wl)] = self.alpha_mH_data[:, i_wl, :, :]
    ...
    def set_device(self, device):
        ...
        if len(self.full_geometries) > 0:
            for i_g in range(len(self.full_geometries)):
                self.full_geometries[i_g] = self.full_geometries[i_g].to(device)
        ...
        self.alpha_pE_data = self.alpha_pE_data.to(device)
        self.alpha_mE_data = self.alpha_mE_data.to(device)
        self.alpha_pH_data = self.alpha_pH_data.to(device)
        self.alpha_mH_data = self.alpha_mH_data.to(device)

        self.selfterms_pE_data = self.selfterms_pE_data.to(device)
        self.selfterms_mE_data = self.selfterms_mE_data.to(device)
        self.selfterms_pH_data = self.selfterms_pH_data.to(device)
        self.selfterms_mH_data = self.selfterms_mH_data.to(device)

        # transfer the lookup tables
        for wl in self.lookup_pE:
            self.lookup_pE[wl] = self.lookup_pE[wl].to(device)
        for wl in self.lookup_mE:
            self.lookup_mE[wl] = self.lookup_mE[wl].to(device)
        for wl in self.lookup_pH:
            self.lookup_pH[wl] = self.lookup_pH[wl].to(device)
        for wl in self.lookup_mH:
            self.lookup_mH[wl] = self.lookup_mH[wl].to(device)
    ...
    def _interpolate_single_alpha(self, wls, a_data, wl_data):
        ...
        a_ip = torch.zeros((len(wls), 3, 3), dtype=DTYPE_COMPLEX, device=self.device)
        for i in range(3):
            for j in range(3):
    ...
    def interpolate_alpha(self, wls, a_data_many, wl_data, lookup=None):
        ...
        # wavelength-interpolation of each tensor component
        a_ip = torch.zeros(
            (len(a_data_many), len(wls), 3, 3), dtype=DTYPE_COMPLEX, device=self.device
        )
        for i_a, a_data in enumerate(a_data_many):
            _a = self._interpolate_single_alpha(wls, a_data, wl_data)
            a_ip[i_a] = _a

        # optional: add wavelengths to lookup
    ...
    def get_polarizability_pE(self, wavelength: float, environment) -> torch.Tensor:
        ...
        """
        if float(wavelength) not in self.lookup_pE:
            warnings.warn(
                "Interpolating 'pE' polarizability at wavelength {:.3f}.".format(
                    float(wavelength)
                )
            )
            self.interpolate_alpha(
                ...
                lookup=self.lookup_pE,
            )

        return self.lookup_pE[float(wavelength)]
    ...
    def get_polarizability_mE(self, wavelength: float, environment) -> torch.Tensor:
        ...
        """
        if float(wavelength) not in self.lookup_mE:
            warnings.warn(
                "Interpolating 'mE' polarizability at wavelength {:.3f}.".format(
                    float(wavelength)
                )
            )
            self.interpolate_alpha(
                ...
                self.alpha_mE_data,
                self.wavelengths_data,
                lookup=self.lookup_mE,
            )

        return self.lookup_mE[float(wavelength)]
    ...
    def get_polarizability_pH(self, wavelength: float, environment) -> torch.Tensor:
        ...
        """
        if float(wavelength) not in self.lookup_pH:
            warnings.warn(
                "Interpolating 'pH' polarizability at wavelength {:.3f}.".format(
                    float(wavelength)
                )
            )
            self.interpolate_alpha(
                ...
                self.alpha_pH_data,
                self.wavelengths_data,
                lookup=self.lookup_pH,
            )

        return self.lookup_pH[float(wavelength)]
    ...
    def get_polarizability_mH(self, wavelength: float, environment) -> torch.Tensor:
        ...
        """
        if float(wavelength) not in self.lookup_mH:
            warnings.warn(
                "Interpolating 'mH' polarizability at wavelength {:.3f}.".format(
                    float(wavelength)
                )
            )
            self.interpolate_alpha(
                ...
                self.alpha_mH_data,
                self.wavelengths_data,
                lookup=self.lookup_mH,
            )

        return self.lookup_mH[float(wavelength)]
    ...
    def plot3d(self, **kwargs):
        ...
        return visu3d.geo3d._plot_structure_eff_pola(self, **kwargs)
    ...
    def rotate(self, alpha, center=torch.as_tensor([0.0, 0.0, 0.0]), axis="z"):
        ...
        # rotate positions (if multiple)
        if len(_struct_rotated.positions) > 1:
            _struct_rotated.positions = (
                torch.matmul(_struct_rotated.positions - center, rot) + center
            )

        # rotate full discretizations (if available)
        for i_g, _geo in enumerate(_struct_rotated.full_geometries):
            if len(_geo) > 1:
                _struct_rotated.full_geometries[i_g] = torch.matmul(
                    _geo - (center + self.r0), rot
                ) + (center + self.r0)

        # rotate polarizability tensors
        rot = rot.to(DTYPE_COMPLEX).unsqueeze(0).unsqueeze(0)
        rotT = rot.transpose(-2, -1)
        _struct_rotated.alpha_pE_data = torch.matmul(
            torch.matmul(rotT, _struct_rotated.alpha_pE_data), rot
        )
        _struct_rotated.alpha_mE_data = torch.matmul(
            torch.matmul(rotT, _struct_rotated.alpha_mE_data), rot
        )
        _struct_rotated.alpha_pH_data = torch.matmul(
            torch.matmul(rotT, _struct_rotated.alpha_pH_data), rot
        )
        _struct_rotated.alpha_mH_data = torch.matmul(
            torch.matmul(rotT, _struct_rotated.alpha_mH_data), rot
        )

        # update lookup tables
        _struct_rotated.create_lookup()
    ...
    def combine(
        ...
        new_struct.alpha_pE_data = torch.concatenate(
            [new_struct.alpha_pE_data, other.alpha_pE_data], dim=0
        )
        new_struct.alpha_mE_data = torch.concatenate(
            [new_struct.alpha_mE_data, other.alpha_mE_data], dim=0
        )
        new_struct.alpha_pH_data = torch.concatenate(
            [new_struct.alpha_pH_data, other.alpha_pH_data], dim=0
        )
        new_struct.alpha_mH_data = torch.concatenate(
            [new_struct.alpha_mH_data, other.alpha_mH_data], dim=0
        )

        new_struct.selfterms_pE_data = torch.concatenate(
            [new_struct.selfterms_pE_data, other.selfterms_pE_data], dim=0
        )
        new_struct.selfterms_mE_data = torch.concatenate(
            [new_struct.selfterms_mE_data, other.selfterms_mE_data], dim=0
        )
        new_struct.selfterms_pH_data = torch.concatenate(
            [new_struct.selfterms_pH_data, other.selfterms_pH_data], dim=0
        )
        new_struct.selfterms_mH_data = torch.concatenate(
            [new_struct.selfterms_mH_data, other.selfterms_mH_data], dim=0
        )
