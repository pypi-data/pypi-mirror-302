...
class LargeScaleBayesianOptimizer(BaseOptimizer):
    ...
    def propose_next_point(self):
        ...
        def min_obj(X):
            ...
            acquisition_value = self.acq_func.evaluation(X.reshape(1, -1), self.Y_iters_large, self.gpL)
    ...
    def optimize(self):
        ...
        self.Y_iters_large = np.array([self.objective_funcL(x) for x in self.X_iters_large]).astype(float)
        ...
        for _ in range(self.n_calls):
            ...
            self.Y_iters_large = np.append(self.Y_iters_large, Y_next).astype(float)
