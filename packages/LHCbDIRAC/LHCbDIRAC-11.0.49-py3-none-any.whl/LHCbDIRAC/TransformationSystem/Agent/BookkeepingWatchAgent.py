...
class BookkeepingWatchAgent(AgentModule, TransformationAgentsUtilities):
    ...
    def __getFiles(self, transID, bkQuery, now):
        ...
        start = time.time()
        lfns_getFiles = returnValueOrRaise(self.bkClient.getFiles(bkQuery))
        ...
        len_diff = len(lfns_getFiles) - len(lfn_run)

        if len_diff:
            raise RuntimeError(
                f"__getFiles mismatch {len_diff=} {bkQuery=} "
                f"{len(lfns_getFiles)=} {len(lfn_run)=} "
                f"{ {x.split('/')[5] for x in  set(lfns_getFiles) - set(lfn_run)} }"
            )
