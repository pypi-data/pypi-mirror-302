...
from tasksch.triggers import MonthlyDOWTrigger
...
def set_monthlydow_schedule_trigger(task: TaskDefinition):
    trigger: MonthlyDOWTrigger = task.triggers.create_trigger(
        TriggerType.TASK_TRIGGER_MONTHLYDOW
    )

    # montly specific values
    trigger.clear_months()
    trigger.november = True
    trigger.december = True

    trigger.clear_weeks_of_month()
    trigger.first_week = True
...
def main(ts: TaskService, demo_folder: TaskFolder, args: DemoArgs):
    """
    Monthly DOW Task Demo
    ---------------------

    Create a scheduled monthly dow task that triggers the first Monday
    of the month.

    and writes to the demo log file.

    """
    task_name = "MONTHLY-DOW-DEMO"
    if args.suffix:
    ...
    set_monthlydow_schedule_trigger(task)
