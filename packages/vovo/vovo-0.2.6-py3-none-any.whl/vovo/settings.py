...
class VovoBaseSettings(BaseSettings):
    ...
    CELERY_TASK_NAME = 'vovo.tasks'
    CELERY_BROKER: str = "redis://localhost:6379/0"  # 示例：Redis 作为 Celery broker
    CELERY_RESULT_BACKEND: str = "redis://localhost:6379/0"  # 示例：Redis 作为 Celery result backend
    CELERY_TASK_SERIALIZER: str = 'json'  # 任务序列化器
