...
def start_liveplotter(properties: List[AsyncProperty], labels: Optional[List[List[str]]] = None, transform: Optional[Callable] = None, window_size: int = 500):
    """
    ...
    :param labels: An optional nested list of labels that also defines the subplots
        and the ordering of data.
        Each sublist corresponds to one subplot and can contain multiple data lines.
        This must have the same shape as the data returned by ``transform``.
        If omitted, all properties are shown on a single subplot and their
        labels correspond to the property names.
    :param transform: An optional callback that can be used to transform the
        recorded properties before they are shown.
        This callback is invoked on every iteration. It takes in the recorded
        properties (one numpy array per argument) and shall return the final
        data to be displayed.
        The output must have the form ``[[y_series_1, y_series_2, ...], ...]``
        and must have the same shape as ``labels``.
        The callback must run synchronously (reading values from the ODrive is
        not allowed from within this callback).
    :param window_size: The size of the x-axis in number of samples.
    """
    run_sync_on_loop(lambda: _start_liveplotter(properties, labels, transform, window_size), get_device_manager().loop)
...
@transform_odrive_objects
async def ram_osci_config(odrv: RuntimeDevice, expressions):
