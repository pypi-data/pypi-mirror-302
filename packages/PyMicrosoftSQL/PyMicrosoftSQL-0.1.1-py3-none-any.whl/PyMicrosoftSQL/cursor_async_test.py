...
class Cursor:
    def __init__(self, connection, async_mode=False):
    ...
    async def _execute_async(self, query, params):
        task = asyncio.create_task(self._execute(query, params))
        await task
        
    def execute(self, query, params=None):
        query = query.split('|')[0]
        if self.async_mode:
            print("Executing in async mode")
            asyncio.run(self._execute_async(query, params))
        else:
