        ...
        for depth in range(z):
            for row in range(y):
                ...
                for col in range(x):
                    ...
                    if row > 0:
                        g.add_edge(nodeIndex, nodeIndex - x, weight=1)
                    if col > 0:
                        g.add_edge(nodeIndex, nodeIndex - 1, weight=1)

                    if depth > 0:
                        g.add_edge(nodeIndex, nodeIndex - (x * y), weight=1)
...
def filterGraph(inputG):
    ...
    for u, v, d in inputG.edges(data=True):
        ...
        if (u_color == 'black' and v_color == 'black') or \
           ...
           (u_color == 'white' and v_color == 'red') or \
           (u_color == 'black' and v_color == 'blue'):
...
def bfs(filteredGraph, black_nodes):
    # Perform BFS starting from each black node connected to the red node
    bfs_results = {}
    for black_node in black_nodes:
        bfs_results[black_node] = nx.single_source_shortest_path(filteredGraph, black_node)
    return bfs_results
