...
def save_resnet_config(num_steps=2):
    ...
    for block_spec_idx in range(11):
        for config_filepath in [f'tr_resnet_train_{block_spec_idx}.yaml', f'tr_resnet_inference_{block_spec_idx}.yaml']:

            config.is_train = ('train' in config_filepath)
            config.num_steps = num_steps
            config.task.name = 'image_classification'
            config.task.model.backbone.resnet.spec_idx = block_spec_idx

            output_path = f'configs/tr/step{num_steps}/{config_filepath}'
...
def save_retinanet_config(num_steps=2):
    ...
    # additional information for MLBConfig
    for block_spec_idx in range(11):
        for config_filepath in [f'tr_retinanet_train_{block_spec_idx}.yaml', f'tr_retinanet_inference_{block_spec_idx}.yaml']:
...
def save_maskrcnn_config(num_steps=2):
    ...
    # additional information for MLBConfig
    for block_spec_idx in range(11):
        for config_filepath in [f'tr_maskrcnn_train_{block_spec_idx}.yaml', f'tr_maskrcnn_inference_{block_spec_idx}.yaml']:
...
def save_bert_config(num_steps=2):
    ...
    bert_encoder_configs = generate_bert_encoder_configs()

    # additional information for MLBConfig
    for idx, bert_encoder_config in enumerate(bert_encoder_configs):

        for config_filepath in [f'tr_bert_train_{idx}.yaml', f'tr_bert_inference_{idx}.yaml']:

            config.is_train = ('train' in config_filepath)
            config.num_steps = num_steps
            config.task.name = 'natural_language_processing'
            config.task.model.encoder.bert = bert_encoder_config
