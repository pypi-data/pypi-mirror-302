...
from actions_lib.actions.user.user import decrease_token_amount, increase_token_amount
...
TOKEN_MAPPING = {
    "ethereum": {
        ...
        # testnet
        "usdc": {
            "address": "0xd10519Aa03FE7Ffb95189B52B74F94Fb33E2C88a",
            "decimal": 6
        },
        # mainnet
        # "usdc": {
    ...
    "base": {
        ...
        # testnet
        "usdc": {
            "address": "0x45b58118a5374dccf7fd6f3f66c66278ab7473d9",
            "decimal": 6
        },
        # mainnet
        # "usdc": {
...
def mpc_transfer(default_address, destination, chain_raw, token_raw, token_address, amount):
    try:
        if chain_raw == 'base' and token_raw == 'usdc':
            transfer = default_address.transfer(amount, 'usdc', destination, gasless=True)
            transfer.wait()
            print(f"Transfer complete: {transfer}")
            print(f"Transfer transfer.status: {transfer.status}")
            print(f"Transaction.Status.COMPLETE: {Transaction.Status.COMPLETE}")
            print(f"transfer.status is Transaction.Status.COMPLETE: {transfer.status is Transaction.Status.COMPLETE}")
            print(f"transfer.status == Transaction.Status.COMPLETE: {transfer.status == Transaction.Status.COMPLETE}")
            transferred_wei = convert_to_wei(float(amount), chain_raw, token_raw)
            if transfer.status == 'complete':
                status = 200
                transfer_status = 1
                print(f"Transfer complete: {transfer.transaction_hash}")
                ...
                # increase_token_amount(redis_client, executor, chain_raw, token_raw, amount_to_transfer)
                status = 500
                transfer_status = 0
                transferred_wei = 0
            ...
            info = {
                "content": generate_tx_link_content(transfer.status, transfer.transaction_link),
                "tx_hash": transfer.transaction_hash,
                "chain": chain_raw,
                "status": transfer_status,
                "code": status,
                "transferred_wei": transferred_wei
            }
            ...
            # increase_token_amount(redis_client, executor, chain_raw, token_raw, amount_to_transfer)
            info = {
                "content": f"In automatic mode, only USDC transfers on the Base mainnet are supported.",
                "code": 500
            }
            return info
...
def transfer(receiver: str, amount: float, step: any, chain: str = "base", token: str = "usdc", **kwargs: Any) -> Dict[str, Any]:
    print(f"receiver: {receiver}, amount: {amount}, chain: {chain}, token: {token}, step: {step}")
    ...
    if run_mode.lower() == 'auto':
        return handle_auto_transfer(w3, eth_w3, mpc_address, executor, receiver, amount, token_address, spender_address, sender_address, control_contract, private_key, redis_client, chain_raw, token_raw, blockchain_explore, executor_balance)
    else:
        return handle_manual_transfer(w3, eth_w3, executor, receiver, amount, token_address, redis_client, chain_raw, token_raw)

def handle_auto_transfer(w3: Web3, eth_w3, mpc_address, executor, receiver, amount, token_address, spender_address, sender_address, control_contract, private_key, redis_client, chain_raw, token_raw, blockchain_explore, executor_balance):
    if token_raw != 'usdc':
        ...
        if token_raw in ['btc', 'cbbtc', 'wbtc']:
            if chain_raw == 'base':
                ...
                token = 'WBTC'
        return {'result': {"code": 500, "content": f"Transfers of the current currency({token}) are not supported in automatic mode."}, 'action': None, 'next_action': None}
    else:
        ...
        # allowance_normalized = convert_to_value(allowance, chain_raw, token_raw)
        # print(f"allowance_normalized: {float(allowance_normalized)}")
        amount_to_transfer = convert_to_wei(float(amount), chain_raw, token_raw)
        try:
            decrease_token_amount(redis_client, executor, chain_raw, token_raw, amount_to_transfer)
        except Exception as e:
            print(f"Exception occurred: {e}", flush=True)
            return {'result': {"content": f"{e}", "code": 500}, 'action': None, 'next_action': None}
        ...
        if code != 200:
            return {'result': {"content": f"Cannot find {receiver} on contacts", "code": 420}, 'action': None, 'next_action': None}
        ...
        try:
            ...
            #     return {'result': info, 'action': None, 'next_action': None}
            # else:
            #     return {'result': {"code": 402, "content": "Your account balance is insufficient. Please top up first, or apply to join the credit list to receive $5 of Ardio Credit."}, 'action': None, 'next_action': None}
        except Exception as e:
...
def generate_tx_link_content(status, tx_link):
    status_text = "successful" if status is Transaction.Status.COMPLETE else "failed"
