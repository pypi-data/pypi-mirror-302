...
def transfer(receiver: str, amount: float, step: any, chain: str = "base", token: str = "usdc", **kwargs: Any) -> Dict[str, Any]:
    print(f"receiver: {receiver}, amount: {amount}, chain: {chain}, token: {token}, kwargs: {kwargs}, step: {step}")
    ...
    token_address = TOKEN_MAPPING[chain_raw][token_raw]['address']
...
def handle_auto_transfer(w3: Web3, eth_w3, mpc_address, executor, receiver, amount, token_address, spender_address, sender_address, control_contract, private_key, redis_client, chain_raw, token_raw, blockchain_explore, executor_balance):
    if token_raw != 'usdc':
        ...
        try:
            ...
            if info['code'] != 200:
                increase_token_amount(redis_client, executor, chain_raw, token, amount_to_transfer)
