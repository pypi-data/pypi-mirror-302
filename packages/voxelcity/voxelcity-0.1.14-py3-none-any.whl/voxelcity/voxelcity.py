...
def get_canopy_height_grid(rectangle_vertices, meshsize, source = 'High Resolution 1m Global Canopy Height Maps', output_dir="output", visualization=True):
...
def get_voxelcity(rectangle_vertices, building_source, land_cover_source, canopy_height_source, dem_source, meshsize, remove_perimeter_object=None, mapvis=False, voxelvis=False, maptiler_API_key=None):
    ...
    canopy_height_grid = get_canopy_height_grid(rectangle_vertices, meshsize, source=canopy_height_source)
    if dem_source == "Flat":
    ...
    return voxelcity_grid, building_height_grid, canopy_height_grid, land_cover_grid, dem_grid, building_geojson