...
class LinearSystemFullInverse(LinearSystemBase):
    ...
    def solve(
        ...
        if verbose:
            ...
            print("done in {:.2f}s.\nLU... ".format(t_interact - t_start), end="")
            print("done in {:.2f}s.\nLU... ".format(t_interact - t_start), end="")
        ...
        if verbose:
            t_solve = time.time()
            print("done in {:.2f}s.".format(t_solve - t_e0))
...
class LinearSystemFullMoreMemoryEfficient(LinearSystemFullInverse):
    ...
    def get_interact(self, sim, wavelength: float, verbose=False) -> torch.Tensor:
        ...
        # calc. (G.alpha)
        interact_rows = []
        for struct2 in tqdm(sim.structures, progress_bar=prgbar):
            interact_cols = []
            for struct1 in sim.structures:
    ...
    def get_all_fields_fill_masked_zeros(
        self,
        sim,
        field_at_polarizable: torch.Tensor,
        field_at_nonpolarizable: torch.Tensor = None,
    ) -> torch.Tensor:
        """create full field tensor with polarizable fields inserted.

        fields at non-polarizable positions can be optionally provided in a separate tensor.
        ...
        Args:
            ...
            field_at_nonpolarizable (torch.Tensor, optional): field values to be inserted at non-polarizable positions. Defaults to None.

        Returns:
            torch.Tensor: full field tensor at all meshpoints with masked regions inserted
        """
        # init full internal field tensor
        ...
        # optional: inset field elements at non-polarizable positions into masked full field tensor
        if field_at_nonpolarizable is not None:
            fields_full.flatten()[
                ~mask_fields.flatten()
            ] = field_at_nonpolarizable.flatten()
    ...
    def solve(
        ...
        eh_inside = self.get_all_fields_fill_masked_zeros(sim, f_masked[..., 0])
        e_inside, h_inside = torch.chunk(eh_inside, 2, dim=2)  # separate E and H fields
