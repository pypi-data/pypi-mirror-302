...
def get_decoder_version(decoder: str):
    # ffmpeg -y -f lavfi -i nullsrc -frames:v 1 -c:v libx264 test.mp4
    output_filename = 'decoder_version.mp4'
    codec = {
        'libx264': 'H.264',
    }
    if decoder not in codec:
        raise ValueError(f'Unsupported decoder {decoder}')

    process = subprocess.Popen(
        [f'ffmpeg -y -f lavfi -i nullsrc -frames:v 1 -c:v {decoder} {output_filename} 2>&1 | grep "{codec[decoder]}"'],
        stdout=subprocess.PIPE,
        universal_newlines=True,
        shell=True,
    )
