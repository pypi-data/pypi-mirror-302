...
from suanpan.stream.sio import StreamServer
...
class Stream(
    ...
    STREAM_ARGUMENTS = [
        ...
        Int("stream-recv-queue-count", default=20),
...
class StreamCallLoop(StreamMQBaseLoop):
    ...
    def subscribe(self, **kwargs):
        return mq.subscribeQueue(
            ...
            group=g.nodeGroup,
            consumer=g.nodeId,
            count=self.stream.options["stream-recv-queue-count"],
...
class StreamSIOLoop(StreamBaseLoop):
    ...
    def __init__(self, stream):
        super().__init__(stream)
        self.sio = socketio.Server(async_mode="threading", cors_allowed_origins="*", json=json,
    ...
    def start(self):
        ...
        if self.web:
            ...
            app = Flask(__name__)
            app.wsgi_app = socketio.WSGIApp(self.sio, static_files=self.staticFiles)
            
        app.run(host='', port=g.port, threaded=True)
