...
def approximate(
    ...
    [approximate in Rust documentation.](https://docs.rs/opendp/0.11.1-nightly.20241015.1/opendp/measures/fn.approximate.html)
...
def new_privacy_profile(
    ...
    * returns delta values only within [0, 1]
    ...
    # No type arguments to standardize.
    # Convert arguments to c types.
    c_curve = py_to_c(curve, c_type=CallbackFn, type_name=f64)

    # Call library function.
    lib_function = lib.opendp_measures__new_privacy_profile
    lib_function.argtypes = [CallbackFn]
    lib_function.restype = FfiResult
