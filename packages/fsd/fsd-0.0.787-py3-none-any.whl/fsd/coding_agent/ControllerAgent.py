...
class ControllerAgent:
    ...
    async def explainer_code_task_pipeline(self, tier, solution, role, original_prompt_language):
        ...
        try:
            ...
            await self.image.get_started_image_generation(tier, solution, original_prompt_language)
        except:
            logger.debug(" #### The `Coding Agent` has encountered some bugs. We apologize for the inconvenience. Please try again!")

    async def regular_code_task_pipeline(self, tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files):
        try:
            """Pipeline for regular coding tasks."""
            logger.info("-------------------------------------------------")
            logger.debug("#### Initiating regular_code_task_pipeline")  
            ...
            while True:
                if not isFirst:
                    ...
                    if user_prompt == "exit":
                        ...
                        working_files = await self.codingProgress(tier, user_prompt, role, original_prompt_language, file_attachments, focused_files)
                        final_working_files.update(working_files)
                else:
                    isFirst = False
                    working_files = await self.codingProgress(tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files)
                    final_working_files.update(working_files)
    ...
    async def codingProgress(self, tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files):
        idea_plan = ""
        ...
        while True:
            ...
            if idea_plan:
                ...
                user_prompt, _, _, _ = parse_payload(self.repo.get_repo_path(), user_prompt_json)
                user_prompt = user_prompt.lower()
            ...
            context = await self.context.get_file_planning(finalPrompt, focused_files)
            logger.info("#### Thinking completed.")

            (self.short_idea if isLight else self.idea).initial_setup(role, crawl_logs, context, file_attachments)
    ...
    async def get_started(self, user_prompt, tier, file_attachments, focused_files):
        ...
        if pipeline in ["1", 1]:
            ...
            await self.regular_code_task_pipeline(tier, user_prompt, role, original_prompt_language, file_attachments, focused_files)
            ...
            await self.image.get_started(tier, user_prompt, original_prompt_language, file_attachments, focused_files)
        else:
