...
class ExplainableFileFinderAgent:
    ...
    async def get_file_planning(self, idea, file_attachments, focused_files):
        ...
        prompt = (
            f"Analyze the user prompt and project structure to identify only the most relevant files for answering the user's request. "
            f"Build a JSON response listing these files. Include only files that are directly related to the user's query. "
            ...
            f"    \"working_files\": []\n"
            "}\n"
            "Only include files that actually exist in the given project structure. "
