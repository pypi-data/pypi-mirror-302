...
class Task(TaskMixin, models.Model):
    ...
    data = JSONField(
        ...
        'on the Import page in the Labelo Data Manager UI.',
    )
    ...
    # Thumbnail image for this task
    compressed = models.ForeignKey(
        'data_import.FileUpload',
        related_name='thumbnail',
        on_delete=models.SET_NULL,
        null=True,
        help_text='Thumbnail used as data source for this task',
    )
...
class Annotation(AnnotationMixin, models.Model):
    ...
    review_status = models.CharField(choices=[('Reject', 'Reject'), ('Fix', 'Fix'), ('Accept', 'Accept')],
                                     null=True, max_length=20)
    reject_comment = models.ForeignKey("tasks.Comments", null=True,
                                       on_delete=models.SET_NULL, related_name='reject_comment')
...
class Review(models.Model):
    """Model representing a review of a task."""
    ...
    annotation = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name='reviews',
        help_text='Annotation being reviewed',
        blank=True,
        null=True,
    )
    # annotation =
    project = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        null=True,
        help_text='Project ID for this review',
    )
    reviewer = models.ForeignKey(
        'TaskAssignee',
        on_delete=models.CASCADE,
        related_name='reviews',
        help_text='Task assignee with type "RV" who reviewed the task',
    )
    was_accepted = models.BooleanField(
        _('was accepted'),
        default=False,
        help_text='User has accepted the annotation')
    ...
    updated_at = models.DateTimeField(
        _('updated at'),
        auto_now=True,
        help_text='Last time when the review was updated',
    )
    ...
    def __str__(self):
        return f'Review of task {self.task.id} by {self.reviewer.assignee.username}'
#
#
...
def delete_compressed_thumbnail(sender, instance, **kwargs):
    """after deleting_task
        delete compressed image
        """
    if instance.compressed:
        instance.compressed.delete()
...
class TaskAssignee(models.Model):
    ...
    assignee = models.ForeignKey(
        ...
        help_text='Assigned user to this task',
    )
    task = models.ForeignKey(
        Task, 
        related_name='assignee',
    ...
    compleated = models.BooleanField(
        _('compleated'),
        default=False,
        help_text='User compleated or not this assigned task',
    )

    annotation_completed = models.ForeignKey(
        'tasks.Annotation',
        on_delete=models.CASCADE,
        related_name='assignee',
        blank=True,
        null=True,
        help_text='Corresponding compleated annotation for Task assign',
    )

@receiver(post_save, sender=Annotation)
def assign_complete(sender, instance, created, **kwargs):
    if created:
        try:
            task_assignee = TaskAssignee.objects.get(type="AN", assignee=instance.completed_by, task=instance.task, compleated=False)
            task_assignee.compleated = True
            task_assignee.annotation_completed = instance
            task_assignee.save(update_fields=['compleated', 'annotation_completed'])
...
class Comments(models.Model):
    ...
    draft = models.ForeignKey(AnnotationDraft, on_delete=models.SET_NULL, null=True)
