...
from briton.briton_pb2_grpc import BritonStub
...
class BritonInteractor(abc.ABC):
    ...
    @abc.abstractmethod
    def create_grpc_stub(self, port: int):
        pass
...
class BritonInteractorImpl(BritonInteractor):
    ...
    def create_grpc_stub(self, port):
...
def load_briton(
    ...
    briton_monitor_thread = threading.Thread(
        target=_briton_monitor, args=(briton_process,)
    )
