...
def remove_action_by_id(actions, action_id):
    """
    Remove the action with the specified id from the actions list.

    :param actions: List of action dictionaries
    :param action_id: The id of the action to remove
    :return: Updated list of actions without the specified action
    """
    return [action for action in actions if action['id'] != action_id]
...
def get_all_actions(user, project):
    ...
    # Remove actions based on project settings
    action_conditions = [
        (project.task_distribution == 'AUTO', 'assign_annotators'),
        (project.review_distribution == 'AUTO', 'assign_reviewer')
    ]

    for condition, action_id in action_conditions:
        if condition:
            actions = remove_action_by_id(actions, action_id)
