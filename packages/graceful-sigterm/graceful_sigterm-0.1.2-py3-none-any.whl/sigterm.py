...
def default_handler(*args, **kwargs):
    ...
    """
    _logger.warning("get TERM signal, prepare to stop server...")
    global _STOP_FLAG
    global _STOP_TIME
    global _STOP_EVENT
    _STOP_FLAG = True
...
def setup(sig=signal.SIGTERM, handler=default_handler):
    """注册KILL事件回调函数。"""
    _logger.warning("signal setup: sig=%s, handler=%s", sig, handler)
    signal.signal(sig, handler)


def execute(sleep_seconds=1):
    """启动工作线程，主线程进入等待。"""
    _logger.warning("start workers...")
    for t in _WORKERS:
        t.start()
    ...
    for t in _WORKERS:
        try:
            t.join()
            _logger.warning("worker %s end.", t)
        except Exception as error:
            _logger.warning("main thread waiting error: %s", error)
    _logger.info("main thread end.")
