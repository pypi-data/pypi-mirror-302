...
from tensorkube.services.train import axolotl_train
...
def train(gpus, gpu_type, env, job_id, axolotl, config_path):
    if axolotl:
        axolotl_train(env=env, secrets=[], gpus=gpus, gpu_type=gpu_type, job_id=job_id, config_path=config_path)
    else:
