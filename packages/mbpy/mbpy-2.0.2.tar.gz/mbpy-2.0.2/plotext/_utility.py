...
def update_size(image, size_new, resample=True):
    """
    Resize an image to the desired size, maintaining the aspect ratio if needed.
    
    Args:
        image (PIL.Image): The input image.
        size_new (tuple): The desired new size as (width, height).
        resample (bool): Whether to use resampling for smoother resizing.
        
    Returns:
        PIL.Image: The resized image.
    """
    # Calculate the aspect ratios of old and new sizes.
    ratio_old = image.height // image.width
    ratio_new = size_new[1] // size_new[0]

    # Adjust size if maintaining aspect ratio.
    if ratio_old != ratio_new:
        if image.width > image.height:
            size_new = (size_new[0], int(size_new[0] * ratio_old))
        else:
            size_new = (int(size_new[1] / ratio_old), size_new[1])
    
    # Resize the image with resampling for better quality.
    resample_method = Image.LANCZOS if resample else Image.NEAREST
    resized_image = image.resize(tuple(round(s) for s in image.size), resample=resample_method)
...
def image_to_matrix(image):
    ...
    Returns:
        list: A 2D list representing pixel values.
    """
    pixels = list(image.getdata())
    width, height = image.size
    # Convert the flat list of pixels into a 2D list (rows and columns).
    return [pixels[i * width:(i + 1) * width] for i in range(height)]


def update_size(image, size_new, resample): # it resize an image to the desired size, maintaining or not its size ratio and adding or not a pixel averaging factor with resample = True
    size_old = image.size
...
if __name__ == "__main__":
    ...
    new_size = (800, 600)  # Example new size

    # Resize the image while maintaining quality.
    resized_image = update_size(image, new_size, resample=True)
    ...
    pixel_matrix = image_to_matrix(resized_image)
    print(f"Matrix size: {len(pixel_matrix)} x {len(pixel_matrix[0])}")