...
class GcsSource(StorageServiceSource):
    ...
    def _load_metadata_file(
        ...
        try:
            logger.info(
                f"Looking for metadata template file at - gs://{bucket.name}/{METAMART_TEMPLATE_FILE_NAME}"
            )
            ...
            logger.warning(
                f"No metadata file found at gs://{bucket.name}/{METAMART_TEMPLATE_FILE_NAME}"
            )
        except Exception as exc:
            logger.debug(traceback.format_exc())
            logger.warning(
                f"Failed loading metadata file gs://{bucket.name}/{METAMART_TEMPLATE_FILE_NAME}-{exc}"
            )
        return None
