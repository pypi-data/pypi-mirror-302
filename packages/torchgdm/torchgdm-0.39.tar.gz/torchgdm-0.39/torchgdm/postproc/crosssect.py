...
from torchgdm.tools.batch import get_p_m_moments, get_e_hfields
...
def ecs(
    ...
    **kwargs,
    ...
    e0_at_p = get_e0(sim, wavelength, r_p, illumination_index)
    h0_at_m = get_h0(sim, wavelength, r_m, illumination_index)
    if normalize_to_local_e0:
        # illumination fields
        e0_at_m = get_e0(sim, wavelength, r_m, illumination_index)
        # intensities
...
def acs(
    ...
    **kwargs,
    ...
    # source positions and dipole moments
    p, m, r_p, r_m = get_p_m_moments(sim, wavelength, illumination_index)
    e_at_p, h_at_m, r_p, r_m = get_e_hfields(sim, wavelength, illumination_index)
    ...
    if radiative_correction:
        rad_corr_p = (
            (2 / 3) * abs_factor_p * k0**3 * (torch.multiply(p, torch.conj(p)))
        ).sum((1, 2))
        rad_corr_m = (
            (2 / 3) * abs_factor_m * k0**3 * (torch.multiply(m, torch.conj(m)))
        ).sum((1, 2))
        cs_abs_p = cs_abs_p - rad_corr_p
        cs_abs_m = cs_abs_m - rad_corr_m

    cs_abs = cs_abs_p + cs_abs_m
