...
class InstructionsExecute:
    def __init__(
        ...
        self._create_cfg_host()

    def _create_cfg_host(self):
        print(self.all_instructions.disable_automations.host_info)
        print(self.all_configs.host_info)
        self.all_configs.host_info.api_key = (
            self.all_instructions.disable_automations.host_info.api_key
        )

        if self.all_instructions.disable_automations.host_info.host:
            self.all_configs.host_info.host = (
                self.all_instructions.disable_automations.host_info.host
            )
        if self.all_instructions.disable_automations.host_info.port:
    ...
    def del_helpers_entities(self):
        ...
        for configuration in self.all_configs.helpers_entities:
            ...
            automation_editor = AutomationsEditor(configuration=configuration,
                                                  full_cfg=full_cfg,
                                                  host_cfg=self.all_configs.host_info,
                                                  )
            try:
                if deletion_flag:
                    for item_id in ids_list:
                        resp = automation_editor.delete_helpers(item_id=item_id)
                        if resp.status_code == 404:
                            logging.error(f"{item_id} {resp.json()['message']}")
                    deletion_flag = False
