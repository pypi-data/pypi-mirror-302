...
class Grid:
    ...
    def SetErrorSaveTo(self, path:str = ""):
        self.__saveto = path
    ...
    def fromFileXML(xml_name: str, holdShadowPrice: bool = False):
        ...
        if bm != "": # No base model
            ...
            from .cases import _get_buses, _get_lines, _DEFAULT_LOAD_SCALE
            if bm.lower() == "ieee33":
    ...
    def solve(self, _t: int, /, *, max_v_pu: float = 1.1, min_v_pu: float = 0.9, max_I_kA: float = 0.866, 
            source_buses:'list[BusID]' = [], source_bus_V_pu:float = 1.0, timeout_s:float = 1) -> 'tuple[GridSolveResult, float]':
        '''Get the best result at time _t, return a tuple: (result status, optimal objective value)'''
        ok, val = self.__solve(_t, max_v_pu, min_v_pu, max_I_kA, source_buses, source_bus_V_pu, timeout_s)
        if ok == GRB.Status.OPTIMAL:
            return GridSolveResult.OK, val
        else:
            ok, val = self.__solve(_t, float("inf"), 0, float("inf"), source_buses, source_bus_V_pu, timeout_s)
            if ok == GRB.Status.OPTIMAL:
                ...
                if self.__saveto != "":
                    p = Path(self.__saveto)
                    p.mkdir(parents=True, exist_ok=True)
                    self.savePQofBus(str(p/f"{_t}_load.csv"), _t)
                return GridSolveResult.Failed, val            
        
    def __solve(self, _t: int, max_v_pu:float, min_v_pu: float, max_I_kA: float, 
            source_buses:'list[BusID]', source_bus_V_pu:float, timeout_s:float) -> 'tuple[int, float]':
        model = Model("model")
        max_v = max_v_pu ** 2
        min_v = min_v_pu ** 2
        ...
        for b in source_buses:
            model.addConstr(v[b] == source_bus_V_pu ** 2, f"src_{b}")
