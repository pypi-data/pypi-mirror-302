        ...
        *,
        ...
        if self._client.enable_logging == True:
            ...
            if not isinstance(response, Stream):
                ...
                asyncio.create_task(self._client.log_chat_completion(input_data, output_data)) # type: ignore
            else:
                output_data ={'content': ''}
                async def process_stream(stream: Stream[ChatCompletionChunk]):
                    full_response = ""
                    for chunk in stream:
                        if chunk.choices and chunk.choices[0].delta.content:
                            full_response += chunk.choices[0].delta.content
                            output_data = {'content': full_response}
                            await self._client.log_chat_completion(input_data, output_data) # type: ignore

                # Tạo task để xử lý stream
                asyncio.create_task(process_stream(response))
        return response
