# cython: language_level=3
# #!/usr/bin/env python
# -*- coding: UTF-8 -*-
"""
@Author: MaJian
@Time: 2024/1/16 17:08
@SoftWare: PyCharm
@Project: mortal
@File: __init__.py
"""
from .build_flask import create_development_app
from .build_main import MortalBuildMain


class MortalBuild(MortalBuildMain):
    def __init__(self, port=None, key=None):
        super().__init__()
        if port:
            app = create_development_app(key, self._build_ext)
            app.run(host="0.0.0.0", port=port, threaded=True)

    def build_config(self):
        return self._build_config()

    def build_ext_wheel_pypi(self, config):
        return self._build_ext_wheel_pypi(config)

    def build_ext(self, config, app_flask=False):
        self._build_ext(config, app_flask)

    def build_ext_wheel(self, config):
        return self._build_ext_wheel(config)

    def build_wheel(self, config):
        self._build_wheel(config)
