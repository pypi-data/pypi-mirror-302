...
#                          FD Coefficient     
##########################################################################
def DiffCoef(order: int, grid_scheme: str) -> np.ndarray:
    """
    ...
    Returns:
        np.ndarray: The calculated differential coefficients.
    """
    ...
    if grid_scheme == 'r':
        B = np.insert(np.zeros(order - 1), 0, 1 / 2)
        A = np.array([[j ** (2 * i - 1) for j in range(1, order + 1)] for i in range(1, order + 1)])
    elif grid_scheme == 's':
        B = np.insert(np.zeros(order - 1), 0, 1)
        A = np.array([[(2 * j - 1) ** (2 * i - 1) for j in range(1, order + 1)] for i in range(1, order + 1)])
    else:
        raise ValueError("Invalid grid scheme. Use 'r' for rational or 's' for symmetric.")

    # Calculate the differential coefficients
    C = np.linalg.inv(A) @ B
...
def pad_torchSingle(data, pml, fs_offset, free_surface=True, device='cpu'):
    ...
    # Determine new dimensions based on padding and free surface condition
    nx_pml = nx + 2 * pml
    ...
    # Place original data in the padded tensor
    if free_surface:
        cc[fs_offset:fs_offset + nz, pml:pml + nx] = data
        cc[:fs_offset, pml:pml + nx] = cc[fs_offset, pml:pml + nx].unsqueeze(0).expand(fs_offset, -1)  # Top padding
    else:
        cc[fs_offset + pml:fs_offset + pml + nz, pml:pml + nx] = data
        cc[:fs_offset + pml, pml:pml + nx] = cc[fs_offset + pml, pml:pml + nx].unsqueeze(0).expand(fs_offset + pml, -1)  # Top padding

    # Bottom padding
    cc[nz_pml - pml:nz_pml, pml:pml + nx] = cc[nz_pml - pml - 1, pml:pml + nx].unsqueeze(0).expand(pml, -1)

    # Left and right padding
    cc[:, :pml] = cc[:, pml].unsqueeze(1).expand(-1, pml)  # Left
    cc[:, nx_pml - pml:nx_pml] = cc[:, nx_pml - pml - 1].unsqueeze(1).expand(-1, pml)  # Right
...
def step_forward_PML(M:int,
    ...
    # Initialize tensors
    vx, vz = vx.clone(), vz.clone()
    txx, tzz, txz = txx.clone(), tzz.clone(), txz.clone()

    rcv_txx = torch.zeros((src_n, nt, rcv_n), dtype=dtype, device=device)
    rcv_tzz = rcv_txx.clone()
    rcv_txz = rcv_txx.clone()
    rcv_vx = rcv_txx.clone()
    rcv_vz = rcv_txx.clone()

    forward_wavefield_shape = (nz, nx)
    forward_wavefield_txx = torch.zeros(forward_wavefield_shape, dtype=dtype, device=device)
    forward_wavefield_tzz = forward_wavefield_txx.clone()
    forward_wavefield_txz = forward_wavefield_txx.clone()
    forward_wavefield_vx = forward_wavefield_txx.clone()
    forward_wavefield_vz = forward_wavefield_txx.clone()
