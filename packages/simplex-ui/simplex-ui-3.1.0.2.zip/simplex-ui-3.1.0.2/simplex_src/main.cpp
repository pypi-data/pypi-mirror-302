...
int RunProcess(picojson::object &inobj, string dataname,
	int rank, int mpiprocesses, double memlim, int serno = -1)
{
...
int simplex_solver(int serno, string input)
{
	int retcode = 0;
...
int main(int argc, char **argv)
{
	...
	if(argc == 2){
		...
		LoadFile(rank, mpiprocesses, argv[2], input);
		if(rank == 0 && isclear){
			remove(argv[2]);
		}
	}
	...
	for(int n = 0; n < inobjs.size(); n++){
		if(RunProcess(inobjs[n], datanames[n], rank, mpiprocesses, memlim) < 0){
			retcode = -1;
		}
