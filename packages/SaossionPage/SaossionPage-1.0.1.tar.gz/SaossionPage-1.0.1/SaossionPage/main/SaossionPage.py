...
class Browser:
    ...
    def get_tree(self,ele:ChromiumElement):
        Tool.tree(ele)
    ...
    def turn_to_canvas0(self,ele):

        cmd=r"""
    function getShot_by_canvas(element) {
        //加载库
    ...
    getShot_by_canvas(this);
       
                """ 
        ele.run_js(cmd);   

    def get_shot_by_canvas(self, tab: ChromiumTab, ele: ChromiumElement, name='shot.png',mode=2):
        load_code = r"""
        ...
        # load_code2 = './js/html2canvas.min.js'
        shot_code = r"""
            //截图
        ...
        if mode==1:
            ...
            print(f'正在载入截图js文件:{js_file}')
            tab.run_js(js_file)    

        tab.wait.eles_loaded('#html2canvas_id')
        tab.wait(2)
        ele.run_js(shot_code)
        tab.wait.eles_loaded('#shot_img', timeout=20)
        tab.ele('#shot_img').save(name=name)
        tab.remove_ele('#shot_img')
    ...
    def listen_all(self,file_type=['mp3'],count=10,timeout=100):
        ...
        def _scan(t:ChromiumTab,key): 
            ...
            for data_packet in t.listen.steps(count=count,timeout=timeout):

                file_url=data_packet.url

                url_list.append(file_url)
                # print(next(c),' ',file_url)
                print(f' {next(c)} {current_time()} {Fore.RED}{data_packet.resourceType.rjust(7)}{Fore.RESET}  {tihuan(file_url)} from {Fore.YELLOW+t.url+Fore.RESET}')
    ...
    def get_screenshot_by_js3(self,ele:ChromiumElement,rename='shot.png'):

        shot_code = '''
                    var canvas = html2canvas(this);
                    var img = canvas.toDataURL("image/png");
                    var link = document.createElement('a');
                    link.download = 'screenshot.png';
                    return img;
        ...
        try:
            ca=ele.run_js(shot_code)
            print(ca)
            self.page.download(ca)
        except:
            self.load_html2canvas()
            ca=ele.run_js(shot_code)
            print(ca)
...
function captureElementAndInsertImage(element) {
...
captureElementAndInsertImage(this);           
        ...
        try:
            ele.run_js(shot_code)
            self.page.latest_tab.wait.ele_loaded('#shot')
            self.page.latest_tab.ele('#shot').click.to_download(rename=rename)
            ...
            # self.page.latest_tab.wait.ele_loaded('#shot')
            self.page.wait(6)
            print(self.page.ele('#shot'))
            self.page.ele('#shot').click.to_download(rename=rename)
...
# from .Listen_Ws import Listen_Ws
# from Listen_Console import Listen_Console

# class Listener:
#     def __init__(self,type) -> None:
#         """
#         :param type: ChromiumBase对象
#         """
#         self._type = type
#         self.listener=None

#     def from_tab(self,tab):
        
#         if(self._type=='ws'):
#             self.listener = Listen_Ws(tab)
#         if(self._type.lower=='console'):
#             self.listener = Listen_Console(tab)
...
if __name__ == '__main__':
    browser = Browser(config=' 忽略证书错误  ')
    url='https://www.doc88.com/p-9943632245208.html?s=like&id=2'
    tab=browser.page.new_tab(url)    

    element=tab.ele('#pageContainer')
    browser.get_shot_by_canvas(tab,element,name='6633ttt36.png')
