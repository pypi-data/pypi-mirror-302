# testing webhook on dev branch 1
import os 
import json 
import cv2 
...
def random_scale(image, bboxes,scale_limit:tuple, prob:float,aug_imgs_path,aug_labels_path,filename_without_ext, aug_name="random_scale"):
    try:
        ...
        aug_image = aug["image"]
        aug_bboxes = aug["bboxes"]
        save_img_annot_pair(aug_imgs_path,aug_labels_path,aug_image,aug_bboxes,img_name_without_ext,prob, aug_name="random_scale")
    except Exception as e:
        print(e)
...
def init_config(config):
    ...
    defaults = {
        'root': '/mnt/sfs-shelfaudit-data/YOLO_DET/DATA/dataset-01',
        'kernel_size': 3,
        'prob': 0.5,
        'erosion_rate': 0.45,
        'dropout_prob': 0.2,
