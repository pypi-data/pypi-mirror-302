...
class InferenceMetricAnalysis(object):
    ...
    def _format_result_to_image(self, metric: InferenceMetric, image_metric_result: Dict):
        ...
        for idx, label in self.label_ids.items():
            _image_label_results = LabelMetricResult(precision=[], recall=[], accuracy=[])
            for item in label:
        ...
        if len(confusion_matrix_list) > 0:
            ...
            if MetricName.image_confusion_matrix.value in image_metric_result:
                ...
                confusion_matrix.column_annotation_specs = [[]] * len(confusion_matrix_list)
                confusion_matrix.row_annotation_specs = [[]] * len(confusion_matrix_list)
                confusion_matrix.result = [ConfusionMatrixMetricResult()] * len(confusion_matrix_list)
            ...
            for index, confusion_matrix_result in enumerate(confusion_matrix_list):
                ...
                for idx, item in enumerate(confusion_matrix_result):
                    ...
                    if MetricName.image_confusion_matrix.value in image_metric_result:
                        ...
                        confusion_matrix.result[index].upper_ound = upper_bound
