...
logger = logging.getLogger(
    "great_expectations.validation_operators.validation_operators.metamart"
)


class MetaMartValidationAction(ValidationAction):
    ...
    def _get_table_entity(
        ...
        if not all([schema_name, table_name]):
            raise ValueError(
                "No Schema or Table name provided. Can't fetch table entity from MetaMart."
            )
    ...
    def _create_ometa_connection(self) -> MetaMart:
        """Create MetaMart API connection"""
        environment = create_jinja_environment(self.config_file_path)
        rendered_config = render_template(environment)

        return MetaMart(create_ometa_connection_obj(rendered_config))
