...
def hex_to_number(
    ...
    """
    This function converts a hexadecimal string to a number (int) and provides additional formatting options.

    :param hex_value: The hexadecimal value as a string.
    :param is_comma: Whether to include commas in the result.
    :param debug: Whether to include the original hex value in the output for debugging.
    :param change: Force change even if the original value is already an integer.
    :param ansi: Whether to add ANSI color codes to the output.
    :param is_tint: Whether to divide the result by a constant (e.g., TINT).
    :param symbol: A symbol to prepend to the converted value (e.g., currency symbol).
    :param show_change: Show whether the value has changed when in debug mode.
    :return: The converted value as an integer, or a formatted string if debug is True.
    """
    _changed = False
    if not ansi:
        _bcolors = Null()
    ...
    if (_changed and change) or is_int(converted_value):
        ...
        if symbol:
            converted_value = f"{converted_value} {symbol}"
    ...
    if debug:
        ...
        if not _changed:
            ...
            debug_info = f"{changed_text} (org: {original_hex_value})"

        # show_change가 True인 경우 변경 여부를 추가
        if show_change:
            change_status = "changed" if _changed else "unchanged"
            debug_info = f"{debug_info}[{change_status}]"

        return f"{converted_value} {_bcolors.ITALIC}{debug_info}{_bcolors.ENDC}".strip()
    else:
