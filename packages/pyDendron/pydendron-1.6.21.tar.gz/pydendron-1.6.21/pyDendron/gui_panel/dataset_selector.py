...
class DatasetSelector(MyViewer):
    ...
    backup_on_load =  param.Boolean(True, doc='Create a backup of the dataset when loading a new dataset.')
    save_context =  param.Boolean(True, doc='Save dataset and configuration when disconnected.')
    ...
    def __init__(self, dataset, template, cfg_path, **param):
        ...
        self._layout = pn.Column(self.wselect, 
                                 ...
                                 pn.Row(self.param.save_auto, self.param.backup_on_load, self.param.save_context)
                                 )
