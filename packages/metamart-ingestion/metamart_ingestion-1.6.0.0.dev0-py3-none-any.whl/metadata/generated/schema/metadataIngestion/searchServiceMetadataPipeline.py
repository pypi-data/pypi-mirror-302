...
class SearchServiceMetadataPipeline(BaseModel):
    ...
    markDeletedSearchIndexes: Annotated[
        ...
        Field(
            True,
            description='Optional configuration to soft delete search indexes in MetaMart if the source search indexes are deleted. Also, if the search index is deleted, all the associated entities like lineage, etc., with that search index will be deleted',
