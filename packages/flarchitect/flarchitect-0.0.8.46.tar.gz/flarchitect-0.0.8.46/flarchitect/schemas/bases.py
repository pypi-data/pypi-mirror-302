...
class AutoSchema(Base):
    ...
    def _add_validation(self, column: Column, field_args: dict):
        ...
        if get_config_or_model_meta(
            ...
            if ("email" in column_name and column.type.python_type == str) or format_name == "email":
                field_args["validate"].append(validate_by_type("email"))
            elif ("url" in column_name and column.type.python_type == str) or format_name in ["url", "uri", "url_path"]:
            ...
            elif ("domain" in column_name and column.type.python_type == str) or format_name == "domain":
            ...
            if dump_type == "url":
                ...
                if request and request.args.get("join"):
                    ...
                    url_naming_function = get_config_or_model_meta(
                        "API_ENDPOINT_NAMER", relationship_property.mapper.class_, default=endpoint_namer
                    )
