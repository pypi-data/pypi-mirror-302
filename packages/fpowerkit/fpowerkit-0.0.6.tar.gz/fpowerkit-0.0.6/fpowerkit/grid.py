...
class Grid:
    ...
    def solve(self, _t: int, /, *, max_v_pu:float = 1.1, min_v_pu: float = 0.9, max_I_kA: float = 0.866, 
            source_buses:'list[BusID]' = [], source_bus_V_pu:float = 1.0) -> 'tuple[GridSolveResult, float]':
        ...
        if model.Status != GRB.Status.OPTIMAL:
            # Remove VI constraints and try again
            state, val = self.solve(_t, max_v_pu=float("inf"), min_v_pu=0, max_I_kA=float("inf"), 
                                    source_buses=source_buses,source_bus_V_pu=source_bus_V_pu)
            if state == GridSolveResult.OK:
        ...
        return GridSolveResult.OK, model.ObjVal
