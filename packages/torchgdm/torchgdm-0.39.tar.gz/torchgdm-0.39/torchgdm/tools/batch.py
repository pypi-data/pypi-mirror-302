    ...
    def wrap(func):
        def batched_func(
            *args,
            ...
            progress_bar=False,
            pgr_bar_title="batched evaluation",
            **kwargs,
        ):
...
def calc_rasterscan(sim, func, wavelength, kw_r="r_focus", **kwargs):
    ...
    Args:
        ...
        kw_r (`str`): kwarg used for first (x) position in rasterscan
...
def get_p_m_moments(sim, wavelength: float, illumination_index: int = None, **kwargs):
    ...
    test_illumination_field_config_exists(
        sim=sim, wavelength=wavelength, illumination_index=illumination_index
    )
    # get dipole moments and their positions for selected illumination
    r_p, r_m = sim._get_polarizable_positions_separate_p_m()
    p = sim._get_polarizable_dipole_moments_p(wavelength)
    m = sim._get_polarizable_dipole_moments_m(wavelength)
...
def get_e_hfields(sim, wavelength: float, illumination_index: int = None, **kwargs):
    """get self-consistent internal fields at polarizable locations and their locations
    ...
    Returns:
        tuple: tuple containing 4x `torch.Tensor` with the E and H-fields and their 3D positions (e, h, r_e and r_h)
    """
    # verify if config exists
    test_illumination_field_config_exists(
        sim=sim, wavelength=wavelength, illumination_index=illumination_index
    ...
    # get dipole moments and their positions for selected illumination
    r_e, r_h = sim._get_polarizable_positions_separate_p_m()
    e = sim._get_polarizablefields_e(wavelength)
    h = sim._get_polarizablefields_h(wavelength)
    if illumination_index is not None:
...
def get_e0(
    ...
    **kwargs,
    ...
    Args:
        sim (`torchgdm.Simulation`): simulation (`sim.run()` needs to have been executed before)
        wavelength (float): in nm
        r (torch.Tensor): position(s) at which to evaluate incident field
        illumination_index (int, optional): optional index of a specific illumination. If None, batch-evaluate all illuminations. Defaults to None.

    Returns:
        torch.Tensor: incident E-field(s)
    """
    e0 = torch.stack(
        [
            f_inc.get_efield(
    ...
    if illumination_index is not None:
        e0 = e0[illumination_index].unsqueeze(0)
    return e0
...
def get_h0(
    ...
    **kwargs,
    ...
    Returns:
        torch.Tensor: incident H-field(s)
    """
    h0 = torch.stack(
        [
            f_inc.get_hfield(
    ...
    if illumination_index is not None:
        h0 = h0[illumination_index].unsqueeze(0)
    return h0
