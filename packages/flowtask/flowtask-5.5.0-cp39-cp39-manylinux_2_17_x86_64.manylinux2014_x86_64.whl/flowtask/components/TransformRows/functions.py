...
def extract_from_array(df: pandas.DataFrame, field: str = "", column: str = "", index = 0):
    ...
    :param index: The index of the element to extract ('first' for the first element, 'last' for the last element, or an integer index).
...
def explode(
    ...
    :param delimiter: Optional, the character or string used as the delimiter to split the values. Defaults to a comma (',').
...
def to_time(
    ...
    :param replace_nulls: Optional, a boolean flag that determines whether to replace null values with a specified value. Defaults to False.
    ...
    :param as_timedelta: Optional, a boolean flag that determines whether to convert the time objects to timedelta. Defaults to False.
...
def split_array(df: pandas.DataFrame, field: str, column="", separator=" ", trim=False):
    """
    Takes a pandas DataFrame and splits the values in a specified column into lists based on a specified separator.
...
def uppercase(df: pandas.DataFrame, field: str, from_column: str = None):
    ...
    :param df: pandas DataFrame to be modified.
    :param field: Name of the column in the df DataFrame to be modified.
    :param from_column: Name of the column to use for the uppercase conversion. If None, the 'field' column is used. Defaults to None.
...
def fill_nulls(df: pandas.DataFrame, field: str, column: str):
    ...
    if field not in df.columns.values:
        ...
        try:
            ...
            logging.error(f"Fill Nulls: Column {field} doesn't exist")
    return df
