...
def _load_frame_list(buffer: BinaryIO) -> List[bytes]:
  ...
  with pydicom.dcmread(buffer) as ds:
    ...
    if _PYDICOM_MAJOR_VERSION <= 2:
      frame_bytes_generator = pydicom.encaps.generate_pixel_data_frame(  # pytype: disable=module-attr
          ds.PixelData, number_of_frames
      )
    else:
      frame_bytes_generator = pydicom.encaps.generate_frames(  # pytype: disable=module-attr
          ds.PixelData, number_of_frames
      )
