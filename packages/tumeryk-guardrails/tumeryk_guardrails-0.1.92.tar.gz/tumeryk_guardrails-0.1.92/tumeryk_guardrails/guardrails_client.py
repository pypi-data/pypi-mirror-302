...
class TumerykGuardrailsClient:
    ...
    def _auto_login(self):
        ...
        if username and password:
            ...
            print("Auto-login skipped: Missing username or password in environment variables")

    def _auto_set_policy(self):
        """Automatically set policy if environment variable is available."""
        policy = os.getenv("TUMERYK_POLICY")
        print(f"Auto-set policy: Policy found: {bool(policy)}")
        if policy:
            try:
                set_policy_result = self.set_policy(policy)
                print(f"Auto-set policy result: {set_policy_result}")
            except ValueError as err:
                print(f"Auto-set policy failed: {err}")
        else:
            print("Auto-set policy skipped: Missing TUMERYK_POLICY in environment variables")

    def _get_headers(self):
        """Helper method to get the headers including authorization."""
        if not self.token:
            print("Authorization token is not set. Please login first.")
    ...
    def tumeryk_completions(self, messages: str, stream: bool = False):
        ...
        if not self.token:
            print("Authorization token not found. Please login to proceed.")
            return {"error": "Authorization token not found. Please login to proceed."}
        if not self.config_id:
            print("Config ID is required. Please pick a policy.")
