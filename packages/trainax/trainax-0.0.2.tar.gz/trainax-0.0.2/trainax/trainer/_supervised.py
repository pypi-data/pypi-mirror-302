    ...
    def __init__(
        ...
        **Arguments:**
        ...
        - `num_rollout_steps`: The number of time steps to autoregressively roll
            out the model. Defaults to 1 (which is one-step supervised
            training).
        ...
        - `cut_bptt`: Whether to cut the backpropagation through time
            ...
            autoregressive network main chain.
        ...
        - `time_level_weights`: An array of length `num_rollout_steps` that
            ...
            argument)
        - `do_sub_stacking`: Whether to use sub-stacking. If `False`, then the
            given reference trajectory will not be sliced into windows of length
            `num_rollout_steps + 1`.
