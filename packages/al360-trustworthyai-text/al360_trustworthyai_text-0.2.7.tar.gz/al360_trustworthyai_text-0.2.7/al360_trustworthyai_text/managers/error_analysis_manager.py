...
from al360_erroranalysis._internal.constants import ModelTask as ErrorAnalysisTask
from al360_erroranalysis._internal.error_analyzer import ModelAnalyzer
from al360_erroranalysis._internal.error_report import as_error_report
from al360_trustworthyai._tools.shared.state_directory_management import \
    DirectoryManager
from al360_trustworthyai.exceptions import UserErrorException
from al360_trustworthyai.managers.error_analysis_manager import \
    ErrorAnalysisManager as BaseErrorAnalysisManager
from al360_trustworthyai.managers.error_analysis_manager import as_error_config
from al360_trustworthyai_text.common.constants import ModelTask
from al360_trustworthyai_text.utils.feature_extractors import get_text_columns
from al360_trustworthyai_text.utils.genai_metrics.metrics import get_genai_metric
...
class ErrorAnalysisManager(BaseErrorAnalysisManager):
    ...
    @staticmethod
    def _load(path, al360_tai_insights):
        """Load the ErrorAnalysisManager from the given path.

        :param path: The directory path to load the ErrorAnalysisManager from.
        :type path: str
        :param al360_tai_insights: The loaded parent AL360_TAIInsights.
        :type al360_tai_insights: AL360_TAIInsights
        ...
        feature_metadata = al360_tai_insights._feature_metadata
        categorical_features = feature_metadata.categorical_features
        inst.__dict__['_categorical_features'] = categorical_features
        target_column = al360_tai_insights.target_column
        true_y = al360_tai_insights._ext_test_df[target_column]
        if isinstance(target_column, list):
            dropped_cols = target_column
        else:
            dropped_cols = [target_column]
        dataset = al360_tai_insights._ext_test_df.drop(columns=dropped_cols)
        inst.__dict__['_dataset'] = dataset
        feature_names = list(dataset.columns)
        inst.__dict__['_feature_names'] = feature_names
        wrapped_model = wrap_model(al360_tai_insights.model, dataset,
                                   al360_tai_insights.task_type)
        is_multilabel = False
        index_classes = al360_tai_insights._classes
        index_dataset = al360_tai_insights.test
        if isinstance(target_column, list):
        ...
        inst.__dict__['_task_type'] = al360_tai_insights.task_type
        text_column = al360_tai_insights._text_column
        index_predictor = ErrorAnalysisManager._create_index_predictor(
            wrapped_model, index_dataset, target_column, text_column,
            is_multilabel, al360_tai_insights.task_type, index_classes)
