...
from arize.proto import public_pb2 as pb2
...
from .casting import cast_dictionary
...
class Client:
    ...
    def __init__(
        ...
        if any(
            not isinstance(key, str) for key in [api_key, space_key, space_id] if key
        ):
        ...
        self._headers = {
            ...
            "Grpc-Metadata-sdk-package-name": "arize-slim",
    ...
    def log(
        ...
        if tags:
            ...
            wrong_tags = [
                tag_name for tag_name in tags.keys() if tag_name in RESERVED_TAG_COLS
            ]
            if wrong_tags:
                raise KeyError(
...
def _get_label(
    ...
    if model_type in NUMERIC_MODEL_TYPES:
        ...
        model_type in CATEGORICAL_MODEL_TYPES or model_type == ModelTypes.GENERATIVE_LLM
    ):
...
def _convert_prediction_id(prediction_id: Union[str, int, float]) -> str:
    if not isinstance(prediction_id, str):
        try:
            ...
            )  # strip ensures we don't receive whitespaces as part of the prediction id
        except Exception:
            raise ValueError(
...
def _validate_prompt_templates_and_llm_config(
    ...
    if prompt_template_version is not None:
        ...
        if len(prompt_template_version) > MAX_PROMPT_TEMPLATE_VERSION_LENGTH:
            ...
            len(prompt_template_version) > MAX_PROMPT_TEMPLATE_VERSION_LENGTH_TRUNCATION
        ):
            logger.warning(
                get_truncation_warning_message(
    ...
    if llm_params is not None:
        ...
        for param_name, param_value in llm_params.items():
            ...
            if val is None:
                continue
            is_correct_type = isinstance(val, (bool, int, float, str)) or is_list_of(
                val, str
            )
