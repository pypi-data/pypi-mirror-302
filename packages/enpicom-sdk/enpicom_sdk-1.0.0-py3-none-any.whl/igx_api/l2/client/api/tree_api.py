...
from igx_api.l2.types.tree import Tree, TreeId
...
class TreeApi:
    ...
    def get_trees(self, cluster_run_id: ClusterRunId, cluster_id: ClusterId, tree_id: TreeId) -> list[Tree]:
        ...
        Args:
            ...
            tree_id (igx_api.l2.types.tree.TreeId): The unique identifier of the tree.
        ...
        Example:
            ```python
                ...
                tree_id = TreeId(456)
                trees = self.get_trees(cluster_run_id=cluster_run_id, cluster_id=cluster_id, tree_id=tree_id)
            ```
        """
        ...
        try:
            get_trees_request = openapi_client.GetTrees(cluster_run_id=cluster_run_id, cluster_id=cluster_id, tree_id=tree_id)
            return [Tree.from_raw(i) for i in tree_api_instance.get_trees(get_trees_request).trees]
    ...
    def start(self, cluster_run_id: ClusterRunId, cluster_id: ClusterId) -> Execution[list[Tree]]:
        ...
        try:
            ...
            tree_job = tree_api_instance.start_tree(tree_work)
        except openapi_client.ApiException as e:
            raise ApiError(e)

        job_id = JobId(tree_job.job_id)

        def wait() -> list[Tree]:
            job_api = JobApi(self._inner_api_client, self._log_level)
            job = job_api.wait_for_job_to_be_completed(job_id)

            tree_id = job_api.get_tree_id_for_tree_job(job.id)

            trees: list[Tree] = self.get_trees(cluster_run_id=cluster_run_id, cluster_id=cluster_id, tree_id=tree_id)
            return trees
    ...
    def export_tree_as_tsv(self, tree: Tree, output_directory: str | Path | None = None) -> Execution[Path]:
        ...
        Args:
            ...
            output_directory (str | Path | None, optional): The directory where to download the TSV file. Defaults to a
        ...
        with ApiErrorContext():
            export_request = openapi_client.StartTreeExportRequest(
                cluster_run_id=tree.cluster_run_id, cluster_id=tree.cluster_id, tree_id=tree.tree_id, type=tree.type
            )
            export_job = tree_api_instance.start_tree_export(export_request)

        job_id = JobId(export_job.job_id)
