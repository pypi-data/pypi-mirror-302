...
def get_pff_api_url() -> str:
    """Retrieves the PFF API URL from the environment or defaults to the predefined constant.
...
def get_pff_api_key() -> str:
    ...
    if not pff_api_key:
        raise ValueError(
            'A PFF API key is required. Please set it in your environment as `PFF_API_KEY`.'
        )
...
def get_client(
    ...
    *,
    ...
    :param pff_api_url: The API URL to connect to. Defaults to the environment variable PFF_API_URL.
    :param pff_api_key: The API key for authentication. Defaults to the environment variable PFF_API_KEY.
