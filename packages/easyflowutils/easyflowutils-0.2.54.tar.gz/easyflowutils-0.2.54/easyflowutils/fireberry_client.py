...
class FireberryClient:
    ...
    def batch_delete(self, object_type: str | int, data: list[str]) -> dict[str, Any]:
        api_prefix = "https://api.fireberry.com/api/v3/record"
    ...
    def get_account_id_by_email(self, email: str) -> Optional[str]:
        account = self.get_account_by_field_and_value("emailaddress1", email)
        if account:
            return account.get('accountid')
        return None

    def get_account_id_by_phone(self, phone: str) -> Optional[str]:
        account = self.get_account_by_field_and_value("telephone1", phone)
        if account:
            return account.get('accountid')
        return None
