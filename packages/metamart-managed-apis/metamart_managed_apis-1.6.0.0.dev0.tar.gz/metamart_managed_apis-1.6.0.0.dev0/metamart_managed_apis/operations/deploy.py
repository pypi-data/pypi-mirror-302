...
from metamart_managed_apis.api.config import (
    AIRFLOW_DAGS_FOLDER,
    DAG_GENERATED_CONFIGS,
    PLUGIN_NAME,
)
from metamart_managed_apis.api.response import ApiResponse
from metamart_managed_apis.api.utils import (
    clean_dag_id,
    get_dagbag,
    import_path,
    scan_dags_job_background,
)
from metamart_managed_apis.utils.logger import operations_logger
...
def dump_with_safe_jwt(ingestion_pipeline: IngestionPipeline) -> str:
    ...
    pipeline_json["metaMartServerConnection"]["securityConfig"][
        "jwtToken"
    ] = ingestion_pipeline.metaMartServerConnection.securityConfig.jwtToken.get_secret_value(
...
class DagDeployer:
    ...
    def __init__(self, ingestion_pipeline: IngestionPipeline):
        ...
        SecretsManagerFactory(
            ingestion_pipeline.metaMartServerConnection.secretsManagerProvider,
            ingestion_pipeline.metaMartServerConnection.secretsManagerLoader,
        )
