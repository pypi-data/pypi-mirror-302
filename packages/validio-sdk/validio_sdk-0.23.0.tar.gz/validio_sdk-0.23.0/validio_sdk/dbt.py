...
async def upload_artifacts(
    ...
    """
    create_response = await client.dbt_artifact_multipart_upload_create(
        DbtArtifactMultipartUploadCreateInput(
            credential_id=credential_id,
            job_name=job_name,
        )
    )
    if len(create_response.errors) > 0:
        raise ValidioError(f"failed to start file upload: {create_response.errors}")
    ...
    for i in range(num_chunks):
        ...
        append_response = await client.dbt_artifact_multipart_upload_append_part(
            DbtArtifactMultipartUploadAppendPartInput(
                id=upload_id,
                part=part,
            )
        )
        if len(append_response.errors) > 0:
            raise ValidioError(f"failed to upload file part: {append_response.errors}")

    complete_response = await client.dbt_artifact_multipart_upload_complete(
        DbtArtifactMultipartUploadCompleteInput(id=upload_id)
    )
    if len(complete_response.errors) > 0:
        raise ValidioError(f"failed to finalize file part: {complete_response.errors}")
