...
from tasks.models import Annotation, AnnotationDraft, Prediction, Task, TaskAssignee, Comments
...
class AnnotationSerializer(FlexFieldsModelSerializer):
    ...
    comments = serializers.SerializerMethodField(default='', read_only=True)

    def get_comments(self, obj):
        comments = Comments.objects.filter(annotation=obj)
...
class CommentsSerializer(ModelSerializer):
    task = serializers.SerializerMethodField(default=[], read_only=True)
    created_by = serializers.SerializerMethodField(default=[], read_only=True)
    class Meta:
        model = Comments
        fields = '__all__'

    def get_task(self, comment):
        if comment.draft:
            return TaskSerializer(instance=comment.draft.task).data
        else:
            return TaskSerializer(instance=comment.annotation.task).data

    def get_created_by(self, comment):
        return UserSerializer(instance=comment.created_by, context=self.context).data
