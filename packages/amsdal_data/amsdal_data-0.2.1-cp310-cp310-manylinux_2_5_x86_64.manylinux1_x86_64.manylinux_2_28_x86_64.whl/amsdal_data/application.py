...
class DataApplication(metaclass=Singleton):
    ...
    def connections_statuses(self) -> list[ConnectionStatus]:
        connection_manager = glue.Container.managers.get(glue.interfaces.ConnectionManager)
        ...
        for _name, _pool in connection_manager.connections.items():
            _connections.append(
                ConnectionStatus(
                    name=_name,
                    is_connected=_pool.is_connected,
                    is_alive=_pool.is_alive,
                )
            )
