...
from gesund_val_library.metrics.object_detection.object_detection_metric_plot import Object_Detection_Plot
...
class ValidationCreation:
    ...
    def plot_metrics(self, metrics, output_dir, plot_configs):
        ...
        for draw_type, config in plot_configs.items():
            ...
            if draw_type == 'mixed_plot':
                plot.draw('mixed_plot', mixed_args=config.get('mixed_plot'), threshold=config.get('threshold'), save_path=save_path)
            elif draw_type == 'top_misses':
                plot.draw('top_misses', top_misses_args=config.get('top_misses_min_miou'), save_path=save_path)
            elif draw_type == 'confidence_histogram':
                plot.draw('confidence_histogram', confidence_histogram_args=config.get('confidence_histogram_labels'), save_path=save_path)
            elif draw_type == 'classbased_table':
                plot.draw('classbased_table', classbased_table_args=config.get('classbased_table_metrics'),threshold=config.get('threshold'), save_path=save_path)
            elif draw_type == 'overall_metrics':
                plot.draw('overall_metrics', overall_args=config.get('overall_metrics_metrics'), threshold=config.get('threshold'), save_path=save_path)
            elif draw_type == 'blind_spot':
                plot.draw('blind_spot', blind_spot_args=config.get('blind_spot_Average'),threshold=config.get('threshold'), save_path=save_path)
