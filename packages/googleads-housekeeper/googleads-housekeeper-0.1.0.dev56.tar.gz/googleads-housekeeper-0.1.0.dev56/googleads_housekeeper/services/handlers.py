...
def migrate_from_old_tasks(
  ...
  with uow:
    for item in uow.tasks.list():
      if not hasattr(item, 'exclusion_level'):
        updated_item_dict = TaskAdapter(item.to_dict()).from_task_v1().to_dict()
        uow.tasks.update(item.id, updated_item_dict)
