...
def login(url, name: Annotated[str, typer.Argument()] = default_pypi_name):
    url, username, password = split_auth_url(url)
    shell_wrapper(f'pdm config pypi.{name}.url {url}')
    if url.startswith('https:'):
        shell_wrapper(f'pdm config pypi.{name}.verify_ssl true')
    shell_wrapper(f'pdm config pypi.{name}.username "{username}"')
    shell_wrapper(f'pdm config pypi.{name}.password "{password}"')
