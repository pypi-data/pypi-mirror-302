...
def run(
    ...
    if bool(os.environ.get("VALIDATION_SPARK_HEAP_SIZE_EXTRA_ROOM")):
        session_args["heap_size_extra_room"] = int(os.environ["VALIDATION_SPARK_HEAP_SIZE_EXTRA_ROOM"])

    if bool(os.environ.get("VALIDATION_SPARK_NUM_PARTITIONS")):
        num_partitions = os.environ["VALIDATION_SPARK_NUM_PARTITIONS"]
        session_args["config"].append(("spark.sql.shuffle.partitions", num_partitions))
        session_args["config"].append(("spark.default.parallelism", num_partitions))

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_MEMORY")):
        session_args["config"].append(
            ("spark.driver.memory", os.environ["VALIDATION_SPARK_DRIVER_MEMORY"])
        )

    if bool(os.environ.get("VALIDATION_SPARK_MEMORY_FRACTION")):
        session_args["config"].append(
            ("spark.memory.fraction", os.environ["VALIDATION_SPARK_MEMORY_FRACTION"])
        )

    if bool(os.environ.get("VALIDATION_SPARK_MEMORY_STORAGE_FRACTION")):
        session_args["config"].append(
            ("spark.memory.storageFraction", os.environ["VALIDATION_SPARK_MEMORY_STORAGE_FRACTION"])
        )

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_CORES")):
        session_args["config"].append(
            ("spark.driver.cores", os.environ["VALIDATION_SPARK_DRIVER_CORES"])
        )

    if bool(os.environ.get("VALIDATION_SPARK_DRIVER_EXTRA_JAVA_OPTIONS")):
        session_args["config"].append(
            ("spark.driver.extraJavaOptions", os.environ["VALIDATION_SPARK_DRIVER_EXTRA_JAVA_OPTIONS"])
        )
