...
class StateMachine:
    ...
    def run(self, input_lines, input_offset=0, context=None,
        ...
        if self.debug:
            print('\nStateMachine.run: input_lines (line_offset={}):\n| {}'.format(self.line_offset, '\n| '.join(self.input_lines)),
                  file=sys.stderr)
        transitions = None
        results = []
            ...
            while True:
                try:
                    try:
                        ...
                        if self.debug:
                            print(f'\nStateMachine.run: {state.__class__.__name__}.eof transition', file=sys.stderr)
        ...
        if next_state:
            if self.debug and next_state != self.current_state:
                print('\nStateMachine.get_state: Changing state from '
                      f'"{self.current_state}" to "{next_state}" (input line {self.abs_line_number()}).', file=sys.stderr)
...
class State:
    ...
    def __init__(self, state_machine: StateMachine, debug=False):
        """
