    ...
    async def __getDaughtersInfoInner(self):
        ...
        async def do_work(prod_id, run_number):
            ...
            return returnValueOrRaise(
                await loop.run_in_executor(None, BookkeepingClient().getInputOuputFilesForProd, prod_id, run_number)
            )
        ...
        with Progress(*PROGRESS_COLUMNS) as progress:
            ...
            for coro in asyncio.as_completed(map(lambda x: do_work(self.prod, x), runs)):
                ...
                aname_idx = coro_result["ParameterNames"].index("AncestorName")
                dname_idx = coro_result["ParameterNames"].index("DescendantName")
