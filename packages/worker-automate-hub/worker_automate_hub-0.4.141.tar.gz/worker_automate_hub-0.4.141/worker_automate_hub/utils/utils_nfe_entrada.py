...
class EMSys():
    ...
    async def verify_warning_and_error(self, warning_title, title="&Yes"):
        try:
            ...
            if btn_no.exists() and btn_no.is_enabled():
                ...
                console.print("Warning - Erro durante a verificação de warnings e errors")
                return {"sucesso": False, "retorno": 'Erro durante a verificação de warnings e errors\n'}
    ...
    async def percorrer_grid(self):
        ...
        try:
            while index_item_atual < index_ultimo_item:
                ...
                # ITERANDO COM O ITEM
                tpage_item = main_window.child_window(class_name="TPanel", found_index=0)
                item = tpage_item.child_window(class_name="TDBIEditString", found_index=1)
                ...
                if 'isqueiro' in item_value.lower():
                    ...
                    console.print("Item não é isqueiro. Continuando para o próximo...\n")
    ...
    async def inserir_vencimento_e_valor(self, data_vencimento, valor):
        ...
        # Incluir registro de vencimento e valor
    ...
    async def verify_max_vatiation(self):
        ...
        while True:
            ...
            if not current_content:
                console.print("Nenhum conteúdo copiado, encerrando loop.", style="bold red")
                break
            ...
            if next_content == last_content:
                ...
                if next_content == last_content:
                    ...
                    if repeat_count >= max_repeats:
                        console.print("Não há mais itens para processar. Fim do grid alcançado.", style="bold green")
                        break
                else:
                    repeat_count = 0
            else:
        ...
        for item in data_list:
            if not (item['custo_min'] - 4 <= item['curto'] <= item['custo_max'] + 4):
                itens_invalidos.append(item['codigo'])

        if itens_invalidos:
            console.print("Itens que Ultrapassaram a Variação Máxima de Custo", style="bold yellow")
            ...
            console.print("Todos os itens estão dentro da variação de custo permitida.", style="bold green")
            send_keys('{ENTER}')
            ...
            if line.startswith('Código'):
                ...
                if i < len(lines):
                    ...
                    if len(headers) == len(data):
                        ...
                        console.print("Número de cabeçalhos e dados não correspondem.", style="bold red")
        ...
        for item in data_list:
            try:
                new_item = {
                    ...
                    'custo_min': float(item['R$ Custo Min.'].replace('.', '').replace(',', '.')),
                    'custo_max': float(item['R$ Custo Máx.'].replace('.', '').replace(',', '.'))
                }
                final_list.append(new_item)
            except Exception as e:
                console.print(f"Erro ao processar item: {item}. Erro: {e}", style="bold red")
        return final_list

    async def download_xml(self, google_drive_folder_id, get_gcp_token, get_gcp_credentials, chave_nota):
