...
export class SharedDrive {
    ...
    async exportFile(toPath) {
        ...
        const type = this._app.shell.currentWidget.context.contentsModel
            .type;
        const format = (_a = this._app.shell.currentWidget.context.contentsModel.format) !== null && _a !== void 0 ? _a : 'text';
        const fileId = this._ydrive.getId(_fromPath);
        const key = `${format}:${type}:${fileId}`;
        const fileProvider = this._fileProviders.get(key);
        const content = fileProvider === null || fileProvider === void 0 ? void 0 : fileProvider.sharedModel.getSource();
        if (fileProvider) {
            const options = {
                type,
                format,
