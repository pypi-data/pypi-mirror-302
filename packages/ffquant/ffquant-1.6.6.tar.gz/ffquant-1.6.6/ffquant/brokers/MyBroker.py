...
class MyBroker(bt.BrokerBase):
    ...
    def get_notification(self):
        notif = None
        if datetime.now().timestamp() - self.last_get_notif_ts > self.MIN_UPDATE_INTERVAL_SECONDS:
            self.last_get_notif_ts = datetime.now().timestamp()
            ...
            if self.debug:
                print(f"get_notification, notif: {notif}")
