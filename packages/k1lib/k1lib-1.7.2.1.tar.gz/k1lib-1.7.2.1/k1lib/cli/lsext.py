...
class PgConn:                                                                    # PgConn
    def __init__(self, p:"PgConns", db:str):                                     # PgConn
        self.p = p; self.db = db; self.conn = None; self._connect()              # PgConn
    ...
    def queryRaw(self, f): cur = self.conn.cursor(); res = f(cur); cur.close(); return res # absolute barebones, just prepare the cursor and handles all errors before # PgConn
...
"""                                                                              # sqltable
        ...
        for k, v in kwargs.items():                                              # sqltable
            ...
            elif isinstance(v, (set, tuple)) or _typeD.get(k, "") == "array" or _typeD.get(k, "").endswith("[]"): vs.append(list(v) if mode == "pg" else json.dumps(list(v))) # sqltable
...
"""                                                                              # sqltable
        ...
        for k, v in kwargs.items():                                              # sqltable
            ...
            elif isinstance(v[0], (set, tuple)) or _typeD.get(k, "") == "array" or _typeD.get(k, "").endswith("[]"): vs.append(list(v) if mode == "pg" else json.dumps(list(v))) # sqltable
...
"""                                                                              # sqltable
        ...
        for k, v in kwargs.items():                                              # sqltable
            ...
            elif isinstance(v, (set, tuple)) or _typeD.get(k, "") == "array" or _typeD.get(k, "").endswith("[]"): values.append(list(v) if mode == "pg" else json.dumps(list(v))) # sqltable
