...
class ProjectManager:
    ...
    async def move_file(self, old_full_path: str, new_full_path: str) -> Optional[str]:
        if not os.path.exists(old_full_path):
            logger.info(f"Move operation failed: File '{os.path.basename(old_full_path)}' not found at '{old_full_path}'.")
            return None

        if os.path.exists(new_full_path):
            logger.info(f"Move operation skipped: Destination file '{os.path.basename(new_full_path)}' already exists at '{new_full_path}'.")
            return None

        self._move_file(old_full_path, new_full_path)
        logger.info(f"File move operation successful:")
        logger.info(f"  - Source: '{os.path.relpath(old_full_path)}'")
        logger.info(f"  - Destination: '{os.path.relpath(new_full_path)}'")
        return new_full_path
    ...
    def _move_file(self, existing_file_path: str, new_full_path: str) -> None:
        ...
        logger.info(f"File moved: '{os.path.basename(existing_file_path)}' has been relocated from '{os.path.dirname(existing_file_path)}' to '{os.path.dirname(new_full_path)}'.")
