...
class CameraOptions(TypedDict, total=False):
    zoom: Optional[float]
...
class FlightOptions(TypedDict, total=False):
    padding: Padding
...
class Map:
  ...
  def fit_to(self, bounds_or_ids: Union[List[str], Bounds], options: Optional[FlightOptions] = None) -> None:
    """
    ...
    Example:
        ...
        ... })
    """
    return self.sdk.map.fitTo(bounds_or_ids, ffi.to_js(options))

  def move_to(self, options: Optional[CameraOptions]) -> Union[Success, Failure]:
    """
    ...
    Example:
        ...
        ... })
    """
    return self.sdk.map.moveTo(ffi.to_js(options))
  ...
  def select_asset(self, asset: Union[Dict[str, Any], Dict[str, Any]]) -> None:
    ...
    Args:
        asset (Union[Dict[str, Any], Dict[str, Any]]): An object containing geometry, type, and ID of the asset to select
        or geometry and ID for the zone.

    Example:
        Select the first zone in the network:

        >>> zone = sdk.network.getZones()[0]
        >>> sdk.map.select_asset(zone)
    """
    return self.sdk.map.selectAsset(ffi.to_js(asset))

  def hide_map_layer(self, type: ElementType, group: Optional[Union[ValveGroup, JunctionGroup, PipeGroup]]) -> None:
    """
    ...
    Args:
        ...
        group (Optional[Union[ValveGroup, JunctionGroup, PipeGroup]]): The asset group to hide, when applicable.

    Example:
        Hide lateral pipe layer:

        >>> sdk.map.hide_map_layer(ElementType.PIPE, PipeGroup.Lateral)
    """
    return self.sdk.map.hideMapLayer(type, ffi.to_js(group))


  def show_map_layer(self, type: ElementType, group: Optional[Union[ValveGroup, JunctionGroup, PipeGroup]]) -> None:
    """
    ...
    Args:
        ...
        group (Optional[Union[ValveGroup, JunctionGroup, PipeGroup]]): The asset group to show, when applicable.

    Example:
        Show customer points:

        >>> sdk.map.show_map_layer(ElementType.JUNCTION, JunctionGroup.CustomerPoint)
    """
  ...
  async def register_icon(self, id: str, svg: str) -> Union[Success, Failure]:
    ...
    """
    return self.sdk.map.registerIcon(id, svg)