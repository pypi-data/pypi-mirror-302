...
class AutomationsEditor:
    ...
    def delete_helpers(self, helpers_list: list[str]) -> (list[dict], list[str]):
        for item_id in helpers_list:
            url = self.host_cfg.host + ":" + self.host_cfg.port + self.host_cfg.delete_helpers_url.format(item_id=item_id)
            headers = {"content-type": "application/json"}
            print(self.ha_request.delete_request(url, headers))

    def delete_entities(self, integrations_list):
