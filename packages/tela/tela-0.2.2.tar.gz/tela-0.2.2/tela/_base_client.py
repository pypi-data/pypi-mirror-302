...
from ._utils import asyncify, is_dict, is_given, is_list, is_mapping
...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def __init__(
        ...
        *,
        ...
        if max_retries is None:  # pyright: ignore[reportUnnecessaryComparison]
            raise TypeError(
                "max_retries cannot be None. If you want to disable retries, pass `0`; if you want unlimited retries, pass `math.inf` or a very high number; if you want the default behavior, pass `tela.DEFAULT_MAX_RETRIES`"
            )
    ...
    def _build_headers(self, options: FinalRequestOptions, *, retries_taken: int = 0) -> httpx.Headers:
        ...
        if "x-retry-count" not in (header.lower() for header in custom_headers):
            headers["x-retry-count"] = str(retries_taken)
