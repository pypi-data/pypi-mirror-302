...
async def entrada_de_notas_505(task):
    """
    ...
    try:
        ...
        if document_type['sucesso'] == True:
            ...
            return {"sucesso": False, "retorno": f"{document_type['retorno']}"}
        ...
        if erro_pop_up['IsOpened'] == True:
            ...
            return {"sucesso": False, "retorno": f"{error_work['retorno']}"}
        ...
        if informacao_nf_eletronica['IsOpened'] == True:
            return {"sucesso": False, "retorno": f"Não foi possivel finalizar os trabalhos na tela de Informações para importação da Nota Fiscal Eletrônica"}
        ...
        if i == max_attempts:
            return {"sucesso": False, "retorno": f"Número máximo de tentativas atingido. A tela para Aguarde não foi encerrada."}
