...
class Chat(BaseLoadedAttrPararamObject, BaseClientObject):
    ...
    def _load_posts(self, start_post_no: int = -50, end_post_no: int = -1) -> List['Post']:
        ...
        if 0 > start_post_no > end_post_no:
            raise PararamioValidationException(
                'range start_post_no must be greater then end_post_no'
            )
        if 0 <= start_post_no > end_post_no:
            raise PararamioValidationException(
                'range start_post_no must be smaller then end_post_no'
            )
        _absolute = abs(end_post_no - start_post_no)
        if start_post_no < 0:
            _absolute = +1
        if _absolute >= POSTS_LIMIT:
            raise PararamioLimitExceededException(f'max post load limit is {POSTS_LIMIT - 1}')
    ...
    def _lazy_posts_loader(
        self, start_post_no: int = -50, end_post_no: int = -1
    ) -> Iterable['Post']:
        if (start_post_no < 0 <= end_post_no) or (start_post_no >= 0 > end_post_no):
            raise PararamioValidationException(
                'start_post_no and end_post_no can only be negative or positive at the same time'
