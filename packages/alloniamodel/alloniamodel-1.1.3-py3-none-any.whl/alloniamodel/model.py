...
class AllOnIAModel(BackwardCompatibleObject):
    ...
    _path_makers: ClassVar = {
        ...
        "open_by": lambda root, name: root / name / "open_by.json",
        "binary": lambda root, name: root / name / "model.alloniamodel",
        "lock": lambda root, name: str(
            (root / name / "model.alloniamodel").with_suffix(".lock")
        ),
    }
    ...
    ) -> Union[DeleteResponse, tuple[DeleteResponse, DeleteResponse]]:
    ...
    def _delete_all(cls, name: str) -> tuple[DeleteResponse, DeleteResponse]:
        ...
        persistent = try_delete(
            model_directory,
            "rmdir",
            recursive=True,
        )
        non_persistent = try_delete(
            model_directory,
            "rmdir",
            recursive=True,
        )
    ...
    def _load(
        ...
        ':obj:`root`/model/:obj:`name`/model.alloniamodel' exists,
    ...
    def get_description(
        cls,
        name: str,
        revision_or_version: Union[None, int, str] = None,
        ...
        Args:
            ...
            revision_or_version: which revision or version to get the
                ...
                and the description extracted from it to create the file.
            path_only: see :obj:`utils.VersionNamedFile.read`.

        Returns:
            The model description, or None if the specified revision was not
                found.
        """
        # skips revision checks to save time
        return cls._make_description_path(
            name, cls._make_paths(name, ("description",))[0]
        ).read(revision_or_version, path_only=path_only)

    @classmethod
    def get_opening_history(cls, name: str) -> Union[None, dict]:
        ...
        Returns:
            ...
            opening timstamps, dans values being another dict containing
            information about who opened the model and when it got closed again.
        """
        # Cast to new S3Path which is NOT persistent
        open_by = S3Path(
            cls._make_paths(name, ("open_by",))[0],
            persistent=False,
            handle_type=True,
        )
    ...
    def unlock(
        ...
        # Cast to new S3Path which is NOT persistent
        response = try_delete(
            S3Path(cls._make_paths(name, ("lock",))[0], persistent=False),
            "rm",
        )
        logger.info(
    ...
    ) -> Union[tuple[DeleteResponse, DeleteResponse], DeleteResponse]:
    ...
    def locked(cls, name: str) -> bool:
        ...
        # Cast to new S3Path which is NOT persistent
        return S3Path(
            cls._make_paths(name, ("lock",))[0], persistent=False
        ).is_file()

    @classmethod
    @typechecked
    def list_versions(cls, model: Union[AllOnIAModel, str]) -> pd.DataFrame:
        """Lists all S3 versions of the given model .alloniamodel file.
    ...
    def __init__(  # noqa: PLR0913, PLR0915
        ...
        version: Union[str, None] = None,  # noqa: ARG002
        revision: Union[int, None] = None,  # noqa: ARG002
        read_only: bool = False,
        ...
        ':obj:`root`/model/:obj:`name`/model.alloniamodel', in which case it is
    ...
    @property
    @getattr_safe_property
    def seldon_implementation(self) -> Union[None, str]:
    ...
    def update_description(
        self, entries: dict[str, str], save_now: bool = False
    ) -> bool:
        """Updates entries in the model's :obj:`description`.
        ...
        Args:
            ...
            save_now: If True, saves the description file now. Only valid if the
                model is read-only, as it is supposed to overwrite the revision
                file corresponding to this model's revision, which will have
                been incremented if the model is not read-only.

        Returns:
            True if something got updated, False otherwise
        """
        if (
            updated := self._description.update_user_entries(entries)
        ) and save_now:
            if not self.read_only:
                raise not_read_only_err()
            if self.new:
                raise new_model_err()
            self.version_named_description_path.write(
                self.version_id, self.description
            )
        return updated
    ...
    @property
    def version_id(self) -> Union[None, str]:
    ...
    @property
    def classes(self) -> Union[None, tuple]:
    ...
    @property
    def validation_set_size(self) -> Union[int, float]:
    ...
    @property
    def test_set_size(self) -> Union[int, float]:
    ...
    @property
    def random_state(self) -> Union[None, int]:
        """Random state used in the :obj:`~train_val_test_split_function`."""
        return self._random_state

    @random_state.setter
    @typechecked
    def random_state(self, value: Union[None, int]):
    ...
    @typechecked
    def add_validators(self, validators: Union[tuple, list]):
    ...
    def raw_set(
        self,
    ) -> Union[np.ndarray, pd.DataFrame, pd.Series, SpecialDataFormat, None]:
    ...
    def health_check(
        self,
        force: bool = False,
        reshape_x: Union[None, tuple[int, ...]] = None,
        reshape_y: Union[None, tuple[int, ...]] = None,
        **kwargs,
    ) -> bool:
        ...
        **kwargs,
        ...
        if self.raw_set is not None:
            self.feature_engineering(
                "raw", **copy(kwargs.get("feature_engineering_kwargs", {}))
            )
        ...
        validation_metrics = self.validate(
            reshape_x=reshape_x, **evaluate_kwargs
        )
        test_metrics = self.test(reshape_x=reshape_x, **evaluate_kwargs)
    ...
    @convert_self_kwarg
    def validate(
        self, reshape_x: Union[None, tuple[int, ...]] = None, **kwargs
    ):
    ...
    @convert_self_kwarg
    def test(self, reshape_x: Union[None, tuple[int, ...]] = None, **kwargs):
    ...
    def apply(self, reshape_x: Union[None, tuple[int, ...]] = None, **kwargs):
        ...
        if self.observations_set is not None:
            self.feature_engineering(
                "observed", **copy(kwargs.get("feature_engineering_kwargs", {}))
            )
        self.predict(
            reshape_x=reshape_x, **copy(kwargs.get("predict_kwargs", {}))
        )
        results = self.postprocess(**copy(kwargs.get("postprocess_kwargs", {})))
    ...
    @convert_self_kwarg
    def predict(self, reshape_x: Union[None, tuple[int, ...]] = None, **kwargs):
    ...
    def save(self):
        ...
        ':obj:`root`/model/:obj:`name`/model.alloniamodel' and
