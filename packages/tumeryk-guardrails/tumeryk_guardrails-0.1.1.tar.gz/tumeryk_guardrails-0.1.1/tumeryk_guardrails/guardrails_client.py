...
class TumerykGuardrailsClient:
    ...
    def tumeryk_completions(self, user_input: str, stream=False):
        ...
        try:
            response = requests.post(guard_url, json=payload, headers=headers, stream=stream)
            response.raise_for_status()
            ...
            if stream:
                ...
                for chunk in response.iter_content(chunk_size=None):
                    if chunk:
                        # Assume the response contains valid JSON chunks that include tokens
                        try:
                            token_data = chunk.decode('utf-8')
                            # If it's properly formatted as JSON, extract the token part
                            yield token_data
                        except ValueError:
                            continue
            else:
