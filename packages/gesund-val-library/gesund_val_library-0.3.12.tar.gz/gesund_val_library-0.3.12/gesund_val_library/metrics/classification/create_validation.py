...
class ValidationCreation:
    ...
    def plot_metrics(self, metrics, output_dir):
        ...
        plot = Classification_Plot(class_distributions_path=os.path.join(output_dir, f'plot_{draw_type}.json'))
        plot.draw('class_distributions', metrics=['normal'], threshold=10, save_path=os.path.join(output_dir, f'{draw_type}.png'))

        draw_type = 'blind_spot'
        plot = Classification_Plot(blind_spot_path=os.path.join(output_dir, f'plot_{draw_type}_metrics.json'))
        plot.draw('blind_spot', class_type='Average', save_path=os.path.join(output_dir, f'{draw_type}.png'))

        draw_type = 'performance_by_threshold'
        plot = Classification_Plot(performance_threshold_path=os.path.join(output_dir, f'plot_class_{draw_type}.json'))
        plot.draw('performance_by_threshold', graph_type='graph_1', metrics=['F1', 'Sensitivity', 'Specificity', 'Precision'], threshold=0.2, save_path=os.path.join(output_dir, f'{draw_type}.png'))
        
        draw_type = 'roc'
        plot = Classification_Plot(roc_statistics_path=os.path.join(output_dir, f'plot_{draw_type}_multiclass_statistics.json'))
        plot.draw('roc', roc_class='normal', save_path=os.path.join(output_dir, f'{draw_type}.png'))

        draw_type = 'precision_recall'
        plot = Classification_Plot(precision_recall_statistics_path=os.path.join(output_dir, f'plot_{draw_type}_multiclass_statistics.json'))
        plot.draw('precision_recall', pr_class='normal', save_path=os.path.join(output_dir, f'{draw_type}.png'))

        draw_type = 'confidence_histogram'
        plot = Classification_Plot(confidence_histogram_path=os.path.join(output_dir, f'plot_{draw_type}_scatter_distribution.json'))
        plot.draw('confidence_histogram', metrics=['TP','FP'], threshold=0.5, save_path=os.path.join(output_dir, f'{draw_type}.png'))  

        draw_type = 'overall_metrics'
        plot = Classification_Plot(overall_json_path=os.path.join(output_dir, f'plot_highlighted_{draw_type}.json'))
        plot.draw('overall_metrics', metrics=['mean AUC', 'fwIoU'],  save_path=os.path.join(output_dir, f'{draw_type}.png'))
