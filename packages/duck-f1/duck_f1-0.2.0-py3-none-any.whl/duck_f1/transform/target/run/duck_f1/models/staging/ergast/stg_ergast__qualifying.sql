  ...
  create view "f1"."staging"."stg_ergast__qualifying__dbt_tmp" as (
    with
...
qualifying_results as (
    select
        ...
        if(qualifying.q1 = '\N' or len(qualifying.q1) = 0, null, 
    to_milliseconds(
        ...
        if(qualifying.q2 = '\N' or len(qualifying.q2) = 0, null, 
    to_milliseconds(
        ...
        if(qualifying.q3 = '\N' or len(qualifying.q3) = 0, null, 
    to_milliseconds(
...
qualifying_windows as (
    select *,
        ...
        q1_time - first(q1_time) over (partition by race_id order by q1_time nulls last) as q1_interval,
        row_number() over (partition by race_id order by q2_time nulls last) as q2_position,
        q2_time - first(q2_time) over (partition by race_id order by q2_time nulls last) as q2_interval,
        row_number() over (partition by race_id order by q3_time nulls last) as q3_position,
        q3_time - first(q3_time) over (partition by race_id order by q3_time nulls last) as q3_interval
    from qualifying_results
    where q1_time is not null
),

formatted as (
    select qualifying.race_id,
        qualifying.driver_id,
        qualifying.constructor_id,
        qualifying.position,
        qualifying.q1_time,
        qualifying.q1_time_label,
        if(q1_time is null, qualifying.position, qualifying.q1_position) as q1_position,
        qualifying.q1_interval,
        qualifying.q2_time,
        qualifying.q2_time_label,
        if(q2_time is null, null, qualifying.q2_position) as q2_position,
        qualifying.q2_interval,
        qualifying.q3_time,
        qualifying.q3_time_label,
        if(q3_time is null, null, qualifying.q3_position) as q3_position,
        qualifying.q3_interval
    from qualifying_windows qualifying
)
