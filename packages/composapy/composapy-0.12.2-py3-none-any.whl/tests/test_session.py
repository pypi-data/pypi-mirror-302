...
def test_session_invalid_uri_error_message():
    ...
    with pytest.raises(ConnectionError) as exc_info:
        session = Session(auth_mode=AuthMode.FORM, credentials=(username, password), uri=invalid_uri)
    expected_message = (
...
def test_session_uri_trailing_slash():
    ...
    session = Session(auth_mode=AuthMode.FORM, credentials=(username, password), uri=uri_without_slash)
    assert session.uri.endswith('/'), "The session URI should end with a trailing slash."