...
def _get_collection_meta(
    panoramax: Panoramax, collection_id: str, client: httpx.Client
):
...
def _get_collection_items(client: httpx.Client, col_location: str, quality: Quality):
...
def _download_collection(
    ...
    pic_list = _get_collection_items(
        client, _get_collection_location(panoramax, collection_id), quality
    )
...
def download(
    ...
    with utils.createClientWithRetry(disable_cert_check) as c:
