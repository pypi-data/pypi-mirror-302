# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

from rbt.cloud.v1alpha1.application import application_pb2 as rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2


class ApplicationInterfaceStub(object):
    """/////////////////////////////////////////////////////////////////////

    """

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.Create = channel.unary_unary(
                '/rbt.cloud.v1alpha1.application.ApplicationInterface/Create',
                request_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.CreateRequest.SerializeToString,
                response_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.CreateResponse.FromString,
                )
        self.Up = channel.unary_unary(
                '/rbt.cloud.v1alpha1.application.ApplicationInterface/Up',
                request_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpRequest.SerializeToString,
                response_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpResponse.FromString,
                )
        self.UpTask = channel.unary_unary(
                '/rbt.cloud.v1alpha1.application.ApplicationInterface/UpTask',
                request_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpRequest.SerializeToString,
                response_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpTaskResponse.FromString,
                )
        self.Down = channel.unary_unary(
                '/rbt.cloud.v1alpha1.application.ApplicationInterface/Down',
                request_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownRequest.SerializeToString,
                response_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownResponse.FromString,
                )
        self.DownTask = channel.unary_unary(
                '/rbt.cloud.v1alpha1.application.ApplicationInterface/DownTask',
                request_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownTaskRequest.SerializeToString,
                response_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownTaskResponse.FromString,
                )


class ApplicationInterfaceServicer(object):
    """/////////////////////////////////////////////////////////////////////

    """

    def Create(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def Up(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def UpTask(self, request, context):
        """The async part of `Up`. Not part of the public API.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def Down(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def DownTask(self, request, context):
        """The async part of `Down`. Not part of the public API.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_ApplicationInterfaceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'Create': grpc.unary_unary_rpc_method_handler(
                    servicer.Create,
                    request_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.CreateRequest.FromString,
                    response_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.CreateResponse.SerializeToString,
            ),
            'Up': grpc.unary_unary_rpc_method_handler(
                    servicer.Up,
                    request_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpRequest.FromString,
                    response_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpResponse.SerializeToString,
            ),
            'UpTask': grpc.unary_unary_rpc_method_handler(
                    servicer.UpTask,
                    request_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpRequest.FromString,
                    response_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpTaskResponse.SerializeToString,
            ),
            'Down': grpc.unary_unary_rpc_method_handler(
                    servicer.Down,
                    request_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownRequest.FromString,
                    response_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownResponse.SerializeToString,
            ),
            'DownTask': grpc.unary_unary_rpc_method_handler(
                    servicer.DownTask,
                    request_deserializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownTaskRequest.FromString,
                    response_serializer=rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownTaskResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'rbt.cloud.v1alpha1.application.ApplicationInterface', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class ApplicationInterface(object):
    """/////////////////////////////////////////////////////////////////////

    """

    @staticmethod
    def Create(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/rbt.cloud.v1alpha1.application.ApplicationInterface/Create',
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.CreateRequest.SerializeToString,
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.CreateResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def Up(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/rbt.cloud.v1alpha1.application.ApplicationInterface/Up',
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpRequest.SerializeToString,
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def UpTask(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/rbt.cloud.v1alpha1.application.ApplicationInterface/UpTask',
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpRequest.SerializeToString,
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.UpTaskResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def Down(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/rbt.cloud.v1alpha1.application.ApplicationInterface/Down',
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownRequest.SerializeToString,
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def DownTask(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/rbt.cloud.v1alpha1.application.ApplicationInterface/DownTask',
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownTaskRequest.SerializeToString,
            rbt_dot_cloud_dot_v1alpha1_dot_application_dot_application__pb2.DownTaskResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
