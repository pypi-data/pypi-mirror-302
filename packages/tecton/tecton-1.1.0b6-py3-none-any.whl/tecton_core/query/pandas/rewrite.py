...
class PandasTreeRewriter:
    def rewrite(
    ...
    ) -> Iterable[Tuple[str, pyarrow.RecordBatchReader]]:
        ...
        Returns generator of (staged table name, Arrow Record Batch Reader)
        Reader must be registered with the Compute under the table name by the caller of this method.
        """
        tree_node = tree.node
        ...
        if isinstance(tree_node, nodes.FeatureViewPipelineNode):
            ...
            yield staging_table_name, pipeline_result
        else:
            for i in tree.inputs:
                yield from self.rewrite(tree=i, data_sources=data_sources, secret_resolver=secret_resolver)
