...
from ._typing import InstrumentKind, SubscribeMethod, channel_options
...
def queue_item_iterator(q: queue.Queue) -> Generator[Any, None, None]:
    while True:
        try:
            item = q.get()
            q.task_done()
    ...
    def subscribe_iter(
        ...
        def response_generator():
            ...
            response_iterator = self._stream.Subscribe(
                queue_item_iterator(q), metadata=self._metadata
            )
            _next()

            log.info("<<<waiting for stream response")

            return sub.id, _next, response_iterator

        subId, _next, response_iterator = response_generator()
        ...
        while not self._finished.is_set():
            try:
                for response in response_iterator:
                    ...
                    # 通知后端可以推送下一个信息
                    _next()
                    log.debug(f"[{subId}]>>>next@{method.value}:{req}")
                ...
                if status_code == grpc.StatusCode.UNAVAILABLE:
                    ...
                    # retry
                    time.sleep(1)
                    log.info(f"[{subId}]Retry to subscribe@{method.value}:{req}")
