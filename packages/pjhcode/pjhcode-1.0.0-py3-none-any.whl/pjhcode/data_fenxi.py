...
def peer_quality(x):  # 交易性质判断
    ...
    life_firm = ['银行', '保险', '贷款', '金融', '嘀嘀', '网银', '保险', '中国', '支付', '\(', '（', '(', '分期乐',
                 '平安', '年卡', '星驿', '分期', '备付金', '天翼', '时空信息交换', '天津舒行', '快手',
                 '滴滴', '付款', '京东', '美团', '三快', '拼多多', '寻梦', '财付通', '云闪付', '抖音',
                 '淘宝', '饿了么', '拉扎斯', '盒马', '理财', '基金', '唯品会', '物流', '顺丰', '微播', '格物致品']
    ...
    if data_deal.luhn_iscard(x['对端卡号']):
        return '卡转卡交易'
...
def cash_level_penetrate(df_df, direction, hour, mothon, collide=pd.DataFrame(), biaoshilie=pd.DataFrame(), only_bank=0,
    ...
    collide_next['对端iscard'] = collide_next['对端卡号'].apply(data_deal.luhn_iscard)
...
def tou_shi(df_toushi):
    all_pt = pd.pivot_table(df_toushi, index=['对端卡号'], values=['交易金额', '主端卡号', '对端姓名'],
                            columns=['收付标志'],
                            aggfunc={"交易金额": 'sum', '主端卡号': ['nunique', 'count'], '对端姓名': data_deal.extract_first_string}).reset_index()
    ...
    print('保存透视结果...')
    all_pt['银行卡'] = all_pt['对端卡号'].apply(data_deal.luhn_iscard)
    return all_pt
...
def agent_under_people(uid: str, invite_id: str):
    ...
    """
    # 读取数据框
    df = pd.read_excel(data_get.get_data(), dtype='str')
