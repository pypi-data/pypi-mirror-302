...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.move_to_active_dir_button = None
    ...
    def move_image_to_active_dir(self):
        ...
        elmer_file = self.scan_all_for_file(strip_name + "_elmer.png")
        ...
        # Move the elmer file
        if elmer_file:
            ## Initial setup for move
            elmer_file_name = os.path.basename(elmer_file)
            cur_active_dir = self.current_active_dir.replace("/", "\\")
            ## Do the actual move
            elmer_save_path = os.path.join(cur_active_dir, elmer_file_name)
            shutil.move(elmer_file, elmer_save_path)  # noqa
    ...
    def export_repeating_file_names_to_cmd(self):
        ...
        start_dir = self.common_img_path  # Replace it with your starting directory
    ...
    def add_active_dir(self):
        ...
        if dir_path:
            ...
            if not dir_path in self.active_dirs:
                ...
                # calculate the common active directory path
                ...
                # create the custom folder widget item for the common active directory
                ...
                # create the custom file item widgets for each file in the active directories
                file_items = []
                if len(self.active_dirs) > 1:
                    for active_dir in self.active_dirs:
                        file_item_name = os.path.relpath(active_dir, self.com_dirs_path).replace('\\', '/')
                        file_item = QListWidgetItem(f"📁 {file_item_name}/")
                        file_items.append(file_item)
                elif len(self.active_dirs) == 1:
                    file_item_name = os.path.basename(self.active_dirs[0])
                    file_item = QListWidgetItem(f"📁 {file_item_name}/")
                    file_items.append(file_item)

                # add all list widgets to the list.
                self.active_dir_list_widget.addItem(folder_item)
                for file_item in file_items:
                    self.active_dir_list_widget.addItem(file_item)
            ...
            # Modify the widget name to reflect the currently selected active directory
    ...
    def save_file(self, _value=False):
        ...
        is_moving_to_a_new_dir = not os.path.dirname(self.file_path) == self.current_active_dir

        image_file_dir = os.path.dirname(self.file_path)
        image_file_name = os.path.basename(self.file_path)
        label_file_name = os.path.splitext(image_file_name)[0] + XML_EXT
        ...
        if self.current_active_dir:
            label_path = os.path.join(self.current_active_dir, label_file_name)
        else:
            label_path = os.path.join(image_file_dir, label_file_name)
        ...
        # Moving the elmer counterpart if the move is to a new directory and the elmer counterpart exists

        ## Setup
        elmer_file_name = os.path.splitext(image_file_name)[0] + "_elmer" + os.path.splitext(image_file_name)[1]
        cur_elmer_path = os.path.join(image_file_dir, elmer_file_name)
        elmer_counter_part_exists = os.path.exists(cur_elmer_path)
        if is_moving_to_a_new_dir and elmer_counter_part_exists:
            ## Do the actual move
            cur_active_dir = self.current_active_dir.replace("/", "\\")
            elmer_save_path = os.path.join(cur_active_dir, elmer_file_name)
            shutil.move(self.file_path, elmer_save_path)  # noqa
