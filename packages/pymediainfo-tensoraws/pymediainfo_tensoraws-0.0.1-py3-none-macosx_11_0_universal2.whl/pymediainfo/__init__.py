...
class MediaInfo:
    ...
    def _get_library(
        ...
        raise OSError("Failed to load library from {} - {}".format(", ".join(library_paths), ", ".join(exceptions)))
    ...
    def parse(
        ...
        if file_size is not None:  # We have a file-like object, use the buffer protocol:
            ...
            if lib.MediaInfo_Open(handle, filename) == 0:
                ...
                raise RuntimeError("An error occured while opening {}" " with libmediainfo".format(filename))
