        ...
        while granted_time < total_interval and not terminate_requested:
            ...
            for esdl_id in self.simulator_configuration.esdl_ids:
                if not terminate_requested:
                    LOGGER.debug(f"Getting all inputs for {esdl_id}")
                    calculation_params_for_esdl_id = calculation_params[esdl_id]
                    ...
                    while not CalculationServiceHelperFunctions.dictionary_has_values_for_all_keys(calculation_params_for_esdl_id):
                        waiting_iterations += 1
                        time.sleep(self.WAIT_FOR_INPUT_ITERATION_DURATION_SECONDS)
                        LOGGER.debug(f"Gathering inputs iteration {waiting_iterations} all inputs for {esdl_id}")
                        LOGGER.debug(calculation_params_for_esdl_id)
                        self._gather_new_inputs(calculation_params_for_esdl_id, esdl_id)
                        if max_amount_of_waiting_iterations == waiting_iterations:
                            raise ValueError("Timeout reached for getting all required values for esdl_id")
                    try:
                        ...
                        pub_values = self.helics_combination_federate_info.calculation_function(calculation_params_for_esdl_id, simulator_time, time_step_information, esdl_id, self.energy_system)
                        ...
                        calculation_params[esdl_id] = CalculationServiceHelperFunctions.clear_dictionary_values(calculation_params_for_esdl_id)
                    except Exception:
