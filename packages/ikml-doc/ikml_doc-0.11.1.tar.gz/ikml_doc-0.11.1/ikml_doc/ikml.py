...
class IKML_Document:
    ...
    def to_txt(self, max_depth=-1):
        # returns IKML text
        return self.root.to_txt(max_depth=max_depth)
    ...
    def find_children(self, tag_name, expand_inline=False, max_depth=-1, fmt="node", apply=None):
        if max_depth <= -1:
            max_depth = 9999
        for node in self.iter():
            try:
    ...
    def find_children_regex(self, tagid_pattern=None, tag_names=set(), max_depth=0, fmt="node", apply=None, no_attributes=True):
        base_depth = None
        ...
        for node in LevelOrderIter(self.root):
            node_id = node.get("id", "")
            if (tagid_pattern is None or re.match(rf"{tagid_pattern}", node_id)) and not (no_attributes and node.is_attribute):
