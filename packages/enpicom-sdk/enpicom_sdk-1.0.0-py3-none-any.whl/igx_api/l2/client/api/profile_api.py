...
from igx_api.l1.openapi_client.models import MatchAFileByItsID, ProfileWorkSequenceTemplatesInner, SequenceTemplateSelector
...
from igx_api.l2.types.collection import CollectionMetadata
...
class ProfileApi:
    ...
    def start(
        ...
        quality_control_template_id: QualityControlTemplateId,
        archive_inputs: bool = False,
        quality_control_template_version: int | None = None,
        clone_id_extraction: CloneIdExtraction | None = None,
        ...
        **<u>Templates can be created and configured only via the [web interface of IGX-Platform](https://igx.bio/)</u>**, it can not
        ...
        Args:
            ...
            quality_control_template_id (igx_api.l2.types.profile.QualityControlTemplateId): The ID of the quality control template to use.
            quality_control_template_version (igx_api.l2.types.profile.QualityControlTemplateId | None): The version of the quality control template to use.
              None defaults to the latest.
            clone_id_extraction (igx_api.l2.types.profile.CloneIdExtraction | None): The configuration for extracting clone IDs. This is only
              ...
              For more information, see the `igx_api.l2.types.profile.CloneIdExtraction` class.
        ...
        Example:
            ...
            quality_control_template_id = QualityControlTemplateId(quality_control_templates[0].id)
            ...
            clone_collection = client.profile_api.start(
                ...
                sequence_templates=[
                    SequenceTemplateConfig(
                        ...
                        selector=FileIdSelector(value=f),
                ...
                archive_inputs=True,
                quality_control_template_id=quality_control_template_id,
            ).wait()
            ```
        """
        ...
        try:
            inner_sequence_templates: list[ProfileWorkSequenceTemplatesInner] = []
            for st in sequence_templates:
                match st.selector.type:
                    case "file_id":
                        inner_sequence_templates.append(
                            ProfileWorkSequenceTemplatesInner(
                                template_id=st.id,
                                template_version=st.version,
                                selector=SequenceTemplateSelector(MatchAFileByItsID(type=st.selector.type, value=st.selector.value)),
                            )
                        )
                    case _:
            ...
            profile_work = openapi_client.ProfileWork(
                ...
                reference_database_id=str(reference_database_revision.reference_database_id),
                reference_database_version=int(reference_database_revision.reference_database_version),
                archive_inputs=archive_inputs,
                quality_control_template_id=quality_control_template_id,
                quality_control_template_version=quality_control_template_version,
        ...
        job_id = JobId(profile_job.job_id)
        ...
        def wait() -> CollectionMetadata:
            ...
            return collection_api.get_collection_metadata_by_id(collection_id)
