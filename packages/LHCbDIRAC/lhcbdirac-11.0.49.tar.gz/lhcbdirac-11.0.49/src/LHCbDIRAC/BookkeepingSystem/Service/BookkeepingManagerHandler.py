...
class BookkeepingManagerHandlerMixin:
    ...
    def _getFilesWithMetadata(cls, in_dict):
        ...
        if columns := in_dict.get("OnlyParameters", None):
            # Only the new implementation supports this
            method = partial(cls.bkkDB._newdb.getFilesWithMetadata, columns=columns)
        ...
        if columns:
            mapping = {k.lower(): k for k in parameters}
            parameters = [mapping.get(c.lower(), c) for c in columns]
        for record in retVal["Value"]:
            records += [list(record)]
        return S_OK({"ParameterNames": parameters, "Records": records, "TotalRecords": len(records)})
    ...
    #############################################################################
    types_getInputOuputFilesForProd = [int, int]

    @classmethod
    def export_getInputOuputFilesForProd(cls, prod_id, run_number):
        ...
        """
        return cls.bkkDB.getInputOuputFilesForProd(prod_id, run_number)
