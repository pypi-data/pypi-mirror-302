...
# %% auto 0
__all__ = ['analysis_utility', 'get_names_from_fasta', 'analysis_manager', 'unit_test_single', 'unit_test_single_2',
...
class analysis_utility(sample_manager.sample_data):
    ...
    def __init__(self, attributes, input_folder, output_folder):
    ...
    def sample_setup(self, analyses_to_run):
        self.logger.info(
        ...
        self.logger.info(f"Analyses to run: {', '.join(analyses_to_run)}")
...
class analysis_manager(sample_manager.input_manager):
    ...
    def __init__(self, input_config):
        try:
            self.base_output_folder: Path = os.path.abspath(
    ...
    def add_sample(self, attributes):
        ...
        self.samples.append(
            analysis_utility(attributes, self.base_input_folder, output_folder)
        )
        return
...
def unit_test_from_folder():
    ...
    input_config["output_folder"] = "output/"
    input_config["analysis_config"] = config["analysis_settings"]["Spyogenes"]

    test = analysis_manager(input_config)
    for x in test:
        print(x.__dict__)
