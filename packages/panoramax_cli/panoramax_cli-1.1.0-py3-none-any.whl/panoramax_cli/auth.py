...
from panoramax_cli.utils import test_panoramax_url, createClientWithRetry
import os
...
def _generate_and_update_credentials(
    client: httpx.Client, creds: Credentials, panoramax: Panoramax
) -> InstanceCredential:
...
def _generate_new_instance_credentials(
    client: httpx.Client, panoramax: Panoramax
) -> InstanceCredential:
...
def _check_if_associated(
    client: httpx.Client, panoramax: Panoramax, creds: InstanceCredential
) -> Optional[str]:
