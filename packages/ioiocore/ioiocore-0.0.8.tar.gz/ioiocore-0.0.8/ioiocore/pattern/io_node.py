...
class IONode(INode, ONode):
    ...
    def _init(self, data, metadata):
        metadataupdated = self.init(data, metadata)
        if self.OutputPorts is not None and len(self.OutputPorts) > 0:
            if metadataupdated is None:
                ONode._set_metadata(self, metadata)
                self.Metadata = ONode._get_metadata(self)
            else:
                ONode._set_metadata(self, metadataupdated)
                self.Metadata = ONode._get_metadata(self)
        elif self.OutputPorts is None or len(self.OutputPorts) == 0:
