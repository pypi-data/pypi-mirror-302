...
Here is some additional information about the author's desired goal for the book:\n\n {goal}"""),
        expected_output="An outline of chapters, with titles and descriptions of what each chapter will contain. Maximum of 3 chapters. MUST 必须是JSON格式",
...
@flow
async def flow_article_gen(req: ArticleGenStateRequest):
    logger = get_run_logger()
    run_context = get_run_context()
    flow_client = get_client()
    # user = await pause_flow_run(wait_for_input=User22)

    if not req.user_id:
        raise ValueError("user_id is required")
    logger.info("开始编写大纲, user_id: %s", req.user_id)
    ...
    filename = f"./{book_title.replace(' ', '_').replace("'", '_').replace('"', '_')}.md"
