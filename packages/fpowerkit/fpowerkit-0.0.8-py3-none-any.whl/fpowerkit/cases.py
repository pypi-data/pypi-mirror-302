...
from .grid import Grid, Bus, Line, Generator
...
class PDNCases:
    ...
    def IEEE33(
        Ub_kV:float=10.0,
        Sb_MVA:float=0.25,
        default_costA_dollar_per_kWh2:float=0.0001,
        default_costB_dollar_per_kWh:float=0.3,
        default_costC_dollar:float=10,
        default_gen_pmin_MW:float=0,
        default_gen_pmax_MW:float=30,
        default_gen_qmin_Mvar:float=-30,
        default_gen_qmax_Mvar:float=30,
        grid_repeat:int = 1,
        ...
        return PDNCases.__ieee(
            33, Ub_kV, Sb_MVA, default_costA_dollar_per_kWh2, default_costB_dollar_per_kWh, default_costC_dollar,
            default_gen_pmin_MW, default_gen_pmax_MW, default_gen_qmin_Mvar, default_gen_qmax_Mvar,
            grid_repeat, changeable_load, load_fluctuation, load_repeat
        )
    ...
    def IEEE69(
        ...
        return PDNCases.__ieee(
            69, Ub_kV, Sb_MVA, default_costA_dollar_per_kWh2, default_costB_dollar_per_kWh, default_costC_dollar,
            default_gen_pmin_MW, default_gen_pmax_MW, default_gen_qmin_Mvar, default_gen_qmax_Mvar,
            grid_repeat, changeable_load, load_fluctuation, load_repeat
        )
    ...
    def __ieee(
        ...
        Zb = Ub**2/Sb
        G_pars = [
            default_gen_pmin_MW/Sb, default_gen_pmax_MW/Sb,
            default_gen_qmin_Mvar/Sb, default_gen_qmax_Mvar/Sb, 
            default_costA_dollar_per_kWh2 * Sb_KVA * Sb_KVA,
            default_costB_dollar_per_kWh * Sb_KVA, 
            default_costC_dollar
        ]
        if case == 33:
            LOAD = _IEEE33_LOAD
            LINE = _IEEE33_LINE
            ...
            raise ValueError("case should be 33 or 69.")
        if grid_repeat == 1:
            L = [Line("L"+str(p[0]), "B"+str(p[1]), "B"+str(p[2]), p[3]/Zb, p[4]/Zb) for p in LINE]
            G = [Generator("G"+p, "B"+p, *G_pars) for p in map(str,_DEFAULT_GEN_POS)]
        else:
            L = []
            G = []
            for i in range(grid_repeat):
                ...
                G.extend([Generator(
                    ...
                    default_gen_pmin_MW/Sb, default_gen_pmax_MW/Sb,
                    default_gen_qmin_Mvar/Sb, default_gen_qmax_Mvar/Sb, 
                    default_costA_dollar_per_kWh2,
                    default_costB_dollar_per_kWh, 
                    default_costC_dollar
                ) for p in _DEFAULT_GEN_POS])
