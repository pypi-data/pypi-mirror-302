...
def skim_and_merge(options: Options, *extra_args):
    ...
    parser.add_argument(
        "--lumi-tuple",
        default="GetIntegratedLuminosity",
        help="Name of TDirectory containing Luminosity information. Set to 'none' to ignore.",
    )
    parser.add_argument(
        "--allow-missing",
        action="store_true",
    ...
    for input_file in input_files:
        ...
        for key in rf.GetListOfKeys():
            ...
            if key_name != extra_opts.lumi_tuple:
                ...
                lumi_tree_found = True
            all_trees.add(key_name)

    if not lumi_tree_found and extra_opts.lumi_tuple != "none":
        log_error(
    ...
    for stream, tree_names in tree_keys.items():
        ...
        merger.AddObjectNames(extra_opts.lumi_tuple)
