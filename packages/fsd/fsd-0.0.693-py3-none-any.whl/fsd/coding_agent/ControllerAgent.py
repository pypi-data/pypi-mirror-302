...
class ControllerAgent:
    ...
    async def log_result(self, main_path: str, result: str):
        ...
        """
        full_log_path = os.path.join(await self.repo.get_repo_path(), "log1.txt")
        ...
        try:
            ...
            await logger.info(f" #### Successfully appended log to {full_log_path}")
        except Exception as e:
            await logger.error(f" #### Error writing to log file {full_log_path}: {str(e)}")
    ...
    async def regular_code_task_pipeline(self, tier, finalPrompt, role, original_prompt_language):
        ...
        while True:
            if not isFirst:
                logger.info(" #### You are in code mode. Type your feedback to continue building, or click 'Exit' to leave code mode. Click version control on bottom right to switch to any version you want, Zinley will work from there.")
                logger.info(
    ...
    async def codingProgress(self, tier, finalPrompt, role, original_prompt_language):
        ...
        final_working_files = set(existing_files + new_adding_files)
