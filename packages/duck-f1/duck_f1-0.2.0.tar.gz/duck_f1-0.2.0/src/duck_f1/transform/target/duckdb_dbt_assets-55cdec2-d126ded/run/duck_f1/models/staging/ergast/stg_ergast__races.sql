...
formatted as (
    select
        ...
        md5(cast(coalesce(cast(race.year as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(race.round as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(session_type as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT))
            as session_id,
        circuit.circuit_id,
