...
def plot_distr1d_comparison(
    ...
    datahist, edges = onp.histogram(data, bins=bins, range=range, weights = data_weights)
    ...
    if isinstance(fit, list) : 
      for i, (f, w) in enumerate(zip(fit, weights)) : 
        fithist1, _ = onp.histogram(f, bins=bins, range=range, weights=w)
      ...
      fithist1, _ = onp.histogram(fit, bins=bins, range=range, weights=weights)
      fitscale = onp.sum(datahist) / onp.sum(fithist1)
      fithist = fithist1 * fitscale
      fitarr = onp.array([fithist, fithist]).T.flatten()
