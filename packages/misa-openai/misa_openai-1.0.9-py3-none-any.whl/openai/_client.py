...
class OpenAI(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        if log_config:
            try:
                database_url = log_config.get("database_url", None)
                database_name = log_config.get("database_name", None)
                collection_name = log_config.get("collection_name", None)
                database_username = log_config.get("username", None)
                database_password = log_config.get("password", None)
                mongo_client = MongoClient(database_url, # type: ignore
                                            username=database_username,
                                            password=database_password)
                db = mongo_client[database_name] # type: ignore
                self.log_collection = db[collection_name] # type: ignore
                self.enable_logging = True
                print(f"Connected to MongoDB: {database_url}", flush=True)
    ...
    def log_chat_completion(self, input_data, output_data): # type: ignore
        try:
            input_data_dict = {}
            for key, value in input_data.items(): # type: ignore
                if value == NOT_GIVEN:
                    input_data_dict[key] = None
                else:
                    input_data_dict[key] = value
            log_data = { # type: ignore
                'timestamp': datetime.now().astimezone(timezone(timedelta(hours = 7))).isoformat(), 
                'input': input_data_dict,
                'output': output_data
            }
            print(f"Logging chat completion: {log_data}", flush=True)
            self.log_collection.insert_one(log_data) # type: ignore
        except Exception as e:
            print(f"Error in log_chat_completion: {e}", flush=True)
    ...
    def copy(
        ...
        *,
        ...
        return self.__class__(
            ...
            log_config=log_config or self.log_config,
...
class AsyncOpenAI(AsyncAPIClient):
        ...
        *,
        ...
        if log_config:
            try:
                ...
                database_username = log_config.get("username", None)
                database_password = log_config.get("password", None)
                mongo_client = AsyncIOMotorClient(database_url, # type: ignore
                                            username=database_username,
                                            password=database_password)
                db = mongo_client[database_name] # type: ignore
                self.log_collection = db[collection_name] # type: ignore
                self.enable_logging = True
                print(f"Connected to MongoDB: {database_url}, {database_name}, {collection_name}", flush=True)
            except Exception as e:
                print(f"Failed to connect to MongoDB: {e}", flush=True)
                self.log_collection = None
                self.enable_logging = False
    ...
    async def log_chat_completion_async(self, input_data, output_data): # type: ignore
        try:
            input_data_dict = {}
            for key, value in input_data.items(): # type: ignore
                if value == NOT_GIVEN:
            ...
            print(f"Logging chat completion", flush=True)
            await self.log_collection.insert_one(log_data) # type: ignore
        except Exception as e:
            print(f"Error in log_chat_completion_async: {e}", flush=True)
