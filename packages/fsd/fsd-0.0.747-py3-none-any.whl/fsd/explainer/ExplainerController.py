...
class ExplainerController:
    ...
    def _parse_user_permission(self, user_permission):
        ...
        return user_prompt, file_attachments, focused_files, tier
            ...
            if not is_first:
                ...
                if last_log:
                    ...
                    if action_status:
                        ...
                        user_prompt, file_attachments, focused_files, tier = self._parse_user_permission(user_permission)

                        if user_permission == "h":
                            await self.coder.explainer_code_task_pipeline(tier, last_log, "Top notch software engineer", "English")
                            self.conversation_history.append({"role": "user", "content": "Apply solution"})
