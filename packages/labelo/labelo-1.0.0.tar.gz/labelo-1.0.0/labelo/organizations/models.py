...
class OrganizationMember(OrganizationMemberMixin, models.Model):
    ...
    #     _('active'),
    #     default=False,
    #     help_text=_('Designates whether to treat this OrganizationMember as active'),
    # )
    ...
    def soft_delete(self):
        with transaction.atomic():
            ...
            # Attempt to find the first active organization
            try:
                org = self.user.organizations.filter(
                    organizationmember__deleted_at__isnull=True
                ).first()
            except OrganizationMember.DoesNotExist:
                org = None
            
            self.user.active_organization = org
            
            # if org:
            #     try:
            #         org_member = OrganizationMember.objects.get(user=self.user, organization=org)
            #         group = Group.objects.get(name=org_member.role)
            #         self.user.groups.set([group])
                    
            #         if org_member.role == 'owner' and self.user.is_owner:
            ...
            #         pass
            # else:
            #     # If no organization found, clear the groups
            #     self.user.groups.clear()
            
            self.user.save(update_fields=['active_organization'])
        
        # Delete all task locks associated with the user
