...
def send_email(config, text, debug=False):
    import smtplib
    print(config)
    ...
    with smtplib.SMTP(config['SMTP']) as connection:
        ...
        connection.sendmail(
            from_addr=MY_EMAIL,
            to_addrs=MY_EMAIL,
            msg=f"Subject:FolioButler Check Orders\n\n{text}"
        )
...
def connected_ib(config, api_ip, api_port, clientId):
    ib = IB()
    try:
        # print(api_ip, api_port, clientId)
...
def starter(env, action, ip, port, quiet, debug, email):
    ...
    if email is not None:
        send_email(config, emailtxt, True)
...
def upload_positions(env, ip, port, clientId, debug):
    ...
    for folioname in list(folios):
        ...
        if f['ib_sync']:
            # if debug:
            #    for key in f['positions']:
            #        v = f['positions'][key]
            #        print("POS: {:3d} * {:4s} @ {} ({})".format(
            #            v['amount'],
            #            key.split('_')[0],
            #            v['price'],
            #            v['name']))
...
if __name__ == "__main__":
    ...
    # starter(g_env, "upload", None, 7497, True, True, None)
    import sys
    sys.exit(0)
