...
class Transetup:
    ...
    def check_and_update_transetup(self, sql_model, connection):
        logger.log(f"sql_model:: {sql_model}")
        ...
        if missing_keys:
            ...
            key_flag = sql_model.get('key_flag', '')
    ...
    def get_table_name(self, join_list):
        ...
        for single_json in join_list:
            if single_json['main_table'] == True:
                logger.log(f"In ::: {single_json['table']}")
    ...
    def process_data(self, conn, sql_models_data, obj_name):
        ...
        logger.log(f"sql_models ::: {self.sql_models}")
        for sql_model in self.sql_models:
            if "sql_model" in sql_model and "joins" in sql_model['sql_model'] and "join_predicates" in sql_model['sql_model']['joins'] and "joins" in sql_model['sql_model']['joins']['join_predicates']:
                ...
                logger.log(f"joins_lst ::: {joins_lst}")
                table_name = self.get_table_name(joins_lst)
                logger.log(f"table_name ::: {table_name}")

            if "form_no" in sql_model:
                if sql_model['form_no'] == '1' or sql_model['form_no'] == 1:
                    trans_id_col_list = []
                    logger.log(f"sql_model ::: {sql_model}")
                    for column in sql_model['sql_model']['columns']:
                        logger.log(f"column['column']['db_name'] ::: {column['column']['db_name'].upper()}")
                        trans_id_col_list.append(column['column']['db_name'].upper())
                    
                    logger.log(f"obj_name ::: w_{obj_name}")
                    logger.log(f"table_name ::: {(sql_model['sql_model']['columns'][0]['column']['table_name']).upper()}")
                    logger.log(f"tran_id_col ::: {','.join(trans_id_col_list)}")
                    ...
                    fixed_model_data = {
                        ...
                        "table_name": f"{(sql_model['sql_model']['columns'][0]['column']['table_name']).upper()}", "application": "", "table_desc": "", "tran_date_col": "", "tran_id__from": "", "tran_id__to": "", 
