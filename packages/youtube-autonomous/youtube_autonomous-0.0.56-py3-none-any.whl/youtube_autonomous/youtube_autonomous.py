...
from youtube_autonomous.enums import StringDuration, SegmentType, EnhancementType
    ...
    def validate_data(self, json_data: dict):
        ...
        for index, segment in enumerate(segments):
            ...
            if type == SegmentType.PREMADE.value:
                # This will raise an Exception if no mandatory parameters are given
                ...
                TextElementBuilder.extract_extra_params(segment, TextElementBuilder.text_premade_name_to_class(keywords).generate, ['self', 'cls', 'args', 'kwargs', 'output_filename', 'duration'])
            ...
            for index, enhancement in enumerate(segment.get(SegmentField.ENHANCEMENTS.value, [])):
                ...
                if enhancement_type == EnhancementType.PREMADE.value:
                    ...
                    EffectElementBuilder.extract_extra_params(enhancement, EffectElementBuilder.effect_name_to_class(keywords).apply, params_to_ignore = ['self', 'cls', 'args', 'kwargs', 'clip'])
                elif enhancement_type == EnhancementType.TEXT.value:
                    TextElementBuilder.extract_extra_params(enhancement, TextElementBuilder.text_premade_name_to_class(keywords).generate, ['self', 'cls', 'args', 'kwargs', 'output_filename', 'duration'])
                # TODO: Validate for another types
