...
class WSNode:
    ...
    def run_task(self, workflow: Workflow, task: Task) -> None:
        wr = workflow.workflowRunner()
        task.output = None
        steps = [False] * len(workflow.step_description)

        def follow_workflow():
            nonlocal workflow, self
            for x, step in enumerate(wr.ongoing_steps()):
                if steps[x]:
                    continue
        ...
        try:
            ...
            thread.start()
            time.sleep(0.5)
            wr.kickoff(task.inputs)
            task.output = wr.output()
