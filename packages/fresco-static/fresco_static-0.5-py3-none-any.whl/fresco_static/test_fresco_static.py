...
def test_it_resolves_source_prefix():
    ...
    with create_tree({'x/foo': '', 'y/foo': ''}) as d:
        ...
        assert sf.resolve_path('x/foo') == ('x', '{}/x/foo'.format(d), {})
        assert sf.resolve_path('y/foo') == ('y', '{}/y/foo'.format(d), {})
...
def test_it_resolves_without_source_prefix():
    ...
    with create_tree({'x/foo': '', 'y/foo/bar': ''}) as d:
        sf.add_directory('x', '{}/x'.format(d))
        sf.add_directory('y', '{}/y'.format(d))
        assert sf.resolve_path('foo') == ('x', '{}/x/foo'.format(d), {})
        assert sf.resolve_path('foo/bar') == ('y', '{}/y/foo/bar'.format(d), {})
...
def test_it_sets_per_source_cache_age():
    ...
    with create_tree({'x/foo': '', 'y/foo': '', 'z/foo': ''}) as d:
        ...
        assert sf.resolve_path('x/foo') == ('x', '{}/x/foo'.format(d), {"Cache-Control": "max-age: 10"})
        assert sf.resolve_path('y/foo') == ('y', '{}/y/foo'.format(d), {"Cache-Control": "max-age: 20"})
        assert sf.resolve_path('z/foo') == ('z', '{}/z/foo'.format(d), {"Cache-Control": "max-age: 30"})
