...
from al360_trustworthyai_vision.common.constants import (MLFlowSchemaLiterals,
                                                   ModelTask)
from al360_trustworthyai_vision.utils.image_reader import (
    get_base64_string_from_path, is_automl_image_model)
from al360_trustworthyai_vision.utils.image_utils import get_images
...
class ErrorAnalysisManager(BaseErrorAnalysisManager):
    ...
    def _load(path, al360_tai_insights):
        ...
        task_type = al360_tai_insights.task_type
        classes = al360_tai_insights._classes
        device = al360_tai_insights.device

        test = al360_tai_insights.test
        image_mode = al360_tai_insights.image_mode
        transformations = al360_tai_insights._transformations
        sample = test.iloc[0:2]
        sample = get_images(sample, image_mode, transformations)
        wrapped_model = wrap_model(
            al360_tai_insights.model, sample, task_type, classes=classes,
            device=device)
        ...
        index_predictor = ErrorAnalysisManager._create_index_predictor(
            ...
            task_type, al360_tai_insights.image_mode,
            al360_tai_insights._transformations,
            al360_tai_insights._classes)
