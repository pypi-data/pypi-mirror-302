...
class FileContentManager:
    ...
    def replace_content(content: str, search: str, replace: str) -> Optional[str]:
        ...
        Attempts exact match first, then normalizes whitespace, and finally uses fuzzy matching.
        """
        # Attempt exact match
        ...
        if search_normalized in content_normalized:
            # Find the start and end indices in the original content
            start_idx = content_normalized.find(search_normalized)
            end_idx = start_idx + len(search_normalized)

            # Map back to the original content
            # This is complex due to differing whitespace, so instead we'll attempt to replace approximately
        ...
        # Use fuzzy matching
        new_content = cls.fuzzy_replace(content, search, replace)
        return new_content
    ...
    def fuzzy_replace(content: str, search: str, replace: str, threshold: float = 0.6) -> Optional[str]:
        ...
        sequence_matcher = difflib.SequenceMatcher(None, content, search)
        match = sequence_matcher.find_longest_match(0, len(content), 0, len(search))

        if match.size == 0:
            return None

        similarity = match.size / len(search)
    ...
    def find_similar_lines(search: str, content: str, num_lines: int = 5) -> str:
        ...
        matches = difflib.get_close_matches(' '.join(search_lines), [' '.join(content_lines)], n=1, cutoff=0.5)
        if matches:
            return matches[0]
        else:
            return ""
