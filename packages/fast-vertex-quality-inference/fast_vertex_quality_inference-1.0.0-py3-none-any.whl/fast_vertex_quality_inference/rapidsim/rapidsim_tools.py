...
RAPIDSIM_particles = os.environ.get('RAPIDSIM_ROOT')+'/config/particles.dat'
RAPIDSIM_particles_df = pd.read_csv(RAPIDSIM_particles, sep=r"\s+")
...
def get_rapidsim_indicies(decay_string):
		...
		for idx, character in enumerate(daughter_string):
			...
			if labels[idx] == level:
				...
				if character == '{':
					...
					mother_in_level = daughter_string[start_idx+1:end_idx].split('->')[0]
					labels[start_idx+1+len(mother_in_level):end_idx] += 1 # dont label the brackets # also bring mother down a level
	...
	for idx in range(len(daughter_split)):
		if daughter_split[idx] not in ['{','}','->','']:
			ordering = np.append(ordering, [[int(label_split[idx][0]), int(-1)]], axis=0)
	...
	return decay_string, ordering_string, stability_string, daughter_string, label_string
...
def compute_combined_param(daughter_info, param):
	...
	if param in ['PX','PY','PZ']:
		...
		for daughter in daughters:
			PE += np.sqrt(daughter_info[daughter]['M']**2 
...
def organise_output_tuple(fileName, dropMissing, naming_scheme, mass_hypotheses, combined_particles, true_PID_scheme):
	...
	with uproot.open(fileName) as f:
		...
		if dropMissing:
			...
			for particle in naming_scheme.values():
				branches_to_keep.extend([branch for branch in tree.keys() if branch[:len(particle)] == particle])
		else:
			branches_to_keep = tree.keys()
		branches = {branch: tree[branch].array(library="np") for branch in branches_to_keep}
		
		# Reset mass hypotheses 
		if mass_hypotheses:
			for mass_hypothesis in mass_hypotheses:
				particle = mass_hypotheses[mass_hypothesis]
				filtered_df = RAPIDSIM_particles_df[(RAPIDSIM_particles_df["part"] == particle) | (RAPIDSIM_particles_df["anti"] == particle)]
				new_mass = float(filtered_df.mass)
				branches[f'{mass_hypothesis}_M'] = np.ones(np.shape(branches[f'{mass_hypothesis}_M']))*new_mass
		...
		if combined_particles:
			for combined_particle in combined_particles:
				...
				for param in ['M','PX','PY','PZ']:
					branches[f'{combined_particle}_{param}'] = compute_combined_param(daughter_info, param)
				branches[f'{combined_particle}_P'] = np.sqrt(branches[f'{combined_particle}_PX']**2 + branches[f'{combined_particle}_PY']**2 + branches[f'{combined_particle}_PZ']**2)
				branches[f'{combined_particle}_PT'] = np.sqrt(branches[f'{combined_particle}_PX']**2 + branches[f'{combined_particle}_PY']**2)

		# True PID
		for particle in true_PID_scheme:
			if f'{particle}_PX' in list(branches.keys()):
				branches[f'{particle}_ID_TRUE'] = np.ones(np.shape(branches[f'{particle}_PX']))*true_PID_scheme[particle]
				# maybe only want to write this out for final state particles?
				branches[f'{particle}_ID'] = np.ones(np.shape(branches[f'{particle}_PX']))*true_PID_scheme[particle]

		# PID from mass hypotheses
		if mass_hypotheses:
			for particle in mass_hypotheses:
				if RAPIDSIM_particles_df[(RAPIDSIM_particles_df["part"] == mass_hypotheses[particle])].shape[0] == 1:
					pdg = int(RAPIDSIM_particles_df[(RAPIDSIM_particles_df["part"] == mass_hypotheses[particle])].ID)
				elif RAPIDSIM_particles_df[(RAPIDSIM_particles_df["anti"] == mass_hypotheses[particle])].shape[0] == 1:
					pdg = int(RAPIDSIM_particles_df[(RAPIDSIM_particles_df["anti"] == mass_hypotheses[particle])].ID*-1)
				else:
					display.error_splash(post_message=f"{particle} not found in {RAPIDSIM_ROOT}/config/particles.dat, or listed multiple times.")
				branches[f'{particle}_ID'] = np.ones(np.shape(branches[f'{particle}_PX']))*pdg
...
def write_params(checked_lines, key, var_list, f):
	param_line = f'{key} :'
	for var in var_list:
		param_line += f' {var},'
...
def check_config(config, USE_EVTGEN, naming_scheme, model_scheme=None, geometry='LHCb', acceptance='AllIn', useEvtGen='TRUE', evtGenUsePHOTOS='TRUE', dropMissing=True):

	blocked_settings = ['paramsTwoBody'] # temporarily blocking this function in rapidsim
	...
	for idx, line in enumerate(f_lines):
		if ":" in line:
			...
			setting = line.split(':')[-1].replace(' ','').replace('\n','').replace(',',', ')
	...
	for key in naming_scheme:
		if naming_scheme[key] != 'NA':
			...
			settings[int(key)]['invisible'] = 'true' # dont include these in mother mass variables


	# EVTGEN models
	for key in model_scheme:
		if not USE_EVTGEN and model_scheme[key] != "PHSP":
			print(f"Cannot use {model_scheme[key]}, $EVTGEN_ROOT not set. Using PHSP instead.")
	...
	if not USE_EVTGEN and useEvtGen == 'TRUE':
		print(f"Cannot set useEvtGen : TRUE, $EVTGEN_ROOT not set.")
	else:
	...
	if evtGenUsePHOTOS != 'TRUE':
		...
		print(f"Cannot set evtGenUsePHOTOS : TRUE, $EVTGEN_ROOT not set.")
	else:
	...
	if 'paramsStable' in settings[-1].keys():
		settings[-1]['paramsStable'] = settings[-1]['paramsStable'].replace(' ','').split(",")
	...
	if 'paramsDecaying' in settings[-1].keys():
		settings[-1]['paramsDecaying'] = settings[-1]['paramsDecaying'].replace(' ','').split(",")
