...
Do What? [rms,rma,edit/e,<ENTER>/next,prev]"""
    ...
    def fastShelf(self,count_field="ShelfCount",set_field="Shelf",mode=""):
        ...
        with Session(ENGINE) as session:
            while True:
                search=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"[SF={set_field},CF={count_field}] Barcode|Code|Name: ",helpText="what are you counting; [SF=set_field,CF=count_field]",data="string")
                if search in [None,]:
                ...
                if ct < 1:
                    ...
                    if isinstance(replacement,int):
                        ...
                        data=self.mkNew(code=code,extra=[count_field,])
                        #print(data)
                        if not self.next_barcode():
                            continue
                        if data in [None,]:
                ...
                if select in [None,]:
                    ...
                    while True:
                        try:
                            selector=Prompt.__init2__(None,func=FormBuilderMkText,ptext="which index?",helpText="number farthest from to the left of screen followed by /",data="integer")
                            if selector in [None,]:
                                if not self.next_barcode():
                                    continue
                                else:
