...
class NeuralTrust:
    ...
    def __init__(
        ...
        full_url = urljoin(base_url, sdk_version)
        
        logger.info(f"Initializing NeuralTrust client with base URL: {full_url}")
        
        self.base_client = NeuralTrustApi(
            api_key=api_key, base_url=full_url, timeout=timeout
        )
        
        # Set allow_redirects to True
        self.base_client._client_wrapper._client.allow_redirects = True
    ...
    def _trace(
        ...
        *,
        ...
        try:
            ...
            logger.error(f"Error sending trace: {str(e)}")
            raise
