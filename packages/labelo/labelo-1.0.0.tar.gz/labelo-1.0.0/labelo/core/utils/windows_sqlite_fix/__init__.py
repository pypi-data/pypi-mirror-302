...
from labelo.core.settings.base import VERIFY_SSL_CERTS, WINDOWS_SQLITE_BINARY_HOST_PREFIX
...
def start_fix():
    ...
    print('finished')
    print(colorama.Fore.LIGHTGREEN_EX + '\nPlease restart Labelo to load the updated sqlite.dll\n')
    print(colorama.Fore.WHITE)
    exit()
...
def windows_dll_fix():
    ...
    if sys.version_info.major == 3 and sys.version_info.minor in [6, 7, 8]:
        ...
        if not auto_agree:
            print(
                colorama.Fore.WHITE + 'Labelo can try to resolve this issue by downloading the correct '
