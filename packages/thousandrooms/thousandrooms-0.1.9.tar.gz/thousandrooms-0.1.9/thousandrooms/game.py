...
class Game:
    ...
    def initialize(self):
        ...
        self.effectResolvers = {
            ...
            "flames": self.flamesResolve,
            "icing": self.icingResolve,
            "acidity": self.acidityResolve,
            "conduction": self.conductionResolve,
            "nightshade": self.nightshadeResolve,
            "strength": self.strengthResolve,
            "curing": self.curingResolve
        }
...
### COMBAT METHODS ###
    ...
    def playerAttack(self):
        ...
        if atkRoll == 20 or atkRoll >= monsterDefense:
            ...
            if self.player.hasCondition("nightshade"):
                self.monster.addCondition("weakness", 3)
                self.player.removeCondition("nightshade")
                self.addResolution(f"{Fore.DARK_ORANGE_3B}The {self.monster.name} looks weaker!")
        else:
        ...
        while self.monster.charges > 0:
            ...
            if specAtkRoll >= playerDefense:
                ...
                if self.monster.special == "drain":
                    ...
                    self.player.addCondition("weakness", 3)
                    self.addResolution(f"{Fore.PALE_TURQUOISE_1}It drains your life essence!")
                    self.addResolution(f"{Fore.PALE_TURQUOISE_1}You feel weaker!")
                    ...
                    if "fire" in self.player.resist:
                        ...
                        burnables = [item for item in self.player.items if item.name == "Scroll" or item.type in ["cloth", "leather"] or item.kind == "bow" or item.name == "Oil"]
                        if len(burnables) > 0:
                    ...
                    if "cold" in self.player.resist:
                        ...
                        freezables = [item for item in self.player.items if item.name == "Potion" or item.name == "Oil"]
                        if len(freezables) > 0:
...
### ITEM METHODS ###
    ...
    def flamesResolve(self):
        self.player.setAttackType("fire")
        self.player.addCondition("elementalAttack", 100)
        self.addResolution(f"{Fore.SPRING_GREEN_3A}Your weapon is wreathed in flames!")
        return True

    def icingResolve(self):
        self.player.setAttackType("cold")
        self.player.addCondition("elementalAttack", 100)
        self.addResolution(f"{Fore.SPRING_GREEN_3A}Your weapon is coated in ice!")
        return True

    def acidityResolve(self):
        self.player.setAttackType("acid")
        self.player.addCondition("elementalAttack", 100)
        self.addResolution(f"{Fore.SPRING_GREEN_3A}Your weapon begins dripping acid!")
        return True

    def conductionResolve(self):
        self.player.setAttackType("electric")
        self.player.addCondition("elementalAttack", 100)
        self.addResolution(f"{Fore.SPRING_GREEN_3A}Your weapon crackles with electricity!")
        return True

    def nightshadeResolve(self):
        self.player.addCondition("nightshade", 5)
        return True

    def strengthResolve(self):
        self.player.addCondition("strength", 4)
        return True

    def curingResolve(self):
        self.player.removeCondition("weakness")
        return True
    ...
    def diggingResolve(self):
        ...
        if len(doors) == 0:
            self.itemListOptions["message"] = f"{Fore.DARK_ORANGE_3B}There's nowhere to dig!"
            return False
...
### LIFECYCLE METHODS ###
    ...
    def nextTurn(self):
        if not self.playerQuit:
            ...
            self.player.processTick()
            if self.monster:
                self.monster.processTick()
            self.takeInput()
