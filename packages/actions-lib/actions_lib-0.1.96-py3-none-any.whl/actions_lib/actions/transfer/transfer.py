...
def record_transaction(redis_client, user_id: str, chain: str, tx_hash: str) -> None:
    key = f"user:tx:{user_id}:{chain}"
    redis_client.rpush(key, tx_hash)
    print(f"Recorded transaction {tx_hash} on {chain} for user {user_id}")

def mpc_transfer(default_address: WalletAddress, destination, chain_raw, token_raw, token_address, amount):
    tx_hash = None
    try:
        if chain_raw == 'base' and token_raw == 'usdc':
            ...
            while str(transfer.status) != 'complete' or str(transfer.status) != 'failed':
                transfer.reload()
