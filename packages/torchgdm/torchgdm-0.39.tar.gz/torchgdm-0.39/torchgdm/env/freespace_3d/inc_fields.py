...
class NullField(IlluminationfieldBase):
    ...
    def get_efield(self, r_probe: torch.Tensor, environment):
    ...
    def get_hfield(self, r_probe: torch.Tensor, environment):
        """evaluate illumination field at position(s) `r_probe`"""
        if len(r_probe.shape) == 1:
            r_probe = r_probe.unsqueeze(0)
        assert r_probe.shape[1] == 3
...
class GaussianParaxial(IlluminationfieldBase):
    ...
    def __init__(
        ...
        Args:
            ...
            NA (float, optional): numerical aperture used for focal spotsize calculation. If None, use explicit `spotsize`. Defaults to None.
            spotsize (float, optional): explicit, constant focus spotsize (in nm). Defaults to None.
        ...
        self.NA = NA
        if self.spotsize is None and self.NA is None:
            raise ValueError(
                "Gaussian Beam Error: Either spotsize or NA must be given."
            )
        if self.spotsize is not None and self.NA is not None:
            warnings.warn("Both, spotsize and NA given. Ignoring `NA`.")
            self.NA = None
        if self.NA is not None:
            self.NA = torch.as_tensor(self.NA, device=device)
        if self.spotsize is not None:
            self.spotsize = torch.as_tensor(self.spotsize, device=device)
    ...
    def __repr__(self, verbose=False):
        ...
        if self.spotsize is None:
            out_str += "\n - numerical aperture: {}".format(
                torch.round(self.NA, decimals=4)
            )
        else:
            out_str += "\n - spotsize (Gaussian width): {}nm".format(
                torch.round(self.spotsize, decimals=1)
            )
        out_str += "\n - incident angle (rad): {}".format(self.inc_angle)
    ...
    def _eval_field(
        ...
        if self.spotsize is None:
            ...
            w0 = torch.as_tensor(self.spotsize, dtype=DTYPE_FLOAT, device=self.device)
...
if __name__ == "__main__":
    ...
    r_probe_xz = tools.geometry.coordinate_map_square_2d(2000, 51, 5, projection="xz")
    r_probe_yz = tools.geometry.coordinate_map_square_2d(2000, 51, 5, projection="yz")
