...
class Order:
    ...
    def __init__(self, order_number:str, address:Address, date=datetime.today().strftime('%Y-%m-%d'), items:List[OrderItem]=[], customer_notes="", internal_notes="", weight=0):
        ...
        self.modify_date = ""
        self.order_status = "awaiting_payment"
...
class ShipStation:
    limiter = RateLimiter(rate_limit=10, period=60)  # 10 calls per 60 seconds
    ...
    def get_orders_by_date_range(self, start_date: str, end_date: str, exclude_voided=True):
        ...
        while True:
            ...
            for shipment in data['shipments']:
                logger.debug(f"Found shipment: {shipment}")
