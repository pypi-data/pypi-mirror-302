  ...
  create view "f1"."staging"."stg_ergast__races__dbt_tmp" as (
    with
raw_races as (select * from "f1"."ingress"."ergast__races"),
...
formatted as (
    select
        ...
        md5(cast(coalesce(cast(race.year as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(race.round as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as event_id,
        md5(cast(coalesce(cast(race.year as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(race.round as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(session_type as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as session_id,
        circuit.circuit_id,
