...
def smooth_line(**kwargs):
    ...
    dataset = kwargs.get('dataset')
    x_dataset = dataset['x']
    number_of_plots = len(dataset) - 1  # Considerando que a primeira chave é 'x'

    # Suavização das curvas
    smoothed_curves = []
    for i in range(1, number_of_plots + 1):
        curve_name = f'curve{i}'
        smoothed_curve = np.convolve(dataset[curve_name], np.ones(5)/5, mode='same')  # Exemplo de suavização por média móvel
        smoothed_curves.append(smoothed_curve)

    mean_curve = np.mean(np.array(list(dataset.values())[1:]), axis=0)
    min_curve = np.min(np.array(list(dataset.values())[1:]), axis=0)
    max_curve = np.max(np.array(list(dataset.values())[1:]), axis=0)
    w_inches, h_inches = convert_si_to_inches_in_chart_size(w, h)
    _, ax = plt.subplots(1, 1, figsize=(w_inches, h_inches), sharex=True)

    # Plotagem das curvas originais (opcional)
    for k in range(number_of_plots):
        if k < len(line_style):
            current_line_style = line_style[k]
        else:
            current_line_style = '-'

        if k < len(line_width):
            current_line_width = line_width[k]
        else:
        ...
        ax.plot(x_dataset,
                ...
                color=current_color,
                alpha=0.3)  # Alpha reduzido para curvas individuais
