...
def _plot_structure_discretized(
    ...
    **kwargs,
    ...
    if projection.lower() == "auto":
        projection = _automatic_projection(struct.positions)
    pos_proj, idx_proj = get_projection(
        struct.positions, projection, return_inverse=True
    )

    if struct.step.dim() == 0:
        steplist = torch.tile(struct.step, (len(struct.positions),))
    else:
...
def _plot_contour_discretized(
    ...
    **kwargs,
    ...
    if type(struct) == torch.Tensor:
        ...
        positions = struct.positions.to("cpu")
...
def _plot_structure_eff_pola(
    ...
    **kwargs,
    ...
    if projection.lower() == "auto":
        projection = _automatic_projection(struct.positions)
    pos_proj = get_projection(struct.positions, projection, unique=False)
...
def contour(
    ...
    **kwargs,
    ...
    """plot structure contour in 2d, projected to e.g. the XY plane
...
if __name__ == "__main__":
    ...
    geometry = tg.struct.volume.discretizer_hexagonalcompact(
        *tg.struct.volume.disc(r_disc, h_disc), step=step, orientation=1
    )
    struct = tg.struct.StructDiscretizedHexagonal3D(geometry, material)
    # geometry = tg.struct.volume.discretizer_cubic(
    #     *tg.struct.volume.disc(r_disc, h_disc), step=step
    # )
    # struct = tg.struct.StructDiscretizedCubic3D(geometry, material)
