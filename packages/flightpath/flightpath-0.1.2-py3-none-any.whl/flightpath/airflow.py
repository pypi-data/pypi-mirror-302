...
def extract_task_instances(base_url: str, dag_ids: list[str], dag_run_id: str, auth: HTTPBasicAuth, page_length: int = 100) -> List[Dict[str, Any]]:
    ...
    for dag_id in dag_ids:
        ...
        while True:
            ...
            dag_task_instances.extend(task_instances)
            
            if len(task_instances) < page_length:
                break
            
            offset += page_length
            logging.info(f"Fetched {len(dag_task_instances)} task instances so far. Continuing to next page...")
...
def etl_task_instances(base_url: str, dag_ids: list[str], dag_run_id: str, auth: HTTPBasicAuth, output_file: str) -> None:
    logging.info(f"Starting to fetch task instances for DAGs: {dag_ids}, DAG Run: {dag_run_id} from base URL: {base_url}")
    task_instances = extract_task_instances(base_url, dag_ids, dag_run_id, auth)
