        ...
        while True:
            if not isFirst:
                ...
                if len(self.conversation_history) >= 2:
                    ...
                    last_log = f"user:{last_user_log} - AI: {last_assistant_log}"
                ...
                if last_log:
                    ...
                    if status == "1" or status == 1:
                        ...
                        if action:
                            ...
                            logger.info(" #### You're in `QA Mode`! Type your question to get support, click `take action` so Zinley can apply above solution to current project, or click `Exit` to leave.")
                            logger.info("\n ### Click take action so Zinley can apply the above solution to the current project:  ")

                            user_permission = input()

                            user_prompt, _, tier, file_attachments, focused_files, _ = parse_payload(self.repo.get_repo_path(), user_permission)
                            ...
                            user_prompt, _, _, file_attachments, focused_files, _ = parse_payload(self.repo.get_repo_path(), user_permission)
                            user_prompt = user_prompt.lower()
            ...
            if user_prompt == "h":
                await self.coder.explainer_code_task_pipeline(tier, last_log, "Top notch software engineer", "English")
                self.conversation_history({"role": "user", "content": "Apply above solution."})
                self.conversation_history({"role": "assistant", "content": "Already done!"})
                logger.info("-------------------------------------------------")
            else:
