...
class AnalyticalQuery:
    ...
    def __call__(
        ...
        try:
            ...
            with tqdm(
                total=polling_interval_sec,
                bar_format="{desc}",
                leave=False,
            ) as pbar:
                while True:
                    pbar.set_description(f"Checking query status")
                    ...
                    if self._status == "running" or self._status == "pending":
                        ...
                        spinner = cycle(["|", "/", "-", "\\"])  # Spinner symbols
                        end_time = time() + polling_interval_sec
                        while time() < end_time:
                            remaining_time = int(end_time - time())
                            pbar.set_description(
                                f"{next(spinner)} Query is {self._status}. Checking status again in {remaining_time}s"
                            )
                            sleep(0.1)
                ...
                if autopaginate and self.has_next():
                    ...
                    with tqdm(
                        total=total_pages, desc="Fetching data", unit="page"
                    ) as pbar:
                        while self.has_next():
