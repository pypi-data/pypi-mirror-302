...
from moviepy.editor import AudioFileClip, VideoFileClip, CompositeVideoClip, concatenate_videoclips, CompositeAudioClip
        ...
        for enhancement in sorted(self.enhancements, key = lambda enhancement: enhancement.start):
            ...
            if enhancement.mode == EnhancementMode.INLINE.value:
                if enhancement.type in [EnhancementType.EFFECT.value, EnhancementType.EFFECT]:
                    # TODO: These 'extra_params' must be set and correctly checked before
                    # Here 'enhancement_clip' is the effect class, so we need to apply it
                    # correctly
                    parameters = enhancement.extra_params
                    premade_name = enhancement.keywords

                    # TODO: All this below must be done in some method, not here
                    ...
                    ElementParameterValidator.validate_effect_name(premade_name)

                    premade_class = EffectElementBuilder.effect_name_to_class(premade_name)
                    parameters_to_ignore = ['self', 'cls', 'args', 'kwargs', 'clip', 'duration']
                    parameters = ElementBuilder.extract_extra_params(enhancement, premade_class.apply, parameters_to_ignore)
                    ...
                    actual_parameters = ParameterObtainer.get_parameters_from_method(premade_class.apply)
                    ...
                    # TODO: All this above must be done in some method, not here

                    enhancement_clip = enhancement_clip.apply(self.video_clip.subclip(enhancement.start, end), **parameters)
