...
class WhatsApp(object):
    ...
    def upload_media(self, media_path: str, mime_type: str) -> str:
        """
        ...
        base_url = "https://graph.facebook.com/v20"
        try:
            # Open the file and prepare the form data
            with open(media_path, "rb") as file:
                files = {
                    "file": (media_path, file, mime_type),
                }
                data = {
                    "messaging_product": "whatsapp",
                    "type": mime_type
                ...
                # Make the POST request to upload media
                response = requests.post(base_url, headers=self.headers, files=files, data=data)

            # Check if the response is successful
            if response.status_code == 200:
                media_id = response.json().get("id")
                logging.info(f"Media uploaded successfully with ID: {media_id}")
                return media_id
            else:
                logging.error(f"Error uploading media: {response.json()}")
                return None
