...
class FileContentManager:
    ...
    async def read_file(file_path: str) -> str:
        ...
        try:
            ...
            logger.info(f" #### The `file manager agent` has successfully read the file: `{file_path}`")
            return content
        except FileNotFoundError:
            logger.info(f" #### The `file manager agent` encountered an issue: File `{file_path}` not found. Returning an empty string.")
            return ""
    ...
    async def write_file(file_path: str, content: str):
        ...
        try:
            ...
            if not os.path.exists(directory):
                ...
                logger.info(f" #### The `file manager agent` has created a new directory: `{directory}` for the file: `{file_path}`")
            
            async with aiofiles.open(file_path, 'w') as file:
                await file.write(content)
            
            if not os.path.exists(file_path):
                logger.info(f" #### The `file manager agent` has successfully created a new file: `{file_path}`")
            else:
                logger.info(f" #### The `file manager agent` has successfully updated the file: `{file_path}`")
        except Exception as e:
    ...
    async def apply_changes(cls, file_path: str, blocks: List[Tuple[str, str]]) -> str:
        ...
        for search, replace in blocks:
            if search:
                ...
                content += f"\n\n{replace}"
        logger.info(f" #### The `file manager agent` has successfully applied changes to the content of file: `{file_path}`")
        return content
    ...
    async def process_coding_agent_response(cls, file_path: str, coding_agent_response: str):
        ...
        logger.info(f" #### The `file manager agent` has automatically applied changes to file: `{file_path}`")
    ...
    async def handle_coding_agent_response(cls, file_path: str, coding_agent_response: str):
        ...
        logger.info(coding_agent_response)
        try:
