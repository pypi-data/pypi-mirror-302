...
from metadata.profiler.source.metadata import MetaMartSource
from metadata.profiler.source.metadata_ext import MetaMartSourceExt
...
class ProfilerWorkflow(IngestionWorkflow):
    ...
    def __init__(self, config: MetaMartWorkflowConfig):
    ...
    def _get_source_class(self):
        if self.config.source.serviceName:
            ...
            return MetaMartSource
        logger.info(
            "Database Service name not provided, we will scan all the tables "
            "available within data source and locate table entity in MetaMart "
            "to ingest profiler data."
        )
        return MetaMartSourceExt
