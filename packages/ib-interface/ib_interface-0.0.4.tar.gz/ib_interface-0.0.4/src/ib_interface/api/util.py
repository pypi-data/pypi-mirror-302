...
def dataclassNonDefaults(obj) -> dict:
    ...
    return {
        ...
        if value != field.default and value == value and not (isinstance(value, list) and value == [])
    }
...
def isnamedtupleinstance(x):
    ...
    t = type(x)
    b = t.__bases__
    if len(b) != 1 or not isinstance(b[0], tuple):
        return False
...
def logToConsole(level=logging.INFO):
    ...
    logger = logging.getLogger()
    stdHandlers = [h for h in logger.handlers if type(h) is logging.StreamHandler and h.stream is sys.stderr]
    if stdHandlers:
...
async def timeRangeAsync(start: Time_t, end: Time_t, step: float) -> AsyncIterator[dt.datetime]:
...
def formatIBDatetime(t: Union[dt.date, dt.datetime, str, None]) -> str:
    ...
    if not t:
        ...
        t = dt.datetime(t.year, t.month, t.day, 23, 59, 59).astimezone(tz=dt.timezone.utc)
        s = t.strftime("%Y%m%d %H:%M:%S UTC")
    else:
        s = t
    return s
