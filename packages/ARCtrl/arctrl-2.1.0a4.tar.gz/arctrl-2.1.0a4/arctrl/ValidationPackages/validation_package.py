...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2766(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2765(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2764(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2763(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2763))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2764))

            return append(singleton("{"), delay(_arrow2765))

        return join("\n", to_list(delay(_arrow2766)))
...
ValidationPackage_reflection = _expr2773
