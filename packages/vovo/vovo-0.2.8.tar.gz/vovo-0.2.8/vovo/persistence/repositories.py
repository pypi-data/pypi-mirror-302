...
class GenericSqlRepository(GenericRepository[T], ABC):
    ...
    async def list(self, limit: int = 100, order_by: str | List[str] | None = None, **filters) -> list[T]:
        """Gets a list of records from the database."""
        query = select(self.model)

        # Apply filters and ordering
        query = await self._apply_filters_and_ordering(query, order_by=order_by, **filters)
    ...
    async def _apply_filters_and_ordering(self, query, order_by: str | List[str] | None = None, **filters: Any):
