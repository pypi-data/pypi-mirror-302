...
def run_code(request: RunCodeRequest, max_attempts: int = 5) -> RunCodeResponse:

    @configurable_retry(max_attempts)
    def _run_code(request: RunCodeRequest) -> RunCodeResponse:
        result = requests.post(f'{SANDBOX_ENDPOINT}/run_code', json=request.dict())
...
def run_jupyter(request: RunJupyterRequest, max_attempts: int = 3) -> RunJupyterResponse:

    @configurable_retry(max_attempts)
    def _run_jupyter(request: RunJupyterRequest) -> RunJupyterResponse:
        result = requests.post(f'{SANDBOX_ENDPOINT}/run_jupyter', json=request.dict())
        if result.status_code != 200:
            raise Exception(f'Faas api responded with code {result.status_code}: {result.text}')
...
def get_prompts(request: GetPromptsRequest) -> List[Prompt]:
    result = requests.post(f'{SANDBOX_ENDPOINT}/get_prompts', json=request.dict())
    if result.status_code != 200:
        raise Exception(f'Faas api responded with code {result.status_code}: {result.text}')
...
def get_prompt_by_id(request: GetPromptByIdRequest) -> Prompt:
    result = requests.post(f'{SANDBOX_ENDPOINT}/get_prompt_by_id', json=request.dict())
    if result.status_code != 200:
        raise Exception(f'Faas api responded with code {result.status_code}: {result.text}')
...
def submit(request: SubmitRequest, max_attempts: int = 5) -> EvalResult:

    @configurable_retry(max_attempts)
    def _submit(request: SubmitRequest) -> EvalResult:
        result = requests.post(f'{SANDBOX_ENDPOINT}/submit', json=request.dict())
        if result.status_code != 200:
            raise Exception(f'Faas api responded with code {result.status_code}: {result.text}')
...
def submit_safe(request: SubmitRequest, max_attempts: int = 5) -> EvalResult:
    try:
        return submit(request, max_attempts)
    except Exception:
