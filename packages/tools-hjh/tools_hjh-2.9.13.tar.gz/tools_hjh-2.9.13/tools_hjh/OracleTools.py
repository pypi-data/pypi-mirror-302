...
class OracleTools:
    ...
    @staticmethod
    # 通过修改get_table_ddl()的结果得到
    def get_table_ddl_pg(dba_conn, username, table):
        ora_sqls = OracleTools.get_table_ddl(dba_conn, username, table, no_fk=False, partition=True)
        is_partition_table = False
        partition_cols = ''
        ora_table_partition_sql = ''
        ora_table_sql = ''
        ora_comment_table_sql = ''
        ora_partition_sql = ''
        ora_comment_column_sqls = []
        ora_index_sqls = []
        ora_constraint_sqls = []
        ...
        for sql in ora_sqls:
            if sql.startswith('create table'):
                ora_table_partition_sql = sql
            elif sql.startswith('comment on table'):
                ora_comment_table_sql = sql
            elif sql.startswith('comment on column'):
                ora_comment_column_sqls.append(sql)
            elif sql.startswith('create') and 'index' in sql:
                ora_index_sqls.append(sql)
            elif sql.startswith('alter tablee') and 'constraint' in sql:
                ora_constraint_sqls.append(sql)
                
        # 建表语句分离为建表语句和分区语句
        if 'partition by' in ora_table_partition_sql:
            ora_table_split = 0
            for line in ora_table_partition_sql.split('\n'):
                if ora_table_split == 0:
                    ora_table_sql = ora_table_sql + line + '\n'
                elif ora_table_split == 1:
                    ora_partition_sql = ora_partition_sql + line + '\n'
                if 'partition by' in line:
                    is_partition_table = True
                    partition_cols = line.split(' (')[1][:-1]
                    ora_table_split = 1
        
        # 处理建表语句
        pg_table_sql = ''
        for line in ora_table_sql.split('\n'):
            if line.startswith('  "'):
                col_name = line.split('" ', 1)[0].lower()
                no_col_name_line = line.split('" ', 1)[1]
                no_col_name_line = no_col_name_line.replace('VARCHAR2', 'VARCHAR')
                no_col_name_line = no_col_name_line.replace('NVARCHAR', 'VARCHAR')
                no_col_name_line = no_col_name_line.replace('NVARCHAR2', 'VARCHAR')
                no_col_name_line = no_col_name_line.replace('NCHAR', 'CHAR')
                no_col_name_line = no_col_name_line.replace('NUMBER', 'NUMERIC')
                no_col_name_line = no_col_name_line.replace('BINARY_INTEGER', 'INTEGER')
                no_col_name_line = no_col_name_line.replace('BINARY_FLOAT', 'FLOAT')
                no_col_name_line = no_col_name_line.replace('DATE', 'TIMESTAMP(0)')
                no_col_name_line = no_col_name_line.replace('TIMESTAMP WITH LOCAL TIME ZONE', 'TIMESTAMPTZ')
                no_col_name_line = no_col_name_line.replace('CLOB', 'TEXT')
                no_col_name_line = no_col_name_line.replace('LONG', 'TEXT')
                no_col_name_line = no_col_name_line.replace('BLOB', 'BYTEA')
                no_col_name_line = no_col_name_line.replace('LONG RAW', 'BYTEA')
                no_col_name_line = no_col_name_line.replace('default SYSDATE', 'default statement_timestamp()')
                pg_table_sql = pg_table_sql + col_name + '" ' + no_col_name_line + '\n'
            else:
                pg_table_sql = pg_table_sql + line + '\n'
        
        # 处理注释
        ora_comment_table_sql_1 = ora_comment_table_sql.split(" is '")[0]
        ora_comment_table_sql_2 = ora_comment_table_sql.split(" is '")[1]
        pg_comment_table_sql = ora_comment_table_sql_1.lower() + " is E'" + ora_comment_table_sql_2
        pg_comment_column_sqls = []
        for ora_comment_column_sql in ora_comment_column_sqls:
            ora_comment_column_sql_1 = ora_comment_column_sql.split(" is '")[0]
            ora_comment_column_sql_2 = ora_comment_column_sql.split(" is '")[1]
            pg_comment_column_sql = ora_comment_column_sql_1.lower() + " is E'" + ora_comment_column_sql_2
            pg_comment_column_sqls.append(pg_comment_column_sql)
            
        # 处理索引
        pg_index_sqls = []
        for ora_index_sql in ora_index_sqls:
            if ora_index_sql.startswith('create UNIQUE index') and is_partition_table:
                cols = []
                ora_index_cols = ora_index_sql.split(' ("')[1].replace('"', '').replace(')', '')
                if ora_index_cols.endswith(' LOCAL'):
                    ora_index_cols = ora_index_cols.replace(' LOCAL', '')
                ora_index_no_col = ora_index_sql.split(' ("')[0]
                for col in partition_cols.replace('"', '').split(','):
                    col = col.strip()
                    if col not in cols:
                        cols.append(col)
                for col in ora_index_cols.split(','):
                    col = col.strip()
                    if col not in cols:
                        cols.append(col)
                pg_index_cols = ''
                for col in cols:
                    pg_index_cols = pg_index_cols + "'" + col.lower() + "', "
                pg_index_cols = pg_index_cols[:-2]
                pg_index_sql = ora_index_no_col + ' (' + pg_index_cols + ')'
                pg_index_sqls.append(pg_index_sql)
            else:
                cols = []
                ora_index_cols = ora_index_sql.split(' ("')[1].replace('"', '').replace(')', '')
                if ora_index_cols.endswith(' LOCAL'):
        ...
        print(pg_table_sql)
        print(pg_comment_table_sql)
        for pg_comment_column_sql in pg_comment_column_sqls:
            print(pg_comment_column_sql)
        for pg_index_sql in pg_index_sqls:
            print(pg_index_sql)
    ...
    def get_table_ddl_2pg(dba_conn, username, table):
        ...
        for idx in metadata['indexes']:
            ...
            for i in partition_columns.split(',') + my_cols.split(','):
                if i not in a:
                    a.append(i)
            cols_partition_columns = str(a).replace('[', '').replace(']', '').replace("'", '').strip(',').strip()
            if type_ == 'NORMAL':
                type_ = ''
            if type_ == 'UNIQUE':
