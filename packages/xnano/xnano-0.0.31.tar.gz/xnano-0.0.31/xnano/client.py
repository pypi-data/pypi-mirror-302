...
# httpx args
class HttpxArgs(PydanticBaseModel):
    """
    Arguments for the httpx client
    """
...
class Completions:
    ...
    def __init__(
        ...
        try:
            self.client = Client(
                base = self.__init_openai_client__(http_client, verify_ssl, proxies) if self.config.provider == "openai" else self.__init_litellm_client__(),
            )
    ...
    def __init_openai_client__(self, http_client : Optional[Any] = None, verify_ssl : Optional[bool] = None, proxies : Optional[Dict[str, str]] = None):
        from openai import OpenAI

        if verify_ssl is not None or proxies is not None:
            ...
            if self.config.verbose:
                logger.info(f"HTTPX arguments detected: {verify_ssl}, {proxies}")
