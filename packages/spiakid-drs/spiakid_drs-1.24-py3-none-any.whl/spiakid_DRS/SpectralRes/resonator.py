...
class Resonator(object):
    ...
    def cal_noise_spectrum(self,noisedata,noisefreqmeas,onres = True,nperseg = 125000,fs = 5e6,
                           combinespectrum = False,method = 'welch'):
        
        #combinespectrum: set the spectrum to be 
        #   1Hz   for f < 50Hz, 
        ...
        if combinespectrum == True:
            freqnoise,Sxx = CombineSpectrum(freqnoise,Syy)
        ...
        if onres == True:
            ...
            self.TNoiseOnRes = np.arange(0,1000,1)*1/fs
        else:
            self.freqOffResNoise = freqnoise
            self.SxxOffRes = Sxx;
            self.SyyOffRes = Syy;
            ...
            self.TNoiseOffRes = np.arange(0,1000,1)*1/fs
