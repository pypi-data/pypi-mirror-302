...
class GitLabBuild:
    ...
    def __get_jobs_metadata(
        ...
        logging.info("%s jobs already collected", len(old_jobs))  # type: ignore
        if old_jobs is None or len(old_jobs) == 0:
            ...
            last_collected_job_id: int = old_jobs["id"].max()  # type: ignore

            page: int = 1
            jobs_df: pd.DataFrame = pd.DataFrame(
                self.__get_jobs_by_page(project=project, page=page)
            )
            logging.info("Collected last %s jobs", len(jobs_df))
            ...
            while last_collected_job_id not in jobs_df["id"].to_list():
                ...
                logging.info("Collected %s additionnal jobs", len(new_page_df))
            ...
            n_additional_jobs = jobs_df.shape[0] - old_jobs.shape[0]  # type: ignore
            logger.info("%s additionnal jobs found", n_additional_jobs)
            jobs = jobs_df.to_dict(orient="records")
            # ensure well-formated JSON
            jobs = repair_json(json.dumps(jobs), return_objects=True)
