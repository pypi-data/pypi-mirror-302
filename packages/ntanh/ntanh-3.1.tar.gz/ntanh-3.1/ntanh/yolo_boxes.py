...
def yolo_str_to_xyxy(yolo_str, imH, imW):
    ...
    x1,y1,x2,y2 = ntanh.yolo_str_to_xyxy(yolo_str, img_width, img_height)

    Args:
        yolo_str (string with id): '0 0.195312 0.416667 0.234375 0.416667'
        img_width (int/float): Rộng ảnh
        img_height (int/float): Cao ảnh
    ...
    return obj_class, int(x_min), int(y_min), int(x_max), int(y_max)
