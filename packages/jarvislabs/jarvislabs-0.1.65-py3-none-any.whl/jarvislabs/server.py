...
class Server:
    ...
    def _create_route(self, method: Callable):
        ...
        async def endpoint(body: dict):            
            ...
            asyncio.create_task(self._run_endpoint(method_name, body))
            
            # Return the prediction_id immediately
            return {"prediction_id": body["prediction_id"], "status": "Processing"}

    async def _run_endpoint(self, method_name: str, body: dict):
        try:
            start_time = time.time()
            result = await self.app.api_endpoints[method_name](body['input'])
            end_time = time.time()
