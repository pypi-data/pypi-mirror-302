...
class SchemeInfoEdit(QWidget):
    ...
    def __setupUi(self):
        ...
        self.name_edit.setPlaceholderText(self.tr(_tr.m[210, "untitled"]))
        self.name_edit.setSizePolicy(QSizePolicy.Expanding,
                                     QSizePolicy.Fixed)
        self.desc_edit = QTextEdit(self)
        self.desc_edit.setTabChangesFocus(True)

        layout.addRow(self.tr(_tr.m[211, "Title"]), self.name_edit)
        layout.addRow(self.tr(_tr.m[212, "Description"]), self.desc_edit)
    ...
    def setScheme(self, scheme):
        ...
        self.scheme = scheme
        if not scheme.title:
            self.name_edit.setText(self.tr(_tr.m[213, "untitled"]))
    ...
    def commit(self):
        ...
        if self.__schemeIsUntitled and \
            self.name_edit.text() == self.tr(_tr.m[214, "untitled"]):
...
class SchemeInfoDialog(QDialog):
    ...
    def __setupUi(self):
        ...
        heading = self.tr(_tr.m[215, "Workflow Info"])
        ...
        self.__showAtNewSchemeCheck = \
            QCheckBox(self.tr(_tr.m[216, "Show when I make a New Workflow."]),
                      self,
        ...
        check_layout.addWidget(
               QLabel(self.tr((_tr.m[217, "You can also edit Workflow Info later "] + _tr.m[218, "(File -> Workflow Info)."])),
                      self,
