...
from tasks.models import Annotation, Prediction, Task, TaskAssignee
...
class TaskListAPI(generics.ListCreateAPIView):
    ...
    def get(self, request):
        ...
        if self.request.user.groups.filter(name='annotater').exists():
            if project.task_distribution == 'MANUEL':
                for task in queryset:
                    try:
                        TaskAssignee.objects.get(type='AN', assignee=self.request.user, task=task)
                    except ObjectDoesNotExist:
                        queryset = queryset.exclude(id=task.id)
        if self.request.user.groups.filter(name='reviewer').exists():
            if project.review_distribution == 'MANUEL':
                for task in queryset:
                    try:
                        TaskAssignee.objects.get(type='RV', assignee=self.request.user, task=task)
                    except ObjectDoesNotExist:
                        queryset = queryset.exclude(id=task.id) 
