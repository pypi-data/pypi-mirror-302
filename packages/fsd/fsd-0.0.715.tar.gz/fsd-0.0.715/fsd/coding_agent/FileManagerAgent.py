...
class FileManagerAgent:
    ...
    async def get_file_planning(self, idea):
        ...
        prompt = (
                ...
                "Moving_files must include only existing files that are explicitly mentioned in the instruction to be moved from a specific location A to a specific location B within the project. Do not include any files in Moving_files unless the instruction clearly states to move an existing file from one location to another."
                "Existing_files must include only the full paths for file names explicitly mentioned in the instruction that will be updated or modified. If a file in Existing_files is also in Moving_files, use the new_full_path instead of the old_full_path in Existing_files. "
