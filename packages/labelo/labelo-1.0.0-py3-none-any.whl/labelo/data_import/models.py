...
class FileUpload(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.format in [".jpg", ".png", ".jpeg"] and not self.image_description:
            self.create_image_description()

    def create_image_description(self):
        model = self.project.description_ml_model
        Model = MODELS.get(model)
        if not model:
            return
        Model = Model["model"]
        args = {}
        params = self.project.ml_params or {}
        if MODELS[model]["args"]:
            for arg in MODELS[model]["args"].keys():
                args[arg] = params[arg]
        print(args)
        path = os.path.join(settings.MEDIA_ROOT, self.file.name)
        model = Model(**args)
        desc = model.generate_desc_with_path(path)
        if desc:
            self.image_description = desc
            self.save(update_fields=['image_description'])
            # send_notification({
            #     "type": "send_notification",
            #     "message": f"Added image description: {self.image_description}",
            #     "toastType": "info"
            # }, "notifications")
