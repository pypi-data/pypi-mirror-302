...
from labelo.core.utils.windows_sqlite_fix import windows_dll_fix
...
from labelo.core.argparser import parse_input_args
from labelo.core.utils.params import get_env
...
def _setup_env():
    ...
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'labelo.core.settings.labelo')
...
def main():
    ...
    if input_args.data_dir:
        ...
        os.environ.setdefault('LABELO_BASE_DATA_DIR', str(data_dir_path.absolute()))
    ...
    from labelo.core.utils.common import collect_versions
    ...
    if input_args.command == 'version' or input_args.version:
        from labelo import __version__

        print('\nLabelo version:', __version__, '\n')
    ...
    # init
        ...
        print('')
        print('Labelo has been successfully initialized.')
        if input_args.command != 'start' and input_args.project_name:
            print('Start the server: labelo start ' + input_args.project_name)
            return

    # start with migrations from old projects, '.' project_name means 'labelo start' without project name
    elif input_args.command == 'start' and input_args.project_name != '.':
        from projects.models import Project

        from labelo.core.old_ls_migration import migrate_existing_project
        ...
        if input_args.project_name and not _project_exists(input_args.project_name):
            ...
            if project_path.exists():
                print('Project directory from previous version of labelo found')
                ...
                print(
                    Fore.LIGHTYELLOW_EX
                    + '\n*** WARNING! ***\n'
                    + f'Project {input_args.project_name} migrated to Labelo Database\n'
            ...
            if not migrated:
                print(
                    ...
                    'Did you miss create it first with `labelo init {project_name}` ?'.format(
                        project_name=input_args.project_name
                    )
                )
                return

    # on `start` command, launch browser if --no-browser is not specified and start labelo server
    if input_args.command == 'start' or input_args.command is None:
        from labelo.core.utils.common import start_browser
        ...
        if cert_file or key_file:
            logger.error(
                "Labelo doesn't support SSL web server with cert and key.\n" 'Use nginx or other servers for it.'
            )
            return
