...
from ib_interface.api.contract import ComboLeg, Contract, ContractDescription, ContractDetails, DeltaNeutralContract
...
from ib_interface.api.order import Order, OrderComboLeg, OrderCondition, OrderState
from ib_interface.api.util import parseIBDatetime, UNSET_DOUBLE, ZoneInfo
...
class Decoder:
    ...
    def __init__(self, wrapper: Wrapper, serverVersion: int):
        ...
        self.handlers = {
            ...
            13: self.wrap("updateMktDepthL2", [int, int, str, int, int, float, float, bool]),
            ...
            47: self.wrap("tickEFP", [int, int, float, str, float, int, str, float, float]),
            49: self.wrap("currentTime", [int]),
            50: self.wrap("realtimeBar", [int, int, float, float, float, float, float, float, int]),
            ...
            95: self.wrap("pnlSingle", [int, float, float, float, float, float], skip=1),
    ...
    def deltaNeutralValidation(self, fields):
        ...
        self.wrapper.deltaNeutralValidation(
            int(reqId), DeltaNeutralContract(int(conId), float(delta or 0), float(price or 0))
        )
    ...
    def securityDefinitionOptionParameter(self, fields):
        ...
        self.wrapper.securityDefinitionOptionParameter(
            int(reqId), exchange, underlyingConId, tradingClass, multiplier, expirations, strikes
        )
    ...
    def mktDepthExchanges(self, fields):
        ...
        descriptions = [
            DepthMktDataDescription(
                exchange=get(), secType=get(), listingExch=get(), serviceDataType=get(), aggGroup=int(get())
            )
            for _ in range(int(n))
        ]
    ...
    def tickByTick(self, fields):
        ...
        if tickType in (1, 2):
            ...
            self.wrapper.tickByTickAllLast(
                reqId, tickType, time, float(price), float(size), attrib, exchange, specialConditions
            )
            ...
            self.wrapper.tickByTickBidAsk(
                reqId, time, float(bidPrice), float(askPrice), float(bidSize), float(askSize), attrib
            )
