...
class TestsimulationClass(unittest.TestCase):
    ...
    def test_class_api(self):
        for device in self.devices:
            ...
            ecs = self.sim.get_ecs_spectra(progress_bar=False)
            torch.testing.assert_close(ecs["ecs"], cs2["ecs"])

            # - field intensity
            # E-field
            r_probe = [0, 0, 100]
            I_nfe = self.sim.get_nf_intensity_spectra_e(r_probe, progress_bar=False)
            ...
            # H-field
            I_nfh = self.sim.get_nf_intensity_spectra_h(r_probe, progress_bar=False)
            ...
            # far-field
            r_probe = [0, 0, 10000]
            I_ff = self.sim.get_ff_intensity_spectra(r_probe, progress_bar=False)
            ...
            mp = self.sim.get_multipole_spec(progress_bar=False)
            ...
            mp_scs = self.sim.get_multipole_scs_spec(progress_bar=False)
            ...
            mp_ecs = self.sim.get_multipole_ecs_spec(progress_bar=False)
