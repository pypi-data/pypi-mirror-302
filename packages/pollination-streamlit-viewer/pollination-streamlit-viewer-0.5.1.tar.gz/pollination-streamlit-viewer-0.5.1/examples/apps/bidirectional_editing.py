...
path = '../files/results_example.vtkjs'
with open(os.path.join(os.path.dirname(__file__), path), 'rb') as input_file:
    fcontent = BytesIO(input_file.read()).getvalue()

content = _file.getvalue() if _file else fcontent
...
with vtkjs_container:
    vtkjs = viewer(
            ...
            screenshot_name="custom_screenshot_name"
            )
