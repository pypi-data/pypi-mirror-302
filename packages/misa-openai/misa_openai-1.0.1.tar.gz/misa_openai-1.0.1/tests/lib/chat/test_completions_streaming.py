...
import misa_openai
from misa_openai import OpenAI, AsyncOpenAI
from misa_openai._utils import assert_signatures_in_sync
from misa_openai._compat import model_copy
from misa_openai.lib.streaming.chat import (
    ContentDoneEvent,
...
from misa_openai.lib._parsing._completions import ResponseFormatT
...
def test_parse_max_tokens_reached(client: OpenAI, respx_mock: MockRouter) -> None:
    ...
    with pytest.raises(misa_openai.LengthFinishReasonError):
...
def test_parse_pydantic_tool(client: OpenAI, respx_mock: MockRouter, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    listener = _make_stream_snapshot_request(
        lambda c: c.beta.chat.completions.stream(
            ...
            tools=[
                misa_openai.pydantic_function_tool(GetWeatherArgs),
            ],
        ),
...
def test_parse_multiple_pydantic_tools(client: OpenAI, respx_mock: MockRouter, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    listener = _make_stream_snapshot_request(
        lambda c: c.beta.chat.completions.stream(
            ...
            tools=[
                misa_openai.pydantic_function_tool(GetWeatherArgs),
                misa_openai.pydantic_function_tool(
