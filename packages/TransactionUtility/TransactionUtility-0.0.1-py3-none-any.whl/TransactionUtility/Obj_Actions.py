...
class Obj_Actions:
    ...
    def insert_or_update_actions(self, actions, connection):
        ...
        if missing_keys:
            ...
            if count > 0:
                logger.log("inside update")

                values = {
                    "obj_name": obj_name, "line_no": line_no, "image": image, "description": description, "service_code": str(service_code)[:60],
    ...
    def process_data(self, conn, sql_models_data):
        ...
        for sql_model in self.sql_models:
            if "sql_model" in sql_model and "actions" in sql_model['sql_model']:
                for actions in sql_model['sql_model']['actions']:
