...
<script lang="ts">
	...
	async function showError(error: string) {
		...
		if (error == 'format') {
			errorMessage = interface_language.startsWith('fr')
			? 'Format non pris en charge'
			: 'Unsupported format';
		} else if (error == 'size') {
			errorMessage = interface_language.startsWith('fr')
			? 'Fichier trop lourd pour être affiché'
...
<Block {visible} {elem_id} {elem_classes} {container} {scale} {min_width} {height}>
    ...
    />
		<div class="viewer-container">
			...
			<div class="button-row">
				{#if Array.isArray(value) && value.length > 1}
					<button class="left-arrow" on:click={previous_file}>
						<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
							<polygon points="12.4,3.8 5.6,9 12.4,14.2"  stroke_linejoin="round" stroke_linecap="round" />
						</svg>
					</button>
				{/if}
				{#if Array.isArray(value) && value.length > 0}
				...
				{/if}
				{#if Array.isArray(value) && value.length > 1}
					<button class="right-arrow" on:click={next_file}>
						<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
							<polygon points="5.6,3.8 12.4,9 5.6,14.2" stroke_linejoin="round" stroke_linecap="round" />
						</svg>
					</button>
				{/if}
			</div>
	...
	<style>
		...
		.left-arrow polygon, .right-arrow polygon {
			...
			stroke: var(--neo-light-grey);
			stroke-width: 1.5;
		}
		.dark .left-arrow polygon, .dark .right-arrow polygon {
		stroke: var(--neo-dark-grey);
