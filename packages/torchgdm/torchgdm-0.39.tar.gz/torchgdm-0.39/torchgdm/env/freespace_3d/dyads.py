...
def G0_Hm(r_probe, r_source, wavelength, eps_env, epsilon_dist: float = 1e-8):
    ...
    """
    return G0_Ep(r_probe, r_source, wavelength, eps_env, epsilon_dist=epsilon_dist)
...
def G0_Em_ff(
    ...
    # wavenumber
    k0 = 2 * torch.pi / wavelength
    k = torch.sqrt(eps_env + epsilon_dist) * k0
    k02n = k * k0
    k_sqrt_eps_lR = k * lR
...
class EnvHomogeneous3D(EnvironmentBase):
    ...
    def __init__(
        self, env_material=1.0, device: torch.device = None
    ):
