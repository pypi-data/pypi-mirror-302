...
def run(
    ...
    logger.info(f"Run validation pipeline, storing annotated output at '{annotated_dataset_path}'")
    before = datetime.now()

    num_rows_total = validate.run_validation_program(
    ...
    if did_pre_validation_pass:
        if not should_check_uniqueness and not should_write_parquet:
            logger.info(f"Validation PASSED and no more checks required, copying input file to output and exiting")
