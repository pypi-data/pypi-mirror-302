...
class GetOpenPositions(GetData):
    ...
    def __init__(
        ...
        self.chain_tokens: dict[str, dict[str, str | int | bool]] = get_available_tokens(config=config)
    ...
    def get_open_positions(self) -> dict[str, Any]:
        ...
        if not raw_positions:
            self.logger.info(f'No positions open for address: "{self.address}" on {self.config.chain.title()}.')
            return {}
    ...
    def _get_data_processing(self) -> dict[str, Any]:
        ...
        """
        market_info = self.markets.info[self._raw_position[0][1]]
