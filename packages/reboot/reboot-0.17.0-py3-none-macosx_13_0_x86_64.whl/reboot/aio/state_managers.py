...
class SidecarStateManager(
    ...
    def __init__(
        ...
        *,
        ...
        self._service_name_by_state_tag: dict[str, ServiceName] = {}

        for serviceable in serviceables:
            # Filter out legacy gRPC serviceables.
            if isinstance(serviceable, RebootServiceable):
                tag = _state_type_tag(
                    service_to_state_type(serviceable.service_name())
                )
                self._service_name_by_state_tag[tag
                                               ] = serviceable.service_name()
    ...
    def _state_type_for_state_ref(self, state_ref: StateRef) -> StateTypeName:
        ...
        service_name = self._service_name_by_state_tag.get(tag, None)

        if service_name is None:
            raise RuntimeError(
                f"No service name found for state '{state_ref.id}'"
            )

        return service_to_state_type(service_name)
