...
"""
Configuration functions for the log package for Python. The core package
...
def fileConfig(fname, defaults=None, disable_existing_loggers=True):
    """
    Read the log configuration from a ConfigParser-format file.
    ...
    try:
        ...
        # Handlers add themselves to log._handlers
...
def _handle_existing_loggers(existing, child_loggers, disable_existing):
    """
    When (re)configuring log, handle loggers which were in the previous
    configuration but are not in the new configuration. There's no point
    deleting them as other threads may continue to hold references to them;
    and by disabling them, you stop them doing any log.
...
class DictConfigurator(BaseConfigurator):
    """
    Configure log using a dictionary-like object to describe the
    configuration.
    """
    ...
    def configure_handler(self, config):
        ...
        config_copy = dict(config)  # for restoring in case of log_error
...
def dictConfig(config):
    """Configure log using a dictionary."""
...
def listen(port=DEFAULT_LOGGING_CONFIG_PORT, verify=None):
    ...
    class ConfigStreamHandler(StreamRequestHandler):
        """
        Handler for a log configuration request.

        It expects a completely new log configuration and uses fileConfig
        to install it.
        """
    ...
    class ConfigSocketReceiver(ThreadingTCPServer):
        """
        A simple TCP socket-based log config receiver.
        """
