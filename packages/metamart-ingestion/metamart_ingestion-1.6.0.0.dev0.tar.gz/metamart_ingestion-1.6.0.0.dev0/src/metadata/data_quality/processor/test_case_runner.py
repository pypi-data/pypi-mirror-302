...
class TestCaseRunner(Processor):
    ...
    def __init__(self, config: MetaMartWorkflowConfig, metadata: MetaMart):
        super().__init__()
    ...
    def _run(self, record: TableAndTests) -> Either:
        ...
        metamart_test_cases = self.filter_for_om_test_cases(test_cases)
        metamart_test_cases = self.filter_incompatible_test_cases(
            record.table, metamart_test_cases
        )
        ...
        logger.debug(
            f"Found {len(metamart_test_cases)} test cases for table {record.table.fullyQualifiedName.root}"
        )
        if len(metamart_test_cases) == 0:
            logger.warning("No test cases found for the table")

        test_results = [
            test_case_result
            for test_case in metamart_test_cases
    ...
    def filter_for_om_test_cases(self, test_cases: List[TestCase]) -> List[TestCase]:
        ...
        for test_case in test_cases:
            ...
            if TestPlatform.MetaMart not in test_definition.testPlatforms:
                logger.debug(
                    f"Test case {test_case.name.root} is not an MetaMart test case."
                )
                continue
