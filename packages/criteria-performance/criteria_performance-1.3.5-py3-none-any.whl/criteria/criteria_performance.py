...
class PerformanceCriteria:
    def __init__(self, data:None, val=100):
        self.__val = val
        self.__data = sortDataFrame(pd.read_csv(data,sep=",")) if type(data).__name__=="str" else sortDataFrame(data)
...
def Opentxt(url):
    data = np.loadtxt(url)
    return asDataFrame(np.vstack((np.array([[k,v] for k,v in zip(data[:,0],data[:,1]) if k==1 ]),np.array([[-1,v] for k,v in zip(data[:,0],data[:,1]) if k!=1 ]))))

def sortDataFrame(data):
    return asDataFrame(np.vstack((np.array([[k,v] for k,v in zip(data[data.columns[0]],data[data.columns[1]]) if k==1 ]),np.array([[-1,v] for k,v in zip(data[data.columns[0]],data[data.columns[1]]) if k!=1 ]))))
