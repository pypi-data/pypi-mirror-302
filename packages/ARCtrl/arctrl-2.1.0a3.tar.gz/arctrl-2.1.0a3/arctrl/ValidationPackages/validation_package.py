...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2821(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2820(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2819(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2818(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2818))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2819))

            return append(singleton("{"), delay(_arrow2820))

        return join("\n", to_list(delay(_arrow2821)))
...
ValidationPackage_reflection = _expr2824
