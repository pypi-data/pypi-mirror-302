...
from odrive.libodrive import Device, DeviceType, DiscoveryDelegate, Interface, LibODrive
...
class Subscription():
    ...
    def _on_found(self, dev: Device):
        try:
            ...
            print(f"Subscription's on_found handler failed: {ex}")
            traceback.print_exc()
    ...
    def _on_lost(self, runtime_device: RuntimeDevice, dev: Device):
        try:
            ...
            print(f"Subscription's on_lost handler failed: {ex}")
            traceback.print_exc()
    ...
    def _on_connected(self, dev: RuntimeDevice):
        ...
        try:
            ...
            print(f"Subscription's on_connected handler failed: {ex}")
            traceback.print_exc()
...
class DeviceManager(DiscoveryDelegate):
    ...
    async def _connect_and_announce_async(self, device: Device, connect_requested_by: List[Subscription]):
        try:
            ...
            print(f"Found ODrive but could not connect (requested by {subs}): {ex}")
            traceback.print_exc()
            for sub in self._subscribers:
